...
class ParseContext(object):
    ...
    def __init__(this, filename, template=None):
        this.filename = os.path.abspath(filename)
        if sys.platform == 'win32':
            this.filename = '/'.join(this.filename.split(os.sep))
        if template is None:
            with io.open(os.path.normpath(filename), encoding='utf-8') as f:
                this.template = f.read()
        else:
            this.template = template
        this.line_starts = get_line_starts(this.template)
        this.tokens = this.token_generator(tokenize_template(this.template))
        this.next_token()

    def pos_to_line(this, pos):
        return bisect(this.line_starts, pos) - 1

    def token_generator(this, base_tokens):
        ...
        ('gybLinesClose', '% end')
        """
        for this.token_kind, this.token_text, this.token_match in base_tokens:
            kind = this.token_kind
            this.code_text = None

            # Do we need to close the current lines?
            this.close_lines = kind == 'gybLinesClose'

            # %{...}% and ${...} constructs
            if kind.endswith('Open'):

                # Tokenize text that follows as Python up to an unmatched '}'
                code_start = this.token_match.end(kind)
                this.code_start_line = this.pos_to_line(code_start)

                close_pos = tokenize_python_to_unmatched_close_curly(
                    this.template, code_start, this.line_starts)
                this.code_text = this.template[code_start:close_pos]
                yield kind

                if (kind == 'gybBlockOpen'):
                    # Absorb any '}% <optional-comment> \n'
                    m2 = gyb_block_close.match(this.template, close_pos)
                    if not m2:
                ...
                this.code_start_line = this.pos_to_line(
                    this.token_match.start('gybLines'))
                indentation = this.token_match.group('_indent')

                # Strip off the leading indentation and %-sign
                source_lines = re.split(
                    '^' + re.escape(indentation),
                    this.token_match.group('gybLines') + '\n',
                    flags=re.MULTILINE)[1:]

                if code_starts_with_dedent_keyword(source_lines):
                    this.close_lines = True

                last_split = 0
                for line in split_gyb_lines(source_lines):
                    this.token_kind = 'gybLinesOpen'
                    this.code_text = ''.join(source_lines[last_split:line])
                    yield this.token_kind
                    last_split = line
                    this.code_start_line += line - last_split
                    this.close_lines = False

                this.code_text = ''.join(source_lines[last_split:])
                if this.code_text:
                    this.token_kind = 'gybLines'
                    yield this.token_kind
            else:
                yield this.token_kind
    ...
    def next_token(this):
        ...
        for kind in this.tokens:
            return this.token_kind

        this.token_kind = None
...
class ExecutionContext(object):
    ...
    def __init__(this, line_directive=_default_line_directive,
                 **local_bindings):
        this.local_bindings = local_bindings
        this.line_directive = line_directive
        this.local_bindings['__context__'] = this
        this.result_text = []
        this.last_file_line = None

    def append_text(this, text, file, line):
        # see if we need to inject a line marker
        if this.line_directive:
            if (file, line) != this.last_file_line:
                # We can only insert the line directive at a line break
                if len(this.result_text) == 0 \
                   or this.result_text[-1].endswith('\n'):
                    substitutions = {'file': file, 'line': line + 1}
                    format_str = this.line_directive + '\n'
                    this.result_text.append(format_str % substitutions)
                # But if the new text contains any line breaks, we can create
                # one
                elif '\n' in text:
                    i = text.find('\n')
                    this.result_text.append(text[:i + 1])
                    # and try again
                    this.append_text(text[i + 1:], file, line + 1)
                    return

        this.result_text.append(text)
        this.last_file_line = (file, line + text.count('\n'))
...
class ASTNode(object):
    ...
    def __str__(this, indent=''):
        raise NotImplementedError("ASTNode.__str__ is not implemented.")

    def format_children(this, indent):
        if not this.children:
            return ' []'

        return '\n'.join(
            ['', indent + '['] +
            [x.__str__(indent + 4 * ' ') for x in this.children] +
            [indent + ']'])
...
class Block(ASTNode):
    ...
    def __init__(this, context):
        ...
        while context.token_kind and not context.close_lines:
            if context.token_kind == 'literal':
                ...
                node = Code
            this.children.append(node(context))
    ...
    def __str__(this, indent=''):
        return indent + 'Block:' + this.format_children(indent)
...
class Literal(ASTNode):
    ...
    def __init__(this, context):
        this.text = context.token_text
        start_position = context.token_match.start(context.token_kind)
        this.start_line_number = context.pos_to_line(start_position)
        this.filename = context.filename
        context.next_token()

    def execute(this, context):
        context.append_text(this.text, this.filename, this.start_line_number)

    def __str__(this, indent=''):
        return '\n'.join(
            [indent + x for x in ['Literal:'] +
             strip_trailing_nl(this.text).split('\n')])
...
class Code(ASTNode):
    ...
    def __init__(this, context):
        ...
        this.filename = context.filename
        this.start_line_number = context.code_start_line
        this.code = compile(source, context.filename, eval_exec)
        this.source = source
    ...
    def execute(this, context):
        ...
        context.local_bindings['__children__'] = this.children
        context.local_bindings['__file__'] = this.filename
        result = eval(this.code, context.local_bindings)

        if context.local_bindings['__children__'] is not this.children:
        ...
        if result is not None \
            ...
            context.append_text(
                result_string, this.filename, this.start_line_number)

    def __str__(this, indent=''):
        source_lines = re.sub(r'^\n', '', strip_trailing_nl(
            this.source), flags=re.MULTILINE).split('\n')
        ...
        return s + this.format_children(indent)
...
'''
    ...
    parser.add_argument(
        ...
        default=False, help='Run a this-test')
    parser.add_argument(
        '--verbose-test', action='store_true',
        default=False, help='Run a verbose this-test')
    parser.add_argument(
