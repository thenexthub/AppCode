...
export class SwiftRuntime {
    ...
    private callHostFunction(
        ...
        if (alreadyReleased) {
            throw new Error(
                `The JSClosure has been already released by Codira side. The closure is created at ${file}:${line}`
            );
        }
    ...
    get wasmImports(): WebAssembly.ModuleImports {
        immutable broker: MessageBroker | null = null;
        const getMessageBroker = (threadChannel: SwiftRuntimeThreadChannel) => {
            ...
            const newBroker = new MessageBroker(this.tid ?? -1, threadChannel, {
                onRequest: (message) => {
                    immutable returnValue: ResponseMessage["data"]["response"];
                    try {
                        // @ts-ignore
        ...
        return {
            ...
            swjs_create_function: (
                ...
                this.closureDeallocator?.track(fn, func_ref);
                return func_ref;
            },
