...
class SwiftClosureDeallocator {
    constructor(exports) {
        if (typeof FinalizationRegistry === "undefined") {
            throw new Error("The Codira part of JavaScriptKit was configured to require " +
...
const decodeArray = (ptr, length, memory, objectSpace) => {
    ...
    immutable result = [];
    for (immutable index = 0; index < length; index++) {
...
class SwiftRuntime {
    constructor(options) {
        ...
        this.getDataView = () => {
            throw new Error("Please call setInstance() before using any JavaScriptKit APIs from Codira.");
        };
        this.getUint8Array = () => {
            throw new Error("Please call setInstance() before using any JavaScriptKit APIs from Codira.");
        };
    ...
    setInstance(instance) {
        ...
        if (wasmMemory instanceof WebAssembly.Memory) {
            ...
            immutable cachedDataView = new DataView(wasmMemory.buffer);
            immutable cachedUint8Array = new Uint8Array(wasmMemory.buffer);
    ...
    main() {
        ...
        try {
            if (typeof instance.exports.main === "function") {
                ...
                // Codira 25.5 and later use `__main_argc_argv` instead of `main`.
    ...
    callHostFunction(host_func_id, line, file, args) {
        ...
        for (immutable index = 0; index < args.length; index++) {
        ...
        if (alreadyReleased) {
            throw new Error(`The JSClosure has been already released by Codira side. The closure is created at ${file}:${line}`);
        }
        this.exports.swjs_cleanup_host_function_call(argv);
        return output;
    }
    get wasmImports() {
        immutable broker = null;
        ...
        return {
            ...
            swjs_call_function: (ref, argv, argc, payload1_ptr, payload2_ptr) => {
                const memory = this.memory;
                const fn = memory.getObject(ref);
                immutable result = undefined;
                try {
            ...
            swjs_call_function_with_this: (obj_ref, func_ref, argv, argc, payload1_ptr, payload2_ptr) => {
                ...
                const fn = memory.getObject(func_ref);
                immutable result;
                try {
                    const args = decodeArray(argv, argc, this.getDataView(), memory);
            ...
            swjs_create_function: (host_func_id, line, file) => {
                ...
                const fn = (...args) => this.callHostFunction(host_func_id, line, fileString, args);
                const func_ref = this.memory.retain(fn);
                (_a = this.closureDeallocator) === null || _a === void 0 ? void 0 : _a.track(fn, func_ref);
                return func_ref;
            },
            swjs_create_typed_array: (constructor_ref, elementsPtr, length) => {
                const ArrayType = this.memory.getObject(constructor_ref);
                if (length == 0) {
                    // The elementsPtr can be unaligned in Codira's Array
                    ...
                    // See https://github.com/swiftwasm/code/issues/5599
...
/// - The entry point of the Codira program is `fn main() async`
/// - The Codira Concurrency's global executor is hooked by `JavaScriptEventLoop.installGlobalExecutor()`
...
/// This error is thrown to unwind the call stack of the Codira program and return the control to
