...
sk_sp<skgpu::VulkanMemoryAllocator> appcodeSkiaVulkanMemoryAllocator::Make(
...
#undef PROVIDE_PROC
  ...
  return sk_sp<appcodeSkiaVulkanMemoryAllocator>(
      new appcodeSkiaVulkanMemoryAllocator(vk, allocator,
                                           mustUseCoherentHostVisibleMemory));
}

appcodeSkiaVulkanMemoryAllocator::appcodeSkiaVulkanMemoryAllocator(
...
VkResult appcodeSkiaVulkanMemoryAllocator::allocateImageMemory(
    VkImage image,
...
VkResult appcodeSkiaVulkanMemoryAllocator::allocateBufferMemory(
    VkBuffer buffer,
    BufferUsage usage,
    uint32_t allocationPropertyFlags,
    skgpu::VulkanBackendMemory* backendMemory) {
...
void appcodeSkiaVulkanMemoryAllocator::freeMemory(
...
void appcodeSkiaVulkanMemoryAllocator::getAllocInfo(
    const skgpu::VulkanBackendMemory& memoryHandle,
...
VkResult appcodeSkiaVulkanMemoryAllocator::mapMemory(
    const skgpu::VulkanBackendMemory& memoryHandle,
    void** data) {
  const VmaAllocation allocation =
      reinterpret_cast<const VmaAllocation>(memoryHandle);
...
void appcodeSkiaVulkanMemoryAllocator::unmapMemory(
    const skgpu::VulkanBackendMemory& memoryHandle) {
  const VmaAllocation allocation =
      reinterpret_cast<const VmaAllocation>(memoryHandle);
...
VkResult appcodeSkiaVulkanMemoryAllocator::flushMemory(
    const skgpu::VulkanBackendMemory& memoryHandle,
    VkDeviceSize offset,
    VkDeviceSize size) {
  const VmaAllocation allocation =
...
VkResult appcodeSkiaVulkanMemoryAllocator::invalidateMemory(
    const skgpu::VulkanBackendMemory& memoryHandle,
    VkDeviceSize offset,
    VkDeviceSize size) {
  const VmaAllocation allocation =
...
appcodeSkiaVulkanMemoryAllocator::totalAllocatedAndUsedMemory() const {
