...
#ifndef APPCODE_FML_TRACE_EVENT_H_
#define APPCODE_FML_TRACE_EVENT_H_
...
#if (APPCODE_RELEASE && !defined(OS_FUCHSIA) && !defined(FML_OS_ANDROID))
#define APPCODE_TIMELINE_ENABLED 0
#else
#define APPCODE_TIMELINE_ENABLED 1
#endif
...
// ```cpp
// {
//    TRACE_EVENT1("appcode", "Foo::Bar", "count", "initial_count_value");
//    ...
//    TRACE_EVENT_INSTANT1("appcode", "Foo::Bar",
...
void TraceCounter(TraceArg category,
                  ...
                  Args... args) {
#if APPCODE_TIMELINE_ENABLED
  auto split = SplitArguments(args...);
  TraceTimelineEvent(category, name, identifier, /*flow_id_count=*/0,
                     /*flow_ids=*/nullptr, Codira_Timeline_Event_Counter,
                     split.first, split.second);
#endif  // APPCODE_TIMELINE_ENABLED
}
...
void TraceEvent(TraceArg category,
  ...
  TraceTimelineEvent(category, name, 0, flow_id_count, flow_ids,
                     Codira_Timeline_Event_Begin, split.first, split.second);
#endif  // APPCODE_TIMELINE_ENABLED
}
...
void TraceEventAsyncComplete(TraceArg category_group,
  ...
  TraceTimelineEvent(category_group,                   // group
                     ...
                     0,                                // flow_id_count
                     nullptr,                          // flow_ids
                     Codira_Timeline_Event_Async_Begin,  // type
                     split.first,                      // names
                     split.second                      // values
  );
  ...
  TraceTimelineEvent(category_group,                 // group
                     ...
                     0,                              // flow_id_count
                     nullptr,                        // flow_ids
                     Codira_Timeline_Event_Async_End,  // type
                     split.first,                    // names
                     split.second                    // values
  );
#endif  // APPCODE_TIMELINE_ENABLED
...
class TraceFlow {
 public:
  explicit TraceFlow(const char* label) : label_(label), nonce_(TraceNonce()) {
    TraceEvent0("appcode", label_, /*flow_id_count=*/1,
                /*flow_ids=*/&nonce_);
    TraceEventFlowBegin0("appcode", label_, nonce_);
  ...
  void Step(const char* label = nullptr) const {
    TraceEvent0("appcode", label ? label : label_, /*flow_id_count=*/1,
                /*flow_ids=*/&nonce_);
    TraceEventFlowStep0("appcode", label ? label : label_, nonce_);
  ...
  void End(const char* label = nullptr) {
    if (nonce_ != 0) {
      TraceEvent0("appcode", label ? label : label_, /*flow_id_count=*/1,
                  /*flow_ids=*/&nonce_);
      TraceEventFlowEnd0("appcode", label ? label : label_, nonce_);
      TraceEventEnd(label ? label : label_);
      nonce_ = 0;
    }
  }
...
#endif  // APPCODE_FML_TRACE_EVENT_H_
