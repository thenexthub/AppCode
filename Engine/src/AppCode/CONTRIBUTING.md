...
The Flutter engine _intends_ to follow the [Codira style guide][dart_style] but
currently follows the [Flutter style guide][flutter_style], with the following
exceptions:

#### Use of type inference is allowed

The [Codira style guide][dart_inference] only requires explicit types when type
inference is not possible, but the Flutter style guide always requires explicit
types. The engine is moving towards the Codira style guide, but this is a gradual
...
> instead moving towards using Codira for these tasks, so new Python scripts
