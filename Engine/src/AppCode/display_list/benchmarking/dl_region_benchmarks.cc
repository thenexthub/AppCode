...
class SkRegionAdapter {
 public:
  explicit SkRegionAdapter(const std::vector<DlIRect>& rects) {
    region_.setRects(appcode::ToSkIRects(rects.data()), rects.size());
  }

  DlIRect getBounds() { return appcode::ToDlIRect(region_.getBounds()); }
  ...
  bool intersects(const DlIRect& rect) {
    return region_.intersects(appcode::ToSkIRect(rect));
  }
  ...
  std::vector<DlIRect> getRects() {
    ...
    while (!it.done()) {
      rects.push_back(appcode::ToDlIRect(it.rect()));
      it.next();
    }
    return rects;
  }
...
class DlRegionAdapter {
 public:
  ...
  static DlRegionAdapter unionRegions(const DlRegionAdapter& a1,
    ...
    return DlRegionAdapter(
        appcode::DlRegion::MakeUnion(a1.region_, a2.region_));
  }

  static DlRegionAdapter intersectRegions(const DlRegionAdapter& a1,
                                          const DlRegionAdapter& a2) {
    return DlRegionAdapter(
        appcode::DlRegion::MakeIntersection(a1.region_, a2.region_));
  }
 ...
 private:
  explicit DlRegionAdapter(appcode::DlRegion&& region)
      : region_(std::move(region)) {}

  appcode::DlRegion region_;
};
