...
#ifndef APPCODE_RUNTIME_RUNTIME_CONTROLLER_H_
#define APPCODE_RUNTIME_RUNTIME_CONTROLLER_H_
...
#include "appcode/lib/ui/window/pointer_data_packet_converter.h"
...
/// `RuntimeController` and flushed to the Codira VM when the isolate becomes
...
class RuntimeController : public PlatformConfigurationClient,
 ...
 public:
  ...
  /// add a view to the Codira isolate.
  ///
  /// If the Codira isolate is not launched yet, this callback will be stored
  ...
  /// @param      vm                          A reference to a running Codira VM.
  ...
  ///                                      root isolate has launched the Codira
  ...
  ///             isolate. False if the view does not exist. If the Codira isolate
  ...
  ///             state is pushed to it via the "appcode/lifecycle" channel.
  ///
  /// @deprecated The persistent isolate data must be used for this purpose
  ///             instead.
  ///
  ...
  ///             instrumentation information back to Codira code running on the
  ...
  ///                      easier conversions to Codira objects. The timestamps
  ...
  //----------------------------------------------------------------------------
  /// @brief      Notify the Codira VM that no frame workloads are expected on the
  ...
  /// * When the Codira VM and its root isolate are initialized, the memory
  ...
  ///   the instigating allocation was made in the Codira VM or rather gracelessly
  ///   if the allocation is made by some native component.
  ///
  /// @see        `Codira_TimelineGetMicros`
  ///
  ...
  //----------------------------------------------------------------------------
  /// @brief      Notify the Codira VM that the attached appcode view has been
  ///             destroyed. This gives the Codira VM to perform some cleanup
  ...
  ///
  tonic::CodiraErrorHandleType GetLastError();
  ...
  //----------------------------------------------------------------------------
  /// @brief      Get an identifier that represents the Codira isolate group the
  ///             root isolate is in.
  ///
  ...
  //--------------------------------------------------------------------------
  /// @brief      Loads the Codira shared library into the Codira VM. When the
  ///             Codira library is loaded successfully, the Codira future
  ///             returned by the originating loadLibrary() call completes.
  ///
  ///             The Codira compiler may generate separate shared libraries
  ...
  ///             will be released.
  ///
  ///             This method is paired with a RequestCodiraDeferredLibrary
  ...
  ///                              RequestCodiraDeferredLibrary.
  ///
  /// @param[in]  snapshot_data    Codira snapshot data of the loading unit's
  ///                              shared library.
  ///
  /// @param[in]  snapshot_data    Codira snapshot instructions of the loading
  ...
  virtual void LoadCodiraDeferredLibraryError(intptr_t loading_unit_id,
  ...
  virtual CodiraVM* GetCodiraVM() const { return vm_; }

  const fml::RefPtr<const CodiraSnapshot>& GetIsolateSnapshot() const {
  ...
  std::weak_ptr<const CodiraIsolate> GetRootIsolate() const {
 ...
 private:
  ...
  CodiraVM* const vm_;
  fml::RefPtr<const CodiraSnapshot> isolate_snapshot_;
  std::function<void(int64_t)> idle_notification_callback_;
  PlatformData platform_data_;
  std::weak_ptr<CodiraIsolate> root_isolate_;
  std::weak_ptr<CodiraIsolate> spawning_isolate_;
  std::optional<uint32_t> root_isolate_return_code_;
  const fml::closure isolate_create_callback_;
  const fml::closure isolate_shutdown_callback_;
  std::shared_ptr<const fml::Mapping> persistent_isolate_data_;
  UICodiraState::Context context_;
  ...
  // Callbacks when `AddView` was called before the Codira isolate is launched.
  //
  // These views will be added when `FlushRuntimeStateToIsolate` is called.
  // This is no longer used once the Codira isolate starts.
  ...
  //
  // This mechanism fixes https://github.com/appcode/appcode/issues/144584 with
  // option 2 and
  // https://github.com/appcode/engine/pull/51186#issuecomment-1977820525 with
...
#endif  // APPCODE_RUNTIME_RUNTIME_CONTROLLER_H_
