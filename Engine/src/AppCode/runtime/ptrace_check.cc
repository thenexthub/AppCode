...
static_assert(appcode_RUNTIME_MODE == appcode_RUNTIME_MODE_DEBUG,
...
static bool IsLaunchedByappcodeCLI(const Settings& vm_settings) {
  // Only the appcode CLI passes "--enable-checked-mode". Therefore, if the flag
...
static bool EnableTracingManually(const Settings& vm_settings) {
  ...
  const char* warning =
      ...
      "Please compile a profile-/release-build, launch your app via \"appcode "
      "run\", or see https://github.com/appcode/appcode/wiki/"
...
static bool EnableTracingIfNecessaryOnce(const Settings& vm_settings) {
  if (IsLaunchedByappcodeCLI(vm_settings)) {
    return true;
  }
