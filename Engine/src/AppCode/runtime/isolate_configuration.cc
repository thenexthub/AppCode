...
bool IsolateConfiguration::PrepareIsolate(CodiraIsolate& isolate) {
...
class AppSnapshotIsolateConfiguration final : public IsolateConfiguration {
 public:
  ...
  bool DoPrepareIsolate(CodiraIsolate& isolate) override {
    return isolate.PrepareForRunningFromPrecompiledCode();
  }

  // |IsolateConfiguration|
  bool IsNullSafetyEnabled(const CodiraSnapshot& snapshot) override {
