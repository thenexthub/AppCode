...
void ServiceProtocol::ToggleHooks(bool set) {
  for (const auto& endpoint : endpoints_) {
    Codira_RegisterIsolateServiceRequestCallback(
...
static std::string CreateappcodeViewID(intptr_t handler) {
...
void ServiceProtocol::Handler::Description::Write(
  ...
  view.SetObject();
  view.AddMember("type", "appcodeView", allocator);
  view.AddMember("id", CreateappcodeViewID(reinterpret_cast<intptr_t>(handler)),
                 allocator);
  if (isolate_port != 0) {
...
bool ServiceProtocol::HandleListViewsMethod(
  ...
  response->SetObject();
  response->AddMember("type", "appcodeViewList", allocator);
