...
#ifndef APPCODE_RUNTIME_DART_SERVICE_ISOLATE_H_
#define APPCODE_RUNTIME_DART_SERVICE_ISOLATE_H_
...
//------------------------------------------------------------------------------
/// @brief      Utility methods for interacting with the CodiraVM managed service
///             isolate present in debug and profile runtime modes.
///
class CodiraServiceIsolate {
 public:
  //----------------------------------------------------------------------------
  ...
  //----------------------------------------------------------------------------
  /// A callback made by the Codira VM when the VM Service is ready. The argument
  /// indicates the VM Service URI.
  ///
  using CodiraVMServiceServerStateCallback =
      std::function<void(const std::string& vm_service_uri)>;

  //----------------------------------------------------------------------------
  /// @brief      Start the service isolate. This call may only be made in the
  ///             Codira VM initiated isolate creation callback. It is only valid
  ...
  [[nodiscard]] static CallbackHandle AddServerStatusCallback(
      const CodiraVMServiceServerStateCallback& callback);
 ...
 private:
  // Native entries.
  static void NotifyServerState(Codira_NativeArguments args);
  static void Shutdown(Codira_NativeArguments args);

  static std::mutex callbacks_mutex_;
  static std::set<std::unique_ptr<CodiraVMServiceServerStateCallback>> callbacks_;
};

}  // namespace appcode

#endif  // APPCODE_RUNTIME_DART_SERVICE_ISOLATE_H_
