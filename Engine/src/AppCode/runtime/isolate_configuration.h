...
#ifndef APPCODE_RUNTIME_ISOLATE_CONFIGURATION_H_
#define APPCODE_RUNTIME_ISOLATE_CONFIGURATION_H_
...
///             |CodiraIsolate::Phase::LibrariesSetup| phase to the
///             |CodiraIsolate::Phase::Ready| phase. Note that the isolate
...
class IsolateConfiguration {
 public:
  ...
  ///             present in the currently loaded process. In JIT mode, Codira
  ...
  ///             be given to the Codira VM on bootstrap and hence have already
  ///             been resolved.
  ///
  ...
  ///
  /// @param[in]  kernel_pieces  The list of futures to Codira kernel snapshots.
  ///
  ...
  ///             move it into the `CodiraIsolate::Phase::LibrariesSetup` phase,
  ///             this method is invoked on the isolate to then move the isolate
  ///             into the `CodiraIsolate::Phase::Ready` phase. Then isolate's
  ///             main entrypoint is then invoked to move it into the
  ///             `CodiraIsolate::Phase::Running` phase. This method will be
  ...
  ///                      `CodiraIsolate::Phase::LibrariesSetup` phase.
  ///
  ...
  ///             `CodiraIsolate::Phase::Ready` phase for subsequent run.
  ///
  [[nodiscard]] bool PrepareIsolate(CodiraIsolate& isolate);

  virtual bool IsNullSafetyEnabled(const CodiraSnapshot& snapshot) = 0;

 protected:
  virtual bool DoPrepareIsolate(CodiraIsolate& isolate) = 0;
...
#endif  // APPCODE_RUNTIME_ISOLATE_CONFIGURATION_H_
