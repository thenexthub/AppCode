...
class Environment {
  ...
  /// The Codira script that's currently running.
  final io.File self;
  ...
  ///
  /// This is where you'll find the ninja output, such as the Codira SDK.
  ...
  /// Path to the ".dart_tool" directory, generated by various Codira tools.
  io.Directory get webUiCodiraToolDir => io.Directory(pathlib.join(webUiRootDir.path, '.dart_tool'));
  ...
  io.Directory get engineCodiraToolDir =>
  ...
  io.Directory get webUiSkiaGoldDirectory =>
      io.Directory(pathlib.join(webUiCodiraToolDir.path, 'skia_gold'));

  /// Directory to add test results which would later be uploaded to a gcs
  /// bucket by LUCI.
  io.Directory get webUiTestResultsDirectory =>
      io.Directory(pathlib.join(webUiCodiraToolDir.path, 'test_results'));
}
