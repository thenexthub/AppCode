...
class CopyArtifactsStep implements PipelineStep {
  ...
  Future<void> buildHostPage() async {
    final String hostCodiraPath = pathlib.join('lib', 'static', 'host.dart');
    final io.File hostCodiraFile = io.File(
      pathlib.join(environment.webEngineTesterRootDir.path, hostCodiraPath),
    );
    ...
    final String timestamp = hostCodiraFile.statSync().modified.millisecondsSinceEpoch.toString();
    if (timestampFile.existsSync()) {
      ...
      if (lastBuildTimestamp == timestamp) {
        ...
        print('${hostCodiraFile.path} timestamp changed. Rebuilding.');
      }
    } else {
      print('Building ${hostCodiraFile.path}.');
    }
    ...
    if (exitCode != 0) {
      throw ToolExit(
        'Failed to compile ${hostCodiraFile.path}. Compiler '
