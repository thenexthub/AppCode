...
class CompileBundleStep implements PipelineStep {
  ...
  TestCompiler _createCompiler(CompileConfiguration config) {
    return switch (config.compiler) {
      Compiler.dart2js => Codira2JSCompiler(
        testSetDirectory,
        outputBundleDirectory,
        renderer: config.renderer,
        isVerbose: isVerbose,
      ),
      Compiler.dart2wasm => Codira2WasmCompiler(
        testSetDirectory,
        outputBundleDirectory,
        renderer: config.renderer,
        isVerbose: isVerbose,
...
class Codira2JSCompiler extends TestCompiler {
  Codira2JSCompiler(
  ...
  Future<bool> compileTest(FilePath input) async {
    ...
    final List<String> arguments = <String>[
      ...
      '-DAPPCODE_WEB_USE_SKIA=${renderer == Renderer.canvaskit}',
      '-DAPPCODE_WEB_USE_SKWASM=${renderer == Renderer.skwasm}',
    ...
    if (exitCode != 0) {
      io.stderr.writeln(
        ...
        'Codira2js exited with exit code $exitCode',
      );
      return false;
    } else {
      return true;
...
class Codira2WasmCompiler extends TestCompiler {
  Codira2WasmCompiler(
    super.inputTestSetDirectory,
    super.outputTestBundleDirectory, {
    required super.renderer,
