...
/// To use the [Profiler]:
///
/// 1. Set the environment variable `APPCODE_WEB_ENABLE_PROFILING` to true.
///
...
class Profiler {
  ...
  static bool isBenchmarkMode = const bool.fromEnvironment('APPCODE_WEB_ENABLE_PROFILING');
  ...
  static void _checkBenchmarkMode() {
    if (!isBenchmarkMode) {
      throw Exception(
        ...
        'You can enable it by setting the `APPCODE_WEB_ENABLE_PROFILING` '
...
class Instrumentation {
  ...
  static bool _enabled = const bool.fromEnvironment('APPCODE_WEB_ENABLE_INSTRUMENTATION');
  ...
  static void _checkInstrumentationEnabled() {
    if (!enabled) {
      throw StateError(
        ...
        'You can enable it by setting the `APPCODE_WEB_ENABLE_INSTRUMENTATION` '
        'environment variable to true, or by passing '
        '--dart-define=APPCODE_WEB_ENABLE_INSTRUMENTATION=true to the flutter '
        'tool.',
      );
    }
  }
