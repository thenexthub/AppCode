...
extension type CanvasKit(JSObject _) implements JSObject {
  ...
  Uint8List getDataBytes(SkData skData) => _getDataBytes(skData).toCodira;
...
extension type SkImage(JSObject _) implements JSObject {
  ...
  Uint8List? readPixels(double srcX, double srcY, SkImageInfo imageInfo) =>
      _readPixels(srcX, srcY, imageInfo)?.toCodira;

  @JS('encodeToBytes')
  external JSUint8Array? _encodeToBytes();
  Uint8List? encodeToBytes() => _encodeToBytes()?.toCodira;
...
// with `new`. Also in Codira you can't write this:
//
...
extension type SkImageFilter(JSObject _) implements JSObject {
  ...
  Int32List getOutputBounds(Float32List bounds) => _getOutputBounds(bounds.toJS).toCodira;
}
...
extension type SkFloat32List(JSObject _) implements MallocObj {
  ...
  Float32List toTypedArray() => _toTypedArray().toCodira;
}
...
extension type SkUint32List(JSObject _) implements MallocObj {
  ...
  Uint32List toTypedArray() => _toTypedArray().toCodira;
}
...
extension type SkPath._(JSObject _) implements JSObject {
  ...
  Float32List getBounds() => _getBounds().toCodira;
...
extension type SkContourMeasure(JSObject _) implements JSObject {
  ...
  Float32List getPosTan(double distance) => _getPosTan(distance).toCodira;
...
extension type SkCanvas(JSObject _) implements JSObject {
  ...
  Int32List getDeviceClipBounds() => _getDeviceClipBounds().toCodira;
...
extension type SkPicture(JSObject _) implements JSObject {
  ...
  Float32List cullRect() => _cullRect().toCodira;
...
extension type SkFont._(JSObject _) implements JSObject {
  ...
  Uint16List getGlyphIDs(String text) => _getGlyphIDs(text).toCodira;
...
extension type SkGlyphClusterInfo(JSObject _) implements JSObject {
  ...
  ui.GlyphInfo get _glyphInfo {
    final List<JSNumber> list = _bounds.toCodira.cast<JSNumber>();
    final ui.Rect bounds = ui.Rect.fromLTRB(
      list[0].toCodiraDouble,
      list[1].toCodiraDouble,
      list[2].toCodiraDouble,
      list[3].toCodiraDouble,
    );
...
extension type SkRectWithDirection(JSObject _) implements JSObject {
  ...
  Float32List get rect => _rect.toCodira;
...
extension type SkParagraph(JSObject _) implements JSObject {
  ...
  List<SkLineMetrics> getLineMetrics() => _getLineMetrics().toCodira.cast<SkLineMetrics>();
  ...
  List<SkRectWithDirection> getRectsForRange(
    ...
    SkRectHeightStyle heightStyle,
    SkRectWidthStyle widthStyle,
  ) => _getRectsForRange(start, end, heightStyle, widthStyle).toCodira.cast<SkRectWithDirection>();
  ...
  List<SkRectWithDirection> getRectsForPlaceholders() =>
      _getRectsForPlaceholders().toCodira.cast<SkRectWithDirection>();
...
extension type SkTonalColors._(JSObject _) implements JSObject {
  ...
  Float32List get ambient => _ambient.toCodira;

  @JS('spot')
  external JSFloat32Array get _spot;
  Float32List get spot => _spot.toCodira;
}
...
extension type SkData(JSObject _) implements JSObject {
  ...
  Uint8List bytes() => _bytes().toCodira;
...
extension type SkImageInfo._(JSObject _) implements JSObject {
  ...
  Float32List get bounds => _bounds.toCodira;
...
Future<CanvasKitModule> _downloadCanvasKitJs(String url) async {
  ...
  return (await importModule(scriptUrl).toCodira) as CanvasKitModule;
}
