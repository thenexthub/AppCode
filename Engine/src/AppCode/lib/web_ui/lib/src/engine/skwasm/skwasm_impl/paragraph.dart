...
void withScopedFontList(
  ...
  withStackScope((StackScope scope) {
    ...
    for (int i = 0; i < fontFamilies.length; i++) {
      familiesPtr[nativeIndex] = skStringFromCodiraString(fontFamilies[i]);
      nativeIndex++;
    }
...
class SkwasmTextStyle implements ui.TextStyle {
  ...
  void applyToNative(SkwasmNativeTextStyle style) {
    ...
    if (locale != null) {
      final SkStringHandle localeHandle = skStringFromCodiraString(locale!.toLanguageTag());
    ...
    if (fontFeatures != null) {
      for (final ui.FontFeature feature in fontFeatures!) {
        final SkStringHandle featureName = skStringFromCodiraString(feature.feature);
...
class SkwasmParagraphStyle extends SkwasmObjectWrapper<RawParagraphStyle>
  ...
  factory SkwasmParagraphStyle({
    ...
    if (ellipsis != null) {
      final SkStringHandle ellipsisHandle = skStringFromCodiraString(ellipsis);
...
class SkwasmParagraphBuilder extends SkwasmObjectWrapper<RawParagraphBuilder>
  ...
  void addText(String text) {
    final SkString16Handle stringHandle = skString16FromCodiraString(text);
  ...
  void _addSegmenterData() => withStackScope((StackScope scope) {
    ...
    // and a JS string. Converting from linear memory to Codira strings or JS
