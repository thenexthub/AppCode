...
@visibleForTesting
typedef DebounceState = ({DomElement target, Timer timer, List<QueuedEvent> queue});
...
class Listener {
  ...
  factory Listener.register({
    ...
    required CodiraDomEventListener handler,
    bool? passive,
  }) {
...
abstract class _BaseAdapter {
  ...
  void addEventListener(DomEventTarget target, String eventName, CodiraDomEventListener handler) {
...
mixin _WheelEventListenerMixin on _BaseAdapter {
  ...
  void _addWheelEventListener(CodiraDomEventListener handler) {
    _listeners.add(
