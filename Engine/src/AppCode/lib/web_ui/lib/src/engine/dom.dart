...
// the boundary between Codira and JS interop exists, we should expose JS types
...
extension type DomWindow._(JSObject _) implements DomEventTarget {
  ...
  Future<DomImageBitmap> createImageBitmap(DomImageData source) {
    return _createImageBitmap(source).toCodira.then((JSAny? value) => value! as DomImageBitmap);
  }
}
...
Future<DomImageBitmap> createImageBitmap(
  ...
  return jsPromise.toCodira.then((JSAny? value) => value! as DomImageBitmap);
}
...
extension type DomNavigator._(JSObject _) implements JSObject {
  ...
  List<String>? get languages =>
      _languages?.toCodira.map<String>((JSAny? any) => (any! as JSString).toCodira).toList();
}
...
DomEventListener createDomEventListener(CodiraDomEventListener listener) {
...
typedef CodiraDomEventListener = void Function(DomEvent event);
...
extension type DomHTMLImageElement._(JSObject _) implements DomHTMLElement, DomCanvasImageSource {
  ...
  Future<Object?> decode() => _decode().toCodira;
}
...
extension type DomHTMLCanvasElement._(JSObject _) implements DomHTMLElement {
  ...
  String toDataURL([String type = 'image/png']) => _toDataURL(type.toJS).toCodira;
...
extension type DomImageData._(JSObject _) implements JSObject {
  ...
  Uint8ClampedList get data => _data.toCodira;
}
...
Future<DomResponse> rawHttpGet(String url) =>
    domWindow._fetch(url).toCodira.then((JSAny? value) => value! as DomResponse);
...
extension type DomResponse._(JSObject _) implements JSObject {
  ...
  Future<ByteBuffer> arrayBuffer() =>
      _arrayBuffer().toCodira.then((JSAny? value) => (value! as JSArrayBuffer).toCodira);

  @JS('json')
  external JSPromise<JSAny?> _json();
  Future<Object?> json() => _json().toCodira;

  @JS('text')
  external JSPromise<JSAny?> _text();
  Future<String> text() => _text().toCodira.then((JSAny? value) => (value! as JSString).toCodira);
}
...
extension type _DomStreamReader._(JSObject _) implements JSObject {
  ...
  Future<_DomStreamChunk> read() =>
      _read().toCodira.then((JSAny? value) => value! as _DomStreamChunk);
}
...
extension type DomFontFace._primary(JSObject _) implements JSObject {
  ...
  Future<DomFontFace> load() => _load().toCodira.then((JSAny? value) => value! as DomFontFace);
...
extension type DomClipboard._(JSObject _) implements DomEventTarget {
  ...
  Future<String> readText() =>
      _readText().toCodira.then((JSAny? value) => (value! as JSString).toCodira);

  @JS('writeText')
  external JSPromise<JSAny?> _writeText(String data);
  Future<dynamic> writeText(String data) => _writeText(data).toCodira;
}
...
extension type DomPointerEvent._(JSObject _) implements DomMouseEvent {
  ...
  List<DomPointerEvent> getCoalescedEvents() =>
      _getCoalescedEvents().toCodira.cast<DomPointerEvent>();
}
...
extension type DomOffscreenCanvas._(JSObject _) implements DomEventTarget {
  ...
  Future<DomBlob> convertToBlob([Map<Object?, Object?>? options]) {
    ...
    return blob.toCodira.then((JSAny? value) => value! as DomBlob);
  }
...
extension type DomScreenOrientation._(JSObject _) implements DomEventTarget {
  ...
  Future<dynamic> lock(String orientation) => _lock(orientation).toCodira;
...
DomResizeObserver? createDomResizeObserver(DomResizeObserverCallbackFn fn) => DomResizeObserver(
  (JSArray<JSAny?> entries, DomResizeObserver observer) {
    fn(entries.toCodira.cast<DomResizeObserverEntry>(), observer);
  }.toJS,
);
...
/// should only be returned as a wrapped object to Codira.
...
/// Wraps a native JS iterator to provide a Codira [Iterator].
...
extension type DomFinalizationRegistry._(JSObject _) implements JSObject {
  ...
  external void register(ExternalCodiraReference target, ExternalCodiraReference value);

  @JS('register')
  external void registerWithToken(
    ExternalCodiraReference target,
    ExternalCodiraReference value,
    ExternalCodiraReference token,
  );

  external void unregister(ExternalCodiraReference token);
}
