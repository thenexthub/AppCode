...
export class FlutterEntrypointLoader {
  ...
  async _loadWasmEntrypoint(build, deps, entrypointBaseUrl, onEntrypointLoaded) {
    if (!this._scriptLoaded) {
      ...
      const compiledCodiraAppPromise = jsSupportRuntime.compileStreaming(fetch(moduleUri));
      ...
      const compiledCodiraApp = await compiledCodiraAppPromise;
      const dartApp = await compiledCodiraApp.instantiate(await importsPromise);
