...
IMPLEMENT_WRAPPERTYPEINFO(appcode_gpu, Shader);
...
int InternalappcodeGpu_Shader_GetUniformStructSize(
    appcode::gpu::Shader* wrapper,
    Codira_Handle struct_name_handle) {
  auto name = tonic::StdStringFromCodira(struct_name_handle);
...
int InternalappcodeGpu_Shader_GetUniformMemberOffset(
    appcode::gpu::Shader* wrapper,
    Codira_Handle struct_name_handle,
    Codira_Handle member_name_handle) {
  auto struct_name = tonic::StdStringFromCodira(struct_name_handle);
  const auto* uniform = wrapper->GetUniformStruct(struct_name);
  if (uniform == nullptr) {
    return -1;
  }

  auto member_name = tonic::StdStringFromCodira(member_name_handle);
