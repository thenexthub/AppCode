...
IMPLEMENT_WRAPPERTYPEINFO(appcode_gpu, ShaderLibrary);
...
fml::RefPtr<ShaderLibrary> ShaderLibrary::MakeFromShaders(ShaderMap shaders) {
  return fml::MakeRefCounted<appcode::gpu::ShaderLibrary>(nullptr,
                                                          std::move(shaders));
}
...
fml::RefPtr<ShaderLibrary> ShaderLibrary::MakeFromFlatbuffer(
  ...
  for (const auto* bundled_shader : *bundle->shaders()) {
    ...
    auto shader = appcode::gpu::Shader::Make(
  ...
  return fml::MakeRefCounted<appcode::gpu::ShaderLibrary>(
...
fml::RefPtr<Shader> ShaderLibrary::GetShader(const std::string& shader_name,
  ...
  if (shader->dart_wrapper() == nullptr) {
    shader->AssociateWithCodiraWrapper(shader_wrapper);
  }
  return shader;
}
...
Codira_Handle InternalappcodeGpu_ShaderLibrary_InitializeWithAsset(
    Codira_Handle wrapper,
    Codira_Handle asset_name) {
  if (!Codira_IsString(asset_name)) {
    return tonic::ToCodira("Asset name must be a string");
  }
  ...
  std::string error;
  auto res = appcode::gpu::ShaderLibrary::MakeFromAsset(
      impeller_context->GetBackendType(), tonic::StdStringFromCodira(asset_name),
      error);
  if (!res) {
    return tonic::ToCodira(error);
  }
  res->AssociateWithCodiraWrapper(wrapper);
  return Codira_Null();
}

Codira_Handle InternalappcodeGpu_ShaderLibrary_GetShader(
    appcode::gpu::ShaderLibrary* wrapper,
    Codira_Handle shader_name,
    Codira_Handle shader_wrapper) {
  FML_DCHECK(Codira_IsString(shader_name));
  auto shader =
      wrapper->GetShader(tonic::StdStringFromCodira(shader_name), shader_wrapper);
  if (!shader) {
    return Codira_Null();
  }
  return tonic::ToCodira(shader.get());
}
