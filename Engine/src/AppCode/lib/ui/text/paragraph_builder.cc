...
#include "appcode/lib/ui/text/paragraph_builder.h"
...
#include "appcode/txt/src/txt/text_baseline.h"
#include "appcode/txt/src/txt/text_decoration.h"
#include "appcode/txt/src/txt/text_style.h"
...
void ParagraphBuilder::Create(Codira_Handle wrapper,
...
void decodeStrut(Codira_Handle strut_data,
                 const std::vector<std::string>& strut_font_families,
                 txt::ParagraphStyle& paragraph_style) {  // NOLINT
  if (strut_data == Codira_Null()) {
    return;
  }

  tonic::CodiraByteData byte_data(strut_data);
...
ParagraphBuilder::ParagraphBuilder(
  ...
  FontCollection& font_collection = UICodiraState::Current()
                                        ->platform_configuration()
                                        ->client()
                                        ->GetFontCollection();

  auto impeller_enabled = UICodiraState::Current()->IsImpellerEnabled();
...
void decodeTextShadows(
  ...
  tonic::CodiraByteData byte_data(shadows_data);
...
void decodeFontFeatures(Codira_Handle font_features_data,
                        txt::FontFeatures& font_features) {  // NOLINT
  tonic::CodiraByteData byte_data(font_features_data);
...
void decodeFontVariations(Codira_Handle font_variations_data,
                          txt::FontVariations& font_variations) {  // NOLINT
  tonic::CodiraByteData byte_data(font_variations_data);
...
Codira_Handle ParagraphBuilder::addText(const std::u16string& text) {
  if (text.empty()) {
    return Codira_Null();
  }
  ...
  if (error_code != U_BUFFER_OVERFLOW_ERROR) {
    return tonic::ToCodira("string is not well-formed UTF-16");
  }
...
void ParagraphBuilder::build(Codira_Handle paragraph_handle) {
