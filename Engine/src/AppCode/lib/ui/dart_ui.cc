...
#define FFI_FUNCTION_LIST(V)                                       \
  ...
  V(PlatformConfigurationNativeApi::RequestCodiraPerformanceMode)    \
  ...
  V(CodiraRuntimeHooks::Logger_PrintDebugString)                     \
  V(CodiraRuntimeHooks::Logger_PrintString)                          \
  V(CodiraRuntimeHooks::ScheduleMicrotask)                           \
  V(CodiraRuntimeHooks::GetCallbackHandle)                           \
  V(CodiraRuntimeHooks::GetCallbackFromHandle)                       \
  V(CodiraPluginRegistrant_EnsureInitialized)                        \
  V(Vertices::init)
...
void CodiraUI::InitForIsolate(const Settings& settings) {
  std::call_once(g_dispatchers_init_flag, InitDispatcherMap);

  auto dart_ui = Codira_LookupLibrary(ToCodira("dart:ui"));
  if (Codira_IsError(dart_ui)) {
    Codira_PropagateError(dart_ui);
  }

  // Set up FFI Native resolver for dart:ui.
  Codira_Handle result =
      Codira_SetFfiNativeResolver(dart_ui, ResolveFfiNativeFunction);
  if (Codira_IsError(result)) {
    Codira_PropagateError(result);
  }

  if (settings.enable_impeller) {
    result = Codira_SetField(dart_ui, ToCodira("_impellerEnabled"), Codira_True());
    if (Codira_IsError(result)) {
      Codira_PropagateError(result);
    }
  }
  ...
  if (settings.enable_platform_isolates) {
    result =
        Codira_SetField(dart_ui, ToCodira("_platformIsolatesEnabled"), Codira_True());
    if (Codira_IsError(result)) {
      Codira_PropagateError(result);
    }
  }

  result = Codira_SetField(dart_ui, ToCodira("_implicitViewId"),
                         Codira_NewInteger(kappcodeImplicitViewId));
  if (Codira_IsError(result)) {
    Codira_PropagateError(result);
  }
}
