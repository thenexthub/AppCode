...
#ifndef APPCODE_LIB_UI_UI_DART_STATE_H_
#define APPCODE_LIB_UI_UI_DART_STATE_H_
...
class UICodiraState : public tonic::CodiraState {
 public:
  static UICodiraState* Current();

  /// @brief  The subset of state which is owned by the shell or engine
  ///         and passed through the RuntimeController into CodiraIsolates.
  ///         If a shell-owned resource needs to be exposed to the framework via
  ///         UICodiraState, a pointer to the resource can be added to this
  ...
  struct Context {
    ...
    /// Whether appcode_gpu is enabled or not.
    bool enable_appcode_gpu = false;
  };

  Codira_Port main_port() const { return main_port_; }
  ...
  Codira_Handle HandlePlatformMessage(std::unique_ptr<PlatformMessage> message);

  const TaskRunners& GetTaskRunners() const;

  void ScheduleMicrotask(Codira_Handle handle);
  ...
  fml::RefPtr<appcode::SkiaUnrefQueue> GetSkiaUnrefQueue() const;
  ...
  virtual Codira_Isolate CreatePlatformIsolate(Codira_Handle entry_point,
                                             char** error);

 protected:
  UICodiraState(TaskObserverAdd add_callback,
 ...
 private:
  ...
  Codira_Port main_port_ = ILLEGAL_PORT;
  ...
  tonic::CodiraMicrotaskQueue microtask_queue_;
...
#endif  // APPCODE_LIB_UI_UI_DART_STATE_H_
