...
MultiFrameCodec::State::State(std::shared_ptr<ImageGenerator> generator)
    : generator_(std::move(generator)),
      ...
      is_impeller_enabled_(UICodiraState::Current()->IsImpellerEnabled()) {}

static void InvokeNextFrameCallback(
    const fml::RefPtr<CanvasImage>& image,
    int duration,
    const std::string& decode_error,
    std::unique_ptr<tonic::CodiraPersistentValue> callback,
    size_t trace_id) {
  std::shared_ptr<tonic::CodiraState> dart_state = callback->dart_state().lock();
  if (!dart_state) {
    FML_DLOG(ERROR) << "Could not acquire Codira state while attempting to fire "
                       "next frame callback.";
    return;
  }
  tonic::CodiraState::Scope scope(dart_state);
  tonic::CodiraInvoke(callback->value(),
                    {tonic::ToCodira(image), tonic::ToCodira(duration),
                     tonic::ToCodira(decode_error)});
}
...
MultiFrameCodec::State::GetNextFrameImage(
    ...
    const fml::RefPtr<appcode::SkiaUnrefQueue>& unref_queue) {
...
Codira_Handle MultiFrameCodec::getNextFrame(Codira_Handle callback_handle) {
  ...
  if (state_->frameCount_ == 0) {
    ...
    task_runners.GetUITaskRunner()->PostTask(fml::MakeCopyable(
        [trace_id, decode_error = std::move(decode_error),
         callback = std::make_unique<tonic::CodiraPersistentValue>(
             tonic::CodiraState::Current(), callback_handle)]() mutable {
