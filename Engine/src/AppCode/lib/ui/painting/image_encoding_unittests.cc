...
TEST_F(ShellTest, EncodeImageGivesExternalTypedData) {
  auto native_encode_image = [&](Codira_NativeArguments args) {
    auto image_handle = Codira_GetNativeArgument(args, 0);
    image_handle =
        Codira_GetField(image_handle, Codira_NewStringFromCString("_image"));
    ASSERT_FALSE(Codira_IsError(image_handle)) << Codira_GetError(image_handle);
    ASSERT_FALSE(Codira_IsNull(image_handle));
    auto format_handle = Codira_GetNativeArgument(args, 1);
    auto callback_handle = Codira_GetNativeArgument(args, 2);

    intptr_t peer = 0;
    Codira_Handle result = Codira_GetNativeInstanceField(
        image_handle, tonic::CodiraWrappable::kPeerIndex, &peer);
    ASSERT_FALSE(Codira_IsError(result));
    CanvasImage* canvas_image = reinterpret_cast<CanvasImage*>(peer);

    int64_t format = -1;
    result = Codira_IntegerToInt64(format_handle, &format);
    ASSERT_FALSE(Codira_IsError(result));

    result = EncodeImage(canvas_image, format, callback_handle);
    ASSERT_TRUE(Codira_IsNull(result));
  };

  auto nativeValidateExternal = [&](Codira_NativeArguments args) {
    auto handle = Codira_GetNativeArgument(args, 0);

    auto typed_data_type = Codira_GetTypeOfExternalTypedData(handle);
    EXPECT_EQ(typed_data_type, Codira_TypedData_kUint8);
...
TEST_F(ShellTest, EncodeImageAccessesSyncSwitch) {
  ...
  auto native_encode_image = [&](Codira_NativeArguments args) {
    ...
    auto io_manager = UICodiraState::Current()->GetIOManager();
...
TEST_F(ShellTest, EncodeImageRetries) {
  ...
  auto turn_off_gpu = [&](Codira_NativeArguments args) {
    auto handle = Codira_GetNativeArgument(args, 0);
    bool value = true;
    ASSERT_TRUE(Codira_IsBoolean(handle));
    Codira_BooleanValue(handle, &value);
    TurnOffGPU(shell.get(), value);
  };

  AddNativeCallback("TurnOffGPU", CREATE_NATIVE_ENTRY(turn_off_gpu));

  auto validate_not_null = [&](Codira_NativeArguments args) {
    auto handle = Codira_GetNativeArgument(args, 0);
    EXPECT_FALSE(Codira_IsNull(handle));
    message_latch.Signal();
  };
...
TEST_F(ShellTest, EncodeImageFailsWithoutGPUImpeller) {
  ...
  auto native_validate_error = [&](Codira_NativeArguments args) {
    auto handle = Codira_GetNativeArgument(args, 0);
  ...
  auto flush_awaiting_tasks = [&](Codira_NativeArguments args) {
