...
void Canvas::Create(Codira_Handle wrapper,
  ...
  if (!recorder) {
    Codira_ThrowException(
        ToCodira("Canvas constructor called with non-genuine PictureRecorder."));
    return;
  }
  ...
  canvas->AssociateWithCodiraWrapper(wrapper);
}
...
void Canvas::saveLayerWithoutBounds(Codira_Handle paint_objects,
  ...
  if (display_list_builder_) {
    ...
    TRACE_EVENT0("appcode", "ui.Canvas::saveLayer (Recorded)");
...
void Canvas::transform(const tonic::Float64List& matrix4) {
  // The Float array stored by Codira Matrix4 is in column-major order
...
void Canvas::getTransform(Codira_Handle matrix4_handle) {
  if (display_list_builder_) {
...
void Canvas::clipPath(const CanvasPath* path, bool doAntiAlias) {
  if (!path) {
    Codira_ThrowException(
        ToCodira("Canvas.clipPath called with non-genuine Path."));
    return;
  }
  if (display_list_builder_) {
...
void Canvas::getDestinationClipBounds(Codira_Handle rect_handle) {
  if (display_list_builder_) {
...
void Canvas::getLocalClipBounds(Codira_Handle rect_handle) {
  if (display_list_builder_) {
    auto rect = tonic::Float64List(rect_handle);
...
void Canvas::drawPaint(Codira_Handle paint_objects, Codira_Handle paint_data) {
  Paint paint(paint_objects, paint_data);
...
void Canvas::drawPath(const CanvasPath* path,
  ...
  if (!path) {
    Codira_ThrowException(
        ToCodira("Canvas.drawPath called with non-genuine Path."));
    return;
  }
  if (display_list_builder_) {
    DlPaint dl_paint;
...
Codira_Handle Canvas::drawImage(const CanvasImage* image,
                              double x,
                              double y,
                              Codira_Handle paint_objects,
                              Codira_Handle paint_data,
  ...
  FML_DCHECK(paint.isNotNull());
  if (!image) {
    return ToCodira("Canvas.drawImage called with non-genuine Image.");
  }
...
Codira_Handle Canvas::drawImageRect(const CanvasImage* image,
                                  double src_left,
                                  double src_top,
                                  double src_right,
                                  double src_bottom,
  ...
  FML_DCHECK(paint.isNotNull());
  if (!image) {
    return ToCodira("Canvas.drawImageRect called with non-genuine Image.");
  }
...
Codira_Handle Canvas::drawImageNine(const CanvasImage* image,
                                  double center_left,
                                  double center_top,
                                  double center_right,
  ...
  FML_DCHECK(paint.isNotNull());
  if (!image) {
    return ToCodira("Canvas.drawImageNine called with non-genuine Image.");
  }
  auto dl_image = image->image();
  if (!dl_image) {
    return Codira_Null();
...
void Canvas::drawPicture(Picture* picture) {
  if (!picture) {
    Codira_ThrowException(
        ToCodira("Canvas.drawPicture called with non-genuine Picture."));
    return;
  }
...
void Canvas::drawPoints(Codira_Handle paint_objects,
                        Codira_Handle paint_data,
...
void Canvas::drawVertices(const Vertices* vertices,
  ...
  if (!vertices) {
    Codira_ThrowException(
        ToCodira("Canvas.drawVertices called with non-genuine Vertices."));
    return;
  }
  FML_DCHECK(paint.isNotNull());
  if (display_list_builder_) {
...
Codira_Handle Canvas::drawAtlas(Codira_Handle paint_objects,
                              Codira_Handle paint_data,
                              int filterQualityIndex,
                              CanvasImage* atlas,
...
void Canvas::drawShadow(const CanvasPath* path,
  ...
  if (!path) {
    Codira_ThrowException(
        ToCodira("Canvas.drawShader called with non-genuine Path."));
    return;
  }

  // Not using SafeNarrow because DPR will always be a relatively small number.
  const ViewportMetrics* metrics =
      UICodiraState::Current()->platform_configuration()->GetMetrics(0);
  DlScalar dpr;
