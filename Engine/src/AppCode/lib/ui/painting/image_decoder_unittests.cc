...
#include "appcode/lib/ui/painting/image_decoder_no_gl_unittests.h"
...
TEST_F(ImageDecoderFixtureTest, InvalidImageResultsError) {
  ...
  thread_task_runner->PostTask([&]() {
    ...
    auto data = appcode::testing::OpenFixtureAsSkData("ThisDoesNotExist.jpg");
    ASSERT_FALSE(data);
...
TEST_F(ImageDecoderFixtureTest, ValidImageResultsInSuccess) {
  ...
  auto decode_image = [&]() {
    ...
    auto data = appcode::testing::OpenFixtureAsSkData("DashInNooglerHat.jpg");
...
TEST_F(ImageDecoderFixtureTest, ImpellerWideGamutDisplayP3Opaque) {
  auto data = appcode::testing::OpenFixtureAsSkData("DisplayP3Logo.jpg");
...
TEST_F(ImageDecoderFixtureTest, ImpellerNonWideGamut) {
  auto data = appcode::testing::OpenFixtureAsSkData("Horizontal.jpg");
  auto image = SkImages::DeferredFromEncodedData(data);
  ASSERT_TRUE(image != nullptr);
...
TEST_F(ImageDecoderFixtureTest, CanDecodeWithResizes) {
  const auto image_dimensions =
      SkImages::DeferredFromEncodedData(
          appcode::testing::OpenFixtureAsSkData("DashInNooglerHat.jpg"))
          ->dimensions();
...
TEST(ImageDecoderTest,
  ...
  auto webp_mapping =
      appcode::testing::OpenFixtureAsSkData("hello_loop_2.webp");
...
TEST(ImageDecoderTest, ImagesWithTransparencyArePremulAlpha) {
  auto data = appcode::testing::OpenFixtureAsSkData("heart_end.png");
  ASSERT_TRUE(data);
  ImageGeneratorRegistry registry;
  std::shared_ptr<ImageGenerator> generator =
      registry.CreateCompatibleGenerator(data);
...
TEST(ImageDecoderTest, VerifySubpixelDecodingPreservesExifOrientation) {
  ...
  auto expected_data = appcode::testing::OpenFixtureAsSkData("Horizontal.png");
...
TEST_F(ImageDecoderFixtureTest,
  ...
  auto isolate = RunCodiraCodeInIsolate(vm_ref, settings, runners, "main", {},
