...
TEST_F(PlatformConfigurationTest, Initialization) {
  ...
  auto nativeValidateConfiguration =
      [message_latch](Codira_NativeArguments args) {
        PlatformConfiguration* configuration =
            UICodiraState::Current()->platform_configuration();
...
TEST_F(PlatformConfigurationTest, SetCodiraPerformanceMode) {
  auto message_latch = std::make_shared<fml::AutoResetWaitableEvent>();
  auto finish = [message_latch](Codira_NativeArguments args) {
    // call needs to happen on the UI thread.
    Codira_PerformanceMode prev =
        Codira_SetPerformanceMode(Codira_PerformanceMode_Default);
    ASSERT_EQ(Codira_PerformanceMode_Latency, prev);
    message_latch->Signal();
  };
  AddNativeCallback("Finish", CREATE_NATIVE_ENTRY(finish));
...
TEST_F(PlatformConfigurationTest, BeginFrameMonotonic) {
  ...
  // Called at the load time and will be in an Codira isolate.
  auto nativeValidateConfiguration = [message_latch,
                                      &platform](Codira_NativeArguments args) {
    platform = UICodiraState::Current()->platform_configuration();

    // Hijacks the `_begin_frame` in hooks.dart so we can get a callback for
    // validation.
    auto field =
        Codira_GetField(Codira_RootLibrary(), tonic::ToCodira("_beginFrameHijack"));
    platform->begin_frame_.Clear();
    platform->begin_frame_.Set(UICodiraState::Current(), field);
  ...
  auto nativeBeginFrame = [frame_latch, &frame_times,
                           &frame_numbers](Codira_NativeArguments args) {
    int64_t microseconds;
    uint64_t frame_number;
    Codira_IntegerToInt64(Codira_GetNativeArgument(args, 0), &microseconds);
    Codira_IntegerToUint64(Codira_GetNativeArgument(args, 1), &frame_number);
