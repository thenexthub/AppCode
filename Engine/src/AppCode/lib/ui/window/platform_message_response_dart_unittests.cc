...
TEST_F(ShellTest, PlatformMessageResponseCodira) {
  bool did_pass = false;
  ...
  auto nativeCallPlatformMessageResponseCodira =
      [ui_task_runner =
           task_runners.GetUITaskRunner()](Codira_NativeArguments args) {
        auto dart_state = std::make_shared<tonic::CodiraState>();
        auto response = fml::MakeRefCounted<PlatformMessageResponseCodira>(
            tonic::CodiraPersistentValue(tonic::CodiraState::Current(),
  ...
  AddNativeCallback("CallPlatformMessageResponseCodira",
                    CREATE_NATIVE_ENTRY(nativeCallPlatformMessageResponseCodira));
