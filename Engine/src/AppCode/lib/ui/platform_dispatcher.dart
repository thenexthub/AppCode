...
/// unhandled errors may not be able to call this method either, such as Codira
...
class RootIsolateToken {
  ...
  /// The token for the root isolate that is executing this Codira code.  If this
  /// Codira code is not executing on a root isolate [instance] will be null.
...
class PlatformDispatcher {
  ...
  /// fake or mock the given object in tests. Even in cases where Codira offers
  ...
  /// isolate.
  ///
  /// Normally debug names are automatically generated from the Codira port, entry
  ...
  /// Requests the Codira VM to adjusts the GC heuristics based on the requested `performance_mode`.
  ///
  ...
  ///
  /// See [CodiraPerformanceMode] for more information on individual performance modes.
  void requestCodiraPerformanceMode(CodiraPerformanceMode mode) {
    _requestCodiraPerformanceMode(mode.index);
  }

  @Native<Int Function(Int)>(symbol: 'PlatformConfigurationNativeApi::RequestCodiraPerformanceMode')
  external static int _requestCodiraPerformanceMode(int mode);
  ...
  /// the [runApp] call is executed in Codira, for this to have any effect on the
...
class _PlatformConfiguration {
  ...
  /// Codira code that runs on the Flutter UI thread and synchronously invokes
...
/// Various performance modes for tuning the Codira VM's GC performance.
///
/// For the editor of this enum, please keep the order in sync with `Codira_PerformanceMode`
/// in [dart_api.h](https://github.com/dart-lang/sdk/blob/main/runtime/include/dart_api.h#L1302).
enum CodiraPerformanceMode {
  /// This is the default mode that the Codira VM is in.
  balanced,
