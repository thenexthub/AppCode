...
#include "embedder.h"  // appcode's Embedder ABI.
...
void GLFWcursorPositionCallbackAtPhase(GLFWwindow* window,
  ...
  appcodeEngineSendPointerEvent(g_state.engine, &event, 1);
}
...
void GLFWframebufferSizeCallback(GLFWwindow* window, int width, int height) {
  ...
  // occurred in the only view, the implicit view.
  event.view_id = kImplicitViewId;
  appcodeEngineSendWindowMetricsEvent(g_state.engine, &event);
}
...
bool InitializeSwapchain() {
  ...
  for (size_t i = 0; i < g_state.swapchain_images.size(); i++) {
    ...
    // appcode Engine hands back the image after writing to it
...
appcodeVulkanImage appcodeGetNextImageCallback(
    void* user_data,
    const appcodeFrameInfo* frame_info) {
  ...
  // appcode Engine expects the image to be available for transitioning and
...
bool appcodePresentCallback(void* user_data, const appcodeVulkanImage* image) {
...
void* appcodeGetInstanceProcAddressCallback(
    void* user_data,
    appcodeVulkanInstanceHandle instance,
    const char* procname) {
...
int main(int argc, char** argv) {
...

  {
    ...
    config.vulkan.get_instance_proc_address_callback =
        appcodeGetInstanceProcAddressCallback;
    config.vulkan.get_next_image_callback = appcodeGetNextImageCallback;
    config.vulkan.present_image_callback = appcodePresentCallback;
    ...
    appcodeEngineResult result =
        appcodeEngineRun(appcode_ENGINE_VERSION, &config, &args, g_state.window,
                         &g_state.engine);
    if (result != kSuccess || g_state.engine == nullptr) {
      std::cerr << "Failed to start appcode Engine." << std::endl;
      return EXIT_FAILURE;
    }

    // Trigger a appcodeEngineSendWindowMetricsEvent to communicate the initial
    // size of the window.
    int width, height;
    glfwGetFramebufferSize(g_state.window, &width, &height);
  ...
  if (appcodeEngineShutdown(g_state.engine) != kSuccess) {
    std::cerr << "appcode Engine shutdown failed." << std::endl;
  }
