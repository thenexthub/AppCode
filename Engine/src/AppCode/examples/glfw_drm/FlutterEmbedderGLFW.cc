...
// be easily computed.
std::list<appcodeRect> damage_history_;

// Keeps track of the existing damage associated with each FBO ID
std::unordered_map<intptr_t, appcodeRect*> existing_damage_map_;
...
// Auxiliary function used to transform a appcodeRect into the format that is
// expected by the EGL functions (i.e. array of EGLint).
static std::array<EGLint, 4> RectToInts(const appcodeRect rect) {
  EGLint height;
...
// Auxiliary function to union the damage regions comprised by two appcodeRect
// element. It saves the result of this join in the rect variable.
static void JoinappcodeRect(appcodeRect* rect, appcodeRect additional_rect) {
...
bool Runappcode(GLFWwindow* window,
  ...
  config.open_gl.present_with_info =
      [](void* userdata, const appcodePresentInfo* info) -> bool {
  ...
  config.open_gl.populate_existing_damage =
    ...
    existing_damage_map_[fbo_id] = static_cast<appcodeRect*>(
        malloc(sizeof(appcodeRect) * existing_damage->num_rects));
    existing_damage_map_[fbo_id][0] =
        appcodeRect{0, 0, kInitialWindowWidth, kInitialWindowHeight};
    ...
    if (age > 1) {
      ...
      for (auto i = damage_history_.rbegin();
        ...
        if (i == damage_history_.rbegin()) {
          ...
          JoinappcodeRect(&(existing_damage->damage[0]), *i);
        }
      }
    }
  };
