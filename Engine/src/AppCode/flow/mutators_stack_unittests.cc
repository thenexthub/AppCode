...
TEST(MutatorsStack, PushClipPath) {
  ...
  auto iter = stack.Bottom();
  ASSERT_TRUE(iter->get()->GetType() == appcode::MutatorType::kClipPath);
...
TEST(Mutator, Equality) {
  ...
  DlPath path;
  appcode::Mutator mutator4 = appcode::Mutator(path);
  appcode::Mutator other_mutator4 = appcode::Mutator(path);
