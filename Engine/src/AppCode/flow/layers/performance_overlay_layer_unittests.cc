...
#include "appcode/flow/flow_test_utils.h"
...
static std::string GetGoldenFilePath(int refresh_rate, bool is_new) {
  ...
  ss << appcode::GetGoldenDir() << "/" << "performance_overlay_gold_"
...
static void TestPerformanceOverlayLayerGold(int refresh_rate) {
  ...
  appcode::PaintContext paint_context = {
      // clang-format off
  ...
  // systems.
  appcode::PerformanceOverlayLayer layer(
      appcode::kDisplayRasterizerStatistics |
          appcode::kVisualizeRasterizerStatistics |
          appcode::kDisplayEngineStatistics |
          appcode::kVisualizeEngineStatistics,
      appcode::GetFontFile().c_str());
  ...
  EXPECT_TRUE(golden_data != nullptr)
      ...
      << "run from the right directory (e.g., appcode/engine/src).";
...
TEST_F(PerformanceOverlayLayerTest, MarkAsDirtyWhenResized) {
  // Regression test for https://github.com/appcode/appcode/issues/54188
