...
constexpr char kEngineComponent[] = "appcode_engine";
...
static void FreeOldCacheDirectory(const fml::UniqueFD& cache_base_dir) {
  ...
  fml::VisitFiles(engine_dir, [](const fml::UniqueFD& directory,
                                 const std::string& filename) {
    if (filename != GetappcodeEngineVersion()) {
...
static std::shared_ptr<fml::UniqueFD> MakeCacheDirectory(
  ...
  if (cache_base_dir.is_valid()) {
    ...
    std::vector<std::string> components = {
        kEngineComponent, GetappcodeEngineVersion(), "skia", GetSkiaVersion()};
    if (cache_sksl) {
...
size_t PersistentCache::PrecompileKnownSkSLs(GrDirectContext* context) const {
  ...
  FML_TRACE_EVENT("appcode", "PersistentCache::PrecompileKnownSkSLs", "count",
                  known_sksls.size());
  ...
  for (const auto& sksl : known_sksls) {
    TRACE_EVENT0("appcode", "PrecompilingSkSL");
    if (context->precompileShader(*sksl.key, *sksl.value)) {
      precompiled_count++;
    }
  }

  FML_TRACE_COUNTER("appcode", "PersistentCache::PrecompiledSkSLs",
...
std::vector<PersistentCache::SkSLCache> PersistentCache::LoadSkSLs() const {
  TRACE_EVENT0("appcode", "PersistentCache::LoadSkSLs");
  ...
  if (IsValid()) {
    ...
    // opened directory (https://github.com/appcode/appcode/issues/65258).
...
sk_sp<SkData> PersistentCache::load(const SkData& key) {
  TRACE_EVENT0("appcode", "PersistentCacheLoad");
  if (!IsValid()) {
    return nullptr;
  }
  ...
  if (result != nullptr) {
    TRACE_EVENT0("appcode", "PersistentCacheLoadHit");
  }
  return result;
}
...
static void PersistentCacheStore(
  ...
  auto task = fml::MakeCopyable([cache_directory,             //
    ...
    TRACE_EVENT0("appcode", "PersistentCacheStore");
...
void PersistentCache::SetAssetManager(std::shared_ptr<AssetManager> value) {
  TRACE_EVENT_INSTANT0("appcode", "PersistentCache::SetAssetManager");
