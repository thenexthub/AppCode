...
#ifndef APPCODE_COMMON_SETTINGS_H_
#define APPCODE_COMMON_SETTINGS_H_
...
// TODO(https://github.com/appcode/appcode/issues/138750): Re-order fields to
// reduce padding.
...
struct Settings {
  ...
  // Path to a library containing the application's compiled Codira code.
  ...
  // Path to a library containing compiled Codira code usable for launching
  ...
  // Whether the Codira VM service should be enabled.
  ...
  // The IP address to which the Codira VM service is bound.
  std::string vm_service_host;

  // The port to which the Codira VM service is bound. When set to `0`, a free
  ...
  // https://github.com/appcode/appcode/issues/95903
  bool leak_vm = true;
  ...
  // to perform native Codira bindings for libraries not built in.
  std::function<void(const CodiraIsolate&)> root_isolate_create_callback;
  // TODO(68738): Update isolate callbacks in settings to accept an additional
  // CodiraIsolate parameter.
...
#endif  // APPCODE_COMMON_SETTINGS_H_
