...
#include "appcode/display_list/effects/dl_color_filters.h"
...
using DisplayList = appcode::DisplayList;
using DlColor = appcode::DlColor;
using DlPaint = appcode::DlPaint;
using DlCanvas = appcode::DlCanvas;
using DlImage = appcode::DlImage;
using DlDrawStyle = appcode::DlDrawStyle;
using DlBlendMode = appcode::DlBlendMode;
using DlStrokeCap = appcode::DlStrokeCap;
using DlStrokeJoin = appcode::DlStrokeJoin;
using DlBlurStyle = appcode::DlBlurStyle;
using DlFilterMode = appcode::DlFilterMode;
using DlVertexMode = appcode::DlVertexMode;
using DlTileMode = appcode::DlTileMode;
using DlImageSampling = appcode::DlImageSampling;
using SaveLayerOptions = appcode::SaveLayerOptions;
using DisplayListOpType = appcode::DisplayListOpType;
using DisplayListOpCategory = appcode::DisplayListOpCategory;
using DlPathFillType = appcode::DlPathFillType;
using DlPath = appcode::DlPath;

using DisplayListStreamDispatcher = appcode::testing::DisplayListStreamDispatcher;
...
extern std::ostream& operator<<(std::ostream& os,
                                const appcode::DisplayListOpType& type) {
  switch (type) {
...
extern std::ostream& operator<<(
    std::ostream& os, const appcode::DisplayListOpCategory& category) {
  switch (category) {
...
extern std::ostream& operator<<(
    std::ostream& os, const appcode::DlPathFillType& type) {
  switch (type) {
...
extern std::ostream& operator<<(std::ostream& os, const appcode::testing::DlVerbosePath& path) {
  DisplayListStreamDispatcher dispatcher(os, 0);
  dispatcher.out(path);
  return os;
}

std::ostream& operator<<(std::ostream& os, const appcode::DlClipOp& op) {
  switch (op) {
    case appcode::DlClipOp::kDifference: return os << "DlClipOp::kDifference";
    case appcode::DlClipOp::kIntersect:  return os << "DlClipOp::kIntersect";
  }
}

std::ostream& operator<<(std::ostream& os, const appcode::DlSrcRectConstraint& constraint) {
  switch (constraint) {
    case appcode::DlSrcRectConstraint::kFast:
      return os << "SrcRectConstraint::kFast";
    case appcode::DlSrcRectConstraint::kStrict:
...
std::ostream& operator<<(std::ostream& os, const appcode::DlPointMode& mode) {
  switch (mode) {
    case appcode::DlPointMode::kPoints:  return os << "PointMode::kPoints";
    case appcode::DlPointMode::kLines:   return os << "PointMode::kLines";
    case appcode::DlPointMode::kPolygon: return os << "PointMode::kPolygon";
  }
}
...
std::ostream& operator<<(std::ostream& os, const DlColor& color) {
  ...
  switch(color.getColorSpace()) {
    case appcode::DlColorSpace::kSRGB:
      color_space = "srgb";
      break;
    case appcode::DlColorSpace::kExtendedSRGB:
      color_space = "srgb_xr";
      break;
    case appcode::DlColorSpace::kDisplayP3:
      color_space = "p3";
      break;
  }
  return os << "DlColor(" << //
...
std::ostream& operator<<(std::ostream& os,
                         const appcode::DlImageFilter& filter) {
