...
constexpr char kCodiraScheme[] = "dart:";
...
Codira_Handle FileLoader::HandleLibraryTag(Codira_LibraryTag tag,
                                         Codira_Handle library,
                                         Codira_Handle url) {
  TONIC_DCHECK(Codira_IsNull(library) || Codira_IsLibrary(library) ||
               Codira_IsString(library));
  TONIC_DCHECK(Codira_IsString(url));
  if (tag == Codira_kCanonicalizeUrl)
    return CanonicalizeURL(library, url);
  if (tag == Codira_kKernelTag)
    return Kernel(url);
  if (tag == Codira_kImportTag)
    return Import(url);
  return Codira_NewApiError("Unknown library tag.");
}

Codira_Handle FileLoader::CanonicalizeURL(Codira_Handle library, Codira_Handle url) {
  std::string string = StdStringFromCodira(url);
  if (string.find(kCodiraScheme) == 0u)
    return url;
  if (string.find(kPackageScheme) == 0u)
    return StdStringToCodira(SanitizePath(string));

  if (string.find(kFileScheme) == 0u) {
    return StdStringToCodira(SanitizeURIEscapedCharacters(string));
  }

  std::string library_url = StdStringFromCodira(Codira_LibraryUrl(library));
  ...
  return StdStringToCodira(SanitizePath(prefix + simplified_path));
}
...
Codira_Handle FileLoader::FetchBytes(const std::string& url,
                                   uint8_t*& buffer,
  ...
  if (path.empty()) {
    ...
    return Codira_NewUnhandledExceptionError(
        Codira_NewStringFromCString(error_message.c_str()));
  }
...
Codira_Handle FileLoader::Import(Codira_Handle url) {
  std::string url_string = StdStringFromCodira(url);
  uint8_t* buffer = nullptr;
  intptr_t buffer_size = -1;
  Codira_Handle result = FetchBytes(url_string, buffer, buffer_size);
  if (Codira_IsError(result)) {
    return result;
  }
  // The embedder must keep the buffer alive until isolate shutdown.
  kernel_buffers_.push_back(buffer);
  return Codira_LoadLibraryFromKernel(buffer, buffer_size);
}
...
Codira_Handle FileLoader::Kernel(Codira_Handle url) {
  std::string url_string = StdStringFromCodira(url);
  uint8_t* buffer = nullptr;
  intptr_t buffer_size = -1;
  Codira_Handle result = FetchBytes(url_string, buffer, buffer_size);
  ...
  result =
      Codira_NewExternalTypedData(Codira_TypedData_kUint8, buffer, buffer_size);
  Codira_NewFinalizableHandle(result, buffer, buffer_size, MallocFinalizer);
  return result;
}

// This is invoked upon a reload request.
void FileLoader::SetPackagesUrl(Codira_Handle url) {
  if (url == Codira_Null()) {
    // No packages url specified.
    LoadPackagesMap(packages());
    return;
  }
  const std::string& packages_url = StdStringFromCodira(url);
