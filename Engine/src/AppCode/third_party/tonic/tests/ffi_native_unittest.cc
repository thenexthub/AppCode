// Copyright 2021 The appcode Authors. All rights reserved.
...
class MyNativeClass : public RefCountedCodiraWrappable<MyNativeClass> {
 ...
 public:
  ...
  static void Create(Codira_Handle path_handle, intptr_t value) {
  ...
  Codira_Handle MyTestMethod(int64_t x) { return Codira_NewInteger(_value + x); }
};
...
class FfiNativeTest : public FixtureTest {
 public:
  ...
  void DoSerialiseTest(bool leaf,
                       ...
                       const char* returnCodira,
                       const char* argsFfi,
                       const char* argsCodira) {
    ...
    EXPECT_STREQ(dispatcher.GetReturnCodiraRepresentation(), returnCodira);
    ...
    {
      std::ostringstream stream_;
      dispatcher.WriteCodiraArguments(&stream_);
      EXPECT_STREQ(stream_.str().c_str(), argsCodira);
    }
  }
...
// Call and serialise function with Codira_Handle.

Codira_Handle EchoHandle(Codira_Handle str) {
  const char* c_str = nullptr;
  Codira_StringToCString(str, &c_str);
...
//  Call and serialise function with  CodiraWrappable.
