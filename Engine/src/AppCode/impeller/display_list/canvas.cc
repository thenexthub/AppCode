...
bool IsPipelineBlendOrMatrixFilter(const appcode::DlColorFilter* filter) {
  return filter->type() == appcode::DlColorFilterType::kMatrix ||
         (filter->type() == appcode::DlColorFilterType::kBlend &&
...
void Canvas::DrawPath(const appcode::DlPath& path, const Paint& paint) {
  Entity entity;
...
bool Canvas::AttemptColorFilterOptimization(
  ...
  if (paint.color_filter->type() == appcode::DlColorFilterType::kBlend) {
    const appcode::DlBlendColorFilter* blend_filter =
    ...
    const appcode::DlMatrixColorFilter* matrix_filter =
...
void Canvas::DrawOval(const Rect& rect, const Paint& paint) {
  ...
  if (paint.style == Paint::Style::kStroke) {
    ...
    DrawPath(appcode::DlPath(PathBuilder{}.AddOval(rect)), paint);
    return;
  }
...
void Canvas::DrawRoundRect(const RoundRect& round_rect, const Paint& paint) {
  ...
  auto path = PathBuilder{}
                  ...
                  .SetBounds(rect)
                  .TakePath();
  DrawPath(appcode::DlPath(path), paint);
}
...
void Canvas::DrawRoundSuperellipse(const RoundSuperellipse& rse,
  ...
  if (paint.style == Paint::Style::kFill) {
    ...
    // https://github.com/appcode/appcode/issues/163893
    Entity entity;
    entity.SetTransform(GetCurrentTransform());
    entity.SetBlendMode(paint.blend_mode);
...
void Canvas::ClipGeometry(const Geometry& geometry,
  ...
  // See https://github.com/appcode/appcode/issues/147021
...
void Canvas::DrawVertices(const std::shared_ptr<VerticesGeometry>& vertices,
  ...
  // is enabled when the appcode engine builds Skia.
  if (!paint.color_source) {
  ...
  // source to a texture.
  if (paint.color_source &&
      paint.color_source->type() == appcode::DlColorSourceType::kImage) {
    const appcode::DlImageColorSource* image_color_source =
...
void Canvas::SaveLayer(const Paint& paint,
                       ...
                       const appcode::DlImageFilter* backdrop_filter,
                       ContentBoundsPromise bounds_promise,
                       uint32_t total_content_depth,
                       bool can_distribute_opacity,
                       std::optional<int64_t> backdrop_id) {
  TRACE_EVENT0("appcode", "Canvas::saveLayer");
  if (IsSkipping()) {
    return SkipUntilMatchingRestore(total_content_depth);
  }
  ...
  // not cut off.
  //
  // See also this bug: https://github.com/appcode/appcode/issues/144213
  //
...
bool Canvas::AttemptBlurredTextOptimization(
  ...
  //              mask filters.
  //              https://github.com/appcode/appcode/issues/133297
...
void Canvas::DrawTextFrame(const std::shared_ptr<TextFrame>& text_frame,
  ...
  if (max_scale * text_frame->GetFont().GetMetrics().point_size >
      kMaxTextScale) {
    fml::StatusOr<appcode::DlPath> path = text_frame->GetPath();
    if (path.ok()) {
      Save(1);
