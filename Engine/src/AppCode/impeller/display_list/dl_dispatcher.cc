...
static impeller::SamplerDescriptor ToSamplerDescriptor(
    const appcode::DlFilterMode options) {
  impeller::SamplerDescriptor desc;
  switch (options) {
    case appcode::DlFilterMode::kNearest:
      desc.min_filter = desc.mag_filter = impeller::MinMagFilter::kNearest;
      desc.label = "Nearest Sampler";
      break;
    case appcode::DlFilterMode::kLinear:
...
static std::optional<const Rect> ToOptRect(const appcode::DlRect* rect) {
  if (rect == nullptr) {
    return std::nullopt;
  }
  return *rect;
...
static Paint::Style ToStyle(appcode::DlDrawStyle style) {
  switch (style) {
    case appcode::DlDrawStyle::kFill:
      return Paint::Style::kFill;
    case appcode::DlDrawStyle::kStroke:
      return Paint::Style::kStroke;
    case appcode::DlDrawStyle::kStrokeAndFill:
      UNIMPLEMENTED;
      break;
  }
  return Paint::Style::kFill;
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::setDrawStyle(appcode::DlDrawStyle style) {
  AUTO_DEPTH_WATCHER(0u);
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::setColor(appcode::DlColor color) {
  AUTO_DEPTH_WATCHER(0u);
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::setStrokeCap(appcode::DlStrokeCap cap) {
  AUTO_DEPTH_WATCHER(0u);

  switch (cap) {
    case appcode::DlStrokeCap::kButt:
      paint_.stroke_cap = Cap::kButt;
      break;
    case appcode::DlStrokeCap::kRound:
      paint_.stroke_cap = Cap::kRound;
      break;
    case appcode::DlStrokeCap::kSquare:
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::setStrokeJoin(appcode::DlStrokeJoin join) {
  AUTO_DEPTH_WATCHER(0u);

  switch (join) {
    case appcode::DlStrokeJoin::kMiter:
      paint_.stroke_join = Join::kMiter;
      break;
    case appcode::DlStrokeJoin::kRound:
      paint_.stroke_join = Join::kRound;
      break;
    case appcode::DlStrokeJoin::kBevel:
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::setColorSource(const appcode::DlColorSource* source) {
  AUTO_DEPTH_WATCHER(0u);
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::setColorFilter(const appcode::DlColorFilter* filter) {
  AUTO_DEPTH_WATCHER(0u);
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::setBlendMode(appcode::DlBlendMode dl_mode) {
  AUTO_DEPTH_WATCHER(0u);

  paint_.blend_mode = dl_mode;
}

static FilterContents::BlurStyle ToBlurStyle(appcode::DlBlurStyle blur_style) {
  switch (blur_style) {
    case appcode::DlBlurStyle::kNormal:
      return FilterContents::BlurStyle::kNormal;
    case appcode::DlBlurStyle::kSolid:
      return FilterContents::BlurStyle::kSolid;
    case appcode::DlBlurStyle::kOuter:
      return FilterContents::BlurStyle::kOuter;
    case appcode::DlBlurStyle::kInner:
      return FilterContents::BlurStyle::kInner;
  }
}

// |appcode::DlOpReceiver|
void DlDispatcherBase::setMaskFilter(const appcode::DlMaskFilter* filter) {
  AUTO_DEPTH_WATCHER(0u);

  // Needs https://github.com/appcode/appcode/issues/95434
  if (filter == nullptr) {
    paint_.mask_blur_descriptor = std::nullopt;
    return;
  }
  switch (filter->type()) {
    case appcode::DlMaskFilterType::kBlur: {
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::setImageFilter(const appcode::DlImageFilter* filter) {
  AUTO_DEPTH_WATCHER(0u);
...
static Entity::ClipOperation ToClipOperation(appcode::DlClipOp clip_op) {
  switch (clip_op) {
    case appcode::DlClipOp::kDifference:
      return Entity::ClipOperation::kDifference;
    case appcode::DlClipOp::kIntersect:
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::drawColor(appcode::DlColor color,
                                 appcode::DlBlendMode dl_mode) {
  AUTO_DEPTH_WATCHER(1u);
...
void DlDispatcherBase::drawArc(const DlRect& oval_bounds,
  ...
  if (paint_.stroke_width >
    ...
    // https://github.com/appcode/appcode/issues/158567
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::drawPoints(appcode::DlPointMode mode,
                                  uint32_t count,
  ...
  switch (mode) {
    case appcode::DlPointMode::kPoints: {
    ...
    case appcode::DlPointMode::kLines:
      ...
      break;
    case appcode::DlPointMode::kPolygon:
      if (count > 1) {
        Point p0 = points[0];
...
void DlDispatcherBase::drawVertices(
    const std::shared_ptr<appcode::DlVertices>& vertices,
    appcode::DlBlendMode dl_mode) {}

// |appcode::DlOpReceiver|
void DlDispatcherBase::drawImage(const sk_sp<appcode::DlImage> image,
                                 const DlPoint& point,
  ...
  drawImageRect(image,                                 // image
                ...
                appcode::DlSrcRectConstraint::kStrict  // constraint
  );
}

// |appcode::DlOpReceiver|
void DlDispatcherBase::drawImageRect(const sk_sp<appcode::DlImage> image,
                                     const DlRect& src,
                                     const DlRect& dst,
                                     appcode::DlImageSampling sampling,
                                     bool render_with_attributes,
                                     appcode::DlSrcRectConstraint constraint =
                                         appcode::DlSrcRectConstraint::kFast) {
  AUTO_DEPTH_WATCHER(1u);
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::drawImageNine(const sk_sp<appcode::DlImage> image,
...
// |appcode::DlOpReceiver|
void DlDispatcherBase::drawAtlas(const sk_sp<appcode::DlImage> atlas,
...
void FirstPassDispatcher::saveLayer(const DlRect& bounds,
  ...
  if (backdrop != nullptr && backdrop_id.has_value()) {
    std::shared_ptr<appcode::DlImageFilter> shared_backdrop =
        backdrop->shared();
...
// |appcode::DlOpReceiver|
void FirstPassDispatcher::setDrawStyle(appcode::DlDrawStyle style) {
  paint_.style = ToStyle(style);
}

// |appcode::DlOpReceiver|
void FirstPassDispatcher::setColor(appcode::DlColor color) {
  paint_.color = skia_conversions::ToColor(color);
}
...
// |appcode::DlOpReceiver|
void FirstPassDispatcher::setStrokeCap(appcode::DlStrokeCap cap) {
  switch (cap) {
    case appcode::DlStrokeCap::kButt:
      paint_.stroke_cap = Cap::kButt;
      break;
...
// |appcode::DlOpReceiver|
void FirstPassDispatcher::setStrokeJoin(appcode::DlStrokeJoin join) {
  switch (join) {
    case appcode::DlStrokeJoin::kMiter:
      paint_.stroke_join = Join::kMiter;
      break;
...
// |appcode::DlOpReceiver|
void FirstPassDispatcher::setImageFilter(const appcode::DlImageFilter* filter) {
  if (filter == nullptr) {
    has_image_filter_ = false;
  } else {
