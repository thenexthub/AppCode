...
TEST_P(TypographerTest, CanConvertTextBlob) {
  SkFont font = appcode::testing::CreateTestFontOfSize(12);
...
TEST_P(TypographerTest, CanCreateGlyphAtlas) {
  ...
  ASSERT_TRUE(context && context->IsValid());
  SkFont sk_font = appcode::testing::CreateTestFontOfSize(12);
...
TEST_P(TypographerTest, LazyAtlasTracksColor) {
  auto host_buffer = HostBuffer::Create(
      ...
      GetContext()->GetCapabilities()->GetMinimumUniformAlignment());
#if FML_OS_MACOSX
  auto mapping = appcode::testing::OpenFixtureAsSkData("Apple Color Emoji.ttc");
#else
  auto mapping = appcode::testing::OpenFixtureAsSkData("NotoColorEmoji.ttf");
#endif
  ASSERT_TRUE(mapping);
...
TEST_P(TypographerTest, RectanglePackerAddsNonoverlapingRectangles) {
  ...
  ASSERT_TRUE(appcode::testing::NumberNear(packer->PercentFull(), 0.02));
  ...
  ASSERT_TRUE(appcode::testing::NumberNear(packer->PercentFull(), 0.65));
...
TEST_P(TypographerTest, GlyphAtlasTextureWillGrowTilMaxTextureSize) {
  ...
  SkFont sk_font_small = appcode::testing::CreateTestFontOfSize(10);
  ...
  for (int i = 0; i < 13; i++) {
    ...
    SkFont sk_font = appcode::testing::CreateTestFontOfSize(50 + i);
    add_char(sk_font, 'A');
