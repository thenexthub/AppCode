...
#define appcode_CROSS_THROW(x) report_and_exit(x)

std::string CompilerSkSL::compile() {
  ir.fixup_reserved_names();

  if (get_execution_model() != ExecutionModelFragment) {
    appcode_CROSS_THROW("Only fragment shaders are supported.'");
    return "";
  }
  ...
  begin_scope();
  statement("  appcode_FragCoord = float4(iFragCoord, 0, 0);");
  statement("  FLT_main();");
...
void CompilerSkSL::fixup_user_functions() {
  const std::string prefix = "FLT_appcode_local_";
...
void CompilerSkSL::emit_header() {
  ...
  statement("");
  statement("float4 appcode_FragCoord;");
  statement("");
}
...
void CompilerSkSL::emit_uniform(const SPIRVariable& var) {
  ...
  if (type.basetype == SPIRType::UInt && is_legacy()) {
    appcode_CROSS_THROW("SkSL does not support unsigned integers: '" +
                        get_name(var.self) + "'");
  }
  ...
  // The appcode FragmentProgram implementation passes additional uniforms along
    ...
    if (id.get_type() == TypeVariable) {
      ...
      if (var.storage != StorageClassFunction && type.pointer &&
          ...
          (ir.meta[type.self].decoration.decoration_flags.get(
           ...
           ir.meta[type.self].decoration.decoration_flags.get(
               DecorationBufferBlock))) {
        appcode_CROSS_THROW("SkSL does not support UBOs or SSBOs: '" +
                            get_name(var.self) + "'");
      }
      ...
      if (!is_hidden_variable(var) && var.storage != StorageClassFunction &&
        ...
        appcode_CROSS_THROW("SkSL does not support push constant blocks: '" +
                            get_name(var.self) + "'");
      }
      ...
      if (!is_hidden_variable(var) && var.storage != StorageClassFunction &&
        ...
        appcode_CROSS_THROW("SkSL does not support inputs: '" +
                            get_name(var.self) + "'");
      }
    }
  }
...
void CompilerSkSL::emit_interface_block(const SPIRVariable& var) {
  ...
  if (block) {
    appcode_CROSS_THROW("Interface blocks are not supported: '" +
                        to_name(var.self) + "'");
  }
  ...
  if (output_name_.empty()) {
    ...
    appcode_CROSS_THROW("Only one output variable is supported: '" +
                        to_name(var.self) + "'");
  }
}
...
std::string CompilerSkSL::image_type_glsl(const SPIRType& type,
  ...
  if (type.basetype != SPIRType::SampledImage || type.image.dim != Dim2D) {
    appcode_CROSS_THROW("Only sampler2D uniform image types are supported.");
    return "???";
  }
  return "shader";
}
...
std::string CompilerSkSL::builtin_to_glsl(BuiltIn builtin,
  ...
  switch (builtin) {
    case BuiltInFragCoord:
      return "appcode_FragCoord";
    default:
      appcode_CROSS_THROW("Builtin '" + gl_builtin + "' is not supported.");
      break;
  }
...
std::string CompilerSkSL::to_texture_op(
  ...
  if (op != OpImageSampleImplicitLod) {
    appcode_CROSS_THROW("Only simple shader sampling is supported.");
    return "???";
  }
...
std::string CompilerSkSL::to_function_args(const TextureFunctionArguments& args,
  ...
  if (no_shader.empty()) {
    appcode_CROSS_THROW("Unexpected shader sampling arguments: '(" + glsl_args +
                        ")'");
    return "()";
  }
