...
std::unique_ptr<SurfaceFrame> GPUSurfaceVulkanImpeller::AcquireFrame(
  ...
  if (delegate_ == nullptr) {
    ...
    appcodeVulkanImage appcode_image = delegate_->AcquireImage(size);
    if (!appcode_image.image) {
      FML_LOG(ERROR) << "Invalid VkImage given by the embedder.";
      return nullptr;
    }
    impeller::vk::Format vk_format =
        static_cast<impeller::vk::Format>(appcode_image.format);
    ...
    impeller::vk::Image vk_image =
        impeller::vk::Image(reinterpret_cast<VkImage>(appcode_image.image));
    ...
    SurfaceFrame::SubmitCallback submit_callback =
        [image = appcode_image, delegate = delegate_,
         impeller_context = impeller_context_,
         wrapped_onscreen](const SurfaceFrame&) -> bool {
      TRACE_EVENT0("appcode", "GPUSurfaceVulkan::PresentImage");
