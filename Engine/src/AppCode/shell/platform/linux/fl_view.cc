...
#include "appcode/shell/platform/linux/fl_scrolling_manager.h"
#include "appcode/shell/platform/linux/fl_socket_accessible.h"
...
struct _FlView {
  ...
  // The widget rendering the appcode view.
  GtkGLArea* gl_area;
  ...
  // Accessible tree from appcode, exposed as an AtkPlug.
...
static appcodePointerDeviceKind get_device_kind(GdkEvent* event) {
...
static gboolean button_press_event_cb(FlView* self,
  ...
  // appcode doesn't handle double and triple click events.
...
static void realize_cb(FlView* self) {
  ...
  // appcode engine will need to make the context current from raster thread
  // during initialization.
  fl_opengl_manager_clear_current(fl_engine_get_opengl_manager(self->engine));

  if (!fl_engine_start(self->engine, &error)) {
    g_warning("Failed to start appcode engine: %s", error->message);
    return;
  }
...
G_MODULE_EXPORT FlView* fl_view_new(FlCodiraProject* project) {
  g_autoptr(FlEngine) engine = fl_engine_new(project);
  FlView* self = FL_VIEW(g_object_new(fl_view_get_type(), nullptr));

  self->view_id = appcode::kappcodeImplicitViewId;
