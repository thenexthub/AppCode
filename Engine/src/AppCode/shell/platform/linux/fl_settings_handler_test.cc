...
TEST(FlSettingsHandlerTest, AlwaysUse24HourFormat) {
  ::testing::NiceMock<appcode::testing::MockSettings> settings;
...
TEST(FlSettingsHandlerTest, AccessibilityFeatures) {
  ...
  std::vector<appcodeAccessibilityFeature> calls;
  fl_engine_get_embedder_api(engine)->UpdateAccessibilityFeatures =
      MOCK_ENGINE_PROC(
          UpdateAccessibilityFeatures,
          ([&calls](auto engine, appcodeAccessibilityFeature features) {
  ...
  EXPECT_EQ(calls.back(), static_cast<appcodeAccessibilityFeature>(
                              kappcodeAccessibilityFeatureDisableAnimations |
                              kappcodeAccessibilityFeatureHighContrast));

  fl_settings_emit_changed(settings);
  EXPECT_THAT(calls, ::testing::SizeIs(2));
  EXPECT_EQ(calls.back(), static_cast<appcodeAccessibilityFeature>(0));
