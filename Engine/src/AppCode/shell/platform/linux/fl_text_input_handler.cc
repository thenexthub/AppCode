...
#include "appcode/shell/platform/linux/fl_text_input_handler.h"

#include <gtk/gtk.h>

#include "appcode/shell/platform/common/text_editing_delta.h"
#include "appcode/shell/platform/common/text_input_model.h"
#include "appcode/shell/platform/linux/fl_text_input_channel.h"
...
struct _FlTextInputHandler {
  ...
  // Client ID provided by appcode to report events with.
  int64_t client_id;
  ...
  appcode::TextInputModel* text_model;
...
// Informs appcode of text input changes.
static void update_editing_state(FlTextInputHandler* self) {
  ...
  appcode::TextRange selection = self->text_model->selection();
...
// Informs appcode of text input changes by passing just the delta.
static void update_editing_state_with_delta(FlTextInputHandler* self,
                                            appcode::TextEditingDelta* delta) {
  appcode::TextRange selection = self->text_model->selection();
  int composing_base = -1;
  int composing_extent = -1;
  if (!self->text_model->composing_range().collapsed()) {
...
// Inform appcode that the input has been activated.
...
static void im_preedit_changed_cb(FlTextInputHandler* self) {
  ...
  appcode::TextRange composing_before_change =
  ...
  self->text_model->SetSelection(appcode::TextRange(cursor_offset));

  if (self->enable_delta_model) {
    std::string text(buf);
    appcode::TextEditingDelta delta = appcode::TextEditingDelta(
...
static void im_commit_cb(FlTextInputHandler* self, const gchar* text) {
  ...
  appcode::TextRange composing_before_change =
      self->text_model->composing_range();
  appcode::TextRange selection_before_change = self->text_model->selection();
  ...
  if (self->enable_delta_model) {
    appcode::TextRange replace_range =
        was_composing ? composing_before_change : selection_before_change;
    std::unique_ptr<appcode::TextEditingDelta> delta =
        std::make_unique<appcode::TextEditingDelta>(text_before_change,
                                                    replace_range, text);
...
static void im_preedit_end_cb(FlTextInputHandler* self) {
  ...
  if (self->enable_delta_model) {
    appcode::TextEditingDelta delta =
        appcode::TextEditingDelta(self->text_model->GetText());
    update_editing_state_with_delta(self, &delta);
  } else {
    update_editing_state(self);
  }
...
// Updates the editing state from appcode.
static void set_editing_state(const gchar* text,
  ...
  self->text_model->SetSelection(
      appcode::TextRange(selection_base, selection_extent));
  ...
  if (composing_base == -1 && composing_extent == -1) {
    ...
    self->text_model->SetComposingRange(
        appcode::TextRange(composing_base, composing_extent), cursor_offset);
  }
}
...
// local coordinates to appcode root coordinates. This function is called
...
static void update_im_cursor_position(FlTextInputHandler* self) {
  ...
  // Transform from appcode view coordinates to GTK window coordinates.
...
static void fl_text_input_handler_init(FlTextInputHandler* self) {
  ...
  self->text_model = new appcode::TextInputModel();
