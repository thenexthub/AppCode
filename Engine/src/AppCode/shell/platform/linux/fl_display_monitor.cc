...
struct _FlDisplayMonitor {
  ...
  appcodeEngineDisplayId next_display_id;
};
...
static void notify_display_update(FlDisplayMonitor* self) {
  ...
  g_autofree appcodeEngineDisplay* displays =
      g_new0(appcodeEngineDisplay, n_monitors);
  for (int i = 0; i < n_monitors; i++) {
    appcodeEngineDisplay* display = &displays[i];

    GdkMonitor* monitor = gdk_display_get_monitor(self->display, i);
    appcodeEngineDisplayId display_id = GPOINTER_TO_INT(
...
appcodeEngineDisplayId fl_display_monitor_get_display_id(FlDisplayMonitor* self,
                                                         GdkMonitor* monitor) {
