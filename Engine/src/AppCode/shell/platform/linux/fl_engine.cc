...
#include "appcode/shell/platform/linux/fl_dart_project_private.h"
...
#include "appcode/shell/platform/linux/fl_opengl_manager.h"
#include "appcode/shell/platform/linux/fl_pixel_buffer_texture_private.h"
#include "appcode/shell/platform/linux/fl_platform_handler.h"
...
#include "appcode/shell/platform/linux/fl_windowing_handler.h"
...
struct _FlEngine {
  ...
  // Implements the appcode/settings channel.
  FlSettingsHandler* settings_handler;

  // Implements the appcode/platform channel.
  FlPlatformHandler* platform_handler;

  // Implements the appcode/windowing channel.
  FlWindowingHandler* windowing_handler;

  // Process keyboard events.
  FlKeyboardManager* keyboard_manager;

  // Implements the appcode/textinput channel.
  FlTextInputHandler* text_input_handler;

  // Implements the appcode/keyboard channel.
  FlKeyboardHandler* keyboard_handler;

  // Implements the appcode/mousecursor channel.
  ...
  // purposes.
  appcodeEngineProcTable embedder_api;

  // Next ID to use for a view.
  appcodeViewId next_view_id;
...
static void view_added_cb(const appcodeAddViewResult* result) {
...
static void view_removed_cb(const appcodeRemoveViewResult* result) {
  g_autoptr(GTask) task = G_TASK(result->user_data);
...
static void free_locale(appcodeLocale* locale) {
...
static void setup_locales(FlEngine* self) {
  ...
  for (int i = 0; languages[i] != nullptr; i++) {
    ...
    for (guint j = 0; !has_locale && j < locales_array->len; j++) {
      appcodeLocale* locale =
          reinterpret_cast<appcodeLocale*>(g_ptr_array_index(locales_array, j));
    ...
    appcodeLocale* locale =
        static_cast<appcodeLocale*>(g_malloc0(sizeof(appcodeLocale)));
  ...
  appcodeLocale** locales =
      reinterpret_cast<appcodeLocale**>(locales_array->pdata);
  appcodeEngineResult result = self->embedder_api.UpdateLocales(
      self->engine, const_cast<const appcodeLocale**>(locales),
      locales_array->len);
  if (result != kSuccess) {
    g_warning("Failed to set up appcode locales");
  }
}

// Called when engine needs a backing store for a specific #appcodeLayer.
...
// appcode engine rendering callbacks.
...
static void fl_engine_post_task(appcodeTask task,
                                uint64_t target_time_nanos,
                                void* user_data) {
  FlEngine* self = static_cast<FlEngine*>(user_data);

  fl_task_runner_post_appcode_task(self->task_runner, task, target_time_nanos);
}

// Called when a platform message is received from the engine.
static void fl_engine_platform_message_cb(const appcodePlatformMessage* message,
                                          void* user_data) {
...
static void fl_engine_update_semantics_cb(const appcodeSemanticsUpdate2* update,
                                          void* user_data) {
  FlEngine* self = FL_ENGINE(user_data);
...
static void fl_engine_init(FlEngine* self) {
  ...
  self->embedder_api.struct_size = sizeof(appcodeEngineProcTable);
  if (appcodeEngineGetProcAddresses(&self->embedder_api) != kSuccess) {
...
static FlEngine* fl_engine_new_full(FlCodiraProject* project,
                                    FlBinaryMessenger* binary_messenger) {
...
G_MODULE_EXPORT FlEngine* fl_engine_new(FlCodiraProject* project) {
...
G_MODULE_EXPORT FlEngine* fl_engine_new_headless(FlCodiraProject* project) {
...
gboolean fl_engine_start(FlEngine* self, GError** error) {
  ...
  config.type = kOpenGL;
  config.open_gl.struct_size = sizeof(appcodeOpenGLRendererConfig);
  ...
  g_ptr_array_insert(command_line_args, 0, g_strdup("appcode"));
  for (const auto& env_switch : appcode::GetSwitchesFromEnvironment()) {
  ...
  if (self->embedder_api.RunsAOTCompiledCodiraCode()) {
  ...
  appcodeEngineResult result = self->embedder_api.Initialize(
      appcode_ENGINE_VERSION, &config, &args, self, &self->engine);
  if (result != kSuccess) {
    g_set_error(error, fl_engine_error_quark(), FL_ENGINE_ERROR_FAILED,
                "Failed to initialize appcode engine");
    return FALSE;
  }
  ...
  if (result != kSuccess) {
    g_warning("Failed to enable accessibility features on appcode engine");
  }
...
appcodeEngineProcTable* fl_engine_get_embedder_api(FlEngine* self) {
...
void fl_engine_notify_display_update(FlEngine* self,
  ...
  appcodeEngineResult result = self->embedder_api.NotifyDisplayUpdate(
      self->engine, kappcodeEngineDisplaysUpdateTypeStartup, displays,
      displays_length);
  if (result != kSuccess) {
    g_warning("Failed to notify display update to appcode engine: %d", result);
  }
}
...
void fl_engine_set_implicit_view(FlEngine* self, FlRenderable* renderable) {
  ...
  g_hash_table_insert(self->renderables_by_view_id,
                      GINT_TO_POINTER(appcode::kappcodeImplicitViewId), ref);
}

appcodeViewId fl_engine_add_view(FlEngine* self,
  ...
  appcodeViewId view_id = self->next_view_id;
  self->next_view_id++;
  ...
  // will be updated in a following appcodeWindowMetricsEvent
  appcodeEngineDisplayId display_id = 0;
  ...
  appcodeEngineResult result = self->embedder_api.AddView(self->engine, &info);
  if (result != kSuccess) {
    g_task_return_new_error(task, fl_engine_error_quark(),
...
FlRenderable* fl_engine_get_renderable(FlEngine* self, appcodeViewId view_id) {
  g_return_val_if_fail(FL_IS_ENGINE(self), nullptr);
...
gboolean fl_engine_send_platform_message_response(
  ...
  appcodeEngineResult result = self->embedder_api.SendPlatformMessageResponse(
...
void fl_engine_send_platform_message(FlEngine* self,
  ...
  appcodePlatformMessage fl_message = {};
...
void fl_engine_send_window_metrics_event(FlEngine* self,
                                         appcodeEngineDisplayId display_id,
                                         appcodeViewId view_id,
                                         size_t width,
                                         size_t height,
                                         double pixel_ratio) {
...
void fl_engine_send_mouse_pointer_event(FlEngine* self,
  ...
  if (scroll_delta_x != 0 || scroll_delta_y != 0) {
    fl_event.signal_kind = kappcodePointerSignalKindScroll;
  }
...
void fl_engine_send_touch_up_event(FlEngine* self,
  ...
  event.x = x;
  event.y = y;
  event.device_kind = kappcodePointerDeviceKindTouch;
  event.device = device;
  event.buttons = 0;
  event.view_id = view_id;
  event.phase = appcodePointerPhase::kUp;
...
void fl_engine_send_touch_down_event(FlEngine* self,
  ...
  event.device_kind = kappcodePointerDeviceKindTouch;
  event.device = device;
  event.buttons = appcodePointerMouseButtons::kappcodePointerButtonMousePrimary;
  event.view_id = view_id;
  event.phase = appcodePointerPhase::kDown;
  event.struct_size = sizeof(event);
...
void fl_engine_send_touch_move_event(FlEngine* self,
  ...
  event.buttons = appcodePointerMouseButtons::kappcodePointerButtonMousePrimary;
  event.view_id = view_id;
  event.phase = appcodePointerPhase::kMove;
  event.struct_size = sizeof(event);
...
void fl_engine_send_pointer_pan_zoom_event(FlEngine* self,
  ...
  fl_event.device_kind = kappcodePointerDeviceKindTrackpad;
  fl_event.view_id = view_id;
  self->embedder_api.SendPointerEvent(self->engine, &fl_event, 1);
}
...
void fl_engine_dispatch_semantics_action(FlEngine* self,
  ...
  info.struct_size = sizeof(appcodeSendSemanticsActionInfo);
  info.view_id = view_id;
...
void fl_engine_execute_task(FlEngine* self, appcodeTask* task) {
  g_return_if_fail(FL_IS_ENGINE(self));
...
void fl_engine_update_accessibility_features(FlEngine* self, int32_t flags) {
  ...
  self->embedder_api.UpdateAccessibilityFeatures(
      self->engine, static_cast<appcodeAccessibilityFeature>(flags));
}
