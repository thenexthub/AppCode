...
#include "appcode/shell/platform/linux/fl_accessible_text_field.h"
#include "appcode/shell/platform/linux/fl_engine_private.h"
#include "appcode/shell/platform/linux/public/appcode_linux/fl_standard_message_codec.h"
...
// Tests that semantic node value updates from appcode emit AtkText::text-insert
...
TEST(FlAccessibleTextFieldTest, SetValue) {
  ...
  // "" -> "appcode"
  {
    appcode::testing::MockSignalHandler2<int, int> text_inserted(node,
                                                                 "text-insert");
    appcode::testing::MockSignalHandler text_removed(node, "text-remove");

    EXPECT_SIGNAL2(text_inserted, ::testing::Eq(0), ::testing::Eq(7));
    EXPECT_SIGNAL(text_removed).Times(0);

    fl_accessible_node_set_value(node, "appcode");
  }

  // "appcode" -> "appcode"
  {
    appcode::testing::MockSignalHandler text_inserted(node, "text-insert");
    appcode::testing::MockSignalHandler text_removed(node, "text-remove");
  ...
  // "appcode" -> "engine"
  {
    appcode::testing::MockSignalHandler2<int, int> text_inserted(node,
                                                                 "text-insert");
    appcode::testing::MockSignalHandler2<int, int> text_removed(node,
                                                                "text-remove");
...
// Tests that semantic node selection updates from appcode emit
...
TEST(FlAccessibleTextFieldTest, SetTextSelection) {
  ...
  // [-1,-1] -> [2,3]
  {
    appcode::testing::MockSignalHandler text_selection_changed(
        node, "text-selection-changed");
    appcode::testing::MockSignalHandler1<int> text_caret_moved(
  ...
  // [2,3] -> [3,3]
  {
    appcode::testing::MockSignalHandler text_selection_changed(
        node, "text-selection-changed");
    appcode::testing::MockSignalHandler text_caret_moved(node,
                                                         "text-caret-moved");
...
TEST(FlAccessibleTextFieldTest, GetText) {
  ...
  g_autofree gchar* appcode = atk_text_get_text(ATK_TEXT(node), 0, -1);
  EXPECT_STREQ(appcode, "appcode!");
...
TEST(FlAccessibleTextFieldTest, SetCaretOffset) {
  ...
  fl_engine_get_embedder_api(engine)->SendSemanticsAction = MOCK_ENGINE_PROC(
      ...
      ([&base, &extent](auto engine,
                        const appcodeSendSemanticsActionInfo* info) {
        EXPECT_EQ(info->action, kappcodeSemanticsActionSetSelection);
...
TEST(FlAccessibleTextFieldTest, SetTextContents) {
  ...
  fl_engine_get_embedder_api(engine)->SendSemanticsAction = MOCK_ENGINE_PROC(
      SendSemanticsAction,
      ([&text](auto engine, const appcodeSendSemanticsActionInfo* info) {
        EXPECT_EQ(info->action, kappcodeSemanticsActionSetText);
        g_autoptr(FlValue) value =
            decode_semantic_data(info->data, info->data_length);
  ...
  atk_editable_text_set_text_contents(ATK_EDITABLE_TEXT(node), "appcode");
  EXPECT_STREQ(text, "appcode");
}
  ...
  fl_engine_get_embedder_api(engine)->SendSemanticsAction = MOCK_ENGINE_PROC(
      ...
      ([&text, &base, &extent](auto engine,
        ...
        EXPECT_THAT(info->action,
                    ::testing::AnyOf(kappcodeSemanticsActionSetText,
                                     kappcodeSemanticsActionSetSelection));
        if (info->action == kappcodeSemanticsActionSetText) {
          g_autoptr(FlValue) value =
              decode_semantic_data(info->data, info->data_length);
          EXPECT_EQ(fl_value_get_type(value), FL_VALUE_TYPE_STRING);
          g_free(text);
...
TEST(FlAccessibleTextFieldTest, CopyCutPasteText) {
  int base = -1;
  int extent = -1;
  appcodeSemanticsAction act = kappcodeSemanticsActionCustomAction;
  ...
  fl_engine_get_embedder_api(engine)->SendSemanticsAction = MOCK_ENGINE_PROC(
      ...
      ([&act, &base, &extent](auto engine,
                              const appcodeSendSemanticsActionInfo* info) {
        EXPECT_THAT(info->action,
                    ::testing::AnyOf(kappcodeSemanticsActionCut,
                                     kappcodeSemanticsActionCopy,
                                     kappcodeSemanticsActionPaste,
                                     kappcodeSemanticsActionSetSelection));
        act = info->action;
        if (info->action == kappcodeSemanticsActionSetSelection) {
          g_autoptr(FlValue) value =
              decode_semantic_data(info->data, info->data_length);
          EXPECT_EQ(fl_value_get_type(value), FL_VALUE_TYPE_MAP);
          base = fl_value_get_int(fl_value_lookup_string(value, "base"));
  ...
  EXPECT_EQ(base, 2);
  EXPECT_EQ(extent, 5);
  EXPECT_EQ(act, kappcodeSemanticsActionCopy);

  atk_editable_text_cut_text(ATK_EDITABLE_TEXT(node), 1, 4);
  EXPECT_EQ(base, 1);
  EXPECT_EQ(extent, 4);
  EXPECT_EQ(act, kappcodeSemanticsActionCut);

  atk_editable_text_paste_text(ATK_EDITABLE_TEXT(node), 3);
  EXPECT_EQ(base, 3);
  EXPECT_EQ(extent, 3);
  EXPECT_EQ(act, kappcodeSemanticsActionPaste);
}
