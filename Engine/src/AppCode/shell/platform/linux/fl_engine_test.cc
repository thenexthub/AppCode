...
TEST(FlEngineTest, NotifyDisplayUpdate) {
  ...
  appcodeEngineDisplay displays[2] = {
      {
          .struct_size = sizeof(appcodeEngineDisplay),
          .display_id = 1,
...
TEST(FlEngineTest, WindowMetrics) {
  ...
  fl_engine_get_embedder_api(engine)->SendWindowMetricsEvent = MOCK_ENGINE_PROC(
      ...
      ([&called](auto engine, const appcodeWindowMetricsEvent* event) {
        called = true;
...
TEST(FlEngineTest, MousePointer) {
  ...
  fl_engine_get_embedder_api(engine)->SendPointerEvent = MOCK_ENGINE_PROC(
      SendPointerEvent,
      ([&called](auto engine, const appcodePointerEvent* events,
                 size_t events_count) {
        called = true;
        ...
        EXPECT_EQ(events[0].signal_kind, kappcodePointerSignalKindScroll);
        EXPECT_EQ(events[0].scroll_delta_x, 1.2);
        EXPECT_EQ(events[0].scroll_delta_y, -3.4);
        EXPECT_EQ(events[0].device_kind, kappcodePointerDeviceKindMouse);
        EXPECT_EQ(events[0].buttons, kappcodePointerButtonMouseSecondary);
  ...
  fl_engine_send_mouse_pointer_event(engine, 1, kDown, 1234567890, 800, 600,
                                     kappcodePointerDeviceKindMouse, 1.2, -3.4,
...
TEST(FlEngineTest, PointerPanZoom) {
  ...
  fl_engine_get_embedder_api(engine)->SendPointerEvent = MOCK_ENGINE_PROC(
      ...
      ([&called](auto engine, const appcodePointerEvent* events,
        ...
        EXPECT_EQ(events[0].signal_kind, kappcodePointerSignalKindNone);
        EXPECT_EQ(events[0].pan_x, 1.5);
        EXPECT_EQ(events[0].pan_y, 2.5);
        EXPECT_EQ(events[0].scale, 3.5);
        EXPECT_EQ(events[0].rotation, 4.5);
        EXPECT_EQ(events[0].device_kind, kappcodePointerDeviceKindTrackpad);
...
TEST(FlEngineTest, DispatchSemanticsAction) {
  ...
  fl_engine_get_embedder_api(engine)->SendSemanticsAction = MOCK_ENGINE_PROC(
      SendSemanticsAction,
      ([&called](auto engine, const appcodeSendSemanticsActionInfo* info) {
        EXPECT_EQ(info->view_id, static_cast<int64_t>(456));
        EXPECT_EQ(info->node_id, static_cast<uint64_t>(42));
        EXPECT_EQ(info->action, kappcodeSemanticsActionTap);
  ...
  fl_engine_dispatch_semantics_action(engine, 456, 42,
                                      kappcodeSemanticsActionTap, data);
...
TEST(FlEngineTest, PlatformMessage) {
  ...
  bool called = false;
  appcodeEngineSendPlatformMessageFnPtr old_handler =
...
TEST(FlEngineTest, PlatformMessageResponse) {
  ...
  fl_engine_get_embedder_api(engine)->SendPlatformMessageResponse =
      MOCK_ENGINE_PROC(
          ...
          ([&called](auto engine,
            ...
            EXPECT_EQ(
                handle,
                reinterpret_cast<const appcodePlatformMessageResponseHandle*>(
                    42));
  ...
  EXPECT_TRUE(fl_engine_send_platform_message_response(
      engine, reinterpret_cast<const appcodePlatformMessageResponseHandle*>(42),
      response, &error));
  EXPECT_EQ(error, nullptr);
...
TEST(FlEngineTest, SettingsHandler) {
  ...
  fl_engine_get_embedder_api(engine)->SendPlatformMessage = MOCK_ENGINE_PROC(
      SendPlatformMessage,
      ([&called](auto engine, const appcodePlatformMessage* message) {
        called = true;

        EXPECT_STREQ(message->channel, "appcode/settings");
...
TEST(FlEngineTest, CodiraEntrypointArgs) {
...
TEST(FlEngineTest, EngineId) {
  ...
  fl_engine_get_embedder_api(engine)->Initialize = MOCK_ENGINE_PROC(
      Initialize,
      ([&engine_id](size_t version, const appcodeRendererConfig* config,
                    const appcodeProjectArgs* args, void* user_data,
                    appcode_API_SYMBOL(appcodeEngine) * engine_out) {
...
TEST(FlEngineTest, UIIsolateOnPlatformTaskRunner) {
  ...
  fl_engine_get_embedder_api(engine)->Initialize = MOCK_ENGINE_PROC(
      Initialize,
      ([&same_task_runner](size_t version, const appcodeRendererConfig* config,
                           const appcodeProjectArgs* args, void* user_data,
                           appcode_API_SYMBOL(appcodeEngine) * engine_out) {
...
TEST(FlEngineTest, UIIsolateOnSeparateThread) {
  ...
  fl_engine_get_embedder_api(engine)->Initialize = MOCK_ENGINE_PROC(
      Initialize,
      ([&separate_thread](size_t version, const appcodeRendererConfig* config,
                          const appcodeProjectArgs* args, void* user_data,
                          appcode_API_SYMBOL(appcodeEngine) * engine_out) {
...
TEST(FlEngineTest, Locales) {
  ...
  fl_engine_get_embedder_api(engine)->UpdateLocales = MOCK_ENGINE_PROC(
      UpdateLocales, ([&called](auto engine, const appcodeLocale** locales,
...
TEST(FlEngineTest, AddView) {
  ...
  fl_engine_get_embedder_api(engine)->AddView = MOCK_ENGINE_PROC(
      AddView, ([&called](auto engine, const appcodeAddViewInfo* info) {
        called = true;
...
TEST(FlEngineTest, AddViewError) {
  ...
  fl_engine_get_embedder_api(engine)->AddView = MOCK_ENGINE_PROC(
      AddView, ([](auto engine, const appcodeAddViewInfo* info) {
        appcodeAddViewResult result;
        result.struct_size = sizeof(appcodeAddViewResult);
        result.added = false;
        result.user_data = info->user_data;
...
TEST(FlEngineTest, RemoveView) {
  ...
  fl_engine_get_embedder_api(engine)->RemoveView = MOCK_ENGINE_PROC(
      RemoveView, ([&called](auto engine, const appcodeRemoveViewInfo* info) {
        called = true;
...
TEST(FlEngineTest, SendKeyEvent) {
  ...
  fl_engine_get_embedder_api(engine)->SendKeyEvent = MOCK_ENGINE_PROC(
      SendKeyEvent,
      ([&called](auto engine, const appcodeKeyEvent* event,
                 appcodeKeyEventCallback callback, void* user_data) {
        called = true;
        EXPECT_EQ(event->timestamp, 1234);
        EXPECT_EQ(event->type, kappcodeKeyEventTypeUp);
        ...
        EXPECT_EQ(event->device_type, kappcodeKeyEventDeviceTypeKeyboard);
        callback(TRUE, user_data);
        return kSuccess;
      }));

  appcodeKeyEvent event = {.struct_size = sizeof(appcodeKeyEvent),
                           .timestamp = 1234,
