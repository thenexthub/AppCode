...
// Checks appcode flags are mapped to appropriate ATK state.
TEST(FlAccessibleNodeTest, SetFlags) {
  ...
  fl_accessible_node_set_flags(
      node, static_cast<appcodeSemanticsFlag>(kappcodeSemanticsFlagIsEnabled |
                                              kappcodeSemanticsFlagIsFocusable |
                                              kappcodeSemanticsFlagIsFocused));
...
// Checks appcode flags are mapped to appropriate ATK roles.
TEST(FlAccessibleNodeTest, GetRole) {
  ...
  fl_accessible_node_set_flags(
      node, static_cast<appcodeSemanticsFlag>(kappcodeSemanticsFlagIsButton));
  EXPECT_EQ(atk_object_get_role(ATK_OBJECT(node)), ATK_ROLE_PUSH_BUTTON);

  fl_accessible_node_set_flags(node, static_cast<appcodeSemanticsFlag>(
                                         kappcodeSemanticsFlagHasCheckedState));
  EXPECT_EQ(atk_object_get_role(ATK_OBJECT(node)), ATK_ROLE_CHECK_BOX);

  fl_accessible_node_set_flags(
      node, static_cast<appcodeSemanticsFlag>(
                kappcodeSemanticsFlagHasCheckedState |
                kappcodeSemanticsFlagIsInMutuallyExclusiveGroup));
  EXPECT_EQ(atk_object_get_role(ATK_OBJECT(node)), ATK_ROLE_RADIO_BUTTON);

  fl_accessible_node_set_flags(node, static_cast<appcodeSemanticsFlag>(
                                         kappcodeSemanticsFlagHasToggledState));
  EXPECT_EQ(atk_object_get_role(ATK_OBJECT(node)), ATK_ROLE_TOGGLE_BUTTON);

  fl_accessible_node_set_flags(node, kappcodeSemanticsFlagIsTextField);
  EXPECT_EQ(atk_object_get_role(ATK_OBJECT(node)), ATK_ROLE_TEXT);

  fl_accessible_node_set_flags(
      node, static_cast<appcodeSemanticsFlag>(kappcodeSemanticsFlagIsTextField |
                                              kappcodeSemanticsFlagIsObscured));
  EXPECT_EQ(atk_object_get_role(ATK_OBJECT(node)), ATK_ROLE_PASSWORD_TEXT);
}

// Checks appcode actions are mapped to the appropriate ATK actions.
TEST(FlAccessibleNodeTest, SetActions) {
  ...
  fl_accessible_node_set_actions(
      node, static_cast<appcodeSemanticsAction>(
                kappcodeSemanticsActionTap | kappcodeSemanticsActionLongPress));
