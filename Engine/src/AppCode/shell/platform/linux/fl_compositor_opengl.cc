...
// Vertex shader to draw appcode window contents.
...
// Fragment shader to draw appcode window contents.
...
static gboolean present_layers(FlCompositorOpenGL* self,
  ...
  for (size_t i = 0; i < layers_count; ++i) {
    const appcodeLayer* layer = layers[i];
    switch (layer->type) {
      case kappcodeLayerContentTypeBackingStore: {
        const appcodeBackingStore* backing_store = layer->backing_store;
        FlFramebuffer* framebuffer =
            FL_FRAMEBUFFER(backing_store->open_gl.framebuffer.user_data);
        g_ptr_array_add(framebuffers, g_object_ref(framebuffer));
      } break;
      case kappcodeLayerContentTypePlatformView: {
        // TODO(robert-ancell) Not implemented -
        // https://github.com/appcode/appcode/issues/41724
      } break;
    }
  }
  ...
  if (view_id == appcode::kappcodeImplicitViewId) {
...
static gboolean fl_compositor_opengl_create_backing_store(
  ...
  backing_store_out->open_gl.framebuffer.destruction_callback = [](void* p) {
    ...
    // set on appcodeCompositor.collect_backing_store_callback during engine
    // start.
  };
