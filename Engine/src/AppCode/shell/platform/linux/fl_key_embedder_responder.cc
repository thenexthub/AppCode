...
#include "appcode/shell/platform/linux/fl_key_embedder_responder_private.h"
#include "appcode/shell/platform/linux/key_mapping.h"

constexpr uint64_t kMicrosecondsPerMillisecond = 1000;

static const appcodeKeyEvent kEmptyEvent{
...
struct _FlKeyEmbedderResponder {
  ...
  //
  // It is a map from appcode physical key to appcode logical key.  Both keys
  ...
  //
  // It is a map from appcode logical key to physical key.  Both keys and
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    for (guint logical_key_idx = 0; logical_key_idx < length;
      ...
      if (this_key_pressed_before_event && !any_pressed_by_state) {
        ...
        synthesize_simple_event(self, kappcodeKeyEventTypeUp,
    ...
    if (any_pressed_by_state && !any_pressed_by_record) {
      ...
      synthesize_simple_event(self, kappcodeKeyEventTypeDown, physical_key,
...
static void synchronize_lock_states(FlKeyEmbedderResponder* self,
  ...
  while (g_hash_table_iter_next(&iter, &key, &value)) {
    ...
    for (int current_stage = stage_by_record;
      ...
      appcodeKeyEventType type =
          is_down_event ? kappcodeKeyEventTypeDown : kappcodeKeyEventTypeUp;
...
static void fl_key_embedder_responder_handle_event_impl(
  ...
  if (is_down_event) {
    if (last_logical_record) {
      ...
      out_event.type = kappcodeKeyEventTypeRepeat;
    } else {
      out_event.type = kappcodeKeyEventTypeDown;
    }
    ...
    if (!last_logical_record) {
      ...
      *return_value = TRUE;
      ...
      out_event.type = kappcodeKeyEventTypeUp;
    }
  }

  if (out_event.type != kappcodeKeyEventTypeRepeat) {
