...
// Maps appcode semantics flags to ATK flags.
static struct {
  AtkStateType state;
  appcodeSemanticsFlag flag;
  gboolean invert;
} flag_mapping[] = {
    {ATK_STATE_SHOWING, kappcodeSemanticsFlagIsObscured, TRUE},
    {ATK_STATE_VISIBLE, kappcodeSemanticsFlagIsHidden, TRUE},
    {ATK_STATE_CHECKABLE, kappcodeSemanticsFlagHasCheckedState, FALSE},
    {ATK_STATE_FOCUSABLE, kappcodeSemanticsFlagIsFocusable, FALSE},
    {ATK_STATE_FOCUSED, kappcodeSemanticsFlagIsFocused, FALSE},
    {ATK_STATE_CHECKED,
     static_cast<appcodeSemanticsFlag>(kappcodeSemanticsFlagIsChecked |
                                       kappcodeSemanticsFlagIsToggled),
     FALSE},
    {ATK_STATE_SELECTED, kappcodeSemanticsFlagIsSelected, FALSE},
    {ATK_STATE_ENABLED, kappcodeSemanticsFlagIsEnabled, FALSE},
    {ATK_STATE_SENSITIVE, kappcodeSemanticsFlagIsEnabled, FALSE},
    {ATK_STATE_READ_ONLY, kappcodeSemanticsFlagIsReadOnly, FALSE},
    {ATK_STATE_EDITABLE, kappcodeSemanticsFlagIsTextField, FALSE},
    {ATK_STATE_INVALID, static_cast<appcodeSemanticsFlag>(0), FALSE},
};

// Maps appcode semantics actions to ATK actions.
typedef struct {
  appcodeSemanticsAction action;
  const gchar* name;
} ActionData;
static ActionData action_mapping[] = {
    {kappcodeSemanticsActionTap, "Tap"},
    {kappcodeSemanticsActionLongPress, "LongPress"},
    {kappcodeSemanticsActionScrollLeft, "ScrollLeft"},
    {kappcodeSemanticsActionScrollRight, "ScrollRight"},
    {kappcodeSemanticsActionScrollUp, "ScrollUp"},
    {kappcodeSemanticsActionScrollDown, "ScrollDown"},
    {kappcodeSemanticsActionIncrease, "Increase"},
    {kappcodeSemanticsActionDecrease, "Decrease"},
    {kappcodeSemanticsActionShowOnScreen, "ShowOnScreen"},
    {kappcodeSemanticsActionMoveCursorForwardByCharacter,
     "MoveCursorForwardByCharacter"},
    {kappcodeSemanticsActionMoveCursorBackwardByCharacter,
     "MoveCursorBackwardByCharacter"},
    {kappcodeSemanticsActionCopy, "Copy"},
    {kappcodeSemanticsActionCut, "Cut"},
    {kappcodeSemanticsActionPaste, "Paste"},
    {kappcodeSemanticsActionDidGainAccessibilityFocus,
     "DidGainAccessibilityFocus"},
    {kappcodeSemanticsActionDidLoseAccessibilityFocus,
     "DidLoseAccessibilityFocus"},
    {kappcodeSemanticsActionCustomAction, "CustomAction"},
    {kappcodeSemanticsActionDismiss, "Dismiss"},
    {kappcodeSemanticsActionMoveCursorForwardByWord, "MoveCursorForwardByWord"},
    {kappcodeSemanticsActionMoveCursorBackwardByWord,
     "MoveCursorBackwardByWord"},
    {kappcodeSemanticsActionFocus, "Focus"},
    {static_cast<appcodeSemanticsAction>(0), nullptr}};
...
static gboolean flag_is_changed(appcodeSemanticsFlag old_flags,
                                appcodeSemanticsFlag flags,
                                appcodeSemanticsFlag flag) {
...
static gboolean has_flag(appcodeSemanticsFlag flags,
                         appcodeSemanticsFlag flag) {
  return (flags & flag) != 0;
}

// Returns TRUE if [action] is set in [actions].
static gboolean has_action(appcodeSemanticsAction actions,
                           appcodeSemanticsAction action) {
...
static AtkRole fl_accessible_node_get_role(AtkObject* accessible) {
  FlAccessibleNodePrivate* priv = FL_ACCESSIBLE_NODE_GET_PRIVATE(accessible);
  if ((priv->flags & kappcodeSemanticsFlagIsButton) != 0) {
    return ATK_ROLE_PUSH_BUTTON;
  }
  if ((priv->flags & kappcodeSemanticsFlagIsInMutuallyExclusiveGroup) != 0 &&
      (priv->flags & kappcodeSemanticsFlagHasCheckedState) != 0) {
    return ATK_ROLE_RADIO_BUTTON;
  }
  if ((priv->flags & kappcodeSemanticsFlagHasCheckedState) != 0) {
    return ATK_ROLE_CHECK_BOX;
  }
  if ((priv->flags & kappcodeSemanticsFlagHasToggledState) != 0) {
    return ATK_ROLE_TOGGLE_BUTTON;
  }
  if ((priv->flags & kappcodeSemanticsFlagIsSlider) != 0) {
    return ATK_ROLE_SLIDER;
  }
  if ((priv->flags & kappcodeSemanticsFlagIsTextField) != 0 &&
      (priv->flags & kappcodeSemanticsFlagIsObscured) != 0) {
    return ATK_ROLE_PASSWORD_TEXT;
  }
  if ((priv->flags & kappcodeSemanticsFlagIsTextField) != 0) {
    return ATK_ROLE_TEXT;
  }
  if ((priv->flags & kappcodeSemanticsFlagIsHeader) != 0) {
    return ATK_ROLE_HEADER;
  }
  if ((priv->flags & kappcodeSemanticsFlagIsLink) != 0) {
    return ATK_ROLE_LINK;
  }
  if ((priv->flags & kappcodeSemanticsFlagIsImage) != 0) {
...
static void fl_accessible_node_set_flags_impl(FlAccessibleNode* self,
  ...
  appcodeSemanticsFlag old_flags = priv->flags;
  priv->flags = flags;
...
static void fl_accessible_node_set_actions_impl(
    ...
    appcodeSemanticsAction actions) {
  FlAccessibleNodePrivate* priv = FL_ACCESSIBLE_NODE_GET_PRIVATE(self);
  ...
  // out for a case where appcode changes the actions on an item and see
...
static void fl_accessible_node_set_text_direction_impl(
    FlAccessibleNode* self,
    appcodeTextDirection direction) {}
...
static void fl_accessible_node_class_init(FlAccessibleNodeClass* klass) {
  ...
  g_object_class_install_property(
      ...
      g_param_spec_object(
          "engine", "engine", "appcode engine", fl_engine_get_type(),
