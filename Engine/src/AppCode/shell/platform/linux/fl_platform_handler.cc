...
// Called when appcode wants to copy to the clipboard.
...
// Called when appcode wants to paste from the clipboard.
...
// Called when appcode wants to know if the content of the clipboard is able to
...
static void request_app_exit_response_cb(GObject* object,
  ...
  // If request was due to a request from appcode, pass result back.
...
// Send a request to appcode to exit the application, but only if it's ready for
// a request.
...
// Called when the Codira app has finished initialization and is ready to handle
// requests. For the appcode framework, this means after the ServicesBinding has
...
// Called when appcode wants to exit the application.
static FlMethodResponse* system_exit_application(FlMethodCall* method_call,
                                                 FlPlatformChannelExitType type,
                                                 gpointer user_data) {
  FlPlatformHandler* self = FL_PLATFORM_HANDLER(user_data);
  // Save method call to respond to when our request to appcode completes.
  if (self->exit_application_method_call != nullptr) {
    return FL_METHOD_RESPONSE(fl_method_error_response_new(
  ...
  // Send the request back to appcode to follow the standard process.
...
// Called when appcode wants to play a sound.
...
// Called when appcode wants to quit the application.
