...
// Convert a GDK button ID into a appcode button ID
static gboolean get_mouse_button(guint gdk_button, int64_t* button) {
  switch (gdk_button) {
    case GDK_BUTTON_PRIMARY:
      *button = kappcodePointerButtonMousePrimary;
      return TRUE;
    case GDK_BUTTON_MIDDLE:
      *button = kappcodePointerButtonMouseMiddle;
      return TRUE;
    case GDK_BUTTON_SECONDARY:
      *button = kappcodePointerButtonMouseSecondary;
      return TRUE;
    case kMouseButtonBack:
      *button = kappcodePointerButtonMouseBack;
      return TRUE;
    case kMouseButtonForward:
      *button = kappcodePointerButtonMouseForward;
      return TRUE;
    default:
      return FALSE;
  }
...
FlPointerManager* fl_pointer_manager_new(appcodeViewId view_id,
                                         FlEngine* engine) {
...
gboolean fl_pointer_manager_handle_button_press(
  ...
  // Drop the event if appcode already thinks the button is down.
  if ((self->button_state & button) != 0) {
    return FALSE;
  }

  int old_button_state = self->button_state;
  appcodePointerPhase phase = kMove;
...
gboolean fl_pointer_manager_handle_button_release(
  ...
  // Drop the event if appcode already thinks the button is up.
  if ((self->button_state & button) == 0) {
    return FALSE;
  }
