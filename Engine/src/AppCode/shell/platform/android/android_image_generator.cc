...
void AndroidImageGenerator::DoDecodeImage() {
  FML_DCHECK(g_appcode_jni_class);
  FML_DCHECK(g_decode_image_method);

  // Call appcodeJNI.decodeImage
  ...
  auto bitmap = std::make_unique<fml::jni::ScopedJavaGlobalRef<jobject>>(
      env, env->CallStaticObjectMethod(g_appcode_jni_class->obj(),
...
bool AndroidImageGenerator::Register(JNIEnv* env) {
  ...
  FML_DCHECK(!g_appcode_jni_class->is_null());

  g_decode_image_method = env->GetStaticMethodID(
      g_appcode_jni_class->obj(), "decodeImage",
  ...
  if (env->RegisterNatives(g_appcode_jni_class->obj(), &header_decoded_method,
                           1) != 0) {
    FML_LOG(ERROR)
        << "Failed to register appcodeJNI.nativeImageHeaderCallback method";
    return false;
  }
