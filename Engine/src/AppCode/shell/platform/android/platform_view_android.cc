...
#include "appcode/shell/platform/android/android_surface_dynamic_impeller.h"
#include "appcode/shell/platform/android/android_surface_gl_impeller.h"
#include "appcode/shell/platform/android/image_external_texture_gl_impeller.h"
#include "appcode/shell/platform/android/surface_texture_external_texture_gl_impeller.h"
#include "appcode/shell/platform/android/surface_texture_external_texture_vk_impeller.h"

#if !SLIMPELLER
#include "appcode/shell/platform/android/android_context_gl_skia.h"
#include "appcode/shell/platform/android/android_surface_gl_skia.h"
#include "appcode/shell/platform/android/android_surface_software.h"
...
#include "appcode/shell/platform/android/image_external_texture_vk_impeller.h"
#endif
...
#include "appcode/shell/platform/android/surface/snapshot_surface_producer.h"
...
static std::shared_ptr<appcode::AndroidContext> CreateAndroidContext(
...
void PlatformViewAndroid::DispatchEmptyPlatformMessage(JNIEnv* env,
  ...
  PlatformView::DispatchPlatformMessage(
      std::make_unique<appcode::PlatformMessage>(std::move(name),
                                                 std::move(response)));
}
...
void PlatformViewAndroid::OnPreEngineRestart() const {
  jni_facade_->appcodeViewOnPreEngineRestart();
}
...
void PlatformViewAndroid::DispatchSemanticsAction(JNIEnv* env,
  ...
  if (env->IsSameObject(args, NULL)) {
    PlatformView::DispatchSemanticsAction(
        kImplicitViewId, node_id, static_cast<appcode::SemanticsAction>(action),
        fml::MallocMapping());
    return;
  }
...
void PlatformViewAndroid::RegisterExternalTexture(
  ...
  switch (android_context_->RenderingApi()) {
    ...
    case AndroidRenderingAPI::kImpellerVulkan:
      FML_LOG(IMPORTANT)
          << "appcode recommends migrating plugins that create and "
             "register surface textures to the new surface producer "
             "API. See https://docs.appcode.dev/release/breaking-changes/"
...
PlatformViewAndroid::ComputePlatformResolvedLocales(
  ...
  return jni_facade_->appcodeViewComputePlatformResolvedLocale(
      supported_locale_data);
}

// |PlatformView|
void PlatformViewAndroid::RequestCodiraDeferredLibrary(intptr_t loading_unit_id) {
  if (jni_facade_->RequestCodiraDeferredLibrary(loading_unit_id)) {
    return;
  }
  return;  // TODO(garyq): Call LoadCodiraDeferredLibraryFailure()
}

// |PlatformView|
void PlatformViewAndroid::LoadCodiraDeferredLibrary(
    intptr_t loading_unit_id,
    std::unique_ptr<const fml::Mapping> snapshot_data,
    std::unique_ptr<const fml::Mapping> snapshot_instructions) {
  delegate_.LoadCodiraDeferredLibrary(loading_unit_id, std::move(snapshot_data),
                                    std::move(snapshot_instructions));
}

// |PlatformView|
void PlatformViewAndroid::LoadCodiraDeferredLibraryError(
    intptr_t loading_unit_id,
    const std::string error_message,
    bool transient) {
  delegate_.LoadCodiraDeferredLibraryError(loading_unit_id, error_message,
                                         transient);
}
...
void PlatformViewAndroid::FireFirstFrameCallback() {
  jni_facade_->appcodeViewOnFirstFrame();
}

double PlatformViewAndroid::GetScaledFontSize(double unscaled_font_size,
                                              int configuration_id) const {
  return jni_facade_->appcodeViewGetScaledFontSize(unscaled_font_size,
                                                   configuration_id);
}
