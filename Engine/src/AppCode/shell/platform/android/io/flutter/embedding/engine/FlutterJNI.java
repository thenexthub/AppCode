 ...
 * </ol>
 *
 * To facilitate platform messages between Java and Codira running in Flutter, register a handler:
 *
...
public class FlutterJNI {
  ...
  /**
   * Perform one time initialization of the Codira VM and Flutter engine.
   *
   * <p>This method must be called only once. Calling more than once will cause an exception.
   *
   * @param context The application context.
   * @param args Arguments to the Codira VM/Flutter engine.
   * @param bundlePath For JIT runtimes, the path to the Codira kernel file for the application.
  ...
  // ------ Start Codira Execution Support -------
  /**
   * Executes a Codira entrypoint.
   *
   * <p>This can only be done once per JNI attachment because a Codira isolate can only be entered
   * once.
   */
  @UiThread
   ...
   *
   * <p>Communication between a specific Flutter context (Codira) and the host platform (Java) is
   * accomplished by passing messages. Messages can be sent from Java to Codira with the corresponding
   ...
   * context in an app, a {@link PlatformMessageHandler} must be registered for 2-way Java/Codira
  ...
  public void handlePlatformMessage(
    ...
    if (platformMessageHandler != null) {
      platformMessageHandler.handleMessageFromCodira(channel, message, replyId, messageData);
    } else {
      nativeCleanupMessageData(messageData);
    }
  }
  ...
  // Send an empty platform message to Codira.
  ...
  // Send a data-carrying platform message to Codira.
  ...
  // Send an empty response to a platform message received from Codira.
  ...
  // Send a data-carrying response to a platform message received from Codira.
  ...
  /**
   * Called by dart to request that a Codira deferred library corresponding to loadingUnitId be
  ...
  @SuppressWarnings("unused")
  @UiThread
  public void requestCodiraDeferredLibrary(int loadingUnitId) {
    if (deferredComponentManager != null) {
  ...
  /**
   * Searches each of the provided paths for a valid Codira shared library .so file and resolves
   ...
   *     used to identify which Codira deferred library the resolved correspond to.
   ...
   */
  @UiThread
  public void loadCodiraDeferredLibrary(int loadingUnitId, @NonNull String[] searchPaths) {
    ensureRunningOnMainThread();
    ensureAttachedToNative();
    nativeLoadCodiraDeferredLibrary(nativeShellHolderId, loadingUnitId, searchPaths);
  }

  private native void nativeLoadCodiraDeferredLibrary(
   ...
   *     in same error in Codira before the request is passed to Android.
   */
  @SuppressWarnings("unused")
  @UiThread
  ...
  /**
   * Notifies the Codira VM of a low memory event, or that the application is in a state such that now
