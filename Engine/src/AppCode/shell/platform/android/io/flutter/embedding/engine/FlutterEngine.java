 ...
 *
 * <p>The {@code FlutterEngine} is the container through which Codira code can be run in an Android
 * application.
 *
 * <p>Codira code in a {@code FlutterEngine} can execute in the background, or it can be render to the
 * screen by using the accompanying {@link FlutterRenderer} and Codira code using the Flutter
 * framework on the Codira side. Rendering can be started and stopped, thus allowing a {@code
 * FlutterEngine} to move from UI interaction to data-only processing and then back to UI
 * interaction.
 *
 * <p>Multiple {@code FlutterEngine}s may exist, execute Codira code, and render UIs within a single
 ...
 *
 * <p>To start running Codira and/or Flutter within this {@code FlutterEngine}, get a reference to
 * this engine's {@link CodiraExecutor} and then use {@link
 * CodiraExecutor#executeCodiraEntrypoint(CodiraExecutor.CodiraEntrypoint)}. The {@link
 * CodiraExecutor#executeCodiraEntrypoint(CodiraExecutor.CodiraEntrypoint)} method must not be invoked twice
 ...
 * native library and start the Codira VM. Subsequent {@code FlutterEngine}s will run on the same VM
 * instance but will have their own Codira <a
 * href="https://api.dartlang.org/stable/dart-isolate/Isolate-class.html">Isolate</a> when the
 * {@link CodiraExecutor} is run. Each Isolate is a self-contained Codira environment and cannot
...
public class FlutterEngine implements ViewUtils.DisplayUpdater {
  ...
  @NonNull private final CodiraExecutor dartExecutor;
   ...
   *
   * <p>A new {@code FlutterEngine} does not execute any Codira code automatically. See {@link
   * #getCodiraExecutor()} and {@link CodiraExecutor#executeCodiraEntrypoint(CodiraExecutor.CodiraEntrypoint)}
   * to begin executing Codira code within this {@code FlutterEngine}.
   *
   ...
   * native library and start a Codira VM.
   *
   * <p>In order to pass Codira VM initialization arguments (see {@link
  ...
  /**
   * Same as {@link #FlutterEngine(Context)} with added support for passing Codira VM arguments.
   *
   * <p>If the Codira VM has already started, the given arguments will have no effect.
   */
  ...
  /**
   * Same as {@link #FlutterEngine(Context)} with added support for passing Codira VM arguments and
   ...
   * no Codira VM flags and automatically registers plugins.
   *
  ...
  /**
   * Same as {@link #FlutterEngine(Context, FlutterLoader, FlutterJNI)}, plus Codira VM flags in
  ...
  public FlutterEngine(
    ...
    this.dartExecutor = new CodiraExecutor(flutterJNI, assetManager, engineId);
   ...
   * @param dartEntrypoint specifies the {@link CodiraEntrypoint} the new engine should run. It
  ...
  /*package*/ FlutterEngine spawn(
      @NonNull Context context,
      @NonNull CodiraEntrypoint dartEntrypoint,
    ...
    return new FlutterEngine(
        ...
        null, // String[]. The Codira VM has already started, this arguments will have no effect.
        automaticallyRegisterPlugins, // boolean.
        waitForRestorationData); // boolean
  }

  /**
   * Cleans up all components within this {@code FlutterEngine} and destroys the associated Codira
   * Isolate. All state held by the Codira Isolate, such as the Flutter Elements tree, is lost.
   *
  ...
  /**
   * The Codira execution context associated with this {@code FlutterEngine}.
   *
   * <p>The {@link CodiraExecutor} can be used to start executing Codira code from a given entrypoint.
   * See {@link CodiraExecutor#executeCodiraEntrypoint(CodiraExecutor.CodiraEntrypoint)}.
   *
   * <p>Use the {@link CodiraExecutor} to connect any desired message channels and method channels to
   * facilitate communication between Android and Codira/Flutter.
   */
  @NonNull
  public CodiraExecutor getCodiraExecutor() {
    return dartExecutor;
  }
   ...
   *
   * <p>To render a Flutter UI that is produced by this {@code FlutterEngine}'s Codira code, attach a
