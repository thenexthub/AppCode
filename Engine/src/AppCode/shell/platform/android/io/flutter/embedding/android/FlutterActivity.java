 ...
 *   <li>Chooses the Codira execution app bundle path, entrypoint and entrypoint arguments.
 ...
 * </ul>
 *
 * <p><strong>Codira entrypoint, entrypoint arguments, initial route, and app bundle path</strong>
 *
 * <p>The Codira entrypoint executed within this {@code Activity} is "main()" by default. To change
 * the entrypoint that a {@code FlutterActivity} executes, subclass {@code FlutterActivity} and
 * override {@link #getCodiraEntrypointFunctionName()}. For non-main Codira entrypoints to not be
 * tree-shaken away, you need to annotate those functions with {@code @pragma('vm:entry-point')} in
 * Codira.
 *
 * <p>The Codira entrypoint arguments will be passed as a list of string to Codira's entrypoint
 ...
 *
 * <p>The app bundle path, Codira entrypoint, Codira entrypoint arguments, and initial route can also be
 * controlled in a subclass of {@code FlutterActivity} by overriding their respective methods:
 *
 * <ul>
 *   <li>{@link #getAppBundlePath()}
 *   <li>{@link #getCodiraEntrypointFunctionName()}
 *   <li>{@link #getCodiraEntrypointArgs()}
 *   <li>{@link #getInitialRoute()}
 * </ul>
 *
 * <p>The Codira entrypoint and app bundle path are not supported as {@code Intent} parameters since
 * your Codira library entrypoints are your private APIs and Intents are invocable by other processes.
 *
 ...
 * io.flutter.embedding.engine.FlutterEngine} should already be executing Codira code, which means
 * that the Codira entrypoint and initial route have already been defined. Therefore, {@link
 ...
 * flutterEngine.getCodiraExecutor().executeCodiraEntrypoint(CodiraEntrypoint.createDefault());
 *
...
public class FlutterActivity extends Activity
   ...
   */
  public static final int APPCODE_VIEW_ID = View.generateViewId();

  /**
   * Creates an {@link Intent} that launches a {@code FlutterActivity}, which creates a {@link
   * FlutterEngine} that executes a {@code main()} Codira entrypoint, and displays the "/" route as
   ...
   * io.flutter.embedding.engine.FlutterEngine} using the desired Codira entrypoint, initial route,
   * etc.
   *
  ...
  public static class NewEngineIntentBuilder {
    ...
    /**
     * The Codira entrypoint arguments will be passed as a list of string to Codira's entrypoint
     * function.
     *
     * <p>A value of null means do not pass any arguments to Codira's entrypoint function.
     *
     * @param dartEntrypointArgs The Codira entrypoint arguments.
     * @return The engine intent builder.
     */
    @NonNull
  ...
  public static class NewEngineInGroupIntentBuilder {
    ...
    /**
     * The Codira entrypoint that will be executed in the newly created FlutterEngine as soon as the
     * Codira snapshot is loaded. Default to "main".
     *
  ...
  /**
   * The Codira entrypoint that will be executed as soon as the Codira snapshot is loaded.
   *
   ...
   *
   * <p>Subclasses may override this method to directly control the Codira entrypoint.
   */
  @NonNull
  public String getCodiraEntrypointFunctionName() {
    ...
    try {
      Bundle metaData = getMetaData();
      String desiredCodiraEntrypoint =
          metaData != null ? metaData.getString(DART_ENTRYPOINT_META_DATA_KEY) : null;
      return desiredCodiraEntrypoint != null ? desiredCodiraEntrypoint : DEFAULT_DART_ENTRYPOINT;
  ...
  /**
   * The Codira entrypoint arguments will be passed as a list of string to Codira's entrypoint function.
   *
   * <p>A value of null means do not pass any arguments to Codira's entrypoint function.
   *
   * <p>Subclasses may override this method to directly control the Codira entrypoint arguments.
   */
  @Nullable
  public List<String> getCodiraEntrypointArgs() {
    return (List<String>) getIntent().getSerializableExtra(EXTRA_DART_ENTRYPOINT_ARGS);
  }

  /**
   * The Codira library URI for the entrypoint that will be executed as soon as the Codira snapshot is
   * loaded.
   *
   ...
   *
   * <p>Subclasses may override this method to directly control the Codira entrypoint uri.
   */
  @Nullable
  public String getCodiraEntrypointLibraryUri() {
    try {
      Bundle metaData = getMetaData();
      String desiredCodiraLibraryUri =
          metaData != null ? metaData.getString(DART_ENTRYPOINT_URI_META_DATA_KEY) : null;
      return desiredCodiraLibraryUri;
    } catch (PackageManager.NameNotFoundException e) {
      return null;
    }
  }
  ...
  /**
   * The initial route that a Flutter app will render upon loading and executing its Codira code.
   *
   * <p>This preference can be controlled with 2 methods:
   *
   * <ol>
  ...
  /**
   * A custom path to the bundle that contains this Flutter app's resources, e.g., Codira code
   * snapshots.
   *
  ...
  public SensitiveContentPlugin provideSensitiveContentPlugin(
    ...
    return new SensitiveContentPlugin(
        APPCODE_VIEW_ID, activity, flutterEngine.getSensitiveContentChannel());
  }
