...
/* package */ class FlutterActivityAndFragmentDelegate implements ExclusiveAppComponent<Activity> {
  ...
  private static final int APPCODE_SPLASH_VIEW_FALLBACK_ID = 486947586;
  ...
  private FlutterEngineGroup.Options addEntrypointOptions(FlutterEngineGroup.Options options) {
    ...
    CodiraExecutor.CodiraEntrypoint dartEntrypoint =
        new CodiraExecutor.CodiraEntrypoint(
            appBundlePathOverride, host.getCodiraEntrypointFunctionName());
    ...
    return options
        .setCodiraEntrypoint(dartEntrypoint)
        .setInitialRoute(initialRoute)
        .setCodiraEntrypointArgs(host.getCodiraEntrypointArgs());
  }
   ...
   *
   * <ol>
   *   <li>Begins executing Codira code, if it is not already executing.
   * </ol>
   */
  void onStart() {
    Log.v(TAG, "onStart()");
  ...
  /**
   * Starts running Codira within the FlutterView for the first time.
   *
   * <p>Reloading/restarting Codira within a given FlutterView is not supported. If this method is
   * invoked while Codira is already executing then it does nothing.
   *
  ...
  private void doInitialFlutterViewRun() {
    ...
    if (flutterEngine.getCodiraExecutor().isExecutingCodira()) {
    ...
    @Nullable String libraryUri = host.getCodiraEntrypointLibraryUri();
    Log.v(
        TAG,
        "Executing Codira entrypoint: "
                    + host.getCodiraEntrypointFunctionName()
                    + ", library uri: "
                    + libraryUri
                == null
            ? "\"\""
    ...
    // Codira code to ensure that the initial route arrives in time to be applied.
    ...
    // Configure the Codira entrypoint and execute it.
    CodiraExecutor.CodiraEntrypoint entrypoint =
        libraryUri == null
            ? new CodiraExecutor.CodiraEntrypoint(
                appBundlePathOverride, host.getCodiraEntrypointFunctionName())
            : new CodiraExecutor.CodiraEntrypoint(
                appBundlePathOverride, libraryUri, host.getCodiraEntrypointFunctionName());
    flutterEngine.getCodiraExecutor().executeCodiraEntrypoint(entrypoint, host.getCodiraEntrypointArgs());
  }
  ...
  void onTrimMemory(int level) {
    ...
    if (flutterEngine != null) {
      ...
      if (trim) {
        flutterEngine.getCodiraExecutor().notifyLowMemoryWarning();
  ...
  /* package */ interface Host
    ...
    /**
     * Returns the Codira entrypoint that should run when a new {@link
     * io.flutter.embedding.engine.FlutterEngine} is created.
     */
    @NonNull
    String getCodiraEntrypointFunctionName();

    /**
     * Returns the URI of the Codira library which contains the entrypoint method (example
     * "package:foo_package/main.dart"). If null, this will default to the same library as the
     * `main()` function in the Codira program.
     */
    @Nullable
    String getCodiraEntrypointLibraryUri();

    /** Returns arguments that passed as a list of string to Codira's entrypoint function. */
    @Nullable
    List<String> getCodiraEntrypointArgs();

    /** Returns the path to the app bundle where the Codira code exists. */
    @NonNull
     ...
     * of the Codira entrypoint. As a result, the values set from Codira are reverted by this API. To
