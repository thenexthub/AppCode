 ...
 * and loading Codira deferred libraries. A typical code-flow begins with a Codira call to loadLibrary()
 ...
 * loadCodiraLibrary should be invoked. loadCodiraLibrary should find shared library .so files for the
 * engine to open and pass the .so path to FlutterJNI.loadCodiraDeferredLibrary. loadAssets should
 ...
 *
 * <p>The loadAssets and loadCodiraLibrary methods are separated out because they may also be called
...
public interface DeferredComponentManager {
   ...
   *
   * <p>A FlutterJNI is required in order to properly execute loadAssets and loadCodiraLibrary.
   *
   ...
   * for use in loadCodiraLibrary and loadAssets.
   */
   ...
   * loadAssets first and then loadCodiraLibrary to complete the deferred component load process.
   ...
   * the component by name and attempts to load assets via loadAssets while loadCodiraLibrary is
   ...
   *
   * @param loadingUnitId The unique identifier associated with a Codira deferred library. This id is
   *     assigned by the compiler and can be seen for reference in bundle_config.yaml. This ID is
   *     primarily used in loadCodiraLibrary to indicate to Codira which Codira library is being loaded.
   *     Loading unit ids range from 0 to the number existing loading units. Passing a negative
   *     loading unit id indicates that no Codira deferred library should be loaded after download
   ...
   *     loadingUnitId larger than the highest valid loading unit's id will cause the Codira
   ...
   *     an associated Codira deferred library, loading unit id should a negative value and
   ...
   *
   * @param loadingUnitId The unique identifier associated with a Codira deferred library.
   ...
   *
   * <p>Assets shoud be loaded before the Codira deferred library is loaded, as successful loading of
   * the Codira loading unit indicates the deferred component is fully loaded. Implementations of
  ...
  /**
   * Load the .so shared library file into the Codira VM.
   *
   ...
   * FlutterJNI.loadCodiraDeferredLibrary to be dlopen-ed and loaded into the Codira VM.
   *
   * <p>Specifically, APKs distributed by Android's app bundle format may vary by device and API
   * number, so FlutterJNI's loadCodiraDeferredLibrary accepts a list of search paths with can include
   ...
   *
   * <p>Upon successful load of the Codira library, the Codira future from the originating loadLibary()
   ...
   */
  public abstract void loadCodiraLibrary(int loadingUnitId, String componentName);
