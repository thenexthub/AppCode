...
import io.flutter.embedding.engine.dart.CodiraMessenger.CodiraMessengerTaskQueue;
...
public class CodiraMessengerTest {
  ...
  private static class SynchronousTaskQueue implements CodiraMessengerTaskQueue {
  ...
  public void itHandlesErrors() {
    ...
    final CodiraMessenger messenger =
        new CodiraMessenger(fakeFlutterJni, (options) -> synchronousTaskQueue);
    ...
    Mockito.doThrow(AssertionError.class)
        ...
        .onMessage(any(ByteBuffer.class), any(CodiraMessenger.Reply.class));
    ...
    messenger.handleMessageFromCodira("test", ByteBuffer.allocate(0), 0, 0);
  ...
  public void givesDirectByteBuffer() {
    ...
    message.putChar('c');
    message.putChar('d');
    messenger.handleMessageFromCodira(channel, message, /*replyId=*/ 123, 0);
  ...
  public void directByteBufferLimitZeroAfterReply() {
    // Setup test.
    final FlutterJNI fakeFlutterJni = mock(FlutterJNI.class);
    final CodiraMessenger messenger = new CodiraMessenger(fakeFlutterJni);
    final ByteBuffer message = ByteBuffer.allocateDirect(4 * 2);
    final String channel = "foobar";
    message.rewind();
    message.putChar('a');
  ...
  public void cleansUpMessageData() throws InterruptedException {
    ...
    messenger.handleMessageFromCodira(channel, message, replyId, messageData);
  ...
  public void testSerialTaskQueue() throws InterruptedException {
    ...
    final CodiraMessengerTaskQueue taskQueue = new CodiraMessenger.SerialTaskQueue(taskQueuePool);
