...
int64_t flagsToInt64(appcode::SemanticsFlags flags) {
  int64_t result = 0;
...
void PlatformViewAndroidDelegate::UpdateSemantics(
    ...
    const appcode::CustomAccessibilityActionUpdates& actions) {
  {
    ...
    for (const auto& value : update) {
      ...
      // sending.
      const appcode::SemanticsNode& node = value.second;
    ...
    for (const auto& value : actions) {
      ...
      // sending.
      const appcode::CustomAccessibilityAction& action = value.second;
    ...
    if (!actions_buffer.empty()) {
      jni_facade_->appcodeViewUpdateCustomAccessibilityActions(actions_buffer,
                                                               action_strings);
    }

    if (!buffer.empty()) {
      jni_facade_->appcodeViewUpdateSemantics(buffer, strings,
                                              string_attribute_args);
    }
  }
}
