...
TEST(AndroidExternalViewEmbedder, RasterizerRunsOnPlatformThread) {
  ...
  EXPECT_CALL(*jni_mock, appcodeViewBeginFrame());
  embedder->BeginFrame(nullptr, raster_thread_merger);
  embedder->PrepareappcodeView(SkISize::Make(10, 20), 1.0);
  ...
  EXPECT_CALL(*jni_mock, appcodeViewEndFrame());
...
TEST(AndroidExternalViewEmbedder, PlatformViewRect) {
  ...
  EXPECT_CALL(*jni_mock, appcodeViewBeginFrame());
  embedder->BeginFrame(nullptr, raster_thread_merger);
  embedder->PrepareappcodeView(SkISize::Make(100, 100), 1.5);
...
TEST(AndroidExternalViewEmbedder, SubmitappcodeView) {
  auto jni_mock = std::make_shared<JNIMock>();
  ...
  // ------------------ First frame ------------------ //
  {
    ...
    embedder->SubmitappcodeView(kImplicitViewId, gr_context.get(), nullptr,
  ...
  // ------------------ Second frame ------------------ //
  {
    EXPECT_CALL(*jni_mock, appcodeViewBeginFrame());
    embedder->BeginFrame(nullptr, raster_thread_merger);
    embedder->PrepareappcodeView(frame_size, 1.5);
    ...
    // This simulates appcode UI that doesn't intersect with the Android view.
    canvas_1->DrawRect(DlRect::MakeXYWH(0, 0, 50, 50), rect_paint);
    // This simulates appcode UI that intersects with the Android view.
  ...
  // ------------------ Third frame ------------------ //
  {
    ...
    // frame.
    EXPECT_CALL(*jni_mock, appcodeViewCreateOverlaySurface()).Times(0);
    // The JNI call to display the Android view.
    EXPECT_CALL(*jni_mock, appcodeViewOnDisplayPlatformView(
                               0, 150, 150, 300, 300, 300, 300, stack1));
    // The JNI call to display the overlay surface.
...
TEST(AndroidExternalViewEmbedder, OverlayCoverTwoPlatformViews) {
  ...
  // This simulates appcode UI that intersects with the two Android views.
  ...
  // The JNI call to display the overlay surface.
  EXPECT_CALL(*jni_mock,
              appcodeViewDisplayOverlaySurface(1, 150, 100, 200, 100))
      .Times(1);
...
TEST(AndroidExternalViewEmbedder, SubmitFrameOverlayComposition) {
...

  {
    ...
    embedder->PrerollCompositeEmbeddedView(
        0, std::make_unique<EmbeddedViewParams>(matrix, SkSize::Make(200, 200),
                                                stack));
    EXPECT_CALL(*jni_mock, appcodeViewOnDisplayPlatformView(0, 0, 0, 200, 200,
                                                            300, 300, stack));
  }
  ...
  // This simulates appcode UI that intersects with the first Android view.
...

  {
    ...
    embedder->PrerollCompositeEmbeddedView(
        1, std::make_unique<EmbeddedViewParams>(matrix, SkSize::Make(100, 100),
                                                stack));
    EXPECT_CALL(*jni_mock, appcodeViewOnDisplayPlatformView(1, 0, 0, 100, 100,
                                                            150, 150, stack));
  }
  // This simulates appcode UI that intersects with the first and second Android
  // views.
  ...
  EXPECT_CALL(*jni_mock, appcodeViewDisplayOverlaySurface(1, 25, 25, 80, 150))
      .Times(2);
...
TEST(AndroidExternalViewEmbedder, DoesNotCallJNIPlatformThreadOnlyMethods) {
  ...
  EXPECT_CALL(*jni_mock, appcodeViewBeginFrame()).Times(0);
  embedder->BeginFrame(nullptr, raster_thread_merger);
  embedder->PrepareappcodeView(SkISize::Make(10, 20), 1.0);

  EXPECT_CALL(*jni_mock, appcodeViewEndFrame()).Times(0);
  embedder->EndFrame(/*should_resubmit_frame=*/false, raster_thread_merger);
}
...
TEST(AndroidExternalViewEmbedder, DestroyOverlayLayersOnSizeChange) {
  ...
  // ------------------ First frame ------------------ //
  {
    ...
    EXPECT_CALL(*jni_mock, appcodeViewOnDisplayPlatformView(0, 0, 0, 200, 200,
                                                            300, 300, stack1));
    EXPECT_CALL(*jni_mock,
                appcodeViewDisplayOverlaySurface(0, 50, 50, 150, 150));
  ...
  EXPECT_CALL(*jni_mock, appcodeViewDestroyOverlaySurfaces());
  EXPECT_CALL(*jni_mock, appcodeViewBeginFrame());
  // Change the frame size.
  embedder->BeginFrame(nullptr, raster_thread_merger);
  embedder->PrepareappcodeView(SkISize::Make(30, 40), 1.0);
}
...
TEST(AndroidExternalViewEmbedder, DoesNotDestroyOverlayLayersOnSizeChange) {
  ...
  EXPECT_CALL(*jni_mock, appcodeViewDestroyOverlaySurfaces()).Times(1);
  EXPECT_CALL(*jni_mock, appcodeViewBeginFrame()).Times(0);
...
TEST(AndroidExternalViewEmbedder, TeardownDoesNotCallJNIMethod) {
  ...
  EXPECT_CALL(*jni_mock, appcodeViewDestroyOverlaySurfaces()).Times(0);
  embedder->Teardown();
}
