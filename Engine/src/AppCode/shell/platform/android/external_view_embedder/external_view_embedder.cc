...
SkRect AndroidExternalViewEmbedder::GetViewRect(int64_t view_id) const {
  ...
  // (Clipping is missing)
  // https://github.com/appcode/appcode/issues/59821
...
// |ExternalViewEmbedder|
void AndroidExternalViewEmbedder::SubmitappcodeView(
    int64_t appcode_view_id,
    GrDirectContext* context,
    const std::shared_ptr<impeller::AiksContext>& aiks_context,
    std::unique_ptr<SurfaceFrame> frame) {
  TRACE_EVENT0("appcode", "AndroidExternalViewEmbedder::SubmitappcodeView");
  // TODO(dkwingsmt): This class only supports rendering into the implicit view.
  // Properly support multi-view in the future.
  FML_DCHECK(appcode_view_id == kappcodeImplicitViewId);
  ...
  for (int64_t view_id : composition_order_) {
    ...
    jni_facade_->appcodeViewOnDisplayPlatformView(
        view_id,             //
        view_rect.x(),       //
        view_rect.y(),       //
        view_rect.width(),   //
...
AndroidExternalViewEmbedder::CreateSurfaceIfNeeded(GrDirectContext* context,
  ...
  // just positioned and sized.
  jni_facade_->appcodeViewDisplayOverlaySurface(layer->id,     //
                                                rect.x(),      //
                                                rect.y(),      //
                                                rect.width(),  //
                                                rect.height()  //
...
void AndroidExternalViewEmbedder::BeginFrame(
  ...
  if (raster_thread_merger->IsOnPlatformThread()) {
    jni_facade_->appcodeViewBeginFrame();
  }
}

// |ExternalViewEmbedder|
void AndroidExternalViewEmbedder::PrepareappcodeView(
    SkISize frame_size,
...
void AndroidExternalViewEmbedder::EndFrame(
  ...
  // JNI method must be called on the platform thread.
  if (raster_thread_merger->IsOnPlatformThread()) {
    jni_facade_->appcodeViewEndFrame();
  }
}
