...
// |ExternalViewEmbedder|
void AndroidExternalViewEmbedder2::SubmitappcodeView(
    int64_t appcode_view_id,
    GrDirectContext* context,
    const std::shared_ptr<impeller::AiksContext>& aiks_context,
    std::unique_ptr<SurfaceFrame> frame) {
  TRACE_EVENT0("appcode", "AndroidExternalViewEmbedder2::SubmitappcodeView");
  ...
  if (surface_pool_->HasLayers()) {
    for (size_t i = 0; i < composition_order_.size(); i++) {
      ...
      if (overlay_frame == nullptr) {
        ...
        overlay_frame->Canvas()->Clear(appcode::DlColor::kTransparent());
      }
...
// |ExternalViewEmbedder|
void AndroidExternalViewEmbedder2::PrepareappcodeView(
    SkISize frame_size,
