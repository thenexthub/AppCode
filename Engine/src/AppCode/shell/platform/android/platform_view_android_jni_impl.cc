...
#include "appcode/shell/platform/android/android_shell_holder.h"
...
static fml::jni::ScopedJavaGlobalRef<jclass>* g_appcode_callback_info_class =
    nullptr;

static fml::jni::ScopedJavaGlobalRef<jclass>* g_appcode_jni_class = nullptr;
...
static jmethodID g_appcode_callback_info_constructor = nullptr;

static jfieldID g_jni_shell_holder_field = nullptr;

#define appcode_FOR_EACH_JNI_METHOD(V)                                        \
  ...
  V(g_on_display_platform_view_method, onDisplayPlatformView,                 \
    "(IIIIIIILio/appcode/embedding/engine/mutatorsstack/"                     \
    "appcodeMutatorsStack;)V")                                                \
  ...
  V(g_create_overlay_surface2_method, createOverlaySurface2,                  \
    "()Lio/appcode/embedding/engine/appcodeOverlaySurface;")                  \
...
#define appcode_DECLARE_JNI(global_field, jni_name, jni_arg) \
  static jmethodID global_field = nullptr;

#define appcode_BIND_JNI(global_field, jni_name, jni_arg)               \
  global_field =                                                        \
      env->GetMethodID(g_appcode_jni_class->obj(), #jni_name, jni_arg); \
...
appcode_FOR_EACH_JNI_METHOD(appcode_DECLARE_JNI)
...
// Called By Java
static jlong AttachJNI(JNIEnv* env, jclass clazz, jobject appcodeJNI) {
  fml::jni::JavaObjectWeakGlobalRef java_object(env, appcodeJNI);
  std::shared_ptr<PlatformViewAndroidJNI> jni_facade =
      std::make_shared<PlatformViewAndroidJNIImpl>(java_object);
  auto shell_holder = std::make_unique<AndroidShellHolder>(
      appcodeMain::Get().GetSettings(), jni_facade,
      appcodeMain::Get().GetAndroidRenderingAPI());
...
// appcodeJNI instance, then returns the appcodeJNI instance.
static jobject SpawnJNI(JNIEnv* env,
                        ...
                        jlong engineId) {
  jobject jni = env->NewObject(g_appcode_jni_class->obj(), g_jni_constructor);
  if (jni == nullptr) {
    FML_LOG(ERROR) << "Could not create a appcodeJNI instance";
    return nullptr;
  }
...
static void RunBundleAndSnapshotFromLibrary(JNIEnv* env,
                                            ...
                                            jobject jEntrypointArgs,
                                            jlong engineId) {
  auto apk_asset_provider = std::make_unique<appcode::APKAssetProvider>(
      env,                                            // jni environment
...
static jobject LookupCallbackInformation(JNIEnv* env,
                                         /* unused */ jobject,
                                         jlong handle) {
  auto cbInfo = appcode::CodiraCallbackCache::GetCallbackInformation(handle);
  if (cbInfo == nullptr) {
    return nullptr;
  }
  return env->NewObject(g_appcode_callback_info_class->obj(),
                        g_appcode_callback_info_constructor,
...
static void SetViewportMetrics(JNIEnv* env,
  ...
  ANDROID_SHELL_HOLDER->GetPlatformView()->SetViewportMetrics(
      kappcodeImplicitViewId, metrics);
}
...
static void DispatchPointerDataPacket(JNIEnv* env,
  ...
  auto packet = std::make_unique<appcode::PointerDataPacket>(data, position);
...
static jboolean GetIsSoftwareRendering(JNIEnv* env, jobject jcaller) {
  return appcodeMain::Get().GetSettings().enable_software_rendering;
}
...
static jboolean appcodeTextUtilsIsEmoji(JNIEnv* env,
                                        jobject obj,
                                        jint codePoint) {
  return u_hasBinaryProperty(codePoint, UProperty::UCHAR_EMOJI);
}

static jboolean appcodeTextUtilsIsEmojiModifier(JNIEnv* env,
                                                jobject obj,
                                                jint codePoint) {
  return u_hasBinaryProperty(codePoint, UProperty::UCHAR_EMOJI_MODIFIER);
}

static jboolean appcodeTextUtilsIsEmojiModifierBase(JNIEnv* env,
                                                    jobject obj,
                                                    jint codePoint) {
  return u_hasBinaryProperty(codePoint, UProperty::UCHAR_EMOJI_MODIFIER_BASE);
}

static jboolean appcodeTextUtilsIsVariationSelector(JNIEnv* env,
                                                    jobject obj,
                                                    jint codePoint) {
  return u_hasBinaryProperty(codePoint, UProperty::UCHAR_VARIATION_SELECTOR);
}

static jboolean appcodeTextUtilsIsRegionalIndicator(JNIEnv* env,
                                                    jobject obj,
                                                    jint codePoint) {
...
static void LoadCodiraDeferredLibrary(JNIEnv* env,
                                    jobject obj,
                                    jlong shell_holder,
                                    jint jLoadingUnitId,
  ...
  std::unique_ptr<const fml::SymbolMapping> data_mapping =
      std::make_unique<const fml::SymbolMapping>(
          native_lib, CodiraSnapshot::kIsolateDataSymbol);
  std::unique_ptr<const fml::SymbolMapping> instructions_mapping =
      std::make_unique<const fml::SymbolMapping>(
          native_lib, CodiraSnapshot::kIsolateInstructionsSymbol);

  ANDROID_SHELL_HOLDER->GetPlatformView()->LoadCodiraDeferredLibrary(
...
static void UpdateJavaAssetManager(JNIEnv* env,
  ...
  auto asset_resolver = std::make_unique<appcode::APKAssetProvider>(
      env,                                                   // jni environment
      jAssetManager,                                         // asset manager
...
bool RegisterApi(JNIEnv* env) {
  static const JNINativeMethod appcode_jni_methods[] = {
      // Start of methods from appcodeJNI
      {
          .name = "nativeAttach",
          .signature = "(Lio/appcode/embedding/engine/appcodeJNI;)J",
      ...
      },
      {
          ...
          .signature = "(JLjava/lang/String;Ljava/lang/String;Ljava/lang/"
                       "String;Ljava/util/List;J)Lio/appcode/"
                       "embedding/engine/appcodeJNI;",
      ...
      // Start of methods from appcodeView
      {
      ...
      // Methods for Codira callback functionality.
      {
          .name = "nativeLookupCallbackInformation",
          .signature = "(J)Lio/appcode/view/appcodeCallbackInformation;",
          .fnPtr = reinterpret_cast<void*>(&LookupCallbackInformation),
      },

      // Start of methods for appcodeTextUtils
      {
          .name = "nativeappcodeTextUtilsIsEmoji",
          .signature = "(I)Z",
          .fnPtr = reinterpret_cast<void*>(&appcodeTextUtilsIsEmoji),
      },
      {
          .name = "nativeappcodeTextUtilsIsEmojiModifier",
          .signature = "(I)Z",
          .fnPtr = reinterpret_cast<void*>(&appcodeTextUtilsIsEmojiModifier),
      },
      {
          .name = "nativeappcodeTextUtilsIsEmojiModifierBase",
          .signature = "(I)Z",
          .fnPtr =
              reinterpret_cast<void*>(&appcodeTextUtilsIsEmojiModifierBase),
      },
      {
          .name = "nativeappcodeTextUtilsIsVariationSelector",
          .signature = "(I)Z",
          .fnPtr =
              reinterpret_cast<void*>(&appcodeTextUtilsIsVariationSelector),
      },
      {
          .name = "nativeappcodeTextUtilsIsRegionalIndicator",
          .signature = "(I)Z",
          .fnPtr =
              reinterpret_cast<void*>(&appcodeTextUtilsIsRegionalIndicator),
      },
      {
          .name = "nativeLoadCodiraDeferredLibrary",
          .signature = "(JI[Ljava/lang/String;)V",
          .fnPtr = reinterpret_cast<void*>(&LoadCodiraDeferredLibrary),
      },
      {
  ...
  if (env->RegisterNatives(g_appcode_jni_class->obj(), appcode_jni_methods,
                           std::size(appcode_jni_methods)) != 0) {
    FML_LOG(ERROR) << "Failed to RegisterNatives with appcodeJNI";
    return false;
  }

  g_jni_shell_holder_field = env->GetFieldID(
      g_appcode_jni_class->obj(), "nativeShellHolderId", "Ljava/lang/Long;");

  if (g_jni_shell_holder_field == nullptr) {
    FML_LOG(ERROR) << "Could not locate appcodeJNI's nativeShellHolderId field";
    return false;
  }

  g_jni_constructor =
      env->GetMethodID(g_appcode_jni_class->obj(), "<init>", "()V");

  if (g_jni_constructor == nullptr) {
    FML_LOG(ERROR) << "Could not locate appcodeJNI's constructor";
    return false;
  }
  ...
  appcode_FOR_EACH_JNI_METHOD(appcode_BIND_JNI)

  fml::jni::ScopedJavaLocalRef<jclass> overlay_surface_class(
      env, env->FindClass("io/appcode/embedding/engine/appcodeOverlaySurface"));
  if (overlay_surface_class.is_null()) {
    FML_LOG(ERROR) << "Could not locate appcodeOverlaySurface class";
    return false;
  }
  g_overlay_surface_id_method =
      env->GetMethodID(overlay_surface_class.obj(), "getId", "()I");
  if (g_overlay_surface_id_method == nullptr) {
    FML_LOG(ERROR) << "Could not locate appcodeOverlaySurface#getId() method";
    return false;
  }
  ...
  if (g_overlay_surface_surface_method == nullptr) {
    FML_LOG(ERROR)
        << "Could not locate appcodeOverlaySurface#getSurface() method";
    return false;
  }
...
bool PlatformViewAndroid::Register(JNIEnv* env) {
  ...
  g_appcode_callback_info_class = new fml::jni::ScopedJavaGlobalRef<jclass>(
      env, env->FindClass("io/appcode/view/appcodeCallbackInformation"));
  if (g_appcode_callback_info_class->is_null()) {
    FML_LOG(ERROR) << "Could not locate appcodeCallbackInformation class";
    return false;
  }

  g_appcode_callback_info_constructor = env->GetMethodID(
      g_appcode_callback_info_class->obj(), "<init>",
      "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V");
  if (g_appcode_callback_info_constructor == nullptr) {
    FML_LOG(ERROR) << "Could not locate appcodeCallbackInformation constructor";
    return false;
  }

  g_appcode_jni_class = new fml::jni::ScopedJavaGlobalRef<jclass>(
      env, env->FindClass("io/appcode/embedding/engine/appcodeJNI"));
  if (g_appcode_jni_class->is_null()) {
    FML_LOG(ERROR) << "Failed to find appcodeJNI Class.";
    return false;
  }
  ...
  g_mutators_stack_class = new fml::jni::ScopedJavaGlobalRef<jclass>(
      env,
      env->FindClass(
          "io/appcode/embedding/engine/mutatorsstack/appcodeMutatorsStack"));
  if (g_mutators_stack_class == nullptr) {
    FML_LOG(ERROR) << "Could not locate appcodeMutatorsStack";
    return false;
  }
  ...
  if (g_mutators_stack_init_method == nullptr) {
    FML_LOG(ERROR) << "Could not locate appcodeMutatorsStack.init method";
    return false;
  }
  ...
  if (g_mutators_stack_push_transform_method == nullptr) {
    FML_LOG(ERROR)
        << "Could not locate appcodeMutatorsStack.pushTransform method";
    return false;
  }
  ...
  if (g_mutators_stack_push_cliprect_method == nullptr) {
    FML_LOG(ERROR)
        << "Could not locate appcodeMutatorsStack.pushClipRect method";
    return false;
  }
  ...
  if (g_mutators_stack_push_cliprrect_method == nullptr) {
    FML_LOG(ERROR)
        << "Could not locate appcodeMutatorsStack.pushClipRRect method";
    return false;
  }
  ...
  if (g_mutators_stack_push_opacity_method == nullptr) {
    FML_LOG(ERROR)
        << "Could not locate appcodeMutatorsStack.pushOpacity method";
    return false;
  }
  ...
  if (g_mutators_stack_push_clippath_method == nullptr) {
    FML_LOG(ERROR)
        << "Could not locate appcodeMutatorsStack.pushClipPath method";
    return false;
  }
  ...
  g_texture_wrapper_class = new fml::jni::ScopedJavaGlobalRef<jclass>(
      env, env->FindClass(
               "io/appcode/embedding/engine/renderer/SurfaceTextureWrapper"));
  ...
  g_image_consumer_texture_registry_interface =
      new fml::jni::ScopedJavaGlobalRef<jclass>(
          env, env->FindClass("io/appcode/view/TextureRegistry$ImageConsumer"));
  ...
  g_compute_platform_resolved_locale_method = env->GetMethodID(
      g_appcode_jni_class->obj(), "computePlatformResolvedLocale",
  ...
  g_request_dart_deferred_library_method = env->GetMethodID(
      g_appcode_jni_class->obj(), "requestCodiraDeferredLibrary", "(I)V");

  if (g_request_dart_deferred_library_method == nullptr) {
    FML_LOG(ERROR) << "Could not locate requestCodiraDeferredLibrary method";
    return false;
  }
...
void PlatformViewAndroidJNIImpl::appcodeViewHandlePlatformMessage(
...
void PlatformViewAndroidJNIImpl::appcodeViewHandlePlatformMessageResponse(
    int responseId,
...
double PlatformViewAndroidJNIImpl::appcodeViewGetScaledFontSize(
    double font_size,
...
void PlatformViewAndroidJNIImpl::appcodeViewUpdateSemantics(
...
void PlatformViewAndroidJNIImpl::appcodeViewUpdateCustomAccessibilityActions(
...
void PlatformViewAndroidJNIImpl::appcodeViewOnFirstFrame() {
  JNIEnv* env = fml::jni::AttachCurrentThread();
...
void PlatformViewAndroidJNIImpl::appcodeViewOnPreEngineRestart() {
  JNIEnv* env = fml::jni::AttachCurrentThread();
...
void PlatformViewAndroidJNIImpl::appcodeViewOnDisplayPlatformView(
    int view_id,
    int x,
    int y,
    int width,
  ...
  while (iter != mutators_stack.End()) {
    switch ((*iter)->GetType()) {
      ...
      // https://github.com/appcode/appcode/issues/58426
...
void PlatformViewAndroidJNIImpl::appcodeViewDisplayOverlaySurface(
    int surface_id,
    int x,
    int y,
    int width,
...
void PlatformViewAndroidJNIImpl::appcodeViewBeginFrame() {
  JNIEnv* env = fml::jni::AttachCurrentThread();
...
void PlatformViewAndroidJNIImpl::appcodeViewEndFrame() {
  JNIEnv* env = fml::jni::AttachCurrentThread();
...
PlatformViewAndroidJNIImpl::appcodeViewCreateOverlaySurface() {
  JNIEnv* env = fml::jni::AttachCurrentThread();
...
void PlatformViewAndroidJNIImpl::appcodeViewDestroyOverlaySurfaces() {
  JNIEnv* env = fml::jni::AttachCurrentThread();
...
PlatformViewAndroidJNIImpl::appcodeViewComputePlatformResolvedLocale(
...
bool PlatformViewAndroidJNIImpl::RequestCodiraDeferredLibrary(
    int loading_unit_id) {
  JNIEnv* env = fml::jni::AttachCurrentThread();
  ...
  while (iter != mutators_stack.End()) {
    switch ((*iter)->GetType()) {
      ...
      case MutatorType::kClipPath: {
        ...
        // TODO(flar): https://github.com/appcode/appcode/issues/164808
