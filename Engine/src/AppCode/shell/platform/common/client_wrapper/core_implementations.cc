...
//
// If https://github.com/appcode/appcode/issues/57146 is fixed, this can be
...
#include "include/appcode/standard_method_codec.h"
...
using appcodeDesktopMessengerScopedLock =
    std::unique_ptr<appcodeDesktopMessenger,
                    decltype(&appcodeDesktopMessengerUnlock)>;
...
void ForwardToHandler(appcodeDesktopMessengerRef messenger,
                      const appcodeDesktopMessage* message,
                      void* user_data) {
  auto* response_handle = message->response_handle;
  auto messenger_ptr = std::shared_ptr<appcodeDesktopMessenger>(
      appcodeDesktopMessengerAddRef(messenger),
  ...
  BinaryReply reply_handler = [messenger_ptr, response_handle](
    ...
    auto lock = appcodeDesktopMessengerScopedLock(
        appcodeDesktopMessengerLock(messenger_ptr.get()),
        &appcodeDesktopMessengerUnlock);
    if (!appcodeDesktopMessengerIsAvailable(messenger_ptr.get())) {
    ...
    appcodeDesktopMessengerSendResponse(messenger_ptr.get(), response_handle,
                                        reply, reply_size);
    // The engine frees the response handle once
    // appcodeDesktopSendMessageResponse is called.
...
BinaryMessengerImpl::BinaryMessengerImpl(
    appcodeDesktopMessengerRef core_messenger)
...
void BinaryMessengerImpl::Send(const std::string& channel,
  ...
  if (reply == nullptr) {
    appcodeDesktopMessengerSend(messenger_, channel.c_str(), message,
                                message_size);
    return;
  }
  struct Captures {
  ...
  bool result = appcodeDesktopMessengerSendWithReply(
...
void BinaryMessengerImpl::SetMessageHandler(const std::string& channel,
  ...
  if (!handler) {
    ...
    appcodeDesktopMessengerSetCallback(messenger_, channel.c_str(), nullptr,
                                       nullptr);
    return;
  }
  ...
  appcodeDesktopMessengerSetCallback(messenger_, channel.c_str(),
...
constexpr char kControlChannelName[] = "dev.appcode/channel-buffers";
...
void ResizeChannel(BinaryMessenger* messenger, std::string name, int new_size) {
  ...
  // https://github.com/appcode/engine/blob/93e8901490e78c7ba7e319cce4470d9c6478c6dc/lib/ui/channel_buffers.dart#L495.
...
TextureRegistrarImpl::TextureRegistrarImpl(
    appcodeDesktopTextureRegistrarRef texture_registrar_ref)
...
int64_t TextureRegistrarImpl::RegisterTexture(TextureVariant* texture) {
  appcodeDesktopTextureInfo info = {};
  if (auto pixel_buffer_texture = std::get_if<PixelBufferTexture>(texture)) {
    info.type = kappcodeDesktopPixelBufferTexture;
    ...
    info.type = kappcodeDesktopGpuSurfaceTexture;
  ...
  int64_t texture_id = appcodeDesktopTextureRegistrarRegisterExternalTexture(
      texture_registrar_ref_, &info);
  return texture_id;
}  // namespace appcode

bool TextureRegistrarImpl::MarkTextureFrameAvailable(int64_t texture_id) {
  return appcodeDesktopTextureRegistrarMarkExternalTextureFrameAvailable(
...
void TextureRegistrarImpl::UnregisterTexture(int64_t texture_id,
  ...
  if (callback == nullptr) {
    appcodeDesktopTextureRegistrarUnregisterExternalTexture(
