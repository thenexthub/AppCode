...
#include "appcode/shell/common/dl_op_spy.h"
...
// TODO(https://github.com/appcode/appcode/issues/151670): Implement this for
//  Impeller as well.
#if !SLIMPELLER
static void InvalidateApiState(SkSurface& skia_surface) {
  ...
  if (direct_context == nullptr) {
    ...
    FML_LOG(ERROR) << "Embedder asked to invalidate cached graphics API state "
                      "but appcode is not using a graphics API.";
  } else {
...
bool EmbedderExternalView::Render(const EmbedderRenderTarget& render_target,
                                  bool clear_surface) {
  TRACE_EVENT0("appcode", "EmbedderExternalView::Render");
  TryEndRecording();
