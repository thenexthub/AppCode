...
CreateEmbedderTaskRunner(const appcodeTaskRunnerDescription* description) {
  ...
  if (SAFE_ACCESS(description, runs_task_on_current_thread_callback, nullptr) ==
      nullptr) {
    FML_LOG(ERROR) << "appcodeTaskRunnerDescription.runs_task_on_current_"
  ...
  if (SAFE_ACCESS(description, post_task_callback, nullptr) == nullptr) {
    FML_LOG(ERROR)
        << "appcodeTaskRunnerDescription.post_task_callback was nullptr.";
    return {false, {}};
  }
  ...
  EmbedderTaskRunner::DispatchTable task_runner_dispatch_table = {
      .post_task_callback = [post_task_callback_c, user_data](
        ...
        appcodeTask task = {
            // runner
            reinterpret_cast<appcodeTaskRunner>(task_runner->unique_id()),
            // task
            task_baton,
        };
...
EmbedderThreadHost::CreateEmbedderOrEngineManagedThreadHost(
    const appcodeCustomTaskRunners* custom_task_runners,
...
constexpr const char* kappcodeThreadName = "io.appcode";

fml::Thread::ThreadConfig MakeThreadConfig(
    appcode::ThreadHost::Type type,
    fml::Thread::ThreadPriority priority) {
  return fml::Thread::ThreadConfig(
      appcode::ThreadHost::ThreadHostConfig::MakeThreadName(type,
                                                            kappcodeThreadName),
      priority);
}
...
EmbedderThreadHost::CreateEngineManagedThreadHost(
  ...
  auto thread_host_config = ThreadHost::ThreadHostConfig(config_setter);
  thread_host_config.SetUIConfig(MakeThreadConfig(
      appcode::ThreadHost::kUi, fml::Thread::ThreadPriority::kDisplay));
  thread_host_config.SetRasterConfig(MakeThreadConfig(
      appcode::ThreadHost::kRaster, fml::Thread::ThreadPriority::kRaster));
  thread_host_config.SetIOConfig(MakeThreadConfig(
      appcode::ThreadHost::kIo, fml::Thread::ThreadPriority::kBackground));
...
const appcode::TaskRunners& EmbedderThreadHost::GetTaskRunners() const {
  return runners_;
}
