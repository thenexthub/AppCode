...
#include "appcode/shell/platform/embedder/tests/embedder_test_backingstore_producer_vulkan.h"
...
bool EmbedderTestCompositorVulkan::UpdateOffscrenComposition(
  ...
  for (size_t i = 0; i < layers_count; ++i) {
    ...
    if (!layer_image && layer->type != kappcodeLayerContentTypePlatformView) {
    ...
    if (layer_image) {
      // The image rendered by appcode already has the correct offset and
