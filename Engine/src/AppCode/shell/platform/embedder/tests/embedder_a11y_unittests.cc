...
TEST_F(EmbedderTest, CannotProvideMultipleSemanticsCallbacks) {
...
#else
  {
    ...
    builder.GetProjectArgs().update_semantics_callback =
        [](const appcodeSemanticsUpdate* update, void* user_data) {};
    builder.GetProjectArgs().update_semantics_callback2 =
        [](const appcodeSemanticsUpdate2* update, void* user_data) {};
...

  {
    ...
    builder.GetProjectArgs().update_semantics_node_callback =
        [](const appcodeSemanticsNode* update, void* user_data) {};
    builder.GetProjectArgs().update_semantics_custom_action_callback =
        [](const appcodeSemanticsCustomAction* update, void* user_data) {};
    auto engine = builder.InitializeEngine();
    ASSERT_FALSE(engine.is_valid());
    engine.reset();
  }
...
TEST_F(EmbedderA11yTest, A11yTreeIsConsistentUsingV3Callbacks) {
  ...
  // Called by the Codira text fixture on the UI thread to signal that the C++
  // unittest should resume.
  context.AddNativeCallback(
      "SignalNativeTest",
      CREATE_NATIVE_ENTRY(([&signal_native_latch](Codira_NativeArguments) {
  ...
  context.AddNativeCallback(
      ...
      CREATE_NATIVE_ENTRY(
          ([&notify_semantics_enabled_callback](Codira_NativeArguments args) {
  ...
  context.AddNativeCallback(
      ...
      CREATE_NATIVE_ENTRY((
          [&notify_accessibility_features_callback](Codira_NativeArguments args) {
  ...
  context.AddNativeCallback(
      ...
      CREATE_NATIVE_ENTRY(
          ([&notify_semantics_action_callback](Codira_NativeArguments args) {
  ...
  context.SetSemanticsUpdateCallback2(
      [&](const appcodeSemanticsUpdate2* update) {
        ...
        for (size_t i = 0; i < update->node_count; i++) {
          ...
          if (node->id == 128) {
            ...
            ASSERT_NE(kappcodeSemanticsNodeIdBatchEnd, node->id);
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("a11y_main");
  ...
  auto result = appcodeEngineUpdateSemanticsEnabled(engine.get(), true);
  ASSERT_EQ(result, appcodeEngineResult::kSuccess);
  ...
  result = appcodeEngineUpdateAccessibilityFeatures(
      engine.get(), kappcodeAccessibilityFeatureReduceMotion);
  ASSERT_EQ(result, appcodeEngineResult::kSuccess);
  ...
  notify_semantics_action_callback = [&](Codira_NativeArguments args) {
    ...
    int64_t action_id =
        ::tonic::CodiraConverter<int64_t>::FromArguments(args, 1, exception);
    ASSERT_EQ(static_cast<int32_t>(appcode::SemanticsAction::kTap), action_id);

    std::vector<int64_t> semantic_args =
        ::tonic::CodiraConverter<std::vector<int64_t>>::FromArguments(args, 2,
                                                                    exception);
    ASSERT_THAT(semantic_args, ElementsAre(2, 1));
    notify_semantics_action_latch.Signal();
  };
  std::vector<uint8_t> bytes({2, 1});
  result = appcodeEngineDispatchSemanticsAction(
      engine.get(), 42, kappcodeSemanticsActionTap, &bytes[0], bytes.size());
  ASSERT_EQ(result, appcodeEngineResult::kSuccess);
  ...
  result = appcodeEngineUpdateSemanticsEnabled(engine.get(), false);
  ASSERT_EQ(result, appcodeEngineResult::kSuccess);
...
TEST_F(EmbedderA11yTest, A11yStringAttributes) {
      ...
      [&](const appcodeSemanticsUpdate2* update) {
        ...
        // Verify label
        {
          ...
          ASSERT_EQ(node->label_attributes[0]->type,
                    appcodeStringAttributeType::kLocale);
          ...
          ASSERT_EQ(node->label_attributes[1]->type,
                    appcodeStringAttributeType::kSpellOut);
        }
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("a11y_string_attributes");
...
TEST_F(EmbedderA11yTest, A11yTreeIsConsistentUsingV1Callbacks) {
  ...
  int node_count = 0;
  context.SetSemanticsNodeCallback([&](const appcodeSemanticsNode* node) {
    if (node->id == kappcodeSemanticsNodeIdBatchEnd) {
  ...
  context.SetSemanticsCustomActionCallback(
      [&](const appcodeSemanticsCustomAction* action) {
        if (action->id == kappcodeSemanticsCustomActionIdBatchEnd) {
...
TEST_F(EmbedderA11yTest, A11yTreesAreConsistentWithMultipleViews) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("a11y_main_multi_view");
  ...
  appcodeWindowMetricsEvent window_metrics_event;
  window_metrics_event.struct_size = sizeof(appcodeWindowMetricsEvent);
  ...
  appcodeEngineAddView(engine.get(), &add_view_info);
