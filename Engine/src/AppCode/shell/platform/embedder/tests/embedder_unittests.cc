...
#include "appcode/flow/raster_cache.h"
...
#include "appcode/shell/platform/embedder/tests/embedder_config_builder.h"
...
#include "appcode/shell/platform/embedder/tests/embedder_unittests_util.h"
...
TEST_F(EmbedderTest, CanInvokeCustomEntrypoint) {
  ...
  Codira_NativeFunction entrypoint = [](Codira_NativeArguments args) {
    latch.Signal();
  };
  context.AddNativeCallback("SayHiFromCustomEntrypoint", entrypoint);
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("customEntrypoint");
  auto engine = builder.LaunchEngine();
  latch.Wait();
  ASSERT_TRUE(engine.is_valid());
}
...
TEST_F(EmbedderTest, CanInvokeCustomEntrypointMacro) {
  ...
  auto entry1 = [&latch1](Codira_NativeArguments args) {
  ...
  auto entry2 = [&latch2](Codira_NativeArguments args) {
  ...
  context.AddNativeCallback(
      ...
      CREATE_NATIVE_ENTRY([&latch3](Codira_NativeArguments args) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("customEntrypoint1");
  auto engine = builder.LaunchEngine();
  latch1.Wait();
  latch2.Wait();
  latch3.Wait();
...
TEST_F(EmbedderTest, CanTerminateCleanly) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("terminateExitCodeHandler");
  auto engine = builder.LaunchEngine();
  ASSERT_TRUE(engine.is_valid());
}
...
TEST_F(EmbedderTest, ExecutableNameNotNull) {
  ...
  // Supply a callback to Codira for the test fixture to pass Platform.executable
  // back to us.
  fml::AutoResetWaitableEvent latch;
  context.AddNativeCallback(
      "NotifyStringValue", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        const auto dart_string = tonic::CodiraConverter<std::string>::FromCodira(
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("executableNameNotNull");
...
TEST_F(EmbedderTest, ImplicitViewNotNull) {
  ...
  // See: https://github.com/appcode/appcode/issues/120306
  auto& context = GetEmbedderContext<EmbedderTestContextSoftware>();

  bool implicitViewNotNull = false;
  fml::AutoResetWaitableEvent latch;
  context.AddNativeCallback(
      "NotifyBoolValue", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        implicitViewNotNull = tonic::CodiraConverter<bool>::FromCodira(
            Codira_GetNativeArgument(args, 0));
        latch.Signal();
      }));
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("implicitViewNotNull");
  auto engine = builder.LaunchEngine();
  latch.Wait();
...
TEST_F(EmbedderTest, CanSpecifyCustomUITaskRunner) {
  ...
  EmbedderTestTaskRunner test_ui_task_runner(
      ui_task_runner, [&](appcodeTask task) {
        std::scoped_lock lock(engine_mutex);
        if (!engine.is_valid()) {
          return;
        }
        appcodeEngineRunTask(engine.get(), &task);
      });
  EmbedderTestTaskRunner test_platform_task_runner(
      platform_task_runner, [&](appcodeTask task) {
        std::scoped_lock lock(engine_mutex);
        if (!engine.is_valid()) {
          return;
        }
  ...
  context.AddNativeCallback(
      "SignalNativeTest", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
  ...
  platform_task_runner->PostTask([&]() {
    ...
    const auto ui_task_runner_description =
        test_ui_task_runner.GetappcodeTaskRunnerDescription();
    const auto platform_task_runner_description =
        test_platform_task_runner.GetappcodeTaskRunnerDescription();
    builder.SetSurface(SkISize::Make(1, 1));
    builder.SetUITaskRunner(&ui_task_runner_description);
    builder.SetPlatformTaskRunner(&platform_task_runner_description);
    builder.SetCodiraEntrypoint("canSpecifyCustomUITaskRunner");
    ...
    ASSERT_EQ(appcodeEngineRunInitialized(engine.get()), kSuccess);
    ASSERT_TRUE(engine.is_valid());
  });
...
TEST_F(EmbedderTest, IgnoresStaleTasks) {
  ...
  static std::mutex engine_mutex;
  UniqueEngine engine;
  appcodeEngine engine_ptr;

  EmbedderTestTaskRunner test_ui_task_runner(
      ui_task_runner, [&](appcodeTask task) {
        // The check for engine.is_valid() is intentionally absent here.
        // appcodeEngineRunTask must be able to detect and ignore stale tasks
        ...
        appcodeEngineRunTask(engine_ptr, &task);
      });
  EmbedderTestTaskRunner test_platform_task_runner(
      platform_task_runner, [&](appcodeTask task) {
        std::scoped_lock lock(engine_mutex);
  ...
  auto appcode_engine = reinterpret_cast<EmbedderEngine*>(engine.get());

  // Schedule task on UI thread that will likely run after the engine has shut
  // down.
  appcode_engine->GetTaskRunners().GetUITaskRunner()->PostDelayedTask(
...
TEST_F(EmbedderTest, MergedPlatformUIThread) {
  ...
  EmbedderTestTaskRunner test_task_runner(task_runner, [&](appcodeTask task) {
    if (!engine.is_valid()) {
      return;
    }
    appcodeEngineRunTask(engine.get(), &task);
...
TEST_F(EmbedderTest, UITaskRunnerFlushesMicrotasks) {
  ...
  ui_task_runner->PostTask([&]() {
    ...
    builder.SetSurface(SkISize::Make(1, 1));
    builder.SetUITaskRunner(&task_runner_description);
    builder.SetCodiraEntrypoint("uiTaskRunnerFlushesMicrotasks");
    engine = builder.LaunchEngine();
    ASSERT_TRUE(engine.is_valid());
  });
  signal_latch.Wait();
...
TEST_F(EmbedderTest, CanSpecifyCustomPlatformTaskRunner) {
  ...
  EmbedderTestTaskRunner test_task_runner(
      platform_task_runner, [&](appcodeTask task) {
        ...
        signaled_once = true;
        ASSERT_TRUE(engine.is_valid());
        ASSERT_EQ(appcodeEngineRunTask(engine.get(), &task), kSuccess);
        latch.Signal();
      });
  ...
  platform_task_runner->PostTask([&]() {
    ...
    builder.SetSurface(SkISize::Make(1, 1));
    builder.SetPlatformTaskRunner(&task_runner_description);
    builder.SetCodiraEntrypoint("invokePlatformTaskRunner");
    std::scoped_lock lock(engine_mutex);
    engine = builder.LaunchEngine();
    ASSERT_TRUE(engine.is_valid());
  });
...
TEST(EmbedderTestNoFixture, CanGetCurrentTimeInNanoseconds) {
  auto point1 = fml::TimePoint::FromEpochDelta(
      fml::TimeDelta::FromNanoseconds(appcodeEngineGetCurrentTime()));
...
TEST_F(EmbedderTest, CanReloadSystemFonts) {
  ...
  auto result = appcodeEngineReloadSystemFonts(engine.get());
...
TEST_F(EmbedderTest, IsolateServiceIdSent) {
  ...
  thread.GetTaskRunner()->PostTask([&]() {
    EmbedderConfigBuilder builder(context);
    builder.SetSurface(SkISize::Make(1, 1));
    builder.SetCodiraEntrypoint("main");
    builder.SetPlatformMessageCallback(
        [&](const appcodePlatformMessage* message) {
          if (strcmp(message->channel, "appcode/isolate") == 0) {
...
TEST_F(EmbedderTest, CanCreateAndCollectCallbacks) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("platform_messages_response");
  context.AddNativeCallback(
      "SignalNativeTest",
  ...
  auto callback = [](const uint8_t* data, size_t size,
                     void* user_data) -> void {};
  auto result = appcodePlatformMessageCreateResponseHandle(
      engine.get(), callback, nullptr, &response_handle);
  ASSERT_EQ(result, kSuccess);
  ASSERT_NE(response_handle, nullptr);

  result = appcodePlatformMessageReleaseResponseHandle(engine.get(),
                                                       response_handle);
  ASSERT_EQ(result, kSuccess);
}

//------------------------------------------------------------------------------
/// Sends platform messages to Codira code than simply echoes the contents of the
...
TEST_F(EmbedderTest, PlatformMessagesCanReceiveResponse) {
  ...
  CreateNewThread()->PostTask([&]() {
    ...
    context.AddNativeCallback(
        "SignalNativeTest",
        CREATE_NATIVE_ENTRY(
            [&ready](Codira_NativeArguments args) { ready.Signal(); }));
    ...
    appcodePlatformMessage message = {};
    message.struct_size = sizeof(appcodePlatformMessage);
    ...
    ready.Wait();
    result = appcodeEngineSendPlatformMessage(engine.get(), &message);
    ASSERT_EQ(result, kSuccess);
...
TEST_F(EmbedderTest, PlatformMessagesCanBeSentWithoutResponseHandles) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("platform_messages_no_response");
  ...
  context.AddNativeCallback(
      "SignalNativeMessage",
      CREATE_NATIVE_ENTRY(
          ([&message, &message_data](Codira_NativeArguments args) {
            auto received_message = tonic::CodiraConverter<std::string>::FromCodira(
                Codira_GetNativeArgument(args, 0));
  ...
  platform_message.struct_size = sizeof(appcodePlatformMessage);
  ...
  auto result =
      appcodeEngineSendPlatformMessage(engine.get(), &platform_message);
  ASSERT_EQ(result, kSuccess);
  message.Wait();
}
...
TEST_F(EmbedderTest, NullPlatformMessagesCanBeSent) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("null_platform_messages");
  ...
  context.AddNativeCallback(
      "SignalNativeMessage",
      CREATE_NATIVE_ENTRY(([&message](Codira_NativeArguments args) {
        auto received_message = tonic::CodiraConverter<std::string>::FromCodira(
            Codira_GetNativeArgument(args, 0));
...
//------------------------------------------------------------------------------
/// Asserts behavior of appcodeProjectArgs::shutdown_dart_vm_when_done (which is
...
TEST_F(EmbedderTest, VMShutsDownWhenNoEnginesInProcess) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  const auto launch_count = CodiraVM::GetVMLaunchCount();

  {
    auto engine = builder.LaunchEngine();
    ASSERT_EQ(launch_count + 1u, CodiraVM::GetVMLaunchCount());
  }

  {
    auto engine = builder.LaunchEngine();
    ASSERT_EQ(launch_count + 2u, CodiraVM::GetVMLaunchCount());
  }
}

//------------------------------------------------------------------------------
///
TEST_F(EmbedderTest, CodiraEntrypointArgs) {
  auto& context = GetEmbedderContext<EmbedderTestContextSoftware>();
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.AddCodiraEntrypointArgument("foo");
  builder.AddCodiraEntrypointArgument("bar");
  builder.SetCodiraEntrypoint("dart_entrypoint_args");
  fml::AutoResetWaitableEvent callback_latch;
  std::vector<std::string> callback_args;
  auto nativeArgumentsCallback = [&callback_args,
                                  &callback_latch](Codira_NativeArguments args) {
    Codira_Handle exception = nullptr;
    callback_args =
        tonic::CodiraConverter<std::vector<std::string>>::FromArguments(
            args, 0, exception);
    callback_latch.Signal();
  };
...
TEST_F(EmbedderTest, CanRenderImplicitView) {
  ...
  builder.SetCodiraEntrypoint("render_implicit_view");
  ...
  context.GetCompositor().SetNextPresentCallback(
      [&](appcodeViewId view_id, const appcodeLayer** layers,
          size_t layers_count) {
        ASSERT_EQ(view_id, kappcodeImplicitViewId);
        latch.Signal();
      });
  ...
  ASSERT_EQ(appcodeEngineSendWindowMetricsEvent(engine.get(), &event),
            kSuccess);
  ASSERT_TRUE(engine.is_valid());
  latch.Wait();
}
...
TEST_F(EmbedderTest,
  ...
  builder.SetSurface(SkISize::Make(800, 600));
  builder.SetCompositor();
  builder.SetCodiraEntrypoint("can_composite_platform_views_with_known_scene");
  ...
  context.GetCompositor().SetNextPresentCallback(
      [&](appcodeViewId view_id, const appcodeLayer** layers,
        ...
        // Layer Root
        {
          appcodeBackingStore backing_store = *layers[0]->backing_store;
          backing_store.type = kappcodeBackingStoreTypeSoftware;
          backing_store.did_update = true;
          backing_store.software.height = 600;

          appcodeRect paint_region_rects[] = {
              appcodeRectMakeLTRB(0, 0, 800, 600),
          };
          appcodeRegion paint_region = {
              .struct_size = sizeof(appcodeRegion),
              .rects_count = 1,
              .rects = paint_region_rects,
          };
          appcodeBackingStorePresentInfo present_info = {
              .struct_size = sizeof(appcodeBackingStorePresentInfo),
          ...
          layer.size = appcodeSizeMake(800.0, 600.0);
          layer.offset = appcodePointMake(0.0, 0.0);
        ...
        // Layer 1
        {
          appcodePlatformView platform_view = *layers[1]->platform_view;
          ...
          layer.size = appcodeSizeMake(50.0, 150.0);
          layer.offset = appcodePointMake(20.0, 20.0);
        ...
        // Layer 2
        {
          appcodeBackingStore backing_store = *layers[2]->backing_store;
          backing_store.type = kappcodeBackingStoreTypeSoftware;
          backing_store.did_update = true;
          backing_store.software.height = 600;

          appcodeRect paint_region_rects[] = {
              appcodeRectMakeLTRB(30, 30, 80, 180),
          };
          appcodeRegion paint_region = {
              .struct_size = sizeof(appcodeRegion),
              .rects_count = 1,
        ...
        // Layer 3
        {
          appcodePlatformView platform_view = *layers[3]->platform_view;
          platform_view.struct_size = sizeof(platform_view);
          ...
          layer.platform_view = &platform_view;
          layer.size = appcodeSizeMake(50.0, 150.0);
          layer.offset = appcodePointMake(40.0, 40.0);
        ...
        // Layer 4
        {
          appcodeBackingStore backing_store = *layers[4]->backing_store;
          backing_store.type = kappcodeBackingStoreTypeSoftware;
          backing_store.did_update = true;
          backing_store.software.height = 600;

          appcodeRect paint_region_rects[] = {
              appcodeRectMakeLTRB(50, 50, 100, 200),
          };
          appcodeRegion paint_region = {
              .struct_size = sizeof(appcodeRegion),
              .rects_count = 1,
  ...
  context.GetCompositor().SetPlatformViewRendererCallback(
      [&](const appcodeLayer& layer, GrDirectContext*
  ...
  context.AddNativeCallback(
      "SignalNativeTest",
      CREATE_NATIVE_ENTRY(
          [&latch](Codira_NativeArguments args) { latch.CountDown(); }));
...
TEST_F(EmbedderTest, NoLayerCreatedForTransparentOverlayOnTopOfPlatformLayer) {
  ...
  builder.SetSurface(SkISize::Make(800, 600));
  builder.SetCompositor();
  builder.SetCodiraEntrypoint("can_composite_platform_views_transparent_overlay");
  ...
  // TODO(https://github.com/appcode/appcode/issues/53784): enable this on all
  // platforms.
...
TEST_F(EmbedderTest, NoLayerCreatedForNoOverlayOnTopOfPlatformLayer) {
  ...
  builder.SetSurface(SkISize::Make(800, 600));
  builder.SetCompositor();
  builder.SetCodiraEntrypoint("can_composite_platform_views_no_overlay");
...
TEST_F(EmbedderTest, CanRunInitializedEngine) {
  ...
  ASSERT_EQ(appcodeEngineRunInitialized(engine.get()), kInvalidArguments);
  engine.reset();
}
...
TEST_F(EmbedderTest, CanDeinitializeAnEngine) {
  ...
  // Cannot re-run an already running engine.
  ASSERT_EQ(appcodeEngineRunInitialized(engine.get()), kInvalidArguments);
  ASSERT_EQ(appcodeEngineDeinitialize(engine.get()), kSuccess);
...
TEST_F(EmbedderTest, CanAddView) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("window_metrics_event_all_view_ids");

  fml::AutoResetWaitableEvent ready_latch, message_latch;
  context.AddNativeCallback(
      "SignalNativeTest",
      CREATE_NATIVE_ENTRY(
          [&ready_latch](Codira_NativeArguments args) { ready_latch.Signal(); }));

  std::string message;
  context.AddNativeCallback("SignalNativeMessage",
                            CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
                              message =
                                  tonic::CodiraConverter<std::string>::FromCodira(
                                      Codira_GetNativeArgument(args, 0));
  ...
  appcodeWindowMetricsEvent metrics = {};
  metrics.struct_size = sizeof(appcodeWindowMetricsEvent);
  metrics.width = 800;
  metrics.height = 600;
  ...
  ASSERT_EQ(appcodeEngineAddView(engine.get(), &info), kSuccess);
  message_latch.Wait();
...
TEST_F(EmbedderTest, AddViewSchedulesFrame) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("add_view_schedules_frame");
  fml::AutoResetWaitableEvent latch;
  context.AddNativeCallback(
      "SignalNativeTest",
      CREATE_NATIVE_ENTRY(
          [&latch](Codira_NativeArguments args) { latch.Signal(); }));

  fml::AutoResetWaitableEvent check_latch;
  context.AddNativeCallback(
      "SignalNativeCount",
      CREATE_NATIVE_ENTRY(
          [&check_latch](Codira_NativeArguments args) { check_latch.Signal(); }));
...
TEST_F(EmbedderTest, CanRemoveView) {
  ...
  appcodeAddViewInfo add_info = {};
  add_info.struct_size = sizeof(appcodeAddViewInfo);
  add_info.view_id = 123;
  add_info.view_metrics = &metrics;
  add_info.add_view_callback = [](const appcodeAddViewResult* result) {
    ASSERT_TRUE(result->added);
  };
  ASSERT_EQ(appcodeEngineAddView(engine.get(), &add_info), kSuccess);
  message_latch.Wait();
  ASSERT_EQ(message, "View IDs: [0, 123]");

  // Remove view 123.
  appcodeRemoveViewInfo remove_info = {};
  remove_info.struct_size = sizeof(appcodeAddViewInfo);
  remove_info.view_id = 123;
  remove_info.remove_view_callback = [](const appcodeRemoveViewResult* result) {
    EXPECT_TRUE(result->removed);
  };
  ASSERT_EQ(appcodeEngineRemoveView(engine.get(), &remove_info), kSuccess);
  message_latch.Wait();
  ASSERT_EQ(message, "View IDs: [0]");
}

// Regression test for:
// https://github.com/appcode/appcode/issues/164564
TEST_F(EmbedderTest, RemoveViewCallbackIsInvokedAfterRasterThreadIsDone) {
  ...
  EmbedderTestTaskRunner render_task_runner(
      render_thread, [&](appcodeTask task) {
  ...
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("remove_view_callback_too_early");
  builder.SetRenderTaskRunner(
      &render_task_runner.GetappcodeTaskRunnerDescription());
  ...
  appcodeRemoveViewInfo remove_view_info = {};
  remove_view_info.struct_size = sizeof(appcodeRemoveViewInfo);
  remove_view_info.view_id = 123;
  remove_view_info.user_data = &remove_view_latch;
  remove_view_info.remove_view_callback =
      [](const appcodeRemoveViewResult* result) {
  ...
  ASSERT_EQ(appcodeEngineRemoveView(engine.get(), &remove_view_info), kSuccess);
  remove_view_latch.Wait();

  // After appcodeEngineRemoveViewCallback is called it should be safe to
  // remove view - raster thread must not be accessing any view resources.
  view_available = false;
  raster_thread_latch.Wait();

  appcodeEngineDeinitialize(engine.get());
}
...
TEST_F(EmbedderTest, CannotRemoveImplicitView) {
  ...
  info.view_id = kappcodeImplicitViewId;
  info.remove_view_callback = [](const appcodeRemoveViewResult* result) {
    FAIL();
  };
  ASSERT_EQ(appcodeEngineRemoveView(engine.get(), &info), kInvalidArguments);
}
...
TEST_F(EmbedderTest, CannotRemoveUnknownView) {
  ...
  ASSERT_EQ(appcodeEngineRemoveView(engine.get(), &info), kSuccess);
  latch.Wait();
}
...
TEST_F(EmbedderTest, ViewOperationsOrdered) {
  ...
  appcodeWindowMetricsEvent metrics123 = {};
  metrics123.struct_size = sizeof(appcodeWindowMetricsEvent);
  metrics123.width = 800;
  metrics123.height = 600;
  metrics123.pixel_ratio = 1.0;
  metrics123.view_id = 123;

  appcodeWindowMetricsEvent metrics456 = {};
  metrics456.struct_size = sizeof(appcodeWindowMetricsEvent);
  ...
  // Add view 123.
  appcodeAddViewInfo add_view_info = {};
  add_view_info.struct_size = sizeof(appcodeAddViewInfo);
  add_view_info.view_id = 123;
  add_view_info.view_metrics = &metrics123;
  add_view_info.user_data = &captures;
  add_view_info.add_view_callback = [](const appcodeAddViewResult* result) {
    auto captures = reinterpret_cast<Captures*>(result->user_data);
  ...
  appcodeAddViewInfo add_duplicate_view_info = {};
  add_duplicate_view_info.struct_size = sizeof(appcodeAddViewInfo);
  ...
  add_duplicate_view_info.add_view_callback =
      [](const appcodeAddViewResult* result) {
        auto captures = reinterpret_cast<Captures*>(result->user_data);
  ...
  // Add view 456.
  appcodeAddViewInfo add_second_view_info = {};
  add_second_view_info.struct_size = sizeof(appcodeAddViewInfo);
  ...
  // Remove view 456.
  appcodeRemoveViewInfo remove_second_view_info = {};
  remove_second_view_info.struct_size = sizeof(appcodeRemoveViewInfo);
  ...
  ASSERT_EQ(appcodeEngineAddView(engine.get(), &add_view_info), kSuccess);
  ASSERT_EQ(appcodeEngineAddView(engine.get(), &add_duplicate_view_info),
            kSuccess);
  ASSERT_EQ(appcodeEngineAddView(engine.get(), &add_second_view_info),
            kSuccess);
  ASSERT_EQ(appcodeEngineRemoveView(engine.get(), &remove_second_view_info),
            kSuccess);
...
TEST_F(EmbedderTest, CanRenderMultipleViews) {
  ...
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCompositor();
  builder.SetCodiraEntrypoint("render_all_views");
  ...
  appcodeWindowMetricsEvent metrics0 = {};
  metrics0.struct_size = sizeof(appcodeWindowMetricsEvent);
  metrics0.width = 800;
  metrics0.height = 600;
  metrics0.pixel_ratio = 1.0;
  metrics0.view_id = 0;
  ASSERT_EQ(appcodeEngineSendWindowMetricsEvent(engine.get(), &metrics0),
            kSuccess);
...
bool operator==(const appcodeViewFocusChangeRequest& lhs,
                const appcodeViewFocusChangeRequest& rhs) {
...
TEST_F(EmbedderTest, SendsViewFocusChangeRequest) {
  ...
  std::vector<appcodeViewFocusChangeRequest> received_requests;
  platform_task_runner->PostTask([&]() {
    EmbedderConfigBuilder builder(context);
    builder.SetSurface(SkISize::Make(1, 1));
    builder.SetCodiraEntrypoint("testSendViewFocusChangeRequest");
    const auto platform_task_runner_description =
        test_platform_task_runner.GetappcodeTaskRunnerDescription();
    builder.SetPlatformTaskRunner(&platform_task_runner_description);
  ...
  std::vector<appcodeViewFocusChangeRequest> expected_requests{
...
TEST_F(EmbedderTest, CanSendViewFocusEvent) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("testSendViewFocusEvent");
  ...
  appcodeViewFocusEvent event1{
      .struct_size = sizeof(appcodeViewFocusEvent),
      .view_id = 1,
      .state = kFocused,
      .direction = kUndefined,
  };
  appcodeEngineResult result =
      appcodeEngineSendViewFocusEvent(engine.get(), &event1);
  ASSERT_EQ(result, kSuccess);
  latch.Wait();
  ASSERT_EQ(last_event,
            "1 ViewFocusState.focused ViewFocusDirection.undefined");

  appcodeViewFocusEvent event2{
      .struct_size = sizeof(appcodeViewFocusEvent),
      .view_id = 2,
      .state = kUnfocused,
      .direction = kBackward,
  ...
  latch.Reset();
  result = appcodeEngineSendViewFocusEvent(engine.get(), &event2);
  ASSERT_EQ(result, kSuccess);
  latch.Wait();
  ASSERT_EQ(last_event,
...
TEST_F(EmbedderTest, BackingStoresCorrespondToTheirViews) {
  constexpr appcodeViewId kSecondViewId = 123;
  constexpr appcodeViewId kThirdViewId = 456;
  auto& context = GetEmbedderContext<EmbedderTestContextSoftware>();
  ...
  builder.GetCompositor() = appcodeCompositor{
      .struct_size = sizeof(appcodeCompositor),
      .user_data = reinterpret_cast<void*>(&compositor_user_data),
      .create_backing_store_callback =
          [](const appcodeBackingStoreConfig* config,
             appcodeBackingStore* backing_store_out, void* user_data) {
      ...
      .collect_backing_store_callback = [](const appcodeBackingStore* renderer,
                                           void* user_data) { return true; },
      .present_layers_callback = nullptr,
      .avoid_backing_store_cache = false,
      .present_view_callback =
          [](const appcodePresentViewInfo* info) {
  ...
  // Give the implicit view a non-zero size so that it renders something.
  appcodeWindowMetricsEvent metrics_implicit = {
      .struct_size = sizeof(appcodeWindowMetricsEvent),
      .width = 800,
      .height = 600,
      .pixel_ratio = 1.0,
      .view_id = 0,
  ...
  ASSERT_EQ(
      appcodeEngineSendWindowMetricsEvent(engine.get(), &metrics_implicit),
      kSuccess);

  // Add the second view.
  appcodeWindowMetricsEvent metrics_add = {
      .struct_size = sizeof(appcodeWindowMetricsEvent),
      .width = 800,
      .height = 600,
      .pixel_ratio = 1.0,
...
TEST_F(EmbedderTest, CanUpdateLocales) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("can_receive_locale_updates");
  fml::AutoResetWaitableEvent latch;
  context.AddNativeCallback(
      "SignalNativeTest",
      CREATE_NATIVE_ENTRY(
  ...
  context.AddNativeCallback(
      "SignalNativeCount",
      CREATE_NATIVE_ENTRY([&check_latch](Codira_NativeArguments args) {
        ASSERT_EQ(tonic::CodiraConverter<int>::FromCodira(
                      Codira_GetNativeArgument(args, 0)),
                  2);
        check_latch.Signal();
      }));
  ...
  std::vector<const appcodeLocale*> locales;
  locales.push_back(&locale1);
  locales.push_back(&locale2);

  ASSERT_EQ(
      appcodeEngineUpdateLocales(engine.get(), locales.data(), locales.size()),
      kInvalidArguments);
...
TEST_F(EmbedderTest, LocalizationCallbacksCalled) {
  ...
  appcode::Shell& shell = ToEmbedderEngine(engine.get())->GetShell();
...
TEST_F(EmbedderTest, CanQueryCodiraAOTMode) {
  ASSERT_EQ(appcodeEngineRunsAOTCompiledCodiraCode(),
            appcode::CodiraVM::IsRunningPrecompiledCode());
}
...
TEST_F(EmbedderTest, VerifyB143464703WithSoftwareBackend) {
  ...
  builder.SetCompositor();
  builder.SetCodiraEntrypoint("verify_b143464703");
  ...
  context.GetCompositor().SetNextPresentCallback(
      [&](appcodeViewId view_id, const appcodeLayer** layers,
        ...
        // Layer 0 (Root)
        {
          ...
          appcodeRect paint_region_rects[] = {
              appcodeRectMakeLTRB(0, 0, 1024, 600),
          };
          appcodeRegion paint_region = {
              .struct_size = sizeof(appcodeRegion),
              .rects_count = 1,
          ...
          layer.type = kappcodeLayerContentTypeBackingStore;
          layer.backing_store = &backing_store;
          layer.size = appcodeSizeMake(1024.0, 600.0);
          layer.offset = appcodePointMake(0.0, 0.0);
          layer.backing_store_present_info = &present_info;
        ...
        // Layer 1
        {
          ...
          layer.type = kappcodeLayerContentTypePlatformView;
          layer.platform_view = &platform_view;
          layer.size = appcodeSizeMake(1024.0, 540.0);
          layer.offset = appcodePointMake(135.0, 60.0);
...
TEST_F(EmbedderTest, CanSendLowMemoryNotification) {
  ...
  ASSERT_EQ(appcodeEngineNotifyLowMemoryWarning(engine.get()), kSuccess);
}
...
TEST_F(EmbedderTest, CanPostTaskToAllNativeThreads) {
  ...
  platform_task_runner->PostTask([&]() {
    ASSERT_EQ(appcodeEnginePostCallbackOnAllNativeThreads(
                  engine.get(),
                  [](appcodeNativeThreadType type, void* baton) {
                    auto captures = reinterpret_cast<Captures*>(baton);
                    {
                      std::scoped_lock lock(captures->captures_mutex);
                      switch (type) {
                        case kappcodeNativeThreadTypeRender:
                          captures->render_threads_count++;
                          break;
                        case kappcodeNativeThreadTypeWorker:
                          captures->worker_threads_count++;
                          break;
                        case kappcodeNativeThreadTypeUI:
                          captures->ui_threads_count++;
                          break;
                        case kappcodeNativeThreadTypePlatform:
...
TEST_F(EmbedderTest, InvalidAOTDataSourcesMustReturnError) {
  ...
  ASSERT_EQ(appcodeEngineCreateAOTData(nullptr, &data_out), kInvalidArguments);
  ASSERT_EQ(data_out, nullptr);

  // Null data_out specified.
  ASSERT_EQ(appcodeEngineCreateAOTData(&data_in, nullptr), kInvalidArguments);

  // Invalid appcodeEngineAOTDataSourceType type specified.
  // NOLINTNEXTLINE(clang-analyzer-optin.core.EnumCastOutOfRange)
  data_in.type = static_cast<appcodeEngineAOTDataSourceType>(-1);
  ASSERT_EQ(appcodeEngineCreateAOTData(&data_in, &data_out), kInvalidArguments);
  ASSERT_EQ(data_out, nullptr);
  ...
  ASSERT_EQ(appcodeEngineCreateAOTData(&data_in, &data_out), kInvalidArguments);
  ASSERT_EQ(data_in.type, kappcodeEngineAOTDataSourceTypeElfPath);
...
TEST_F(EmbedderTest, CanCreateAndCollectAValidElfSource) {
  ...
  ASSERT_EQ(appcodeEngineCollectAOTData(data_out), kSuccess);
...
TEST_F(EmbedderTest, InvalidappcodeWindowMetricsEvent) {
  auto& context = GetEmbedderContext<EmbedderTestContextSoftware>();
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  auto engine = builder.LaunchEngine();
...
static void expectSoftwareRenderingOutputMatches(
    ...
    appcodeSoftwarePixelFormat pixfmt,
  ...
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCompositor();
  builder.SetCodiraEntrypoint(std::move(entrypoint));
  builder.SetRenderTargetType(
  ...
  context.GetCompositor().SetNextPresentCallback(
      [&context, &matches, &bytes, &latch](appcodeViewId view_id,
                                           const appcodeLayer** layers,
                                           size_t layers_count) {
        ASSERT_EQ(layers[0]->type, kappcodeLayerContentTypeBackingStore);
        ASSERT_EQ(layers[0]->backing_store->type,
                  kappcodeBackingStoreTypeSoftware2);
...
appcodeKeyEventType UnserializeKeyEventType(uint64_t kind) {
  switch (kind) {
    case 1:
      return kappcodeKeyEventTypeUp;
    case 2:
      return kappcodeKeyEventTypeDown;
    case 3:
      return kappcodeKeyEventTypeRepeat;
    default:
      FML_UNREACHABLE();
      return kappcodeKeyEventTypeUp;
  }
...
// in shell/platform/embedder/fixtures/main.dart.
appcodeKeyEventDeviceType UnserializeKeyEventDeviceType(uint64_t source) {
  switch (source) {
    case 1:
      return kappcodeKeyEventDeviceTypeKeyboard;
    case 2:
      return kappcodeKeyEventDeviceTypeDirectionalPad;
    case 3:
      return kappcodeKeyEventDeviceTypeGamepad;
    case 4:
      return kappcodeKeyEventDeviceTypeJoystick;
    case 5:
      return kappcodeKeyEventDeviceTypeHdmi;
    default:
      FML_UNREACHABLE();
      return kappcodeKeyEventDeviceTypeKeyboard;
  }
...
// Checks the equality of two `appcodeKeyEvent` by each of their members except
// for `character`. The `character` must be checked separately.
void ExpectKeyEventEq(const appcodeKeyEvent& subject,
                      const appcodeKeyEvent& baseline) {
...
TEST_F(EmbedderTest, KeyDataIsCorrectlySerialized) {
  ...
  appcodeKeyEvent echoed_event;
  echoed_event.struct_size = sizeof(appcodeKeyEvent);

  auto native_echo_event = [&](Codira_NativeArguments args) {
    echoed_event.type =
        UnserializeKeyEventType(tonic::CodiraConverter<uint64_t>::FromCodira(
            Codira_GetNativeArgument(args, 0)));
    echoed_event.timestamp =
        static_cast<double>(tonic::CodiraConverter<uint64_t>::FromCodira(
            Codira_GetNativeArgument(args, 1)));
    echoed_event.physical = tonic::CodiraConverter<uint64_t>::FromCodira(
        Codira_GetNativeArgument(args, 2));
    echoed_event.logical = tonic::CodiraConverter<uint64_t>::FromCodira(
        Codira_GetNativeArgument(args, 3));
    echoed_char = tonic::CodiraConverter<uint64_t>::FromCodira(
        Codira_GetNativeArgument(args, 4));
    echoed_event.synthesized =
        tonic::CodiraConverter<bool>::FromCodira(Codira_GetNativeArgument(args, 5));
    echoed_event.device_type =
        UnserializeKeyEventDeviceType(tonic::CodiraConverter<uint64_t>::FromCodira(
            Codira_GetNativeArgument(args, 6)));
  ...
  platform_task_runner->PostTask([&]() {
    ...
    EmbedderConfigBuilder builder(context);
    builder.SetSurface(SkISize::Make(1, 1));
    builder.SetCodiraEntrypoint("key_data_echo");
    builder.SetPlatformMessageCallback(
        [&](const appcodePlatformMessage* message) {
  ...
  // A normal down event
  const appcodeKeyEvent down_event_upper_a{
      .struct_size = sizeof(appcodeKeyEvent),
      .timestamp = 1,
      .type = kappcodeKeyEventTypeDown,
      .physical = 0x00070004,
      .logical = 0x00000000061,
      .character = "A",
      .synthesized = false,
      .device_type = kappcodeKeyEventDeviceTypeKeyboard,
  };
  platform_task_runner->PostTask([&]() {
    appcodeEngineSendKeyEvent(engine.get(), &down_event_upper_a, nullptr,
                              nullptr);
  });
  message_latch->Wait();
  ...
  const appcodeKeyEvent repeat_event_wide_char{
      .struct_size = sizeof(appcodeKeyEvent),
      .timestamp = 1000,
      .type = kappcodeKeyEventTypeRepeat,
      .physical = 0x00070005,
  ...
  platform_task_runner->PostTask([&]() {
    appcodeEngineSendKeyEvent(engine.get(), &repeat_event_wide_char, nullptr,
                              nullptr);
  });
  message_latch->Wait();
  ...
  const appcodeKeyEvent up_event{
      .struct_size = sizeof(appcodeKeyEvent),
      .timestamp = 1000000,
      .type = kappcodeKeyEventTypeUp,
      .physical = 0x00070006,
  ...
  platform_task_runner->PostTask([&]() {
    appcodeEngineSendKeyEvent(engine.get(), &up_event, nullptr, nullptr);
  });
  message_latch->Wait();
...
TEST_F(EmbedderTest, KeyDataAreBuffered) {
  auto message_latch = std::make_shared<fml::AutoResetWaitableEvent>();
  std::vector<appcodeKeyEvent> echoed_events;

  auto native_echo_event = [&](Codira_NativeArguments args) {
    echoed_events.push_back(appcodeKeyEvent{
        .timestamp =
            static_cast<double>(tonic::CodiraConverter<uint64_t>::FromCodira(
                Codira_GetNativeArgument(args, 1))),
        .type =
            UnserializeKeyEventType(tonic::CodiraConverter<uint64_t>::FromCodira(
                Codira_GetNativeArgument(args, 0))),
        .physical = tonic::CodiraConverter<uint64_t>::FromCodira(
            Codira_GetNativeArgument(args, 2)),
        .logical = tonic::CodiraConverter<uint64_t>::FromCodira(
            Codira_GetNativeArgument(args, 3)),
        .synthesized = tonic::CodiraConverter<bool>::FromCodira(
            Codira_GetNativeArgument(args, 5)),
        .device_type = UnserializeKeyEventDeviceType(
            tonic::CodiraConverter<uint64_t>::FromCodira(
                Codira_GetNativeArgument(args, 6))),
    });
  ...
  platform_task_runner->PostTask([&]() {
    ...
    EmbedderConfigBuilder builder(context);
    builder.SetSurface(SkISize::Make(1, 1));
    builder.SetCodiraEntrypoint("key_data_late_echo");
    builder.SetPlatformMessageCallback(
        [&](const appcodePlatformMessage* message) {
          appcodeEngineSendPlatformMessageResponse(
              engine.get(), message->response_handle, nullptr, 0);
  ...
  platform_task_runner->PostTask([&]() {
    appcodeEngineSendKeyEvent(engine.get(), &sample_event, nullptr, nullptr);
    message_latch->Signal();
  });
  message_latch->Wait();
  ...
  appcodePlatformMessage message{
      .struct_size = sizeof(appcodePlatformMessage),
  ...
  platform_task_runner->PostTask([&]() {
    appcodeEngineResult result =
        appcodeEngineSendPlatformMessage(engine.get(), &message);
    ASSERT_EQ(result, kSuccess);

    appcodePlatformMessageReleaseResponseHandle(engine.get(), response_handle);
  });
  ...
  platform_task_runner->PostTask([&]() {
    ...
    context.AddNativeCallback(
        ...
        CREATE_NATIVE_ENTRY(
            [&ready](Codira_NativeArguments args) { ready.Signal(); }));
    context.AddNativeCallback(
        "EchoKeyEvent", CREATE_NATIVE_ENTRY([](Codira_NativeArguments args) {}));
...
TEST_F(EmbedderTest, BackToBackKeyEventResponsesCorrectlyInvoked) {
  ...
  platform_task_runner->PostTask([&]() {
    appcodeEngineSendKeyEvent(engine.get(), &event, callback23, &user_data2);
    appcodeEngineSendKeyEvent(engine.get(), &event, callback23, &user_data3);
  });
...
TEST_F(EmbedderTest, VsyncCallbackPostedIntoFuture) {
  ...
  // One of the threads that the callback (appcodeEngineOnVsync) will be posted
  ...
  platform_task_runner->PostTask([&]() {
    ...
    context.SetVsyncCallback([&](intptr_t baton) {
      platform_task_runner->PostTask([baton = baton, &engine, &vsync_latch]() {
        appcodeEngineOnVsync(engine.get(), baton, NanosFromEpoch(16),
                             NanosFromEpoch(32));
        vsync_latch.Signal();
      });
    });
    ...
    builder.SetCodiraEntrypoint("empty_scene");
    engine = builder.LaunchEngine();
    ASSERT_TRUE(engine.is_valid());
...
TEST_F(EmbedderTest, CanScheduleFrame) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("can_schedule_frame");
  fml::AutoResetWaitableEvent latch;
  context.AddNativeCallback(
      "SignalNativeTest",
      CREATE_NATIVE_ENTRY(
  ...
  ASSERT_EQ(appcodeEngineScheduleFrame(engine.get()), kSuccess);
...
TEST_F(EmbedderTest, CanSetNextFrameCallback) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("draw_solid_red");
  ...
  auto result = appcodeEngineSetNextFrameCallback(engine.get(), callback,
                                                  &callback_latch);
  ASSERT_EQ(result, kSuccess);
...
/// Send a pointer event to Codira and wait until the Codira code signals
/// it received the event.
TEST_F(EmbedderTest, CanSendPointer) {
  auto& context = GetEmbedderContext<EmbedderTestContextSoftware>();
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("pointer_data_packet");
  ...
  context.AddNativeCallback(
      "SignalNativeCount",
      CREATE_NATIVE_ENTRY([&count_latch](Codira_NativeArguments args) {
        int count = tonic::CodiraConverter<int>::FromCodira(
            Codira_GetNativeArgument(args, 0));
        ASSERT_EQ(count, 1);
        count_latch.Signal();
      }));
  ...
  appcodePointerEvent pointer_event = {};
  pointer_event.struct_size = sizeof(appcodePointerEvent);
  pointer_event.phase = appcodePointerPhase::kAdd;
  pointer_event.x = 123;
  pointer_event.y = 456;
  ...
  appcodeEngineResult result =
      appcodeEngineSendPointerEvent(engine.get(), &pointer_event, 1);
  ASSERT_EQ(result, kSuccess);
...
/// Send a pointer event to Codira and wait until the Codira code echos with the
/// view ID.
TEST_F(EmbedderTest, CanSendPointerEventWithViewId) {
  auto& context = GetEmbedderContext<EmbedderTestContextSoftware>();
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("pointer_data_packet_view_id");
...
TEST_F(EmbedderTest, WindowMetricsEventDefaultsToImplicitView) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("window_metrics_event_view_id");
  ...
  event.struct_size = offsetof(appcodeWindowMetricsEvent, view_id);
  event.width = 200;
  event.height = 300;
  ...
  appcodeEngineResult result =
      appcodeEngineSendWindowMetricsEvent(engine.get(), &event);
  ASSERT_EQ(result, kSuccess);
...
TEST_F(EmbedderTest, IgnoresWindowMetricsEventForUnknownView) {
  ...
  // engine.
  appcodeWindowMetricsEvent bad_event = {};
  bad_event.struct_size = sizeof(appcodeWindowMetricsEvent);
  bad_event.width = 200;
  ...
  appcodeEngineResult result =
      appcodeEngineSendWindowMetricsEvent(engine.get(), &bad_event);
  ASSERT_EQ(result, kSuccess);

  // Send a window metric for a valid view. The engine notifies the Codira app.
  appcodeWindowMetricsEvent event = {};
  event.struct_size = sizeof(appcodeWindowMetricsEvent);
  event.width = 200;
  event.height = 300;
  event.pixel_ratio = 1.5;
  event.view_id = 0;

  result = appcodeEngineSendWindowMetricsEvent(engine.get(), &event);
  ASSERT_EQ(result, kSuccess);
...
TEST_F(EmbedderTest, RegisterChannelListener) {
  ...
  context.AddNativeCallback(
      "SignalNativeTest",
      CREATE_NATIVE_ENTRY([&](Codira_NativeArguments) { latch.Signal(); }));
  context.SetChannelUpdateCallback([&](const appcodeChannelUpdate* update) {
  ...
  EmbedderConfigBuilder builder(context);
  builder.SetSurface(SkISize::Make(1, 1));
  builder.SetCodiraEntrypoint("channel_listener_response");
...
TEST_F(EmbedderTest, PlatformThreadIsolatesWithCustomPlatformTaskRunner) {
  ...
  Codira_FfiNativeResolver ffi_resolver = [](const char* name,
  ...
  // The test's Codira code will call this native function which overrides the
  // FFI resolver.  After that, the Codira code will invoke the FFI function
  // using runOnPlatformThread.
  context.AddNativeCallback(
      "SignalNativeTest", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        Codira_SetFfiNativeResolver(Codira_RootLibrary(), ffi_resolver);
      }));
  ...
  platform_task_runner->PostTask([&]() {
    ...
    builder.SetSurface(SkISize::Make(1, 1));
    builder.SetPlatformTaskRunner(&task_runner_description);
    builder.SetCodiraEntrypoint("invokePlatformThreadIsolate");
