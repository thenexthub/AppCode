...
#include "appcode/shell/platform/embedder/tests/embedder_test_context_vulkan.h"

#include <memory>
#include <utility>

#include "appcode/fml/logging.h"
#include "appcode/shell/platform/embedder/tests/embedder_test_compositor_vulkan.h"
#include "appcode/testing/test_vulkan_context.h"
#include "appcode/testing/test_vulkan_surface.h"
...
EmbedderTestContextVulkan::EmbedderTestContextVulkan(std::string assets_path)
  ...
  renderer_config_.type = appcodeRendererType::kVulkan;
  renderer_config_.vulkan = {
      .struct_size = sizeof(appcodeVulkanRendererConfig),
      ...
      .get_next_image_callback =
          [](void* context,
             const appcodeFrameInfo* frame_info) -> appcodeVulkanImage {
        VkImage image =
      ...
      .present_image_callback = [](void* context,
                                   const appcodeVulkanImage* image) -> bool {
...
void EmbedderTestContextVulkan::SetVulkanInstanceProcAddressCallback(
    appcodeVulkanInstanceProcAddressCallback callback) {
