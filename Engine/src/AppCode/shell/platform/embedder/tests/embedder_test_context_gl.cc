...
#include "appcode/shell/platform/embedder/tests/embedder_test_compositor_gl.h"
...
EmbedderTestContextGL::EmbedderTestContextGL(std::string assets_path)
  ...
  renderer_config_.type = appcodeRendererType::kOpenGL;
  renderer_config_.open_gl = {
      .struct_size = sizeof(appcodeOpenGLRendererConfig),
      ...
      .surface_transformation = [](void* context) -> appcodeTransformation {
        return reinterpret_cast<EmbedderTestContextGL*>(context)
      ...
      .present_with_info = [](void* context,
                              const appcodePresentInfo* present_info) -> bool {
...
void EmbedderTestContextGL::SetOpenGLFBOCallBack() {
  ...
  FML_CHECK(renderer_config_.type == appcodeRendererType::kOpenGL);
...
bool EmbedderTestContextGL::GLPresent(appcodePresentInfo present_info) {
  FML_CHECK(gl_surface_) << "GL surface must be initialized.";
...
uint32_t EmbedderTestContextGL::GLGetFramebuffer(appcodeFrameInfo frame_info) {
  FML_CHECK(gl_surface_) << "GL surface must be initialized.";
