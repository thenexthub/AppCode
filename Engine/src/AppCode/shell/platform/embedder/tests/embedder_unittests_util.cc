...
sk_sp<SkSurface> CreateRenderSurface(const appcodeLayer& layer,
...
void ConfigureBackingStore(appcodeBackingStore& backing_store,
                           EmbedderTestContextType backend,
                           bool opengl_framebuffer) {
  switch (backend) {
    case EmbedderTestContextType::kVulkanContext:
      backing_store.type = kappcodeBackingStoreTypeVulkan;
      break;
    case EmbedderTestContextType::kOpenGLContext:
      if (opengl_framebuffer) {
      ...
      break;
    case EmbedderTestContextType::kMetalContext:
      backing_store.type = kappcodeBackingStoreTypeMetal;
      break;
    case EmbedderTestContextType::kSoftwareContext:
...
void FilterMutationsByType(
    const appcodePlatformViewMutation** mutations,
    size_t count,
    appcodePlatformViewMutationType type,
    const std::function<void(const appcodePlatformViewMutation& mutation)>&
        handler) {
  ...
  for (size_t i = 0; i < count; ++i) {
    const appcodePlatformViewMutation* mutation = mutations[i];
...
void FilterMutationsByType(
    const appcodePlatformView* view,
    appcodePlatformViewMutationType type,
    const std::function<void(const appcodePlatformViewMutation& mutation)>&
        handler) {
...
SkMatrix GetTotalMutationTransformationMatrix(
  ...
  FilterMutationsByType(
      mutations, count, kappcodePlatformViewMutationTypeTransformation,
      [&](const auto& mutation) {
...
SkMatrix GetTotalMutationTransformationMatrix(const appcodePlatformView* view) {
