...
EmbedderConfigBuilder::EmbedderConfigBuilder(
  ...
  project_args_.platform_message_callback =
      [](const appcodePlatformMessage* message, void* context) {
        reinterpret_cast<EmbedderTestContext*>(context)
            ->PlatformMessageCallback(message);
      };

  custom_task_runners_.struct_size = sizeof(appcodeCustomTaskRunners);
...
appcodeProjectArgs& EmbedderConfigBuilder::GetProjectArgs() {
...
void EmbedderConfigBuilder::SetCodiraEntrypoint(std::string entrypoint) {
  if (entrypoint.empty()) {
    return;
  }
...
void EmbedderConfigBuilder::AddCodiraEntrypointArgument(std::string arg) {
  if (arg.empty()) {
    return;
  }
...
void EmbedderConfigBuilder::SetPlatformTaskRunner(
    const appcodeTaskRunnerDescription* runner) {
  if (runner == nullptr) {
    return;
  }
...
void EmbedderConfigBuilder::SetViewFocusChangeRequestCallback(
    const std::function<void(const appcodeViewFocusChangeRequest*)>& callback) {
...
void EmbedderConfigBuilder::SetCompositor(bool avoid_backing_store_cache,
  ...
  if (use_present_layers_callback) {
    compositor_.present_layers_callback = [](const appcodeLayer** layers,
                                             size_t layers_count,
                                             void* user_data) {
      ...
      return compositor->Present(kappcodeImplicitViewId, layers, layers_count);
    };
  } else {
    compositor_.present_view_callback = [](const appcodePresentViewInfo* info) {
      auto compositor =
...
appcodeCompositor& EmbedderConfigBuilder::GetCompositor() {
  return compositor_;
}
...
UniqueEngine EmbedderConfigBuilder::SetupEngine(bool run) const {
  ...
  appcodeProjectArgs project_args = project_args_;
  ...
  auto result = run ? appcodeEngineRun(appcode_ENGINE_VERSION,
                                       &context_.GetRendererConfig(),
                                       &project_args, &context_, &engine)
                    : appcodeEngineInitialize(
                          appcode_ENGINE_VERSION, &context_.GetRendererConfig(),
                          &project_args, &context_, &engine);
