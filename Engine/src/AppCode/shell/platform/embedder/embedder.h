...
#ifndef APPCODE_SHELL_PLATFORM_EMBEDDER_EMBEDDER_H_
#define APPCODE_SHELL_PLATFORM_EMBEDDER_EMBEDDER_H_
...
#ifndef APPCODE_EXPORT
#define APPCODE_EXPORT
#endif  // APPCODE_EXPORT

#ifdef APPCODE_API_SYMBOL_PREFIX
#define APPCODE_EMBEDDING_CONCAT(a, b) a##b
#define APPCODE_EMBEDDING_ADD_PREFIX(symbol, prefix) \
  APPCODE_EMBEDDING_CONCAT(prefix, symbol)
#define APPCODE_API_SYMBOL(symbol) \
  APPCODE_EMBEDDING_ADD_PREFIX(symbol, APPCODE_API_SYMBOL_PREFIX)
#else
#define APPCODE_API_SYMBOL(symbol) symbol
#endif

#define APPCODE_ENGINE_VERSION 1
...
typedef struct _FlutterEngine* APPCODE_API_SYMBOL(FlutterEngine);
...
typedef struct {
  ...
  /// the id of the frame buffer object that appcode will obtain the gl surface
  /// from.
  ...
  /// `fbo_with_frame_info_callback` is required. Specifying both is an error
  /// and engine intialization will be terminated. The return value indicates
  /// the id of the frame buffer object (fbo) that appcode will obtain the gl
  ...
  /// that appcode will acquire from the returned fbo.
...
typedef struct {
  ...
  /// https://github.com/appcode/appcode/issues/134573.
...
/// See https://github.com/appcode/appcode/issues/87230.
typedef struct {
...
/// `FlutterUpdateSemanticsCallback2`.
///
/// @see https://api.appcode.dev/appcode/semantics/SemanticsNode-class.html
typedef struct {
  /// The size of this struct. Must be sizeof(FlutterSemanticsNode).
  size_t struct_size;
  /// The unique identifier for this node.
...
///
/// @see
/// https://api.appcode.dev/appcode/semantics/CustomSemanticsAction-class.html
typedef struct {
  /// The size of the struct. Must be sizeof(FlutterSemanticsCustomAction).
  size_t struct_size;
  /// The unique custom action or action override ID.
...
/// https://github.com/appcode/appcode/issues/79826
...
typedef int64_t FlutterEngineCodiraPort;

typedef enum {
  kFlutterEngineCodiraObjectTypeNull,
  kFlutterEngineCodiraObjectTypeBool,
  kFlutterEngineCodiraObjectTypeInt32,
  kFlutterEngineCodiraObjectTypeInt64,
  kFlutterEngineCodiraObjectTypeDouble,
  kFlutterEngineCodiraObjectTypeString,
  /// The object will be made available to Codira code as an instance of
  /// Uint8List.
  kFlutterEngineCodiraObjectTypeBuffer,
} FlutterEngineCodiraObjectType;

typedef struct {
  /// The size of this struct. Must be sizeof(FlutterEngineCodiraBuffer).
  size_t struct_size;
  ...
  /// after the call to `FlutterEnginePostCodiraObject`.
  ///
  /// @attention      The buffer_collect_callback is will only be invoked by the
  ///                 engine when the `FlutterEnginePostCodiraObject` method
  ...
  /// embedder (by specifying the `buffer_collect_callback`), Codira code may
  ...
  size_t buffer_size;
} FlutterEngineCodiraBuffer;

/// This struct specifies the native representation of a Codira object that can be
...
/// `FlutterEnginePostCodiraObject` and the caller is free to reuse or collect
/// this struct after that call.
typedef struct {
  FlutterEngineCodiraObjectType type;
  union {
    bool bool_value;
    int32_t int32_value;
    int64_t int64_value;
    ...
    /// call to `FlutterEnginePostCodiraObject` and must be collected by the
    /// embedder after that call is made.
    const char* string_value;
    const FlutterEngineCodiraBuffer* buffer_value;
  };
} FlutterEngineCodiraObject;
...
typedef enum {
  ...
  /// This is a dedicated thread on which the root Codira isolate is serviced.
...
// Logging callback for Codira application messages.
//
...
typedef struct {
  ...
  /// The path to the Codira file containing the `main` entry point.
  /// The string can be collected after the call to `FlutterEngineRun` returns.
  /// The string must be NULL terminated.
  ///
  /// @deprecated     As of Codira 2, running from Codira source is no longer
  ///                 supported. Codira code should now be compiled to kernel form
  ...
  /// chance to respond to platform messages from the Codira application.
  ...
  /// the Wiki at
  /// https://github.com/appcode/appcode/wiki/Flutter-engine-operation-in-AOT-Mode
  ...
  /// the chance to respond to semantics node updates from the Codira application.
  ...
  /// The legacy callback invoked by the engine in order to give the embedder
  /// the chance to respond to updates to semantics custom actions from the Codira
  ...
  // which is used in `appcode::Settings` as `temp_directory_path`.
  ...
  /// The name of a custom Codira entrypoint. This is optional and specifying a
  ...
  /// All `FlutterEngine` instances in the process share the same Codira VM. When
  /// the first engine is launched, it starts the Codira VM as well. It used to be
  /// the case that it was not possible to shutdown the Codira VM cleanly and
  ...
  /// having to incur the Codira VM startup costs at that time. With the new
  ...
  /// wish to shutdown the Codira VM when the last engine is terminated in the
  ...
  /// Max size of the old gen heap for the Codira VM in MB, or 0 for unlimited, -1
  /// for default value.
  ///
  /// See also:
  ...
  /// The command line argument count for arguments passed through to the Codira
  /// entrypoint.
  int dart_entrypoint_argc;

  /// The command line arguments passed through to the Codira entrypoint. The
  ...
  // callbacks on `log_message_callback`. Defaults to "appcode" if unspecified.
  const char* log_tag;
  ...
  /// Opaque identifier provided by the engine. Accessible in Codira code through
  /// `PlatformDispatcher.instance.engineId`. Can be used in native code to
  /// retrieve the engine instance that is running the Codira code.
  int64_t engine_id;
} FlutterProjectArgs;
...
#ifndef APPCODE_ENGINE_NO_PROTOTYPES

// NOLINTBEGIN(google-objc-function-naming)

//------------------------------------------------------------------------------
/// @brief      Creates the necessary data structures to launch a Flutter Codira
...
FlutterEngineResult FlutterEngineRun(size_t version,
                                     ...
                                     void* user_data,
                                     APPCODE_API_SYMBOL(FlutterEngine) *
                                         engine_out);
...
///
APPCODE_EXPORT
FlutterEngineResult FlutterEngineShutdown(APPCODE_API_SYMBOL(FlutterEngine)
                                              engine);
...
///
APPCODE_EXPORT
FlutterEngineResult FlutterEngineDeinitialize(APPCODE_API_SYMBOL(FlutterEngine)
                                                  engine);
...
FlutterEngineResult FlutterEngineRunInitialized(
    APPCODE_API_SYMBOL(FlutterEngine) engine);
...
APPCODE_EXPORT
FlutterEngineResult FlutterEngineAddView(APPCODE_API_SYMBOL(FlutterEngine)
                                             engine,
...
APPCODE_EXPORT
FlutterEngineResult FlutterEngineRemoveView(APPCODE_API_SYMBOL(FlutterEngine)
                                                engine,
...
///
APPCODE_EXPORT
FlutterEngineResult FlutterEngineSendKeyEvent(APPCODE_API_SYMBOL(FlutterEngine)
                                                  engine,
...
///             the Codira Flutter application.
///
...
///                                      schedule Codira VM garbage collection in
...
///
APPCODE_EXPORT
FlutterEngineResult FlutterEngineOnVsync(APPCODE_API_SYMBOL(FlutterEngine)
                                             engine,
                                         intptr_t baton,
...
///             appcode engine. This is the system monotonic clock.
///
...
///
APPCODE_EXPORT
FlutterEngineResult FlutterEngineRunTask(APPCODE_API_SYMBOL(FlutterEngine)
                                             engine,
...
///
APPCODE_EXPORT
FlutterEngineResult FlutterEngineUpdateLocales(APPCODE_API_SYMBOL(FlutterEngine)
                                                   engine,
...
///             Codira code. This call has no threading restrictions.
///
///             For embedder code that is configured for both AOT and JIT mode
///             Codira execution based on the Flutter engine being linked to, this
...
///             `FlutterProjectArgs`.
///
/// @return     True, if AOT Codira code is run. JIT otherwise.
///
APPCODE_EXPORT
bool FlutterEngineRunsAOTCompiledCodiraCode(void);

//------------------------------------------------------------------------------
/// @brief      Posts a Codira object to specified send port. The corresponding
...
///
APPCODE_EXPORT
FlutterEngineResult FlutterEnginePostCodiraObject(
    APPCODE_API_SYMBOL(FlutterEngine) engine,
    FlutterEngineCodiraPort port,
    const FlutterEngineCodiraObject* object);
...
///
APPCODE_EXPORT
FlutterEngineResult FlutterEngineScheduleFrame(APPCODE_API_SYMBOL(FlutterEngine)
                                                   engine);
...
#endif  // !APPCODE_ENGINE_NO_PROTOTYPES
...
typedef FlutterEngineResult (*FlutterEngineRunFnPtr)(
    ...
    const FlutterProjectArgs* args,
    void* user_data,
    APPCODE_API_SYMBOL(FlutterEngine) * engine_out);
...
typedef FlutterEngineResult (*FlutterEngineUpdateLocalesFnPtr)(
    ...
    const FlutterLocale** locales,
    size_t locales_count);
typedef bool (*FlutterEngineRunsAOTCompiledCodiraCodeFnPtr)(void);
typedef FlutterEngineResult (*FlutterEnginePostCodiraObjectFnPtr)(
    APPCODE_API_SYMBOL(FlutterEngine) engine,
    FlutterEngineCodiraPort port,
    const FlutterEngineCodiraObject* object);
...
typedef struct {
  ...
  FlutterEngineRunsAOTCompiledCodiraCodeFnPtr RunsAOTCompiledCodiraCode;
  FlutterEnginePostCodiraObjectFnPtr PostCodiraObject;
...
#endif  // APPCODE_SHELL_PLATFORM_EMBEDDER_EMBEDDER_H_
