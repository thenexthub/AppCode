...
EmbedderSemanticsUpdate::EmbedderSemanticsUpdate(
  ...
  update_ = {
      .struct_size = sizeof(appcodeSemanticsUpdate),
...
appcodeSemanticsFlag SemanticsFlagsToInt(const SemanticsFlags& flags) {
  int result = 0;
...
void EmbedderSemanticsUpdate::AddNode(const SemanticsNode& node) {
  ...
  // Do not add new members to appcodeSemanticsNode.
  // This would break the forward compatibility of appcodeSemanticsUpdate.
  // All new members must be added to appcodeSemanticsNode2 instead.
  nodes_.push_back({
      sizeof(appcodeSemanticsNode),
      node.id,
      SemanticsFlagsToInt(node.flags),
      static_cast<appcodeSemanticsAction>(node.actions),
      ...
      static_cast<appcodeTextDirection>(node.textDirection),
      appcodeRect{node.rect.fLeft, node.rect.fTop, node.rect.fRight,
                  node.rect.fBottom},
      appcode_transform,
...
void EmbedderSemanticsUpdate::AddAction(
  ...
  // Do not add new members to appcodeSemanticsCustomAction.
  // This would break the forward compatibility of appcodeSemanticsUpdate.
  // All new members must be added to appcodeSemanticsCustomAction2 instead.
  actions_.push_back({
      sizeof(appcodeSemanticsCustomAction),
      action.id,
      static_cast<appcodeSemanticsAction>(action.overrideId),
      action.label.c_str(),
      action.hint.c_str(),
  });
}
...
EmbedderSemanticsUpdate2::EmbedderSemanticsUpdate2(
  ...
  update_ = {.struct_size = sizeof(appcodeSemanticsUpdate2),
...
void EmbedderSemanticsUpdate2::AddAction(
    const CustomAccessibilityAction& action) {
  actions_.push_back({
      sizeof(appcodeSemanticsCustomAction2),
      action.id,
      static_cast<appcodeSemanticsAction>(action.overrideId),
      action.label.c_str(),
      action.hint.c_str(),
...
EmbedderSemanticsUpdate2::CreateStringAttributes(
  ...
  // is destroyed.
  auto result = std::make_unique<std::vector<const appcodeStringAttribute*>>();
  result->reserve(attributes.size());

  for (const auto& attribute : attributes) {
    auto embedder_attribute = std::make_unique<appcodeStringAttribute>();
    embedder_attribute->struct_size = sizeof(appcodeStringAttribute);
    embedder_attribute->start = attribute->start;
    embedder_attribute->end = attribute->end;

    switch (attribute->type) {
      case StringAttributeType::kLocale: {
        std::shared_ptr<appcode::LocaleStringAttribute> locale_attribute =
            std::static_pointer_cast<appcode::LocaleStringAttribute>(attribute);

        auto embedder_locale = std::make_unique<appcodeLocaleStringAttribute>();
        embedder_locale->struct_size = sizeof(appcodeLocaleStringAttribute);
        embedder_locale->locale = locale_attribute->locale.c_str();
        locale_attributes_.push_back(std::move(embedder_locale));

        embedder_attribute->type = appcodeStringAttributeType::kLocale;
        embedder_attribute->locale = locale_attributes_.back().get();
        break;
      }
      case appcode::StringAttributeType::kSpellOut: {
        // All spell out attributes are identical and share a lazily created
        // instance.
        if (!spell_out_attribute_) {
          auto spell_out_attribute_ =
              std::make_unique<appcodeSpellOutStringAttribute>();
          spell_out_attribute_->struct_size =
              sizeof(appcodeSpellOutStringAttribute);
        }

        embedder_attribute->type = appcodeStringAttributeType::kSpellOut;
