...
std::unique_ptr<VulkanSurface> VulkanSurfacePool::GetCachedOrCreateSurface(
    const SkISize& size) {
  TRACE_EVENT2("appcode", "VulkanSurfacePool::GetCachedOrCreateSurface",
  ...
  // First try to find a surface that exactly matches |size|.
  {
    ...
    if (exact_match_it != available_surfaces_.end()) {
      ...
      TRACE_EVENT_INSTANT0("appcode", "Exact match found");
...
void VulkanSurfacePool::SubmitSurface(
  ...
  TRACE_EVENT0("appcode", "VulkanSurfacePool::SubmitSurface");

  // This cast is safe because |VulkanSurface| is the only implementation of
  // |SurfaceProducerSurface| for appcode on Fuchsia.  Additionally, it is
...
std::unique_ptr<VulkanSurface> VulkanSurfacePool::CreateSurface(
    const SkISize& size) {
  TRACE_EVENT2("appcode", "VulkanSurfacePool::CreateSurface", "width",
...
void VulkanSurfacePool::RecycleSurface(std::unique_ptr<VulkanSurface> surface) {
  ...
  TRACE_EVENT0("appcode", "VulkanSurfacePool::RecycleSurface");
  ...
  if (available_surfaces_.size() < kMaxSurfaces) {
    ...
    TRACE_EVENT_INSTANT0("appcode", "Too many surfaces in pool, dropping");
  }
  TraceStats();
}

void VulkanSurfacePool::AgeAndCollectOldBuffers() {
  TRACE_EVENT0("appcode", "VulkanSurfacePool::AgeAndCollectOldBuffers");
  ...
  TRACE_EVENT1("appcode", "AgeAndCollect", "aged surfaces",
  ...
  if (surface_to_remove_it != available_surfaces_.end()) {
    TRACE_EVENT_INSTANT0("appcode", "replacing surface with smaller one");
...
void VulkanSurfacePool::ShrinkToFit() {
  TRACE_EVENT0("appcode", "VulkanSurfacePool::ShrinkToFit");
...
void VulkanSurfacePool::TraceStats() {
  ...
  TRACE_COUNTER("appcode", "SurfacePoolCounts", 0u, "CachedCount",
  ...
  TRACE_COUNTER("appcode", "SurfacePoolBytes", 0u,          //
