...
#include "appcode_vma/appcode_skia_vma.h"
...
//   * Over budget ("appcode", "GPURasterizer::Draw") durations
...
//     "SkiaCacheBytes" field in the ("appcode", "SurfacePool") trace counter
...
bool VulkanSurfaceProducer::Initialize() {
  ...
  // appcode_runner in order to work. As a result, we can use the presence
  // of the layers to mean that we want the layers enabled.
  application_ = std::make_unique<vulkan::VulkanApplication>(
      *vk_, "appcodeRunner", std::move(extensions), VK_MAKE_VERSION(1, 0, 0),
  ...
  memory_allocator_ = appcode::appcodeSkiaVulkanMemoryAllocator::Make(
  ...
  // Use local limits specified in this file above instead of appcode defaults.
...
void VulkanSurfaceProducer::SubmitSurfaces(
  ...
  TRACE_EVENT0("appcode", "VulkanSurfaceProducer::SubmitSurfaces");

  // Do a single flush for all canvases derived from the context.
  {
    TRACE_EVENT0("appcode", "GrDirectContext::flushAndSignalSemaphores");
