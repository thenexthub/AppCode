...
#include "appcode/shell/common/serialization_callbacks.h"
...
std::unique_ptr<appcode::PlatformMessage> MakeLocalizationPlatformMessage(
    const fuchsia::intl::Profile& intl_profile) {
  return std::make_unique<appcode::PlatformMessage>(
      "appcode/localization", MakeLocalizationPlatformMessageData(intl_profile),
      nullptr);
}
...
appcode::ThreadHost Engine::CreateThreadHost(
    const std::string& name_prefix,
  ...
  appcode::ThreadConfigSetter config_setter = fml::Thread::SetCurrentThreadName;
  ...
  if (services) {
    ...
    // requirement for appcode::ThreadConfigSetter (i.e. std::function<>) that
  ...
  appcode::ThreadHost::ThreadHostConfig thread_host_config{config_setter};

  thread_host_config.SetRasterConfig(
      {appcode::ThreadHost::ThreadHostConfig::MakeThreadName(
           appcode::ThreadHost::Type::kRaster, name_prefix),
       fml::Thread::ThreadPriority::kRaster});
  thread_host_config.SetUIConfig(
      {appcode::ThreadHost::ThreadHostConfig::MakeThreadName(
           appcode::ThreadHost::Type::kUi, name_prefix),
       fml::Thread::ThreadPriority::kDisplay});
  thread_host_config.SetIOConfig(
      {appcode::ThreadHost::ThreadHostConfig::MakeThreadName(
           appcode::ThreadHost::Type::kIo, name_prefix),
       fml::Thread::ThreadPriority::kNormal});

  return appcode::ThreadHost(thread_host_config);
}
...
void Engine::Initialize(
  ...
  const appcode::TaskRunners task_runners(
      thread_label_,                                // Codira thread labels
  ...
  AccessibilityBridge::DispatchSemanticsActionCallback
      dispatch_semantics_action_callback =
          [this](int32_t node_id, appcode::SemanticsAction action) {
            auto platform_view = shell_->GetPlatformView();
  ...
  auto run_configuration = appcode::RunConfiguration::InferFromSettings(
  ...
  OnSemanticsNodeUpdateCallback on_semantics_node_update_callback =
      [this](appcode::SemanticsNodeUpdates updates, float pixel_ratio) {
  ...
  appcode::Shell::CreateCallback<appcode::PlatformView>
      on_create_platform_view = fml::MakeCopyable(
          [this, view_ref = std::move(platform_view_ref),
           ...
           product_config, svc](appcode::Shell& shell) mutable {
                ...
                on_shader_warmup_callback =
                    [this, &shell](
                      ...
                      WarmupSkps(
                          ...
                          appcode::PersistentCache::GetCacheForProcess()
                              ->asset_manager(),
                          skp_names, completion_callback);
                    };
              } else {
                WarmupSkps(shell.GetCodiraVM()
                               ->GetConcurrentMessageLoop()
                               ->GetTaskRunner()
                               .get(),
                           shell.GetTaskRunners().GetRasterTaskRunner().get(),
            ...
            return std::make_unique<appcode_runner::PlatformView>(
  ...
  {
    TRACE_EVENT0("appcode", "CreateShell");
    shell_ = appcode::Shell::Create(
        appcode::PlatformData(),             // default window data
  ...
  // message.  The engine will be fully functional, except that the appcode
  // shell will not be notified when memory is low.
  {
    ...
    // The appcode Shell should be notified when memory is low regardless of
  ...
  // locale preferences would not be communicated to appcode engine.
  {
    intl_property_provider_.set_error_handler([](zx_status_t status) {
      FML_LOG(WARNING) << "Failed to connect to "
                       ...
                       << " preferences will not be forwarded to appcode apps";
    });

    // Note that we're using the runner's services, not the component's.
    // appcode locales should be updated regardless of whether the component has
  ...
  shell_->GetTaskRunners().GetUITaskRunner()->PostTask(
      fml::MakeCopyable([engine = shell_->GetEngine(),                      //
        ...
        if (engine->Run(std::move(run_configuration)) ==
            appcode::Engine::RunStatus::Failure) {
          on_run_failure();
        }
      }));
}
...
std::unique_ptr<appcode::Surface> Engine::CreateSurface() {
...
void Engine::WriteProfileToTrace() const {
  Codira_Port main_port = shell_->GetEngine()->GetUIIsolateMainPort();
  char* error = NULL;
  bool success = Codira_WriteProfileToTimeline(main_port, &error);
  if (!success) {
    FML_LOG(ERROR) << "Failed to write Codira profile to trace: " << error;
    free(error);
  }
}
...
void Engine::WarmupSkps(
  ...
  concurrent_task_runner->PostTask([raster_task_runner, size,
    ...
    TRACE_DURATION("appcode", "DeserializeSkps");
    ...
    for (auto& mapping : skp_mappings) {
      ...
      procs.fImageProc = appcode::DeserializeImageWithoutData;
      procs.fTypefaceProc = appcode::DeserializeTypefaceWithoutData;
      ...
      raster_task_runner->PostTask([picture, skp_warmup_surface, size,
        ...
        TRACE_DURATION("appcode", "WarmupSkp");
...
void Engine::OnLevelChanged(
  ...
  if (latest_memory_pressure_level_ == fuchsia::memorypressure::Level::NORMAL &&
    ...
    FML_LOG(WARNING)
        << "memorypressure watcher: notifying appcode that memory is low";
