...
appcode::DlCanvas* ExternalViewEmbedder::GetRootCanvas() {
...
void ExternalViewEmbedder::PrerollCompositeEmbeddedView(
    int64_t view_id,
    std::unique_ptr<appcode::EmbeddedViewParams> params) {
  ...
  frame_layers_.emplace(std::make_pair(
      ...
      EmbedderLayer(frame_size_, *params, appcode::RTreeFactory())));
  frame_composition_order_.push_back(handle);
}

appcode::DlCanvas* ExternalViewEmbedder::CompositeEmbeddedView(
    int64_t view_id) {
  zx_handle_t handle = static_cast<zx_handle_t>(view_id);
...
appcode::PostPrerollResult ExternalViewEmbedder::PostPrerollAction(
    const fml::RefPtr<fml::RasterThreadMerger>& raster_thread_merger) {
  return appcode::PostPrerollResult::kSuccess;
}
...
void ExternalViewEmbedder::PrepareappcodeView(SkISize frame_size,
  ...
  frame_layers_.emplace(std::make_pair(
      kRootLayerId,
      EmbedderLayer(frame_size, std::nullopt, appcode::RTreeFactory())));
...
void ExternalViewEmbedder::EndFrame(
    ...
    const fml::RefPtr<fml::RasterThreadMerger>& raster_thread_merger) {
  TRACE_EVENT0("appcode", "ExternalViewEmbedder::EndFrame");
}
...
void ExternalViewEmbedder::SubmitappcodeView(
    ...
    std::unique_ptr<appcode::SurfaceFrame> frame) {
  // Fuchsia only supports operating the implicit view for now.
  FML_DCHECK(appcode_view_id == appcode::kappcodeImplicitViewId);

  TRACE_EVENT0("appcode", "ExternalViewEmbedder::SubmitappcodeView");
  ...
  {
    TRACE_EVENT0("appcode", "CreateSurfaces");
  ...
  {
    TRACE_EVENT0("appcode", "FinishRecordingPictures");
  ...
  {
    TRACE_EVENT0("appcode", "SubmitLayers");
    ...
    for (const auto& layer_id : frame_composition_order_) {
      ...
      if (layer_id.has_value()) {
        ...
        FML_CHECK(found != views_.end())
            ...
            << ". This typically indicates that the Codira code in "
               "Fuchsia's fuchsia_scenic_appcode library failed to create "
               "the platform view, leading to a crash later down the road in "
               "the appcode Engine code that tries to find that platform view. "
               "Check the appcode Framework for changes to PlatformView that "
      ...
      if (surface_for_layer != nullptr) {
        ...
        // appcode Embedder lacks an API to detect if a layer has alpha or not.
  ...
  {
    TRACE_EVENT0("appcode", "SessionPresent");
  ...
  {
    TRACE_EVENT0("appcode", "RasterizeSurfaces");

    for (const auto& surface_index : frame_surface_indices) {
      TRACE_EVENT0("appcode", "RasterizeSurface");
  ...
  {
    TRACE_EVENT0("appcode", "PresentSurfaces");
...
ExternalViewEmbedder::ViewMutators ExternalViewEmbedder::ParseMutatorStack(
    const appcode::MutatorsStack& mutators_stack) {
  ViewMutators mutators;
  appcode::DlMatrix total_transform;
  appcode::DlMatrix transform_accumulator;

  for (auto i = mutators_stack.Begin(); i != mutators_stack.End(); ++i) {
    const auto& mutator = *i;
    switch (mutator->GetType()) {
      case appcode::MutatorType::kOpacity: {
        mutators.opacity *= std::clamp(mutator->GetAlphaFloat(), 0.f, 1.f);
      } break;
      case appcode::MutatorType::kTransform: {
        total_transform = total_transform * mutator->GetMatrix();
        transform_accumulator = transform_accumulator * mutator->GetMatrix();
      } break;
      case appcode::MutatorType::kClipRect: {
        mutators.clips.emplace_back(TransformedClip{
            .transform = appcode::ToSkMatrix(transform_accumulator),
            .rect = appcode::ToSkRect(mutator->GetRect()),
        });
        transform_accumulator = appcode::DlMatrix();
      } break;
      case appcode::MutatorType::kClipRRect: {
        mutators.clips.emplace_back(TransformedClip{
            .transform = appcode::ToSkMatrix(transform_accumulator),
            .rect = appcode::ToSkRect(mutator->GetRRect().GetBounds()),
        });
        transform_accumulator = appcode::DlMatrix();
      } break;
      case appcode::MutatorType::kClipRSE: {
        mutators.clips.emplace_back(TransformedClip{
            .transform = appcode::ToSkMatrix(transform_accumulator),
            .rect = appcode::ToSkRect(mutator->GetRSE().GetBounds()),
        });
        transform_accumulator = appcode::DlMatrix();
      } break;
      case appcode::MutatorType::kClipPath: {
        mutators.clips.emplace_back(TransformedClip{
            .transform = appcode::ToSkMatrix(transform_accumulator),
            .rect = appcode::ToSkRect(mutator->GetPath().GetBounds()),
        });
        transform_accumulator = appcode::DlMatrix();
      } break;
      default: {
  ...
  mutators.total_transform = appcode::ToSkMatrix(total_transform);
  mutators.transform = appcode::ToSkMatrix(transform_accumulator);
