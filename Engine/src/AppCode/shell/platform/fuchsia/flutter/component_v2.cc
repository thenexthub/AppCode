...
// "data" and "assets" are arguments that are specific to the appcode runner.
...
constexpr char kRunnerConfigPath[] = "/config/data/appcode_runner_config";
...
ComponentV2::ComponentV2(
  ...
  // TODO(fxb/88391): Codira launch arguments.
  ...
  fuchsia::io::DirectoryHandle appcode_public_dir;
  {
    auto request = appcode_public_dir.NewRequest().TakeChannel();
  ...
  composed_service_dir->SetFallback(
      fidl::ClientEnd<fuchsia_io::Directory>(appcode_public_dir.TakeChannel()));
    ...
    for (auto& dir_str : other_dirs) {
      ...
      if (status == ZX_OK) {
        ...
        FML_LOG(ERROR) << "could not add out directory entry(" << dir_str
                       << ") for appcode component(" << debug_label_
                       << "): " << zx_status_get_string(status);
      }
    }
  };
  ...
  if (dart_utils::ReadFileToString(kRunnerConfigPath, &json_string)) {
    product_config_ = appcodeRunnerProductConfiguration(json_string);
  ...
  // For AOT, compare with appcode_aot_app in appcode_app.gni.
  // For JIT, compare appcode_jit_runner in BUILD.gn.
  if (appcode::CodiraVM::IsRunningPrecompiledCode()) {
    ...
    if (snapshot->Load(component_data_directory_.get(),
      ...
      isolate_snapshot_ = fml::MakeRefCounted<appcode::CodiraSnapshot>(
  ...
  // Compare appcode_jit_app in appcode_app.gni.
  settings_.application_kernel_list_asset = "app.dilplist";

  settings_.log_tag = debug_label_ + std::string{"(appcode)"};
  ...
  // Don't collect CPU samples from Codira VM C++ code.
    ...
    if (weak) {
      ...
      platform_task_runner->PostTask([weak, runner_incoming_services,
        ...
        if (weak) {
          ...
          FML_LOG(WARNING)
              << "Exception was thrown which was not caught in appcode app: "
              << error;
        }
      });
    } else {
