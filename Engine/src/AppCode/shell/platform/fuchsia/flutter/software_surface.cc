...
bool SoftwareSurface::SetupSkiaSurface(
  ...
  // Allocate a "local" sysmem token to represent appcode's handle to the
  // sysmem buffer.
  ...
  // Acquire appcode's local handle to the sysmem buffer.
  ...
  // Set appcode's constraints on the sysmem buffer.  Software rendering only
  ...
  // Now that the buffer is CPU-readable, it's safe to discard appcode's
...
void SoftwareSurface::SignalWritesFinished(
  ...
  // Inform scenic that appcode is finished writing to the surface.
