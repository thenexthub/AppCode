...
#ifndef APPCODE_SHELL_PLATFORM_FUCHSIA_APPCODE_POINTER_DELEGATE_H_
#define APPCODE_SHELL_PLATFORM_FUCHSIA_APPCODE_POINTER_DELEGATE_H_
...
// surfaces them to PlatformView as appcode::PointerData events for further
...
class PointerDelegate {
 public:
  ...
  // data and transforms them into appcode::PointerData structs. It then calls
  // the supplied callback with a vector of appcode::PointerData, which (1) does
  // last processing (applies metrics), and (2) packs these appcode::PointerData
  // in a appcode::PointerDataPacket for transport to the Engine.
  void WatchLoop(
 ...
 private:
  ...
  std::unordered_map<fuchsia::ui::pointer::TouchInteractionId,
                     std::vector<appcode::PointerData>,
                     IxnHasher>
      touch_buffer_;
  ...
  // A mouse is considered appcode::PointerData::Change::kDown if any button is
  // pressed. This set is used to correctly set the phase in
  // appcode::PointerData.change, with this high-level algorithm:
...
#endif  // APPCODE_SHELL_PLATFORM_FUCHSIA_APPCODE_POINTER_DELEGATE_H_
