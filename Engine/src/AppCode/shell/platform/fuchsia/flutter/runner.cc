...
static void SetProcessName() {
  ...
  stream << "io.appcode.product_runner.";
#else
  stream << "io.appcode.runner.";
#endif
...
#if !defined(DART_PRODUCT)
// Register native symbol information for the Codira VM's profiler.
static void RegisterProfilerSymbols(const char* symbols_path,
  ...
  if (dart_utils::ReadFileToString(symbols_path, symbols)) {
    Codira_AddSymbols(dso_name, symbols->data(), symbols->size());
  } else {
...
Runner::Runner(fml::RefPtr<fml::TaskRunner> task_runner,
  ...
  SetThreadName("io.appcode.runner.main");
...
#if !defined(DART_PRODUCT)
  if (Codira_IsPrecompiledRuntime()) {
    RegisterProfilerSymbols("pkg/data/appcode_aot_runner.dartprofilersymbols",
                            "");
  } else {
    RegisterProfilerSymbols("pkg/data/appcode_jit_runner.dartprofilersymbols",
                            "");
  }
#endif  // !defined(DART_PRODUCT)
}
...
void Runner::Start(
  ...
  TRACE_EVENT1("appcode", "Start", "url", url_copy.c_str());
