  ...
  void ExtendRealm() override {
    ...
    realm_builder()->AddRoute(
        Route{.capabilities = {Protocol{
              ...
              .targets = {kappcodeJitRunnerRef, kMouseInputViewRef}});
...
TEST_P(MouseInputTest, DISABLED_appcodeMouseMove) {
  LaunchClient();
  ...
  // If the first mouse event is cursor movement, appcode first sends an ADD
...
TEST_P(MouseInputTest, DISABLED_appcodeMouseDown) {
  LaunchClient();
  ...
  // If the first mouse event is a button press, appcode first sends an ADD
  ...
  // Then appcode sends a DOWN pointer event with the buttons we care about.
  VerifyEvent(
      event_down,
      /*expected_x=*/static_cast<double>(display_width()) / 2.f,
      /*expected_y=*/static_cast<double>(display_height()) / 2.f,
  ...
  // Then appcode sends a MOVE pointer event with no new information.
  VerifyEvent(
      event_noop_move,
      /*expected_x=*/static_cast<double>(display_width()) / 2.f,
      /*expected_y=*/static_cast<double>(display_height()) / 2.f,
...
TEST_P(MouseInputTest, DISABLED_appcodeMouseDownUp) {
  LaunchClient();
...
TEST_P(MouseInputTest, DISABLED_appcodeMouseDownMoveUp) {
  LaunchClient();
...
// the first event to appcode.
// 1. expect appcode app receive 2 events: ADD - Scroll, but got 3 events: Move
// - Scroll - Scroll.
// 2. the first event appcode app received has random value in buttons field
// Disabled until appcode rolls, since it changes the behavior of this issue.
TEST_P(MouseInputTest, DISABLED_appcodeMouseWheelIssue103098) {
  LaunchClient();
  ...
  // appcode will scale the count of ticks to pixel.
...
TEST_P(MouseInputTest, DISABLED_appcodeMouseWheel) {
  LaunchClient();
