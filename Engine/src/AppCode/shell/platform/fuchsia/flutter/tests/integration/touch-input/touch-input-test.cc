...
#include "appcode/shell/platform/fuchsia/appcode/tests/integration/utils/portable_ui_test.h"
...
constexpr auto kEmbeddingappcodeView = "embedding-appcode-view";
constexpr auto kEmbeddingappcodeViewRef = ChildRef{kEmbeddingappcodeView};
constexpr auto kEmbeddingappcodeViewUrl =
    "fuchsia-pkg://fuchsia.com/embedding-appcode-view#meta/"
    "embedding-appcode-view.cm";
...
class appcodeTapTestBase : public PortableUITest, public ::testing::Test {
 protected:
  ~appcodeTapTestBase() override {
...
class appcodeTapTest : public appcodeTapTestBase {
 private:
  void ExtendRealm() override {
    ...
    // Route the TouchInput protocol capability to the Codira component
    realm_builder()->AddRoute(
        Route{.capabilities = {Protocol{
                  fuchsia::ui::test::input::TouchInputListener::Name_}},
              .source = kMockTouchInputListenerRef,
              .targets = {kappcodeJitRunnerRef, kTouchInputViewRef}});
...
class appcodeEmbedTapTest : public appcodeTapTestBase {
 protected:
  void SetUp() override {
  ...
  // by embedding-appcode-view.dart
  //
  ...
  void AddComponentArgument(std::string component_arg) {
    ...
    realm_builder()->RouteReadOnlyDirectory(
        "config-data", {kEmbeddingappcodeViewRef},
 ...
 private:
  void ExtendRealm() override {
    ...
    // Add embedding-appcode-view to the Realm
    // This component will embed touch-input-view as a child view
    realm_builder()->AddChild(kEmbeddingappcodeView, kEmbeddingappcodeViewUrl,
                              component_testing::ChildOptions{
                                  .environment = kappcodeRunnerEnvironment,
                              });
    ...
    realm_builder()->AddRoute(
        Route{.capabilities = {Protocol{
              ...
              .source = kMockTouchInputListenerRef,
              .targets = {kappcodeJitRunnerRef, kTouchInputViewRef,
                          kEmbeddingappcodeViewRef}});

    realm_builder()->AddRoute(
        Route{.capabilities = {Protocol{fuchsia::ui::app::ViewProvider::Name_}},
              .source = kEmbeddingappcodeViewRef,
              .targets = {ParentRef()}});
    realm_builder()->AddRoute(
        Route{.capabilities = {Protocol{fuchsia::ui::app::ViewProvider::Name_}},
              .source = kTouchInputViewRef,
              .targets = {kEmbeddingappcodeViewRef}});
  }
};

TEST_F(appcodeTapTest, appcodeTap) {
...
TEST_F(appcodeEmbedTapTest, appcodeEmbedTap) {
  // Launch view
  FML_LOG(INFO) << "Initializing scene";
...

  {
    ...
    RunLoopUntil([this] {
      return LastEventReceivedMatches(
          ...
          /*component_name=*/"embedding-appcode-view");
    });
  }
...
TEST_F(appcodeEmbedTapTest, appcodeEmbedOverlayEnabled) {
  FML_LOG(INFO) << "Initializing scene";
