...
#include "appcode/shell/platform/fuchsia/appcode/platform_view.h"
...
#include "appcode/shell/platform/fuchsia/appcode/surface.h"
#include "appcode/shell/platform/fuchsia/appcode/task_runner_adapter.h"
...
class MockExternalViewEmbedder : public appcode::ExternalViewEmbedder {
 public:
  appcode::DlCanvas* GetRootCanvas() override { return nullptr; }
  ...
  void PrepareappcodeView(SkISize frame_size,
                          double device_pixel_ratio) override {}

  void SubmitappcodeView(
...
class MockPlatformViewDelegate : public appcode::PlatformView::Delegate {
 public:
  void Reset() {
    message_ = nullptr;
    metrics_ = appcode::ViewportMetrics{};
    semantics_features_ = 0;
    semantics_enabled_ = false;
    pointer_packets_.clear();
  }

  // |appcode::PlatformView::Delegate|
  void OnPlatformViewCreated(std::unique_ptr<appcode::Surface> surface) {
  ...
  void OnPlatformViewAddView(int64_t view_id,
                             const appcode::ViewportMetrics& viewport_metrics,
  ...
  // |appcode::PlatformView::Delegate|
  const appcode::Settings& OnPlatformViewGetSettings() const {
    return settings_;
  }
  // |appcode::PlatformView::Delegate|
  ...
  void OnPlatformViewDispatchKeyDataPacket(
      std::unique_ptr<appcode::KeyDataPacket> packet,
  ...
  void OnPlatformViewRegisterTexture(
      std::shared_ptr<appcode::Texture> texture) {}
  // |appcode::PlatformView::Delegate|
  ...
  void UpdateAssetResolverByType(
      std::unique_ptr<appcode::AssetResolver> updated_asset_resolver,
      appcode::AssetResolver::AssetResolverType type) {}
  // |appcode::PlatformView::Delegate|
  void OnPlatformViewSendViewFocusEvent(const appcode::ViewFocusEvent& event) {
  };

  appcode::Surface* surface() const { return surface_.get(); }
  appcode::PlatformMessage* message() const { return message_.get(); }
  const appcode::ViewportMetrics& metrics() const { return metrics_; }
  int32_t semantics_features() const { return semantics_features_; }
  bool semantics_enabled() const { return semantics_enabled_; }
  const std::vector<std::unique_ptr<appcode::PointerDataPacket>>&
  pointer_packets() const {
    return pointer_packets_;
  }
  std::vector<std::unique_ptr<appcode::PointerDataPacket>>
 ...
 private:
  std::unique_ptr<appcode::Surface> surface_;
  std::unique_ptr<appcode::PlatformMessage> message_;
  appcode::ViewportMetrics metrics_;
  std::vector<std::unique_ptr<appcode::PointerDataPacket>> pointer_packets_;
  int32_t semantics_features_ = 0;
  bool semantics_enabled_ = false;
  appcode::Settings settings_;
};

class MockResponse : public appcode::PlatformMessageResponse {
 public:
  MOCK_METHOD(void, Complete, (std::unique_ptr<fml::Mapping> data), (override));
  MOCK_METHOD(void, CompleteEmpty, (), (override));
};

class TestPlatformMessageResponse : public appcode::PlatformMessageResponse {
 public:
...
class PlatformViewBuilder {
 public:
  PlatformViewBuilder(appcode::PlatformView::Delegate& delegate,
  ...
  PlatformViewBuilder& SetExternalViewEmbedder(
      std::shared_ptr<appcode::ExternalViewEmbedder> embedder) {
 ...
 private:
  ...
  appcode::PlatformView::Delegate& delegate_;
  appcode::TaskRunners task_runners_;
  std::pair<fuchsia::ui::views::ViewRefControl, fuchsia::ui::views::ViewRef>
      view_ref_pair_;

  std::shared_ptr<appcode::ExternalViewEmbedder>
...
void UnpackPointerPacket(std::vector<appcode::PointerData>& output,  // NOLINT
                         std::unique_ptr<appcode::PointerDataPacket> packet) {
  for (size_t i = 0; i < packet->GetLength(); i++) {
    appcode::PointerData pointer_data = packet->GetPointerData(i);
...
TEST_F(PlatformViewTests, InvalidPlatformMessageRequest) {
  ...
  appcode::TaskRunners task_runners =
      appcode::TaskRunners("test_runners", nullptr, nullptr, nullptr, nullptr);
  ...
  auto base_view = static_cast<appcode::PlatformView*>(&platform_view);
...
TEST_F(PlatformViewTests, CreateSurfaceTest) {
  ...
  appcode::TaskRunners task_runners =
      appcode::TaskRunners("test_runners",  // label
                           nullptr,         // platform
                           appcode_runner::CreateFMLTaskRunner(
  ...
  sk_sp<GrDirectContext> gr_context = GrDirectContext::MakeMock(
      nullptr,
      appcode::MakeDefaultContextOptions(appcode::ContextType::kRender));
  ...
  auto CreateSurfaceCallback = [&external_view_embedder, gr_context]() {
    return std::make_unique<appcode_runner::Surface>(
...
TEST_F(PlatformViewTests, SetViewportMetrics) {
  ...
  MockPlatformViewDelegate delegate;
  EXPECT_EQ(delegate.metrics(), appcode::ViewportMetrics());

  MockParentViewportWatcher watcher;
  appcode::TaskRunners task_runners("test_runners", nullptr, nullptr, nullptr,
                                    nullptr);
  auto platform_view = PlatformViewBuilder(delegate, std::move(task_runners))
  ...
  EXPECT_EQ(delegate.metrics(),
            appcode::ViewportMetrics(kDPR, std::round(width * kDPR),
...
TEST_F(PlatformViewTests, ChangesAccessibilitySettings) {
  ...
  EXPECT_EQ(delegate.semantics_features(),
            static_cast<int32_t>(
                appcode::AccessibilityFeatureFlag::kAccessibleNavigation));
...
// "appcode/platform_views" channel for EnableWireframe.
TEST_F(PlatformViewTests, EnableWireframeTest) {
  ...
  std::unique_ptr<appcode::PlatformMessage> message =
      std::make_unique<appcode::PlatformMessage>(
          "appcode/platform_views", fml::MallocMapping::Copy(txt, sizeof(txt)),
          fml::RefPtr<appcode::PlatformMessageResponse>());
...
// This test makes sure that the PlatformView forwards messages on the
// "appcode/platform_views" channel for Createview.
TEST_F(PlatformViewTests, CreateViewTest) {
  ...
  auto CreateViewCallback =
      [&create_view_called, this](
          int64_t view_id, appcode_runner::ViewCallback on_view_created,
          appcode_runner::ViewCreatedCallback on_view_bound, bool hit_testable,
          bool focusable) {
  ...
  // 'appcode/platform_views' channel when a view gets created.
...
// This test makes sure that the PlatformView forwards messages on the
// "appcode/platform_views" channel for UpdateView.
TEST_F(PlatformViewTests, UpdateViewTest) {
  ...
  const uint8_t json_false[] =
      ...
      "    }"
      "}";
  std::unique_ptr<appcode::PlatformMessage> message_false =
      std::make_unique<appcode::PlatformMessage>(
          "appcode/platform_views",
  ...
  const uint8_t json_occlusion_hint[] =
      ...
      "    }"
      "}";
  std::unique_ptr<appcode::PlatformMessage> message_occlusion_hint =
      std::make_unique<appcode::PlatformMessage>(
          "appcode/platform_views",
...
// This test makes sure that the PlatformView forwards messages on the
// "appcode/platform_views" channel for DestroyView.
TEST_F(PlatformViewTests, DestroyViewTest) {
  ...
  auto on_destroy_view = [&destroy_view_called](
                             int64_t view_id,
                             appcode_runner::ViewIdCallback on_view_unbound) {
  ...
  auto on_create_view = [&create_view_called, this](
                            int64_t view_id,
                            appcode_runner::ViewCallback on_view_created,
                            appcode_runner::ViewCreatedCallback on_view_bound,
                            bool hit_testable, bool focusable) {
    create_view_called = true;
    on_view_created();
    fuchsia::ui::composition::ContentId content_id;
  ...
  base_view->HandlePlatformMessage(create_response->WithMessage(
      "appcode/platform_views", create_message.str()));
  RunLoopUntilIdle();
  ...
  // 'appcode/platform_views' channel when a view gets destroyed.
...
// This test makes sure that the PlatformView forwards messages on the
// "appcode/platform_views" channel for View.focus.getCurrent and
// View.focus.getNext.
TEST_F(PlatformViewTests, GetFocusStatesTest) {
  ...
  for (std::size_t i = 0; i < vrf_states.size(); ++i) {
    ...
    auto response1 = FakePlatformMessageResponse::Create();
    base_view->HandlePlatformMessage(response1->WithMessage(
        "appcode/platform_views", "{\"method\":\"View.focus.getNext\"}"));
    ...
    auto response3 = FakePlatformMessageResponse::Create();
    base_view->HandlePlatformMessage(response3->WithMessage(
        "appcode/platform_views", "{\"method\":\"View.focus.getCurrent\"}"));
...
// This test makes sure that the PlatformView forwards messages on the
// "appcode/platform_views" channel for View.focus.request.
TEST_F(PlatformViewTests, RequestFocusTest) {
  ...
  base_view->HandlePlatformMessage(focus_response->WithMessage(
      "appcode/platform_views", focus_message.str()));
  RunLoopUntilIdle();
...
TEST_F(PlatformViewTests, RequestFocusDisposedTest) {
  ...
  base_view->HandlePlatformMessage(dispose_response->WithMessage(
      "appcode/platform_views", dispose_message.str()));
  RunLoopUntilIdle();
  EXPECT_TRUE(destroy_view_called);
...
TEST_F(PlatformViewTests, OnShaderWarmup) {
  ...
  static_cast<appcode::PlatformView*>(&platform_view)
      ->HandlePlatformMessage(std::make_unique<appcode::PlatformMessage>(
...
TEST_F(PlatformViewTests, TouchSourceLogicalToPhysicalConversion) {
  ...
  RunLoopUntilIdle();
  EXPECT_EQ(delegate.metrics(),
            appcode::ViewportMetrics(1, width, height, -1, 0));
  ...
  // Unpack
  std::vector<std::unique_ptr<appcode::PointerDataPacket>> packets =
      delegate.TakePointerDataPackets();
  ASSERT_EQ(packets.size(), 1u);
  std::vector<appcode::PointerData> appcode_events;
  UnpackPointerPacket(appcode_events, std::move(packets[0]));

  // Examine phases
  ASSERT_EQ(appcode_events.size(), 2u);
  EXPECT_EQ(appcode_events[0].change, appcode::PointerData::Change::kAdd);
  EXPECT_EQ(appcode_events[1].change, appcode::PointerData::Change::kDown);

  // Examine coordinates
  EXPECT_EQ(appcode_events[0].physical_x, width / 2);
  EXPECT_EQ(appcode_events[0].physical_y, height / 2);
  EXPECT_EQ(appcode_events[1].physical_x, width / 2);
  EXPECT_EQ(appcode_events[1].physical_y, height / 2);
}
