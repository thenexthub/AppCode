...
#include "appcode/shell/platform/fuchsia/appcode/surface_producer.h"

#include "gmock/gmock.h"  // For EXPECT_THAT and matchers
#include "gtest/gtest.h"

using appcode::DlColor;
using appcode::DlMatrix;
using appcode::DlRect;
using appcode::DlSize;
...
Matcher<FakeGraph> IsappcodeGraph(
...
Matcher<std::shared_ptr<FakeTransform>> IsInputShield() {
  return Pointee(AllOf(
      ...
      Field("hit_regions", &FakeTransform::hit_regions,
            Contains(appcode_runner::testing::kInfiniteHitRegion))));
}
...
void DrawSimpleFrame(ExternalViewEmbedder& external_view_embedder,
                     SkISize frame_size,
                     float frame_dpr,
                     std::function<void(appcode::DlCanvas*)> draw_callback) {
  external_view_embedder.BeginFrame(nullptr, nullptr);
  external_view_embedder.PrepareappcodeView(frame_size, frame_dpr);
  {
    appcode::DlCanvas* root_canvas = external_view_embedder.GetRootCanvas();
...
void DrawFrameWithView(
    ...
    float frame_dpr,
    int view_id,
    appcode::EmbeddedViewParams& view_params,
    std::function<void(appcode::DlCanvas*)> background_draw_callback,
    std::function<void(appcode::DlCanvas*)> overlay_draw_callback) {
  external_view_embedder.BeginFrame(nullptr, nullptr);
  external_view_embedder.PrepareappcodeView(frame_size, frame_dpr);
  {
    appcode::DlCanvas* root_canvas = external_view_embedder.GetRootCanvas();
    external_view_embedder.PrerollCompositeEmbeddedView(
        view_id, std::make_unique<appcode::EmbeddedViewParams>(view_params));
    external_view_embedder.PostPrerollAction(nullptr);
    background_draw_callback(root_canvas);
    appcode::DlCanvas* overlay_canvas =
...
TEST_F(ExternalViewEmbedderTest, RootScene) {
  ...
  EXPECT_THAT(fake_flatland().graph(),
              IsappcodeGraph(parent_viewport_watcher, viewport_creation_token,
                             view_ref_clone));
}
...
TEST_F(ExternalViewEmbedderTest, SceneWithOneView) {
  ...
  auto mutators_stack = appcode::MutatorsStack();
  mutators_stack.PushOpacity(kOpacity);
  mutators_stack.PushTransform(matrix);

  appcode::EmbeddedViewParams child_view_params(
      appcode::ToSkMatrix(matrix), child_view_size_signed, mutators_stack);
  ...
  DrawFrameWithView(
      ...
      [](appcode::DlCanvas* canvas) {
        const DlSize canvas_size(canvas->GetBaseLayerDimensions());
        appcode::DlPaint rect_paint;
        rect_paint.setColor(appcode::DlColor::kRed());
...
TEST_F(ExternalViewEmbedderTest, SceneWithOneClippedView) {
  ...
  auto new_mutators_stack = appcode::MutatorsStack();
  new_mutators_stack.PushOpacity(kOpacity);
  new_mutators_stack.PushTransform(new_matrix);
  appcode::EmbeddedViewParams new_child_view_params(
      appcode::ToSkMatrix(new_matrix), child_view_size_signed,
      new_mutators_stack);
  DrawFrameWithView(
      external_view_embedder, frame_size_signed, kDPR, child_view_id,
...
TEST_F(ExternalViewEmbedderTest, ViewportCoveredWithInputInterceptor) {
  ...
  const fuchsia::math::SizeU frame_size{
      static_cast<uint32_t>(frame_size_signed.width()),
      static_cast<uint32_t>(frame_size_signed.height())};
  auto background_draw_callback = [](appcode::DlCanvas* canvas) {
    const DlSize canvas_size(canvas->GetBaseLayerDimensions());
    appcode::DlPaint rect_paint;
    rect_paint.setColor(appcode::DlColor::kGreen());
    canvas->Translate(canvas_size.width / 4.f, canvas_size.height / 2.f);
  ...
  auto overlay_draw_callback = [](appcode::DlCanvas* canvas) {
    const DlSize canvas_size(canvas->GetBaseLayerDimensions());
    appcode::DlPaint rect_paint;
    rect_paint.setColor(appcode::DlColor::kRed());
