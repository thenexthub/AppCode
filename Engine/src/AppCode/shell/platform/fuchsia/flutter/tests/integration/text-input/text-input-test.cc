...
/// |KeyboardInputListener| is a local test protocol that our test appcode app
 ...
 public:
  ...
  void ReportTextInput(
      fuchsia::ui::test::input::KeyboardInputListenerReportTextInputRequest
          request) override {
    FML_LOG(INFO) << "appcode app sent: '" << request.text() << "'";
...
class TextInputTest : public PortableUITest,
 ...
 private:
  void ExtendRealm() override {
    ...
    // Route KeyboardInputListener to the runner and appcode app
    realm_builder()->AddRoute(
        Route{.capabilities = {Protocol{
                  fuchsia::ui::test::input::KeyboardInputListener::Name_}},
              .source = kKeyboardInputListenerRef,
              .targets = {kappcodeJitRunnerRef, kTextInputViewRef}});

    // Expose fuchsia.ui.app.ViewProvider from the appcode app.
    realm_builder()->AddRoute(
    ...
    realm_builder()->AddRoute(Route{
        ...
        .targets = {ParentRef(), kappcodeJitRunnerRef}});
  }
};
