...
static constexpr char kappcodePlatformChannel[] = "appcode/platform";
static constexpr char kAccessibilityChannel[] = "appcode/accessibility";
static constexpr char kappcodePlatformViewsChannel[] = "appcode/platform_views";
...
static constexpr int64_t kappcodeImplicitViewId = 0ll;
...
PlatformView::PlatformView(
    ...
    : appcode::PlatformView(delegate, std::move(task_runners)),
  ...
  focus_delegate_->WatchLoop([weak = weak_factory_.GetWeakPtr()](bool focused) {
    ...
    // Ensure last_text_state_ is set to make sure appcode actually wants
    // an IME.
  ...
  pointer_delegate_->WatchLoop([weak = weak_factory_.GetWeakPtr()](
                                   std::vector<appcode::PointerData> events) {
    if (!weak) {
      FML_LOG(WARNING) << "PlatformView use-after-free attempted. Ignoring.";
      return;
    }
    ...
    auto packet = std::make_unique<appcode::PointerDataPacket>(events.size());
    for (size_t i = 0; i < events.size(); ++i) {
      auto& event = events[i];
      // Translate logical to physical coordinates, as per
      // appcode::PointerData contract. Done here because pixel ratio comes
...
void PlatformView::RegisterPlatformMessageHandlers() {
  platform_message_handlers_[kappcodePlatformChannel] =
      std::bind(&PlatformView::HandleappcodePlatformChannelPlatformMessage,
                this, std::placeholders::_1);
  platform_message_handlers_[kTextInputChannel] =
      std::bind(&TextDelegate::HandleappcodeTextInputChannelPlatformMessage,
  ...
  platform_message_handlers_[kAccessibilityChannel] =
      std::bind(&PlatformView::HandleAccessibilityChannelPlatformMessage, this,
                std::placeholders::_1);
  platform_message_handlers_[kappcodePlatformViewsChannel] =
      std::bind(&PlatformView::HandleappcodePlatformViewsChannelPlatformMessage,
                this, std::placeholders::_1);
...
static appcode::PointerData::Change GetChangeFromPointerEventPhase(
...
static appcode::PointerData::DeviceKind GetKindFromPointerType(
...
void PlatformView::OnGetLayout(fuchsia::ui::composition::LayoutInfo info) {
  ...
  SetViewportMetrics(kappcodeImplicitViewId, metrics);
...
bool PlatformView::OnHandlePointerEvent(
  ...
  TRACE_EVENT0("appcode", "PlatformView::OnHandlePointerEvent");
  ...
  switch (pointer_data.change) {
    ...
    case appcode::PointerData::Change::kMove:
      if (down_pointers_.count(pointer_data.device) == 0) {
        pointer_data.change = appcode::PointerData::Change::kHover;
      }
      break;
  ...
  auto packet = std::make_unique<appcode::PointerDataPacket>(1);
...
std::unique_ptr<appcode::VsyncWaiter> PlatformView::CreateVSyncWaiter() {
  return std::make_unique<appcode_runner::VsyncWaiter>(
      await_vsync_callback_, await_vsync_for_secondary_callback_callback_,
      task_runners_);
}

// |appcode::PlatformView|
std::unique_ptr<appcode::Surface> PlatformView::CreateRenderingSurface() {
...
void PlatformView::HandlePlatformMessage(
  ...
  if (found == platform_message_handlers_.end()) {
    ...
    appcode::PlatformView::HandlePlatformMessage(std::move(message));
    return;
  }
...
void PlatformView::SetSemanticsEnabled(bool enabled) {
  appcode::PlatformView::SetSemanticsEnabled(enabled);
  if (enabled) {
...
bool PlatformView::HandleAccessibilityChannelPlatformMessage(
  ...
  const appcode::StandardMessageCodec& standard_message_codec =
      appcode::StandardMessageCodec::GetInstance(nullptr);
  std::unique_ptr<appcode::EncodableValue> decoded =
      standard_message_codec.DecodeMessage(message->data().GetMapping(),
                                           message->data().GetSize());

  appcode::EncodableMap map = std::get<appcode::EncodableMap>(*decoded);
  std::string type =
      std::get<std::string>(map.at(appcode::EncodableValue("type")));
  if (type == "announce") {
    appcode::EncodableMap data_map = std::get<appcode::EncodableMap>(
        map.at(appcode::EncodableValue("data")));
    std::string text =
        std::get<std::string>(data_map.at(appcode::EncodableValue("message")));
...
// Channel handler for kappcodePlatformChannel
bool PlatformView::HandleappcodePlatformChannelPlatformMessage(
    std::unique_ptr<appcode::PlatformMessage> message) {
  FML_DCHECK(message->channel() == kappcodePlatformChannel);
...
bool PlatformView::HandleappcodePlatformViewsChannelPlatformMessage(
    std::unique_ptr<appcode::PlatformMessage> message) {
  FML_DCHECK(message->channel() == kappcodePlatformViewsChannel);
