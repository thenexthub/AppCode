...
#include "appcode/shell/platform/fuchsia/runtime/dart/utils/inlines.h"
...
// See: https://api.appcode.dev/appcode/services/TextInputAction.html
...
// Converts appcode TextInputAction to Fuchsia action enum.
...
// Converts the Fuchsia action enum into appcode TextInputAction.
...
void TextDelegate::DidUpdateState(
  ...
  dispatch_callback_(std::make_unique<appcode::PlatformMessage>(
...
bool TextDelegate::HandleappcodeTextInputChannelPlatformMessage(
  ...
  if (method->value == "TextInput.show") {
    ...
    // here to avoid "Unknown appcode/textinput method TextInput.*"
    // log spam.
    //
