...
// Fixture to exercise appcode runner's implementation for
...
class PointerDelegateTest : public ::testing::Test {
 protected:
  PointerDelegateTest() : loop_(&kAsyncLoopConfigAttachToCurrentThread) {
    ...
    pointer_delegate_ = std::make_unique<appcode_runner::PointerDelegate>(
  ...
  std::unique_ptr<appcode_runner::PointerDelegate> pointer_delegate_;
...
TEST_F(PointerDelegateTest, Data_FuchsiaTimeVersusappcodeTime) {
  std::optional<std::vector<appcode::PointerData>> pointers;
  pointer_delegate_->WatchLoop(
      [&pointers](std::vector<appcode::PointerData> events) {
        pointers = std::move(events);
      });
  RunLoopUntilIdle();  // Server gets watch call.

  // Fuchsia ADD -> appcode ADD+DOWN
...
TEST_F(PointerDelegateTest, Phase_appcodePhasesAreSynthesized) {
  std::optional<std::vector<appcode::PointerData>> pointers;
  pointer_delegate_->WatchLoop(
      [&pointers](std::vector<appcode::PointerData> events) {
        pointers = std::move(events);
  ...
  ASSERT_TRUE(pointers.has_value());
  ASSERT_EQ(pointers->size(), 2u);
  EXPECT_EQ((*pointers)[0].change, appcode::PointerData::Change::kAdd);
  EXPECT_EQ((*pointers)[1].change, appcode::PointerData::Change::kDown);

  // Fuchsia CHANGE -> appcode MOVE
  ...
  EXPECT_EQ((*pointers)[0].change, appcode::PointerData::Change::kMove);

  // Fuchsia REMOVE -> appcode UP+REMOVE
  events = TouchEventBuilder::New()
               .AddTime(3333000u)
  ...
  ASSERT_TRUE(pointers.has_value());
  ASSERT_EQ(pointers->size(), 2u);
  EXPECT_EQ((*pointers)[0].change, appcode::PointerData::Change::kUp);
  EXPECT_EQ((*pointers)[1].change, appcode::PointerData::Change::kRemove);
}

TEST_F(PointerDelegateTest, Phase_FuchsiaCancelBecomesappcodeCancel) {
  std::optional<std::vector<appcode::PointerData>> pointers;
  pointer_delegate_->WatchLoop(
      [&pointers](std::vector<appcode::PointerData> events) {
        pointers = std::move(events);
  ...
  ASSERT_TRUE(pointers.has_value());
  ASSERT_EQ(pointers->size(), 1u);
  EXPECT_EQ((*pointers)[0].change, appcode::PointerData::Change::kCancel);
}
...
TEST_F(PointerDelegateTest, Protocol_FirstResponseIsEmpty) {
  bool called = false;
  pointer_delegate_->WatchLoop(
      [&called](std::vector<appcode::PointerData> events) { called = true; });
  RunLoopUntilIdle();  // Server gets Watch call.
...
TEST_F(PointerDelegateTest, Protocol_LateGrant) {
  ...
  EXPECT_EQ(pointers.value()[0].change, appcode::PointerData::Change::kAdd);
  EXPECT_EQ(pointers.value()[1].change, appcode::PointerData::Change::kDown);
  EXPECT_EQ(pointers.value()[2].change, appcode::PointerData::Change::kMove);
  pointers = {};
  ...
  EXPECT_EQ(pointers.value()[0].change, appcode::PointerData::Change::kMove);
...
TEST_F(PointerDelegateTest, Protocol_LateGrantCombo) {
  ...
  EXPECT_EQ(pointers.value()[3].change, appcode::PointerData::Change::kMove);
...
TEST_F(PointerDelegateTest, Protocol_PointersAreIndependent) {
  ...
  // to fit in appcode's 64-bit device ID. However, appcode's pointer_identifier
...
TEST_F(PointerDelegateTest, MouseWheel_TickBased) {
  ...
  ASSERT_TRUE(pointers.has_value());
  ASSERT_EQ(pointers.value().size(), 1u);
  EXPECT_EQ(pointers.value()[0].change, appcode::PointerData::Change::kHover);
  EXPECT_EQ(pointers.value()[0].signal_kind,
            appcode::PointerData::SignalKind::kScroll);
  EXPECT_EQ(pointers.value()[0].kind, appcode::PointerData::DeviceKind::kMouse);
