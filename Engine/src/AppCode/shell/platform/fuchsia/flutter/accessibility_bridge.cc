...
// Converts appcode semantic node flags to a string representation.
std::string NodeFlagsToString(const appcode::SemanticsNode& node) {
  std::string output;
...
// Converts appcode semantic node actions to a string representation.
std::string NodeActionsToString(const appcode::SemanticsNode& node) {
  std::string output;

  if (node.HasAction(appcode::SemanticsAction::kCopy)) {
    output += "kCopy|";
  }
  if (node.HasAction(appcode::SemanticsAction::kCustomAction)) {
    output += "kCustomAction|";
  }
  if (node.HasAction(appcode::SemanticsAction::kCut)) {
    output += "kCut|";
  }
  if (node.HasAction(appcode::SemanticsAction::kDecrease)) {
    output += "kDecrease|";
  }
  if (node.HasAction(appcode::SemanticsAction::kDidGainAccessibilityFocus)) {
    output += "kDidGainAccessibilityFocus|";
  }
  if (node.HasAction(appcode::SemanticsAction::kDidLoseAccessibilityFocus)) {
    output += "kDidLoseAccessibilityFocus|";
  }
  if (node.HasAction(appcode::SemanticsAction::kDismiss)) {
    output += "kDismiss|";
  }
  if (node.HasAction(appcode::SemanticsAction::kIncrease)) {
    output += "kIncrease|";
  }
  if (node.HasAction(appcode::SemanticsAction::kLongPress)) {
    output += "kLongPress|";
  }
  if (node.HasAction(
          appcode::SemanticsAction::kMoveCursorBackwardByCharacter)) {
    output += "kMoveCursorBackwardByCharacter|";
  }
  if (node.HasAction(appcode::SemanticsAction::kMoveCursorBackwardByWord)) {
    output += "kMoveCursorBackwardByWord|";
  }
  if (node.HasAction(appcode::SemanticsAction::kMoveCursorForwardByCharacter)) {
    output += "kMoveCursorForwardByCharacter|";
  }
  if (node.HasAction(appcode::SemanticsAction::kMoveCursorForwardByWord)) {
    output += "kMoveCursorForwardByWord|";
  }
  if (node.HasAction(appcode::SemanticsAction::kPaste)) {
    output += "kPaste|";
  }
  if (node.HasAction(appcode::SemanticsAction::kScrollDown)) {
    output += "kScrollDown|";
  }
  if (node.HasAction(appcode::SemanticsAction::kScrollLeft)) {
    output += "kScrollLeft|";
  }
  if (node.HasAction(appcode::SemanticsAction::kScrollRight)) {
    output += "kScrollRight|";
  }
  if (node.HasAction(appcode::SemanticsAction::kScrollUp)) {
    output += "kScrollUp|";
  }
  if (node.HasAction(appcode::SemanticsAction::kScrollToOffset)) {
    output += "kScrollToOffset|";
  }
  if (node.HasAction(appcode::SemanticsAction::kSetSelection)) {
    output += "kSetSelection|";
  }
  if (node.HasAction(appcode::SemanticsAction::kSetText)) {
    output += "kSetText|";
  }
  if (node.HasAction(appcode::SemanticsAction::kShowOnScreen)) {
    output += "kShowOnScreen|";
  }
  if (node.HasAction(appcode::SemanticsAction::kTap)) {
    output += "kTap|";
  }
  if (node.HasAction(appcode::SemanticsAction::kFocus)) {
    output += "kFocus|";
  }
...
// Returns a string representation of the appcode semantic node absolut
// location.
...
std::string NodeChildrenToString(const appcode::SemanticsNode& node) {
...
AccessibilityBridge::AccessibilityBridge(
  ...
  fuchsia_semantics_manager_.set_error_handler([](zx_status_t status) {
    FML_LOG(ERROR) << "appcode cannot connect to SemanticsManager with status: "
...
AccessibilityBridge::GetNodeAttributes(const appcode::SemanticsNode& node,
...
fuchsia::accessibility::semantics::States AccessibilityBridge::GetNodeStates(
  ...
  // appcode's definition of a hidden node is different from Fuchsia, so it must
  // not be set here.
...
AccessibilityBridge::GetNodeActions(const appcode::SemanticsNode& node,
                                    size_t* additional_size) const {
...
fuchsia::accessibility::semantics::Role AccessibilityBridge::GetNodeRole(
  ...
  // If a appcode node supports the kIncrease or kDecrease actions, it can be
  ...
  if (node.HasAction(appcode::SemanticsAction::kIncrease) ||
      node.HasAction(appcode::SemanticsAction::kDecrease)) {
    return fuchsia::accessibility::semantics::Role::SLIDER;
  }

  // If a appcode node has a checked state, then we assume it is either a
    ...
    if (it != nodes_.end()) {
      ...
      for (const auto& child : node.childrenInHitTestOrder) {
        if (descendents.find(child) == descendents.end()) {
          ...
          // appcode should never let this happen, but the engine API does not
          // explicitly forbid it right now.
          // TODO(http://fxbug.dev/75905): Crash appcode accessibility bridge
...
static uint32_t appcodeIdToFuchsiaId(int32_t appcode_node_id) {
  FML_DCHECK(appcode_node_id >= 0)
      << "Unexpectedly received a negative semantics node ID.";
  return static_cast<uint32_t>(appcode_node_id);
}
...
void AccessibilityBridge::PruneUnreachableNodes(
  ...
  while (iter != nodes_.end()) {
    ...
    if (reachable_nodes.find(id) == reachable_nodes.end()) {
      atomic_update->AddNodeDeletion(appcodeIdToFuchsiaId(id));
...
void AccessibilityBridge::AddSemanticsNodeUpdate(
  ...
  for (const auto& [appcode_node_id, appcode_node] : update) {
    ...
    // TODO(http://fxbug.dev/75908): Investigate appcode a11y bridge refactor
    // after removal of the inverse view pixel ratio transformation in scenic
    // view).
    if (appcode_node.id == kRootNodeId) {
      root_appcode_semantics_node_ = appcode_node;
      has_root_node_update = true;
      continue;
    }
    // Store the nodes for later hit testing and logging.
    nodes_[appcode_node.id].data = appcode_node;

    fuchsia::accessibility::semantics::Node fuchsia_node;
    std::vector<uint32_t> child_ids;
    // Send the nodes in traversal order, so the manager can figure out
    // traversal.
    for (int32_t appcode_child_id : appcode_node.childrenInTraversalOrder) {
      child_ids.push_back(appcodeIdToFuchsiaId(appcode_child_id));
    }
    // TODO(http://fxbug.dev/75910): check the usage of appcodeIdToFuchsiaId in
    // the appcode accessibility bridge.
    fuchsia_node.set_node_id(appcode_node.id)
        .set_role(GetNodeRole(appcode_node))
        .set_location(GetNodeLocation(appcode_node))
        .set_transform(GetNodeTransform(appcode_node))
        .set_attributes(GetNodeAttributes(appcode_node, &this_node_size))
        .set_states(GetNodeStates(appcode_node, &this_node_size))
        .set_actions(GetNodeActions(appcode_node, &this_node_size))
        .set_child_ids(child_ids);
    this_node_size +=
        kNodeIdSize * appcode_node.childrenInTraversalOrder.size();
...
fuchsia::accessibility::semantics::Node AccessibilityBridge::GetRootNodeUpdate(
  ...
  for (int32_t appcode_child_id :
       root_appcode_semantics_node_.childrenInTraversalOrder) {
    child_ids.push_back(appcodeIdToFuchsiaId(appcode_child_id));
  }
  ...
  SkM44 result = root_appcode_semantics_node_.transform *
                 inverse_view_pixel_ratio_transform;
  nodes_[root_appcode_semantics_node_.id].data = root_appcode_semantics_node_;

  // TODO(http://fxbug.dev/75910): check the usage of appcodeIdToFuchsiaId in
  // the appcode accessibility bridge.
  root_fuchsia_node.set_node_id(root_appcode_semantics_node_.id)
      .set_role(GetNodeRole(root_appcode_semantics_node_))
      .set_location(GetNodeLocation(root_appcode_semantics_node_))
      .set_transform(ConvertSkiaTransformToMat4(result))
      .set_attributes(
          GetNodeAttributes(root_appcode_semantics_node_, &node_size))
      .set_states(GetNodeStates(root_appcode_semantics_node_, &node_size))
      .set_actions(GetNodeActions(root_appcode_semantics_node_, &node_size))
      .set_child_ids(child_ids);
  node_size += kNodeIdSize *
               root_appcode_semantics_node_.childrenInTraversalOrder.size();
...
void AccessibilityBridge::UpdateScreenRects() {
  ...
  // to the root node. However, this transform is not persisted in the appcode
...
std::optional<appcode::SemanticsAction>
AccessibilityBridge::GetappcodeSemanticsAction(
  ...
  switch (fuchsia_action) {
    ...
    case fuchsia::accessibility::semantics::Action::DEFAULT:
      return appcode::SemanticsAction::kTap;
    ...
    case fuchsia::accessibility::semantics::Action::SECONDARY:
      return appcode::SemanticsAction::kLongPress;
    ...
    case fuchsia::accessibility::semantics::Action::SHOW_ON_SCREEN:
      return appcode::SemanticsAction::kShowOnScreen;
    case fuchsia::accessibility::semantics::Action::INCREMENT:
      return appcode::SemanticsAction::kIncrease;
    case fuchsia::accessibility::semantics::Action::DECREMENT:
      return appcode::SemanticsAction::kDecrease;
    default:
...
void AccessibilityBridge::OnAccessibilityActionRequested(
  ...
  std::optional<appcode::SemanticsAction> appcode_action =
      GetappcodeSemanticsAction(action, node_id);
  if (!appcode_action.has_value()) {
    callback(false);
    return;
  }
  dispatch_semantics_action_callback_(static_cast<int32_t>(node_id),
                                      appcode_action.value());
  callback(true);
}
...
#if !appcode_RELEASE
void AccessibilityBridge::FillInspectTree(int32_t appcode_node_id,
                                          int32_t current_level,
                                          inspect::Node inspect_node,
                                          inspect::Inspector* inspector) const {
  const auto it = nodes_.find(appcode_node_id);
  if (it == nodes_.end()) {
  ...
  for (int32_t appcode_child_id : semantic_node.data.childrenInTraversalOrder) {
    const auto inspect_name = "node_" + std::to_string(appcode_child_id);
    FillInspectTree(appcode_child_id, current_level + 1,
