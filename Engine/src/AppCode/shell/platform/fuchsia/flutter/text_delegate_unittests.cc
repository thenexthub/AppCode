 ...
 protected:
  TextDelegateTest()
    ...
    text_delegate_ = std::make_unique<TextDelegate>(
        ...
        [this](std::unique_ptr<appcode::PlatformMessage> message) {
  ...
  std::unique_ptr<appcode::PlatformMessage> last_message_;
};
...
TEST_F(TextDelegateTest, ActivateIme) {
  auto fake_platform_message_response = FakePlatformMessageResponse::Create();
  {
    ...
    text_delegate_->HandleappcodeTextInputChannelPlatformMessage(
        std::move(message));
    RunLoopUntilIdle();
  ...
  {
    // Notify appcode that the action key has been pressed.
