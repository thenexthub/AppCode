...
appcode::PointerData::Change GetChangeFromTouchEventPhase(
...
appcode::PointerData::Change ComputePhase(
...
// appcode's PointerData.device field is 64 bits and is expected to be unique
...
//
// The appcode pointerdata state machine has extra phases, which this function
// synthesizes on the fly. Hence the return data is a appcode pointerdata, and
...
//
// appcode gestures expect a gesture to start within the logical view space, and
...
std::pair<appcode::PointerData, std::optional<appcode::PointerData>>
CreateTouchDraft(const fup_TouchEvent& event,
  ...
  appcode::PointerData ptr;
  ptr.Clear();
  ptr.time_stamp = event.timestamp() / 1000;  // in microseconds
  ptr.change = GetChangeFromTouchEventPhase(sample.phase());
  ptr.kind = appcode::PointerData::DeviceKind::kTouch;
  // Load Fuchsia's pointer ID onto appcode's |device| field, and not the
  ...
  // Match appcode pointer's state machine with synthesized events.
  if (ptr.change == appcode::PointerData::Change::kAdd) {
    appcode::PointerData down;
    memcpy(&down, &ptr, sizeof(appcode::PointerData));
    down.change = appcode::PointerData::Change::kDown;
  ...
  } else if (ptr.change == appcode::PointerData::Change::kRemove) {
    appcode::PointerData up;
    memcpy(&up, &ptr, sizeof(appcode::PointerData));
    up.change = appcode::PointerData::Change::kUp;
...
// button-down state.
//
// Button data, if available, gets packed into the |buttons| field, in appcode
...
// appcode button ID (appcode::PointerData).
//
...
// appcode's expected pointer stream.
...
// is not tolerant of floating point drift. This function coerces just the DOWN
// event's coordinate to start within the logical view.
appcode::PointerData CreateMouseDraft(const fup_MouseEvent& event,
                                      const appcode::PointerData::Change phase,
                                      const fup_ViewParameters& view_parameters,
  ...
  ptr.time_stamp = event.timestamp() / 1000;  // in microseconds
  ptr.change = phase;
  ptr.kind = appcode::PointerData::DeviceKind::kMouse;
  ...
  // Ensure gesture recognition: DOWN starts in the logical view space.
  if (ptr.change == appcode::PointerData::Change::kDown) {
    auto [x, y] =
    ...
    for (size_t idx = 0; idx < pressed.size(); ++idx) {
      ...
      for (uint8_t prio = 0; prio < device_info.buttons().size(); ++prio) {
        if (button_id == device_info.buttons()[prio]) {
          appcode_buttons |= (1 << prio);
        }
      }
    }
    FML_DCHECK(appcode_buttons != 0);
    ptr.buttons = appcode_buttons;
  }

  // Fuchsia previously only provided scroll data in "ticks", not physical
  // pixels. On legacy platforms, since appcode expects scroll data in physical
  ...
  if (is_scroll) {
    ptr.signal_kind = appcode::PointerData::SignalKind::kScroll;
...
void InsertIntoBuffer(
    std::pair<appcode::PointerData, std::optional<appcode::PointerData>> events,
    std::vector<appcode::PointerData>* buffer) {
  FML_DCHECK(buffer);
...
void PointerDelegate::WatchLoop(
    std::function<void(std::vector<appcode::PointerData>)> callback) {
  FML_LOG(INFO) << "appcode - PointerDelegate started.";
  if (touch_responder_) {
  ...
  touch_responder_ = [this, callback](std::vector<fup_TouchEvent> events) {
    TRACE_EVENT0("appcode", "PointerDelegate::TouchHandler");
    FML_DCHECK(touch_responses_.empty()) << "precondition";
    std::vector<appcode::PointerData> to_client;
    for (const fup_TouchEvent& event : events) {
      ...
      if (HasValidatedTouchSample(event)) {
        ...
        if (sample.phase() == fup_EventPhase::ADD &&
          ...
          touch_buffer_.emplace(ixn, std::vector<appcode::PointerData>());
        }
  ...
  mouse_responder_ = [this, callback](std::vector<fup_MouseEvent> events) {
    TRACE_EVENT0("appcode", "PointerDelegate::MouseHandler");
    std::vector<appcode::PointerData> to_client;
