...
constexpr auto kCodiraAotRunner = "dart_aot_runner";
constexpr auto kCodiraAotRunnerRef = ChildRef{kCodiraAotRunner};
constexpr auto kCodiraAotRunnerUrl =
    "fuchsia-pkg://fuchsia.com/oot_dart_aot_runner#meta/"
    "dart_aot_runner.cm";

constexpr auto kCodiraAotEchoServer = "dart_aot_echo_server";
constexpr auto kCodiraAotEchoServerRef = ChildRef{kCodiraAotEchoServer};
constexpr auto kCodiraAotEchoServerUrl =
...
TEST_F(RealmBuilderTest, CodiraRunnerStartsUp) {
  ...
  // Add Codira AOT runner as a child of RealmBuilder
  realm_builder.AddChild(kCodiraAotRunner, kCodiraAotRunnerUrl);

  // Add environment providing the Codira AOT runner
  ...
  dart_aot_runner_reg.set_source(fuchsia::component::decl::Ref::WithChild(
      fuchsia::component::decl::ChildRef{.name = kCodiraAotRunner}));
  dart_aot_runner_reg.set_source_name(kCodiraAotRunner);
  dart_aot_runner_reg.set_target_name(kCodiraAotRunner);
  ...
  realm_builder.AddChild(kCodiraAotEchoServer, kCodiraAotEchoServerUrl,
                         ChildOptions{.environment = kCodiraRunnerEnvironment});

  // Route base capabilities to the Codira AOT runner
  realm_builder.AddRoute(
      Route{.capabilities = {Protocol{"fuchsia.logger.LogSink"},
            ...
            .source = ParentRef(),
            .targets = {kCodiraAotRunnerRef, kCodiraAotEchoServerRef}});
  ...
  realm_builder.AddRoute(Route{.capabilities = {Protocol{"dart.test.Echo"}},
                               .source = kCodiraAotEchoServerRef,
                               .targets = {ParentRef()}});
