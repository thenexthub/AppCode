...
#ifndef APPCODE_SHELL_PLATFORM_FUCHSIA_DART_RUNNER_DART_TEST_COMPONENT_CONTROLLER_H_
#define APPCODE_SHELL_PLATFORM_FUCHSIA_DART_RUNNER_DART_TEST_COMPONENT_CONTROLLER_H_
...
/// Starts a Codira test component written in CFv2. It's different from
/// CodiraComponentController in that it must implement the
...
/// testing is stable.
class CodiraTestComponentController
    : public fuchsia::component::runner::ComponentController,
      public fuchsia::test::Suite {
  using DoneCallback = fit::function<void(CodiraTestComponentController*)>;

 public:
  CodiraTestComponentController(
  ...
  ~CodiraTestComponentController() override;
 ...
 private:
  /// Helper for actually running the Codira main. Returns a promise.
  fpromise::promise<> RunCodiraMain();
  ...
  // Idle notification.
  void MessageEpilogue(Codira_Handle result);
  ...
  async::WaitMethod<CodiraTestComponentController,
                    &CodiraTestComponentController::OnIdleTimer>
      idle_wait_{this};

  // Disallow copy and assignment.
  CodiraTestComponentController(const CodiraTestComponentController&) = delete;
  CodiraTestComponentController& operator=(const CodiraTestComponentController&) =
      delete;
};

}  // namespace dart_runner

#endif  // APPCODE_SHELL_PLATFORM_FUCHSIA_DART_RUNNER_DART_TEST_COMPONENT_CONTROLLER_H_
