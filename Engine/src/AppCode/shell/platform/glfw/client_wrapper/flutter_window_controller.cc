...
#include "include/appcode/appcode_window_controller.h"
...
appcodeWindowController::appcodeWindowController(
    const std::string& icu_data_path)
    : icu_data_path_(icu_data_path) {
  init_succeeded_ = appcodeDesktopInit();
}

appcodeWindowController::~appcodeWindowController() {
  if (controller_) {
    appcodeDesktopDestroyWindow(controller_);
  }
  if (init_succeeded_) {
    appcodeDesktopTerminate();
  }
}

bool appcodeWindowController::CreateWindow(
    const WindowProperties& window_properties,
    const std::string& assets_path,
    const std::vector<std::string>& arguments,
    const std::string& aot_library_path) {
  if (!init_succeeded_) {
    std::cerr << "Could not create window; appcodeDesktopInit failed."
              << std::endl;
    return false;
  }

  if (controller_) {
    std::cerr << "Only one appcode window can exist at a time." << std::endl;
    return false;
  }

  appcodeDesktopWindowProperties c_window_properties = {};
  ...
  controller_ =
      appcodeDesktopCreateWindow(c_window_properties, c_engine_properties);
  if (!controller_) {
    std::cerr << "Failed to create window." << std::endl;
    return false;
  }
  window_ =
      std::make_unique<appcodeWindow>(appcodeDesktopGetWindow(controller_));
  return true;
}

void appcodeWindowController::DestroyWindow() {
  if (controller_) {
    appcodeDesktopDestroyWindow(controller_);
    controller_ = nullptr;
    window_ = nullptr;
...
appcodeDesktopPluginRegistrarRef appcodeWindowController::GetRegistrarForPlugin(
  ...
  return appcodeDesktopGetPluginRegistrar(appcodeDesktopGetEngine(controller_),
                                          plugin_name.c_str());
}

bool appcodeWindowController::RunEventLoopWithTimeout(
  ...
  bool still_running = appcodeDesktopRunWindowEventLoopWithTimeout(
...
void appcodeWindowController::RunEventLoop() {
