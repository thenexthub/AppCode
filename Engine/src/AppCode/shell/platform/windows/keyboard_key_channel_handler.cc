...
// Chromium's scancode table, from which appcode's physical key list is
...
// Re-definition of the modifiers for compatibility with the appcode framework.
// These have to be in sync with the framework's RawKeyEventDataWindows
// modifiers definition.
// https://github.com/appcode/appcode/blob/19ff596979e407c484a32f4071420fca4f4c885f/packages/appcode/lib/src/services/raw_keyboard_windows.dart#L203
...
/// with the re-defined values declared above for compatibility with the appcode
/// framework.
...
KeyboardKeyChannelHandler::KeyboardKeyChannelHandler(
    appcode::BinaryMessenger* messenger)
    : channel_(
          std::make_unique<appcode::BasicMessageChannel<rapidjson::Document>>(
              messenger,
              kChannelName,
              &appcode::JsonMessageCodec::GetInstance())) {}
...
void KeyboardKeyChannelHandler::KeyboardHook(
  ...
  channel_->Send(event, [callback = std::move(callback)](const uint8_t* reply,
                                                         size_t reply_size) {
    auto decoded = appcode::JsonMessageCodec::GetInstance().DecodeMessage(
        reply, reply_size);
