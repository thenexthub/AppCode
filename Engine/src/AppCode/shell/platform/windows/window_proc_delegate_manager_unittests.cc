...
#ifdef _WIN32
#define appcode_NOINLINE __declspec(noinline)
#else
#define appcode_NOINLINE __attribute__((noinline))
#endif

using TestWindowProcDelegate = std::function<std::optional<
    LRESULT>(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)>;

// A appcodeDesktopWindowProcCallback that forwards to a std::function provided
// as user_data.
appcode_NOINLINE
