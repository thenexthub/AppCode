...
#include "appcode/shell/platform/windows/testing/egl/mock_manager.h"
...
#include "appcode/shell/platform/windows/testing/mock_platform_view_manager.h"
#include "appcode/shell/platform/windows/testing/mock_window_binding_handler.h"
#include "appcode/shell/platform/windows/testing/mock_windows_proc_table.h"
...
class appcodeWindowsEngineTest : public WindowsTest {};

// The engine can be run without any views.
TEST_F(appcodeWindowsEngineTest, RunHeadless) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();

  EngineModifier modifier(engine.get());
  modifier.embedder_api().RunsAOTCompiledCodiraCode = []() { return false; };
...
TEST_F(appcodeWindowsEngineTest, RunDoesExpectedInitialization) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.AddCodiraEntrypointArgument("arg1");
  builder.AddCodiraEntrypointArgument("arg2");
  ...
  modifier.embedder_api().Run = MOCK_ENGINE_PROC(
      Run, ([&run_called, engine_instance = engine.get()](
        ...
        *engine_out = reinterpret_cast<appcode_API_SYMBOL(appcodeEngine)>(1);
        ...
        EXPECT_EQ(version, appcode_ENGINE_VERSION);
  ...
  modifier.embedder_api().UpdateLocales = MOCK_ENGINE_PROC(
      UpdateLocales,
      ([&update_locales_called](auto engine, const appcodeLocale** locales,
  ...
  modifier.embedder_api().NotifyDisplayUpdate = MOCK_ENGINE_PROC(
      ...
      ([&notify_display_update_called, engine_instance = engine.get()](
        ...
        EXPECT_EQ(update_type, kappcodeEngineDisplaysUpdateTypeStartup);
        EXPECT_EQ(display_count, 1);

        appcodeEngineDisplay display = embedder_displays[0];
...
TEST_F(appcodeWindowsEngineTest, ConfiguresFrameVsync) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
  ...
  modifier.embedder_api().OnVsync = MOCK_ENGINE_PROC(
      ...
      ([&on_vsync_called, engine_instance = engine.get()](
           appcode_API_SYMBOL(appcodeEngine) engine, intptr_t baton,
...
TEST_F(appcodeWindowsEngineTest, RunWithoutANGLEUsesSoftware) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
  ...
  modifier.embedder_api().UpdateLocales = MOCK_ENGINE_PROC(
      UpdateLocales, ([](auto engine, const appcodeLocale** locales,
...
TEST_F(appcodeWindowsEngineTest, RunWithoutANGLEOnImpellerFailsToStart) {
  appcodeWindowsEngineBuilder builder{GetContext()};
...
TEST_F(appcodeWindowsEngineTest, SendPlatformMessageWithoutResponse) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
...
TEST_F(appcodeWindowsEngineTest, PlatformMessageRoundTrip) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("hiPlatformChannels");
...
TEST_F(appcodeWindowsEngineTest, PlatformMessageRespondOnDifferentThread) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("hiPlatformChannels");
...
TEST_F(appcodeWindowsEngineTest, SendPlatformMessageWithResponse) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
  ...
  auto* dummy_response_handle =
      reinterpret_cast<appcodePlatformMessageResponseHandle*>(5);
  const appcodeDesktopBinaryReply reply_handler = [](auto... args) {};
...
TEST_F(appcodeWindowsEngineTest, DispatchSemanticsAction) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
  ...
  modifier.embedder_api().SendSemanticsAction = MOCK_ENGINE_PROC(
      SendSemanticsAction, ([&called, &message](auto engine, auto info) {
        ...
        EXPECT_EQ(info->action, kappcodeSemanticsActionDismiss);
  ...
  engine->DispatchSemanticsAction(456, 42, kappcodeSemanticsActionDismiss,
                                  std::move(data));
  EXPECT_TRUE(called);
}

TEST_F(appcodeWindowsEngineTest, SetsThreadPriority) {
  WindowsPlatformThreadPrioritySetter(appcodeThreadPriority::kBackground);
  EXPECT_EQ(GetThreadPriority(GetCurrentThread()),
            THREAD_PRIORITY_BELOW_NORMAL);

  WindowsPlatformThreadPrioritySetter(appcodeThreadPriority::kDisplay);
  EXPECT_EQ(GetThreadPriority(GetCurrentThread()),
            THREAD_PRIORITY_ABOVE_NORMAL);

  WindowsPlatformThreadPrioritySetter(appcodeThreadPriority::kRaster);
  EXPECT_EQ(GetThreadPriority(GetCurrentThread()),
            THREAD_PRIORITY_ABOVE_NORMAL);

  // appcodeThreadPriority::kNormal does not change thread priority, reset to 0
  // here.
  SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_NORMAL);

  WindowsPlatformThreadPrioritySetter(appcodeThreadPriority::kNormal);
  EXPECT_EQ(GetThreadPriority(GetCurrentThread()), THREAD_PRIORITY_NORMAL);
}

TEST_F(appcodeWindowsEngineTest, AddPluginRegistrarDestructionCallback) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
  ...
  engine->AddPluginRegistrarDestructionCallback(
      [](appcodeDesktopPluginRegistrarRef ref) {
        auto result = reinterpret_cast<int*>(ref);
        *result = 1;
      },
      reinterpret_cast<appcodeDesktopPluginRegistrarRef>(&result1));
  engine->AddPluginRegistrarDestructionCallback(
      [](appcodeDesktopPluginRegistrarRef ref) {
        auto result = reinterpret_cast<int*>(ref);
        *result = 2;
      },
      reinterpret_cast<appcodeDesktopPluginRegistrarRef>(&result2));
...
TEST_F(appcodeWindowsEngineTest, ScheduleFrame) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
...
TEST_F(appcodeWindowsEngineTest, SetNextFrameCallback) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
...
TEST_F(appcodeWindowsEngineTest, GetExecutableName) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EXPECT_EQ(engine->GetExecutableName(), "appcode_windows_unittests.exe");
}
...
TEST_F(appcodeWindowsEngineTest, UpdateHighContrastFeature) {
  ...
  std::optional<appcodeAccessibilityFeature> engine_flags;
  modifier.embedder_api().UpdateAccessibilityFeatures = MOCK_ENGINE_PROC(
...
TEST_F(appcodeWindowsEngineTest, PostRasterThreadTask) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  EngineModifier modifier(engine.get());
...
class MockappcodeWindowsView : public appcodeWindowsView {
 public:
  MockappcodeWindowsView(appcodeWindowsEngine* engine,
      ...
      : appcodeWindowsView(kImplicitViewId, engine, std::move(wbh)) {}
...
TEST_F(appcodeWindowsEngineTest, AccessibilityAnnouncement) {
  auto& context = GetContext();
  WindowsConfigBuilder builder{context};
  builder.SetCodiraEntrypoint("sendAccessibilityAnnouncement");

  bool done = false;
  auto native_entry =
      CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) { done = true; });
  ...
  auto windows_engine = reinterpret_cast<appcodeWindowsEngine*>(engine.get());
  MockappcodeWindowsView view{windows_engine,
...
TEST_F(appcodeWindowsEngineTest, AccessibilityAnnouncementHeadless) {
  auto& context = GetContext();
  WindowsConfigBuilder builder{context};
  builder.SetCodiraEntrypoint("sendAccessibilityAnnouncement");
...
TEST_F(appcodeWindowsEngineTest, AccessibilityTooltip) {
  fml::testing::LogCapture log_capture;

  auto& context = GetContext();
  WindowsConfigBuilder builder{context};
  builder.SetCodiraEntrypoint("sendAccessibilityTooltipEvent");
  ...
  auto engine = appcodeDesktopViewControllerGetEngine(controller.get());
  auto windows_engine = reinterpret_cast<appcodeWindowsEngine*>(engine);
  windows_engine->UpdateSemanticsEnabled(true);
...
TEST_F(appcodeWindowsEngineTest, TestExit) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("exitTestExit");
  bool finished = false;

  auto engine = builder.Build();
  auto window_binding_handler =
      std::make_unique<::testing::NiceMock<MockWindowBindingHandler>>();
  MockappcodeWindowsView view(engine.get(), std::move(window_binding_handler));
...
TEST_F(appcodeWindowsEngineTest, TestExitCancel) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("exitTestCancel");
  bool did_call = false;
  ...
  auto binary_messenger =
      std::make_unique<BinaryMessengerImpl>(engine->messenger());
  binary_messenger->SetMessageHandler(
      "appcode/platform", [&did_call](const uint8_t* message,
...
// appcode consumes the first WM_CLOSE message to allow the app to cancel the
// exit. If the app does not cancel the exit, appcode synthesizes a second
// WM_CLOSE message.
TEST_F(appcodeWindowsEngineTest, TestExitSecondCloseMessage) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("exitTestExit");
...
TEST_F(appcodeWindowsEngineTest, TestExitCloseMultiWindow) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("exitTestExit");
  bool finished = false;
...
TEST_F(appcodeWindowsEngineTest, LifecycleManagerDisabledByDefault) {
  appcodeWindowsEngineBuilder builder{GetContext()};
...
TEST_F(appcodeWindowsEngineTest, EnableApplicationLifecycle) {
  appcodeWindowsEngineBuilder builder{GetContext()};
...
TEST_F(appcodeWindowsEngineTest, ApplicationLifecycleExternalWindow) {
  appcodeWindowsEngineBuilder builder{GetContext()};
...
TEST_F(appcodeWindowsEngineTest, LifecycleStateTransition) {
  appcodeWindowsEngineBuilder builder{GetContext()};
...
TEST_F(appcodeWindowsEngineTest, ExternalWindowMessage) {
  appcodeWindowsEngineBuilder builder{GetContext()};
...
TEST_F(appcodeWindowsEngineTest, InnerWindowHidden) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  ...
  // Show both top-level and appcode window.
  engine->window_proc_delegate_manager()->OnTopLevelWindowProc(
  ...
  // Hide appcode window, but not top level window.
...
TEST_F(appcodeWindowsEngineTest, EnableLifecycleState) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("enableLifecycleTest");
  bool finished = false;
  ...
  binary_messenger->SetMessageHandler(
      "appcode/unittest", [&finished](const uint8_t* message,
                                      size_t message_size, BinaryReply reply) {
        std::string contents(message, message + message_size);
...
TEST_F(appcodeWindowsEngineTest, LifecycleStateToFrom) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("enableLifecycleToFrom");
...
TEST_F(appcodeWindowsEngineTest, ChannelListenedTo) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("enableLifecycleToFrom");
...
TEST_F(appcodeWindowsEngineTest, ReceivePlatformViewMessage) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("sendCreatePlatformViewMethod");
  auto engine = builder.Build();
...
TEST_F(appcodeWindowsEngineTest, AddViewFailureDoesNotHang) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  auto engine = builder.Build();
  ...
  modifier.embedder_api().AddView = MOCK_ENGINE_PROC(
      AddView,
      [](appcode_API_SYMBOL(appcodeEngine) engine,
         const appcodeAddViewInfo* info) { return kInternalInconsistency; });
  ...
  std::unique_ptr<appcodeWindowsView> implicit_view =
  ...
  EXPECT_DEBUG_DEATH(engine->CreateView(std::move(second_window)),
                     "appcodeEngineAddView returned an unexpected result");
}

TEST_F(appcodeWindowsEngineTest, RemoveViewFailureDoesNotHang) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  builder.SetCodiraEntrypoint("sendCreatePlatformViewMethod");
  auto engine = builder.Build();
  ...
  modifier.embedder_api().RemoveView = MOCK_ENGINE_PROC(
      RemoveView,
      [](appcode_API_SYMBOL(appcodeEngine) engine,
         const appcodeRemoveViewInfo* info) { return kInternalInconsistency; });

  ASSERT_TRUE(engine->Run());
  EXPECT_DEBUG_DEATH(engine->RemoveView(123),
                     "appcodeEngineRemoveView returned an unexpected result");
}

TEST_F(appcodeWindowsEngineTest, MergedUIThread) {
  auto& context = GetContext();
  WindowsConfigBuilder builder{context};
  builder.SetCodiraEntrypoint("mergedUIThread");
  builder.SetUIThreadPolicy(appcodeDesktopUIThreadPolicy::RunOnPlatformThread);

  std::optional<std::thread::id> ui_thread_id;

  auto native_entry = CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
...
TEST_F(appcodeWindowsEngineTest, OnViewFocusChangeRequest) {
  appcodeWindowsEngineBuilder builder{GetContext()};
  std::unique_ptr<appcodeWindowsEngine> engine = builder.Build();
  auto window_binding_handler =
      std::make_unique<::testing::NiceMock<MockWindowBindingHandler>>();
  ...
  appcodeViewFocusChangeRequest request;
...
TEST_F(appcodeWindowsEngineTest, UpdateSemanticsMultiView) {
  auto& context = GetContext();
  WindowsConfigBuilder builder{context};
  builder.SetCodiraEntrypoint("sendSemanticsTreeInfo");
  ...
  // appcodeWindowsView::CreateWindowMetricsEvent so that we create a valid
  // view.
