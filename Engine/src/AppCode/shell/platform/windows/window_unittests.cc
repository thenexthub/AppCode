...
#include "appcode/shell/platform/windows/testing/mock_direct_manipulation.h"
#include "appcode/shell/platform/windows/testing/mock_text_input_manager.h"
...
TEST(MockWindow, VerticalScroll) {
  ...
  EXPECT_CALL(window, OnScroll(0, -scroll_amount / 120.0,
                               kappcodePointerDeviceKindMouse, 0))
      .Times(1);
...
TEST(MockWindow, MouseLeave) {
  ...
  EXPECT_CALL(window, OnPointerMove(mouse_x, mouse_y,
                                    kappcodePointerDeviceKindMouse, 0, 0))
      .Times(1);
...
// See: https://github.com/appcode/appcode/issues/109412
