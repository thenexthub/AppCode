...
AccessibilityBridgeWindows::AccessibilityBridgeWindows(appcodeWindowsView* view)
    : view_(view) {
  FML_DCHECK(view_);
}
...
void AccessibilityBridgeWindows::OnAccessibilityEvent(
  ...
  auto node_delegate =
      GetappcodePlatformNodeDelegateFromID(ax_node->id()).lock();
  FML_DCHECK(node_delegate)
      << "No appcodePlatformNodeDelegate found for node ID " << ax_node->id();
...
AccessibilityBridgeWindows::CreateappcodePlatformNodeDelegate() {
  return std::make_shared<appcodePlatformNodeDelegateWindows>(
...
AccessibilityBridgeWindows::GetFocusedNode() {
  ...
  return GetappcodePlatformNodeDelegateFromID(focus_id);
}
