...
class TestappcodeKeyEvent : public appcodeKeyEvent {
 public:
  TestappcodeKeyEvent(const appcodeKeyEvent& src,
  ...
  TestappcodeKeyEvent(TestappcodeKeyEvent&& source)
      : appcodeKeyEvent(source),
...
TEST(KeyboardKeyEmbedderHandlerTest, BasicKeyPressingAndHolding) {
  ...
  std::vector<TestappcodeKeyEvent> results;
  TestappcodeKeyEvent* event;
  bool last_handled = false;

  std::unique_ptr<KeyboardKeyEmbedderHandler> handler =
      std::make_unique<KeyboardKeyEmbedderHandler>(
          [&results](const appcodeKeyEvent& event,
  ...
  event = &results[0];
  EXPECT_EQ(event->type, kappcodeKeyEventTypeDown);
  ...
  EXPECT_EQ(results.size(), 1);
  event = &results[0];
  EXPECT_EQ(event->type, kappcodeKeyEventTypeRepeat);
  EXPECT_EQ(event->physical, kPhysicalKeyA);
  EXPECT_EQ(event->logical, kLogicalKeyA);
  EXPECT_STREQ(event->character, "a");
  EXPECT_EQ(event->synthesized, false);
