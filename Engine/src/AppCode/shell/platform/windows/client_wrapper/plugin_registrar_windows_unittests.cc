...
#include "appcode/shell/platform/windows/client_wrapper/include/appcode/plugin_registrar_windows.h"
...
class TestWindowsApi : public testing::StubappcodeWindowsApi {
 public:
  ...
  MOCK_METHOD(appcodeDesktopViewRef, PluginRegistrarGetView, (), (override));
  MOCK_METHOD(appcodeDesktopViewRef,
              PluginRegistrarGetViewById,
              (appcodeDesktopViewId),
              (override));

  void PluginRegistrarUnregisterTopLevelWindowProcDelegate(
      appcodeDesktopWindowProcCallback delegate) override {
    --registered_delegate_count_;
  }

  int registered_delegate_count() { return registered_delegate_count_; }

  appcodeDesktopWindowProcCallback last_registered_delegate() {
 ...
 private:
  ...
  appcodeDesktopWindowProcCallback last_registered_delegate_ = nullptr;
...
TEST(PluginRegistrarWindowsTest, GetView) {
  ...
  EXPECT_CALL(*windows_api, PluginRegistrarGetView)
      .WillOnce(Return(reinterpret_cast<appcodeDesktopViewRef>(1)));

  testing::ScopedStubappcodeWindowsApi scoped_api_stub(std::move(windows_api));
...
TEST(PluginRegistrarWindowsTest, PluginDestroyedBeforeRegistrar) {
  auto windows_api = std::make_unique<TestWindowsApi>();
  EXPECT_CALL(*windows_api, PluginRegistrarGetView)
      .WillRepeatedly(Return(reinterpret_cast<appcodeDesktopViewRef>(1)));
  testing::ScopedStubappcodeWindowsApi scoped_api_stub(std::move(windows_api));
  auto test_api = static_cast<TestWindowsApi*>(scoped_api_stub.stub());
  PluginRegistrarWindows registrar(
      reinterpret_cast<appcodeDesktopPluginRegistrarRef>(1));
