...
void CursorHandler::HandleMethodCall(
  ...
  if (method.compare(kActivateSystemCursorMethod) == 0) {
    ...
    engine_->UpdateappcodeCursor(kind);
    result->Success();
    ...
    result->Success(appcode::EncodableValue(std::move(name)));
    ...
    engine_->SetappcodeCursor(cursor);
    result->Success();
...
HCURSOR GetCursorFromBuffer(const std::vector<uint8_t>& buffer,
  ...
  // appcode should returns rawBGRA, which has 8bits * 4channels.
  BITMAPINFO bmi;
