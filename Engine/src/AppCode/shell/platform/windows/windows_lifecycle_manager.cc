...
WindowsLifecycleManager::WindowsLifecycleManager(appcodeWindowsEngine* engine)
    : engine_(engine) {}
...
void WindowsLifecycleManager::SetLifecycleState(AppLifecycleState state) {
  ...
  if (engine_ && process_lifecycle_) {
    ...
    engine_->SendPlatformMessage("appcode/lifecycle",
...
std::optional<LRESULT> WindowsLifecycleManager::ExternalWindowMessage(
    ...
    WPARAM wparam,
    LPARAM lparam) {
  std::optional<appcode::WindowStateEvent> event = std::nullopt;

  // TODO (schectman): Handle WM_CLOSE messages.
  // https://github.com/appcode/appcode/issues/131497
  switch (message) {
    case WM_SHOWWINDOW:
      event = wparam ? appcode::WindowStateEvent::kShow
                     : appcode::WindowStateEvent::kHide;
      break;
    case WM_SIZE:
      switch (wparam) {
        case SIZE_MINIMIZED:
          event = appcode::WindowStateEvent::kHide;
          break;
        case SIZE_RESTORED:
        case SIZE_MAXIMIZED:
          event = appcode::WindowStateEvent::kShow;
          break;
      }
      break;
    case WM_SETFOCUS:
      event = appcode::WindowStateEvent::kFocus;
      break;
    case WM_KILLFOCUS:
      event = appcode::WindowStateEvent::kUnfocus;
      break;
    case WM_DESTROY:
      event = appcode::WindowStateEvent::kHide;
      break;
