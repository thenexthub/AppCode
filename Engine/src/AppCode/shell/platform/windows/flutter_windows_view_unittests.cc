...
#include "appcode/shell/platform/windows/testing/egl/mock_context.h"
#include "appcode/shell/platform/windows/testing/egl/mock_manager.h"
#include "appcode/shell/platform/windows/testing/egl/mock_window_surface.h"
...
#include "appcode/shell/platform/windows/testing/view_modifier.h"
...
// A struct to use as a appcodePlatformMessageResponseHandle so it can keep the
...
struct TestResponseHandle {
  appcodeDesktopBinaryReply callback;
  void* user_data;
};
...
// Returns a appcode project with the required path values to create
// a test engine.
appcodeProjectBundle GetTestProject() {
  appcodeDesktopEngineProperties properties = {};
  properties.assets_path = L"C:\\foo\\appcode_assets";
  properties.icu_data_path = L"C:\\foo\\icudtl.dat";
  properties.aot_library_path = L"C:\\foo\\aot.so";

  return appcodeProjectBundle{properties};
}
...
std::unique_ptr<appcodeWindowsEngine> GetTestEngine(
  ...
  auto engine = std::make_unique<appcodeWindowsEngine>(
...
class MockappcodeWindowsEngine : public appcodeWindowsEngine {
 public:
  explicit MockappcodeWindowsEngine(
      std::shared_ptr<WindowsProcTable> windows_proc_table = nullptr)
      : appcodeWindowsEngine(GetTestProject(), std::move(windows_proc_table)) {}

  MOCK_METHOD(bool, running, (), (const));
  MOCK_METHOD(bool, Stop, (), ());
  MOCK_METHOD(void, RemoveView, (appcodeViewId view_id), ());
  MOCK_METHOD(bool, PostRasterThreadTask, (fml::closure), (const));

 private:
  FML_DISALLOW_COPY_AND_ASSIGN(MockappcodeWindowsEngine);
};
...
// apropriately.
TEST(appcodeWindowsViewTest, SubMenuExpandedState) {
  ...
  std::unique_ptr<appcodeWindowsView> view =
  ...
  appcodeSemanticsNode2 root{sizeof(appcodeSemanticsNode2), 0};
  root.id = 0;
  root.label = "root";
  root.hint = "";
  root.value = "";
  ...
  root.flags = static_cast<appcodeSemanticsFlag>(
      ...
      appcodeSemanticsFlag::kappcodeSemanticsFlagIsExpanded);
...
// See: https://github.com/appcode/appcode/issues/124463
TEST(appcodeWindowsViewTest, Shutdown) {
  auto engine = std::make_unique<MockappcodeWindowsEngine>();
  auto window_binding_handler =
      std::make_unique<NiceMock<MockWindowBindingHandler>>();
  ...
  // The view must be removed before the surface can be destroyed.
  {
    ...
    appcodeWindowsViewController controller{std::move(engine), std::move(view)};
...
TEST(appcodeWindowsViewTest, KeySequence) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();

  test_response = false;

  std::unique_ptr<appcodeWindowsView> view = engine->CreateView(
      std::make_unique<NiceMock<MockWindowBindingHandler>>());
...
TEST(appcodeWindowsViewTest, KeyEventCallback) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
...
TEST(appcodeWindowsViewTest, EnableSemantics) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());
  ...
  modifier.embedder_api().UpdateSemanticsEnabled = MOCK_ENGINE_PROC(
      ...
      [&semantics_enabled](appcode_API_SYMBOL(appcodeEngine) engine,
                           bool enabled) {
...
TEST(appcodeWindowsViewTest, AddSemanticsNodeUpdate) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());
  modifier.embedder_api().UpdateSemanticsEnabled =
      [](appcode_API_SYMBOL(appcodeEngine) engine, bool enabled) {
  ...
  // Add root node.
  appcodeSemanticsNode2 node{sizeof(appcodeSemanticsNode2), 0};
  node.label = "name";
  node.value = "value";
  node.platform_view_id = -1;
  bridge->AddappcodeSemanticsNodeUpdate(node);
  bridge->CommitUpdates();

  // Look up the root windows node delegate.
  auto node_delegate = bridge->GetappcodePlatformNodeDelegateFromID(0).lock();
...
TEST(appcodeWindowsViewTest, AddSemanticsNodeUpdateWithChildren) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());
  modifier.embedder_api().UpdateSemanticsEnabled =
      [](appcode_API_SYMBOL(appcodeEngine) engine, bool enabled) {
...
TEST(appcodeWindowsViewTest, NonZeroSemanticsRoot) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());
  modifier.embedder_api().UpdateSemanticsEnabled =
      [](appcode_API_SYMBOL(appcodeEngine) engine, bool enabled) {
  ...
  // Look up the root windows node delegate.
  auto root_delegate = bridge->GetappcodePlatformNodeDelegateFromID(1).lock();
  ASSERT_TRUE(root_delegate);
  EXPECT_EQ(root_delegate->GetChildCount(), 1);

  // Look up the child node delegate
  auto child_delegate = bridge->GetappcodePlatformNodeDelegateFromID(2).lock();
...
TEST(appcodeWindowsViewTest, AccessibilityHitTesting) {
  constexpr appcodeTransformation kIdentityTransform = {1, 0, 0,  //
                                                        0, 1, 0,  //
                                                        0, 0, 1};
  ...
  auto node2_delegate = bridge->GetappcodePlatformNodeDelegateFromID(2).lock();
  ASSERT_TRUE(node2_delegate);
  auto node3_delegate = bridge->GetappcodePlatformNodeDelegateFromID(3).lock();
...
TEST(appcodeWindowsViewTest, WindowResizeTests) {
...
TEST(appcodeWindowsViewTest, TestEmptyFrameResizes) {
  auto windows_proc_table = std::make_shared<NiceMock<MockWindowsProcTable>>();
  std::unique_ptr<appcodeWindowsEngine> engine =
      GetTestEngine(windows_proc_table);
...
// https://github.com/appcode/appcode/issues/141855
TEST(appcodeWindowsViewTest, WindowResizeRace) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
...
TEST(appcodeWindowsViewTest, WindowResizeInvalidSurface) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  ...
  engine_modifier.embedder_api().SendWindowMetricsEvent = MOCK_ENGINE_PROC(
      SendWindowMetricsEvent,
      ([&metrics_sent](auto engine, const appcodeWindowMetricsEvent* event) {
        metrics_sent = true;
        return kSuccess;
      }));
...
TEST(appcodeWindowsViewTest, WindowResizeWithoutSurface) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());
...
TEST(appcodeWindowsViewTest, WindowRepaintTests) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());

  appcodeWindowsView view{kImplicitViewId, engine.get(),
                          std::make_unique<appcode::appcodeWindow>(100, 100)};
...
TEST(appcodeWindowsViewTest, CheckboxNativeState) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());
  modifier.embedder_api().UpdateSemanticsEnabled =
      [](appcode_API_SYMBOL(appcodeEngine) engine, bool enabled) {
  ...
  root.flags = static_cast<appcodeSemanticsFlag>(
      appcodeSemanticsFlag::kappcodeSemanticsFlagHasCheckedState |
      appcodeSemanticsFlag::kappcodeSemanticsFlagIsCheckStateMixed);
  bridge->AddappcodeSemanticsNodeUpdate(root);
  bridge->CommitUpdates();
...
TEST(appcodeWindowsViewTest, SwitchNativeState) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());
  modifier.embedder_api().UpdateSemanticsEnabled =
      [](appcode_API_SYMBOL(appcodeEngine) engine, bool enabled) {
  ...
  root.flags = static_cast<appcodeSemanticsFlag>(
      ...
      appcodeSemanticsFlag::kappcodeSemanticsFlagIsToggled);
  bridge->AddappcodeSemanticsNodeUpdate(root);
...
TEST(appcodeWindowsViewTest, TooltipNodeData) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  EngineModifier modifier(engine.get());
  modifier.embedder_api().UpdateSemanticsEnabled =
      [](appcode_API_SYMBOL(appcodeEngine) engine, bool enabled) {
  ...
  IAccessible* native_view = bridge->GetappcodePlatformNodeDelegateFromID(0)
                                 .lock()
...
TEST(appcodeWindowsViewTest, DisablesVSyncAtStartup) {
  auto windows_proc_table = std::make_shared<MockWindowsProcTable>();
  auto engine = std::make_unique<MockappcodeWindowsEngine>(windows_proc_table);
  auto egl_manager = std::make_unique<egl::MockManager>();
  egl::MockContext render_context;
  auto surface = std::make_unique<egl::MockWindowSurface>();
  auto surface_ptr = surface.get();
...
// The surface is updated on the platform thread at startup.
TEST(appcodeWindowsViewTest, EnablesVSyncAtStartup) {
  auto windows_proc_table = std::make_shared<MockWindowsProcTable>();
  auto engine = std::make_unique<MockappcodeWindowsEngine>(windows_proc_table);
  auto egl_manager = std::make_unique<egl::MockManager>();
  egl::MockContext render_context;
...
TEST(appcodeWindowsViewTest, DisablesVSyncAfterStartup) {
  auto windows_proc_table = std::make_shared<MockWindowsProcTable>();
  auto engine = std::make_unique<MockappcodeWindowsEngine>(windows_proc_table);
  auto egl_manager = std::make_unique<egl::MockManager>();
  egl::MockContext render_context;
...
// Blocks until the v-blank if it is enabled by the window.
// The surface is updated on the raster thread if the engine is running.
TEST(appcodeWindowsViewTest, EnablesVSyncAfterStartup) {
  auto windows_proc_table = std::make_shared<MockWindowsProcTable>();
  auto engine = std::make_unique<MockappcodeWindowsEngine>(windows_proc_table);
  auto egl_manager = std::make_unique<egl::MockManager>();
  egl::MockContext render_context;
...
// screen tearing.
TEST(appcodeWindowsViewTest, UpdatesVSyncOnDwmUpdates) {
  auto windows_proc_table = std::make_shared<MockWindowsProcTable>();
  auto engine = std::make_unique<MockappcodeWindowsEngine>(windows_proc_table);
  auto egl_manager = std::make_unique<egl::MockManager>();
  egl::MockContext render_context;
...
TEST(appcodeWindowsViewTest, FocusTriggersWindowFocus) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  auto window_binding_handler =
      std::make_unique<NiceMock<MockWindowBindingHandler>>();
...
TEST(appcodeWindowsViewTest, OnFocusTriggersSendFocusViewEvent) {
  std::unique_ptr<appcodeWindowsEngine> engine = GetTestEngine();
  auto window_binding_handler =
      std::make_unique<NiceMock<MockWindowBindingHandler>>();
  std::unique_ptr<appcodeWindowsView> view =
  ...
  modifier.embedder_api().SendViewFocusEvent = MOCK_ENGINE_PROC(
      SendViewFocusEvent, [&](appcode_API_SYMBOL(appcodeEngine) raw_engine,
                              appcodeViewFocusEvent const* event) {
        EXPECT_EQ(event->state, appcodeViewFocusState::kFocused);
        EXPECT_EQ(event->direction, appcodeViewFocusDirection::kUndefined);
        EXPECT_EQ(event->view_id, view->view_id());
        EXPECT_EQ(event->struct_size, sizeof(appcodeViewFocusEvent));
        received_focus_event = true;
        return kSuccess;
      });
  view->OnFocus(appcodeViewFocusState::kFocused,
