...
// Prevent regression: https://github.com/appcode/appcode/issues/86617
TEST(WindowsNoFixtureTest, GetTextureRegistrar) {
  ...
  auto engine = appcodeDesktopEngineCreate(&properties);
  ASSERT_NE(engine, nullptr);
  auto texture_registrar = appcodeDesktopEngineGetTextureRegistrar(engine);
  EXPECT_NE(texture_registrar, nullptr);
  appcodeDesktopEngineDestroy(engine);
}
...
TEST_F(WindowsTest, LaunchMainHasNoOutput) {
  ...
  // output printed by Codira, such as the Codira VM service startup message)
...
// appcodeDesktopEngineRun parameter when no entrypoint is specified in
// appcodeDesktopEngineProperties.dart_entrypoint.
//
...
TEST_F(WindowsTest, LaunchCustomEntrypointInEngineRunInvocation) {
  ...
  ASSERT_TRUE(appcodeDesktopEngineRun(engine.get(), "customEntrypoint"));
}
...
TEST_F(WindowsTest, LaunchHeadlessEngine) {
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("signalViewIds");
  ...
  context.AddNativeFunction(
      "SignalStringValue", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        auto handle = Codira_GetNativeArgument(args, 0);
        ASSERT_FALSE(Codira_IsError(handle));
        view_ids = tonic::CodiraConverter<std::string>::FromCodira(handle);
        signaled = true;
      }));
...
TEST_F(WindowsTest, EngineCanTransitionToHeadless) {
  ...
  {
    appcodeDesktopViewControllerProperties properties = {};
    ViewControllerPtr controller{
        appcodeDesktopEngineCreateViewController(engine.get(), &properties)};
  ...
  auto engine_ptr = reinterpret_cast<appcodeWindowsEngine*>(engine.get());
...
TEST_F(WindowsTest, LaunchRefreshesAccessibility) {
  ...
  ViewControllerPtr controller{
      appcodeDesktopViewControllerCreate(0, 0, engine.release())};

  ASSERT_TRUE(called);
}

// Verify that engine fails to launch when a conflicting entrypoint in
// appcodeDesktopEngineProperties.dart_entrypoint and the
// appcodeDesktopEngineRun parameter.
//
// TODO(cbracken): https://github.com/appcode/appcode/issues/109285
TEST_F(WindowsTest, LaunchConflictingCustomEntrypoints) {
  ...
  ASSERT_FALSE(appcodeDesktopEngineRun(engine.get(), "conflictingEntrypoint"));
}
...
TEST_F(WindowsTest, VerifyNativeFunction) {
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("verifyNativeFunction");

  bool signaled = false;
  auto native_entry =
      CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) { signaled = true; });
...
TEST_F(WindowsTest, VerifyNativeFunctionWithParameters) {
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("verifyNativeFunctionWithParameters");

  bool bool_value = false;
  bool signaled = false;
  auto native_entry = CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
    auto handle = Codira_GetNativeBooleanArgument(args, 0, &bool_value);
    ASSERT_FALSE(Codira_IsError(handle));
    signaled = true;
  });
...
TEST_F(WindowsTest, PlatformExecutable) {
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("readPlatformExecutable");
  ...
  auto native_entry = CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
    auto handle = Codira_GetNativeArgument(args, 0);
    ASSERT_FALSE(Codira_IsError(handle));
    executable_name = tonic::CodiraConverter<std::string>::FromCodira(handle);
    signaled = true;
  });
  ...
  EXPECT_EQ(executable_name, "appcode_windows_unittests.exe");
}
...
TEST_F(WindowsTest, VerifyNativeFunctionWithReturn) {
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("verifyNativeFunctionWithReturn");

  bool bool_value_to_return = true;
  int count = 2;
  auto bool_return_entry = CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
    Codira_SetBooleanReturnValue(args, bool_value_to_return);
    --count;
  });
  context.AddNativeFunction("SignalBoolReturn", bool_return_entry);

  bool bool_value_passed = false;
  auto bool_pass_entry = CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
    auto handle = Codira_GetNativeBooleanArgument(args, 0, &bool_value_passed);
    ASSERT_FALSE(Codira_IsError(handle));
    --count;
  });
...
TEST_F(WindowsTest, NextFrameCallback) {
  ...
  CreateNewThread("test_platform_thread")->PostTask([&]() {
    ...
    auto& context = GetContext();
    WindowsConfigBuilder builder(context);
    builder.SetCodiraEntrypoint("drawHelloWorld");
...
TEST_F(WindowsTest, PresentHeadless) {
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("renderImplicitView");
...
TEST_F(WindowsTest, GetViewId) {
  ...
  appcodeDesktopViewId view_id =
      appcodeDesktopViewControllerGetViewId(controller.get());

  ASSERT_EQ(view_id, static_cast<appcodeDesktopViewId>(kImplicitViewId));
}
...
TEST_F(WindowsTest, GetGraphicsAdapter) {
  ...
  ViewControllerPtr controller{builder.Run()};
  ASSERT_NE(controller, nullptr);
  auto view = appcodeDesktopViewControllerGetView(controller.get());

  Microsoft::WRL::ComPtr<IDXGIAdapter> dxgi_adapter;
  dxgi_adapter = appcodeDesktopViewGetGraphicsAdapter(view);
...
TEST_F(WindowsTest, GetGraphicsAdapterWithLowPowerPreference) {
  ...
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetGpuPreference(appcodeDesktopGpuPreference::LowPowerPreference);
  ViewControllerPtr controller{builder.Run()};
  ASSERT_NE(controller, nullptr);
  auto view = appcodeDesktopViewControllerGetView(controller.get());
...
TEST_F(WindowsTest, PluginRegistrarGetImplicitView) {
  ...
  appcodeDesktopEngineRef engine =
      appcodeDesktopViewControllerGetEngine(controller.get());
  appcodeDesktopPluginRegistrarRef registrar =
      appcodeDesktopEngineGetPluginRegistrar(engine, "foo_bar");
  appcodeDesktopViewRef implicit_view =
      appcodeDesktopPluginRegistrarGetView(registrar);
...
TEST_F(WindowsTest, PluginRegistrarGetView) {
  ...
  appcodeDesktopViewId view_id =
      appcodeDesktopViewControllerGetViewId(controller.get());
  appcodeDesktopViewRef view =
      appcodeDesktopPluginRegistrarGetViewById(registrar, view_id);

  appcodeDesktopViewRef view_123 = appcodeDesktopPluginRegistrarGetViewById(
      registrar, static_cast<appcodeDesktopViewId>(123));
...
TEST_F(WindowsTest, PluginRegistrarGetViewHeadless) {
  ...
  appcodeDesktopPluginRegistrarRef registrar =
      appcodeDesktopEngineGetPluginRegistrar(engine.get(), "foo_bar");
...
TEST_F(WindowsTest, Lifecycle) {
  ...
  appcodeDesktopViewControllerProperties properties = {0, 0};
  ...
  appcodeDesktopViewRef view =
      appcodeDesktopViewControllerGetView(controller.get());
  ASSERT_NE(view, nullptr);

  HWND hwnd = appcodeDesktopViewGetHWND(view);
...
TEST_F(WindowsTest, GetKeyboardStateHeadless) {
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("sendGetKeyboardState");
  ...
  context.AddNativeFunction(
      "SignalStringValue", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        auto handle = Codira_GetNativeArgument(args, 0);
        ASSERT_FALSE(Codira_IsError(handle));
        auto value = tonic::CodiraConverter<std::string>::FromCodira(handle);
...
TEST_F(WindowsTest, AddRemoveView) {
  ...
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("onMetricsChangedSignalViewIds");

  bool ready = false;
  context.AddNativeFunction(
      "Signal",
      CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) { ready = true; }));
  ...
  appcodeDesktopEngineRef engine =
      appcodeDesktopViewControllerGetEngine(first_controller.get());
  appcodeDesktopViewControllerProperties properties = {};
  properties.width = 100;
  properties.height = 100;
  ViewControllerPtr second_controller{
      appcodeDesktopEngineCreateViewController(engine, &properties)};
...
TEST_F(WindowsTest, EngineId) {
  auto& context = GetContext();
  WindowsConfigBuilder builder(context);
  builder.SetCodiraEntrypoint("testEngineId");

  std::optional<int64_t> engineId;
  context.AddNativeFunction(
      "NotifyEngineId", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        const auto argument = Codira_GetNativeArgument(args, 0);
        if (!Codira_IsNull(argument)) {
          const auto handle = tonic::CodiraConverter<int64_t>::FromCodira(argument);
          engineId = handle;
        }
      }));
  // Create the implicit view.
  ...
  auto engine = appcodeDesktopViewControllerGetEngine(first_controller.get());
  EXPECT_EQ(engine, appcodeDesktopEngineForId(*engineId));
}
