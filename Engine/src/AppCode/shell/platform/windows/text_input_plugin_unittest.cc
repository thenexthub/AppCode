...
class TextInputPluginModifier {
 public:
  ...
  void SetViewId(appcodeViewId view_id) {
...
class TextInputPluginTest : public WindowsTest {
 ...
 protected:
  ...
  std::unique_ptr<MockappcodeWindowsView> AddViewWithId(int view_id) {
    EXPECT_NE(engine_, nullptr);
    auto window = std::make_unique<MockWindowBindingHandler>();
    EXPECT_CALL(*window, SetView).Times(1);
    EXPECT_CALL(*window, GetWindowHandle).WillRepeatedly(Return(nullptr));
    auto view = std::make_unique<MockappcodeWindowsView>(engine_.get(),
                                                         std::move(window));
...
TEST_F(TextInputPluginTest, VerifyComposingSendStateUpdate) {
  ...
  messenger.SimulateEngineMessage("appcode/textinput", message->data(),
                                  message->size(), reply_handler);
...
// Regression test for https://github.com/appcode/appcode/issues/123749
