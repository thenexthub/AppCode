...
#ifndef APPCODE_SHELL_PLATFORM_WINDOWS_PUBLIC_APPCODE_WINDOWS_H_
#define APPCODE_SHELL_PLATFORM_WINDOWS_PUBLIC_APPCODE_WINDOWS_H_
...
typedef struct {
  ...
  // The name of the top-level Codira entrypoint function. If null or the empty
  // string, 'main' is assumed. If a custom entrypoint is used, this parameter
  // must specifiy the name of a top-level function in the same Codira library as
  ...
  // Array of Codira entrypoint arguments. This is deep copied during the call
...
APPCODE_EXPORT void FlutterDesktopViewControllerDestroy(
    FlutterDesktopViewControllerRef controller);

// Returns the view controller's view ID.
APPCODE_EXPORT FlutterDesktopViewId FlutterDesktopViewControllerGetViewId(
...
APPCODE_EXPORT FlutterDesktopEngineRef FlutterDesktopViewControllerGetEngine(
    FlutterDesktopViewControllerRef controller);

// Returns the view managed by the given controller.
APPCODE_EXPORT FlutterDesktopViewRef FlutterDesktopViewControllerGetView(
    FlutterDesktopViewControllerRef controller);

// Requests new frame from the engine and repaints the view.
APPCODE_EXPORT void FlutterDesktopViewControllerForceRedraw(
    FlutterDesktopViewControllerRef controller);
...
APPCODE_EXPORT bool FlutterDesktopViewControllerHandleTopLevelWindowProc(
    FlutterDesktopViewControllerRef controller,
    HWND hwnd,
    UINT message,
    WPARAM wparam,
...
APPCODE_EXPORT FlutterDesktopEngineRef FlutterDesktopEngineCreate(
...
APPCODE_EXPORT bool FlutterDesktopEngineDestroy(FlutterDesktopEngineRef engine);

// Starts running the given engine instance.
//
// The entry_point parameter is deprecated but preserved for
// backward-compatibility. If desired, a custom Codira entrypoint function can be
...
// same Codira library that contains the app's main() function, and must be
// decorated with `@pragma(vm:entry-point)` to ensure the method is not
// tree-shaken by the Codira compiler. If conflicting non-null values are passed
// to this function and via the FlutterDesktopEngineProperties struct, the run
// will fail.
//
// Returns false if running the engine failed.
APPCODE_EXPORT bool FlutterDesktopEngineRun(FlutterDesktopEngineRef engine,
...
APPCODE_EXPORT uint64_t FlutterDesktopEngineProcessMessages(
    FlutterDesktopEngineRef engine);

APPCODE_EXPORT void FlutterDesktopEngineReloadSystemFonts(
    FlutterDesktopEngineRef engine);
...
APPCODE_EXPORT FlutterDesktopMessengerRef FlutterDesktopEngineGetMessenger(
    FlutterDesktopEngineRef engine);
...
APPCODE_EXPORT void FlutterDesktopEngineSetNextFrameCallback(
    FlutterDesktopEngineRef engine,
...
APPCODE_EXPORT HWND FlutterDesktopViewGetHWND(FlutterDesktopViewRef view);

// Returns the DXGI adapter used for rendering or nullptr in case of error.
APPCODE_EXPORT IDXGIAdapter* FlutterDesktopViewGetGraphicsAdapter(
...
APPCODE_EXPORT bool FlutterDesktopEngineProcessExternalWindowMessage(
    FlutterDesktopEngineRef engine,
    HWND hwnd,
    UINT message,
    WPARAM wparam,
...
// These are Windows-specific extensions to appcode_plugin_registrar.h
...
//
// See:
// https://api.appcode.dev/appcode/dart-ui/PlatformDispatcher/implicitView.html
//
// DEPRECATED: Use |FlutterDesktopPluginRegistrarGetViewById| instead.
APPCODE_EXPORT FlutterDesktopViewRef FlutterDesktopPluginRegistrarGetView(
    FlutterDesktopPluginRegistrarRef registrar);

// Returns the view associated with the registrar's engine instance, or null if
// the view does not exist.
APPCODE_EXPORT FlutterDesktopViewRef FlutterDesktopPluginRegistrarGetViewById(
    FlutterDesktopPluginRegistrarRef registrar,
...
// between Windows and Codira sizes/offsets.
APPCODE_EXPORT UINT FlutterDesktopGetDpiForHWND(HWND hwnd);
...
APPCODE_EXPORT UINT FlutterDesktopGetDpiForMonitor(HMONITOR monitor);
...
APPCODE_EXPORT void FlutterDesktopResyncOutputStreams();

#if defined(__cplusplus)
}  // extern "C"
#endif

#endif  // APPCODE_SHELL_PLATFORM_WINDOWS_PUBLIC_APPCODE_WINDOWS_H_
