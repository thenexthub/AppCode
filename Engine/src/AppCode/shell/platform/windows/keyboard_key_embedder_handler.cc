...
void KeyboardKeyEmbedderHandler::KeyboardHookImpl(
  ...
  if (sequence_logical_key == VK_PROCESSKEY) {
    ...
    // presses are considered handled and not sent to appcode. These events must
      ...
      if (key_info.toggled_on != pre_event_toggled) {
        ...
        if (target_is_pressed) {
          SendEvent(SynthesizeSimpleEvent(
                        kappcodeKeyEventTypeUp, key_info.physical_key,
        ...
        SendEvent(SynthesizeSimpleEvent(kappcodeKeyEventTypeDown,
...
void KeyboardKeyEmbedderHandler::SynchronizeCriticalPressedStates(
    ...
    bool is_event_down,
    bool event_key_can_be_repeat) {
  // During an incoming event, there might be a synthesized appcode event for
  // each key of each pressing goal, followed by an eventual main appcode
  // event.
  //
  //    NowState ---------------->  PreEventState --------------> TrueState
  //              Synchronization                      Event
  ...
  for (auto& kv : critical_keys_) {
    ...
    if (key_info.check_pressed) {
      ...
      if (is_event_down) {
        ...
        //
        // In that case, normally appcode should synthesize nothing since the
        // resulting event can adapt to the current state by dispatching either
        // a down or a repeat event. However, in certain cases (when appcode has
      ...
      if (now_pressed != pre_event_pressed) {
        ...
        SendEvent(
            SynthesizeSimpleEvent(
                now_pressed ? kappcodeKeyEventTypeUp : kappcodeKeyEventTypeDown,
...
void KeyboardKeyEmbedderHandler::SyncModifiersIfNeeded(int modifiers_state) {
  // TODO(bleroux): consider exposing these constants in appcode_key_map.g.cc?
...
void KeyboardKeyEmbedderHandler::SendSynthesizeDownEvent(uint64_t physical,
                                                         uint64_t logical) {
  SendEvent(
      SynthesizeSimpleEvent(kappcodeKeyEventTypeDown, physical, logical, ""),
      nullptr, nullptr);
...
void KeyboardKeyEmbedderHandler::SendSynthesizeUpEvent(uint64_t physical,
                                                       uint64_t logical) {
  SendEvent(
      SynthesizeSimpleEvent(kappcodeKeyEventTypeUp, physical, logical, ""),
      nullptr, nullptr);
...
appcodeKeyEvent KeyboardKeyEmbedderHandler::SynthesizeSimpleEvent(
    appcodeKeyEventType type,
    uint64_t physical,
    uint64_t logical,
    const char* character) {
  return appcodeKeyEvent{
      .struct_size = sizeof(appcodeKeyEvent),
      .timestamp = static_cast<double>(
          std::chrono::duration_cast<std::chrono::microseconds>(
              std::chrono::high_resolution_clock::now().time_since_epoch())
...
void KeyboardKeyEmbedderHandler::SendEvent(const appcodeKeyEvent& event,
