...
CompositorOpenGL::CompositorOpenGL(appcodeWindowsEngine* engine,
...
bool CompositorOpenGL::CreateBackingStore(
  ...
  result->type = kappcodeBackingStoreTypeOpenGL;
  result->open_gl.type = kappcodeOpenGLTargetTypeFramebuffer;
...
bool CompositorOpenGL::CollectBackingStore(const appcodeBackingStore* store) {
  FML_DCHECK(is_initialized_);
  FML_DCHECK(store->type == kappcodeBackingStoreTypeOpenGL);
  FML_DCHECK(store->open_gl.type == kappcodeOpenGLTargetTypeFramebuffer);
...
bool CompositorOpenGL::Present(appcodeWindowsView* view,
  ...
  // See: https://github.com/appcode/appcode/issues/31713
  FML_DCHECK(is_initialized_);
  FML_DCHECK(layers_count == 1);
  FML_DCHECK(layers[0]->offset.x == 0 && layers[0]->offset.y == 0);
  FML_DCHECK(layers[0]->type == kappcodeLayerContentTypeBackingStore);
  FML_DCHECK(layers[0]->backing_store->type == kappcodeBackingStoreTypeOpenGL);
  FML_DCHECK(layers[0]->backing_store->open_gl.type ==
             kappcodeOpenGLTargetTypeFramebuffer);
  ...
  // Prevents regressions like: https://github.com/appcode/appcode/issues/140828
...
bool CompositorOpenGL::Clear(appcodeWindowsView* view) {
  FML_DCHECK(is_initialized_);
