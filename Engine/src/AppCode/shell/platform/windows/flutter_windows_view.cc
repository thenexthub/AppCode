...
#include "appcode/shell/platform/windows/text_input_plugin.h"
...
bool SurfaceWillUpdate(size_t cur_width,
  ...
  // TODO (https://github.com/appcode/appcode/issues/65061) : Avoid special
...
void UpdateVsync(const appcodeWindowsEngine& engine,
...
/// Destroys a rendering surface that backs a appcode view.
void DestroyWindowSurface(const appcodeWindowsEngine& engine,
...
appcodeWindowsView::appcodeWindowsView(
...
bool appcodeWindowsView::OnEmptyFrameGenerated() {
...
bool appcodeWindowsView::OnFrameGenerated(size_t width, size_t height) {
  // Called on the raster thread.
  std::unique_lock<std::mutex> lock(resize_mutex_);
...
void appcodeWindowsView::ForceRedraw() {
...
bool appcodeWindowsView::OnWindowSizeChanged(size_t width, size_t height) {
...
void appcodeWindowsView::OnWindowRepaint() {
  ForceRedraw();
}

void appcodeWindowsView::OnPointerMove(double x,
                                       double y,
...
void appcodeWindowsView::OnPointerDown(
    double x,
    double y,
    appcodePointerDeviceKind device_kind,
    int32_t device_id,
    appcodePointerMouseButtons appcode_button) {
  if (appcode_button != 0) {
    auto state = GetOrCreatePointerState(device_kind, device_id);
    state->buttons |= appcode_button;
    SendPointerDown(x, y, state);
  }
}

void appcodeWindowsView::OnPointerUp(
    double x,
    double y,
    appcodePointerDeviceKind device_kind,
    int32_t device_id,
...
void appcodeWindowsView::OnPointerLeave(double x,
                                        double y,
                                        appcodePointerDeviceKind device_kind,
                                        int32_t device_id) {
...
void appcodeWindowsView::OnPointerPanZoomStart(int32_t device_id) {
  PointerLocation point = binding_handler_->GetPrimaryPointerLocation();
  SendPointerPanZoomStart(device_id, point.x, point.y);
}

void appcodeWindowsView::OnPointerPanZoomUpdate(int32_t device_id,
                                                double pan_x,
                                                double pan_y,
                                                double scale,
                                                double rotation) {
...
void appcodeWindowsView::OnPointerPanZoomEnd(int32_t device_id) {
  SendPointerPanZoomEnd(device_id);
}

void appcodeWindowsView::OnText(const std::u16string& text) {
  SendText(text);
}

void appcodeWindowsView::OnKey(int key,
                               int scancode,
                               int action,
                               char32_t character,
                               bool extended,
...
void appcodeWindowsView::OnFocus(appcodeViewFocusState focus_state,
                                 appcodeViewFocusDirection direction) {
  SendFocus(focus_state, direction);
}

void appcodeWindowsView::OnComposeBegin() {
  SendComposeBegin();
}

void appcodeWindowsView::OnComposeCommit() {
  SendComposeCommit();
}

void appcodeWindowsView::OnComposeEnd() {
  SendComposeEnd();
}

void appcodeWindowsView::OnComposeChange(const std::u16string& text,
                                         int cursor_pos) {
  SendComposeChange(text, cursor_pos);
}

void appcodeWindowsView::OnScroll(double x,
                                  double y,
                                  double delta_x,
                                  double delta_y,
...
void appcodeWindowsView::OnScrollInertiaCancel(int32_t device_id) {
  PointerLocation point = binding_handler_->GetPrimaryPointerLocation();
  SendScrollInertiaCancel(device_id, point.x, point.y);
}

void appcodeWindowsView::OnUpdateSemanticsEnabled(bool enabled) {
  engine_->UpdateSemanticsEnabled(enabled);
}

gfx::NativeViewAccessible appcodeWindowsView::GetNativeViewAccessible() {
...
void appcodeWindowsView::OnCursorRectUpdated(const Rect& rect) {
  binding_handler_->OnCursorRectUpdated(rect);
}

void appcodeWindowsView::OnResetImeComposing() {
  binding_handler_->OnResetImeComposing();
}

// Sends new size information to appcodeEngine.
void appcodeWindowsView::SendWindowMetrics(size_t width,
                                           size_t height,
...
appcodeWindowMetricsEvent appcodeWindowsView::CreateWindowMetricsEvent() const {
...
void appcodeWindowsView::SendInitialBounds() {
...
appcodeWindowsView::PointerState* appcodeWindowsView::GetOrCreatePointerState(
    appcodePointerDeviceKind device_kind,
    int32_t device_id) {
...
void appcodeWindowsView::SetEventPhaseFromCursorButtonState(
    appcodePointerEvent* event_data,
    const PointerState* state) const {
  // For details about this logic, see appcodePointerPhase in the embedder.h
  // file.
  if (state->buttons == 0) {
    event_data->phase = state->appcode_state_is_down
                            ? appcodePointerPhase::kUp
                            : appcodePointerPhase::kHover;
  } else {
    event_data->phase = state->appcode_state_is_down
                            ? appcodePointerPhase::kMove
                            : appcodePointerPhase::kDown;
  }
}

void appcodeWindowsView::SendPointerMove(double x,
                                         double y,
                                         PointerState* state) {
...
void appcodeWindowsView::SendPointerDown(double x,
                                         double y,
                                         PointerState* state) {
  appcodePointerEvent event = {};
  event.x = x;
  ...
  state->appcode_state_is_down = true;
}

void appcodeWindowsView::SendPointerUp(double x,
                                       double y,
                                       PointerState* state) {
  appcodePointerEvent event = {};
  event.x = x;
  ...
  SetEventPhaseFromCursorButtonState(&event, state);
  SendPointerEventWithData(event, state);
  if (event.phase == appcodePointerPhase::kUp) {
    state->appcode_state_is_down = false;
  }
}

void appcodeWindowsView::SendPointerLeave(double x,
                                          double y,
                                          PointerState* state) {
  appcodePointerEvent event = {};
  event.x = x;
  event.y = y;
  event.phase = appcodePointerPhase::kRemove;
  SendPointerEventWithData(event, state);
}

void appcodeWindowsView::SendPointerPanZoomStart(int32_t device_id,
                                                 double x,
                                                 double y) {
  auto state =
      GetOrCreatePointerState(kappcodePointerDeviceKindTrackpad, device_id);
  state->pan_zoom_start_x = x;
  state->pan_zoom_start_y = y;
  appcodePointerEvent event = {};
  event.x = x;
  event.y = y;
  event.phase = appcodePointerPhase::kPanZoomStart;
  SendPointerEventWithData(event, state);
}

void appcodeWindowsView::SendPointerPanZoomUpdate(int32_t device_id,
                                                  double pan_x,
                                                  double pan_y,
                                                  double scale,
                                                  double rotation) {
  ...
  event.phase = appcodePointerPhase::kPanZoomUpdate;
  SendPointerEventWithData(event, state);
}

void appcodeWindowsView::SendPointerPanZoomEnd(int32_t device_id) {
  auto state =
      GetOrCreatePointerState(kappcodePointerDeviceKindTrackpad, device_id);
  appcodePointerEvent event = {};
  event.x = state->pan_zoom_start_x;
  event.y = state->pan_zoom_start_y;
  event.phase = appcodePointerPhase::kPanZoomEnd;
  SendPointerEventWithData(event, state);
}

void appcodeWindowsView::SendText(const std::u16string& text) {
  engine_->text_input_plugin()->TextHook(text);
}

void appcodeWindowsView::SendKey(int key,
                                 int scancode,
                                 int action,
                                 char32_t character,
                                 bool extended,
...
void appcodeWindowsView::SendFocus(appcodeViewFocusState focus_state,
                                   appcodeViewFocusDirection direction) {
  appcodeViewFocusEvent event = {};
  event.struct_size = sizeof(event);
  event.view_id = view_id_;
...
void appcodeWindowsView::SendComposeBegin() {
  engine_->text_input_plugin()->ComposeBeginHook();
}

void appcodeWindowsView::SendComposeCommit() {
  engine_->text_input_plugin()->ComposeCommitHook();
}

void appcodeWindowsView::SendComposeEnd() {
  engine_->text_input_plugin()->ComposeEndHook();
}

void appcodeWindowsView::SendComposeChange(const std::u16string& text,
                                           int cursor_pos) {
  engine_->text_input_plugin()->ComposeChangeHook(text, cursor_pos);
}

void appcodeWindowsView::SendScroll(double x,
                                    double y,
                                    double delta_x,
                                    double delta_y,
                                    int scroll_offset_multiplier,
...
void appcodeWindowsView::SendScrollInertiaCancel(int32_t device_id,
                                                 double x,
                                                 double y) {
  auto state =
      GetOrCreatePointerState(kappcodePointerDeviceKindTrackpad, device_id);
  ...
  event.y = y;
  event.signal_kind =
      appcodePointerSignalKind::kappcodePointerSignalKindScrollInertiaCancel;
  SetEventPhaseFromCursorButtonState(&event, state);
  SendPointerEventWithData(event, state);
}

void appcodeWindowsView::SendPointerEventWithData(
    const appcodePointerEvent& event_data,
    PointerState* state) {
  // If sending anything other than an add, and the pointer isn't already added,
  // synthesize an add to satisfy appcode's expectations about events.
  if (!state->appcode_state_is_added &&
  ...
  if (event_data.phase == appcodePointerPhase::kAdd) {
    state->appcode_state_is_added = true;
...
void appcodeWindowsView::OnFramePresented() {
...
bool appcodeWindowsView::ClearSoftwareBitmap() {
  return binding_handler_->OnBitmapSurfaceCleared();
}

bool appcodeWindowsView::PresentSoftwareBitmap(const void* allocation,
                                               size_t row_bytes,
                                               size_t height) {
...
appcodeViewId appcodeWindowsView::view_id() const {
  return view_id_;
}

bool appcodeWindowsView::IsImplicitView() const {
  return view_id_ == kImplicitViewId;
}

void appcodeWindowsView::CreateRenderSurface() {
...
bool appcodeWindowsView::ResizeRenderSurface(size_t width, size_t height) {
  FML_DCHECK(surface_ != nullptr);
  ...
  // See: https://github.com/appcode/appcode/issues/79427
  if (!surface_->Destroy()) {
...
egl::WindowSurface* appcodeWindowsView::surface() const {
  return surface_.get();
}

void appcodeWindowsView::OnHighContrastChanged() {
  engine_->UpdateHighContrastMode();
}

HWND appcodeWindowsView::GetWindowHandle() const {
  return binding_handler_->GetWindowHandle();
}

appcodeWindowsEngine* appcodeWindowsView::GetEngine() const {
  return engine_;
}

void appcodeWindowsView::AnnounceAlert(const std::wstring& text) {
...
void appcodeWindowsView::NotifyWinEventWrapper(ui::AXPlatformNodeWin* node,
...
ui::AXFragmentRootDelegateWin* appcodeWindowsView::GetAxFragmentRootDelegate() {
  return accessibility_bridge_.get();
}

ui::AXPlatformNodeWin* appcodeWindowsView::AlertNode() const {
  return binding_handler_->GetAlert();
}

std::shared_ptr<AccessibilityBridgeWindows>
appcodeWindowsView::CreateAccessibilityBridge() {
  return std::make_shared<AccessibilityBridgeWindows>(this);
}

void appcodeWindowsView::UpdateSemanticsEnabled(bool enabled) {
...
void appcodeWindowsView::OnDwmCompositionChanged() {
  UpdateVsync(*engine_, surface_.get(), NeedsVsync());
}

void appcodeWindowsView::OnWindowStateEvent(HWND hwnd, WindowStateEvent event) {
  engine_->OnWindowStateEvent(hwnd, event);
}

bool appcodeWindowsView::Focus() {
  return binding_handler_->Focus();
}

bool appcodeWindowsView::NeedsVsync() const {
