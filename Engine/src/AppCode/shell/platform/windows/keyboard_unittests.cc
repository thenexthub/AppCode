...
#include "appcode/shell/platform/windows/keyboard_manager.h"
...
using namespace ::appcode::testing::keycodes;
...
typedef struct {
  ...
  appcodeKeyEvent key_event;     // For kKeyCallOnKey
  std::u16string text;           // For kKeyCallOnText
  std::string text_method_call;  // For kKeyCallTextMethodCall
} KeyCall;

// A appcodeWindowsView that spies on text.
class TestappcodeWindowsView : public appcodeWindowsView {
 public:
  TestappcodeWindowsView(appcodeWindowsEngine* engine,
 ...
 private:
  ...
  FML_DISALLOW_COPY_AND_ASSIGN(TestappcodeWindowsView);
};
...
class KeyboardTester {
 public:
  ...
  explicit KeyboardTester(WindowsTestContext& context)
    ...
    view_ = std::make_unique<TestappcodeWindowsView>(
        engine_.get(),
  ...
  TestappcodeWindowsView& GetView() { return *view_; }
 ...
 private:
  ...
  std::unique_ptr<TestappcodeWindowsView> view_;
  ...
  std::unique_ptr<appcodeWindowsEngine> GetTestEngine(
    ...
    appcodeWindowsEngineBuilder builder{context};
    ...
    key_response_controller->SetEmbedderResponse(
        [&key_calls = key_calls, &callback_handler = callback_handler_](
          ...
          appcodeKeyEvent clone_event = *event;
  ...
  static MockKeyResponseController::EmbedderCallbackHandler RespondValue(
      bool value) {
    return [value](const appcodeKeyEvent* event,
                   MockKeyResponseController::ResponseCallback callback) {
      callback(value);
    };
  }
...
TEST_F(KeyboardTest, LowerCaseAHandled) {
  ...
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeDown,
  ...
  EXPECT_EQ(tester.key_calls.size(), 1);
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeUp,
...
TEST_F(KeyboardTest, ShiftLeftUnhandled) {
  ...
  EXPECT_EQ(tester.key_calls.size(), 1);
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeRepeat,
                       kPhysicalShiftLeft, kLogicalShiftLeft, "",
                       kNotSynthesized);
  tester.clear_key_calls();
  EXPECT_EQ(tester.RedispatchedMessageCountAndClear(), 1);
...
TEST_F(KeyboardTest, RepeatA) {
  ...
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeDown,
                       kPhysicalKeyA, kLogicalKeyA, "a", kNotSynthesized);
  EXPECT_CALL_IS_EVENT(tester.key_calls[1], kappcodeKeyEventTypeRepeat,
                       kPhysicalKeyA, kLogicalKeyA, "a", kNotSynthesized);
  EXPECT_EQ(tester.RedispatchedMessageCountAndClear(), 0);
}
...
TEST_F(KeyboardTest, RestartClearsKeyboardState) {
  ...
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeDown,
                       kPhysicalKeyA, kLogicalKeyA, "a", kNotSynthesized);
  EXPECT_CALL_IS_EVENT(tester.key_calls[1], kappcodeKeyEventTypeDown,
                       kPhysicalKeyA, kLogicalKeyA, "a", kNotSynthesized);
  EXPECT_EQ(tester.RedispatchedMessageCountAndClear(), 0);
}
...
TEST_F(KeyboardTest, AltGrModifiedKey) {
  ...
  // Release AltGr. Win32 doesn't dispatch ControlLeft up. Instead appcode will
  ...
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeUp,
                       kPhysicalControlLeft, kLogicalControlLeft, "",
                       kNotSynthesized);
  EXPECT_CALL_IS_EVENT(tester.key_calls[1], kappcodeKeyEventTypeUp,
                       kPhysicalAltRight, kLogicalAltRight, "",
                       kNotSynthesized);
  tester.clear_key_calls();
...
TEST_F(KeyboardTest, AltGrTwice) {
  ...
  tester.InjectKeyboardChanges(std::vector<KeyboardChange>{
      WmKeyUpInfo{VK_CONTROL, kScanCodeControl, kNotExtended}.Build(
          kWmResultZero)});
  EXPECT_EQ(tester.key_calls.size(), 1);
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeDown, 0, 0, "",
                       kNotSynthesized);
  tester.clear_key_calls();
  EXPECT_EQ(tester.RedispatchedMessageCountAndClear(), 0);
}
...
TEST_F(KeyboardTest, DeadKeyThatDoesNotCombine) {
  ...
  // https://github.com/appcode/appcode/issues/98306
  EXPECT_EQ(tester.RedispatchedMessageCountAndClear(), 2);
...
TEST_F(KeyboardTest, SynthesizeModifiers) {
  ...
  // https://github.com/appcode/appcode/issues/98533
  EXPECT_EQ(tester.key_calls.size(), 2);
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeDown,
  ...
  EXPECT_CALL_IS_EVENT(tester.key_calls[1], kappcodeKeyEventTypeDown,
                       kPhysicalNumLock, kLogicalNumLock, "", kSynthesized);
  EXPECT_CALL_IS_EVENT(tester.key_calls[2], kappcodeKeyEventTypeUp,
                       kPhysicalNumLock, kLogicalNumLock, "", kSynthesized);
  tester.clear_key_calls();
  EXPECT_EQ(tester.RedispatchedMessageCountAndClear(), 1);
}
...
// any events.
//
// Regression test for https://github.com/appcode/appcode/issues/95888 .
...
//
// Regression test for https://github.com/appcode/appcode/issues/104169. These
...
TEST_F(KeyboardTest, UpOnlyImeEventsAreCorrectlyHandled) {
  ...
  EXPECT_CALL_IS_EVENT(tester.key_calls[0], kappcodeKeyEventTypeDown,
                       kPhysicalShiftLeft, kLogicalShiftLeft, "",
                       kNotSynthesized);
  EXPECT_CALL_IS_EVENT(tester.key_calls[1], kappcodeKeyEventTypeDown, 0, 0, "",
                       kNotSynthesized);
  EXPECT_CALL_IS_EVENT(tester.key_calls[2], kappcodeKeyEventTypeUp,
                       kPhysicalShiftLeft, kLogicalShiftLeft, "",
                       kNotSynthesized);
  EXPECT_CALL_IS_EVENT(tester.key_calls[3], kappcodeKeyEventTypeDown, 0, 0, "",
                       kNotSynthesized);
  tester.clear_key_calls();
}
...
TEST_F(KeyboardTest, SlowFrameworkResponse) {
  ...
  EXPECT_CALL_IS_TEXT(tester.key_calls[1], u"a");
  EXPECT_CALL_IS_EVENT(tester.key_calls[2], kappcodeKeyEventTypeRepeat,
                       kPhysicalKeyA, kLogicalKeyA, "a", kNotSynthesized);
  EXPECT_EQ(tester.RedispatchedMessageCountAndClear(), 2);
...
// Regression test for https://github.com/appcode/appcode/issues/84210.
//
...
TEST_F(KeyboardTest, TextInputSubmit) {
  ...
  tester.InjectPlatformMessage(
      "appcode/textinput", "TextInput.setClient",
  ...
  //
  // Regression test for https://github.com/appcode/appcode/issues/97706.
