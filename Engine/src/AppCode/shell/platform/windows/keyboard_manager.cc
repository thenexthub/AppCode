...
// when pressed, as reported in https://github.com/appcode/appcode/issues/78005.
//
...
// with a up event, which is fine until appcode redispatches the CtrlDown
// event, which Win32 then interprets as a real event, leaving both Win32 and
// the appcode framework thinking that CtrlLeft is still pressed.
//
// To resolve this, appcode recognizes this fake CtrlLeft down event using the
// following AltRight down event. appcode then forges a CtrlLeft key up event
...
bool KeyboardManager::HandleMessage(UINT const action,
  ...
  switch (action) {
    ...
    case WM_SYSCHAR: {
      ...
      if (current_session_.front().IsGeneralKeyDown()) {
        ...
        if (action == WM_DEADCHAR || action == WM_SYSDEADCHAR) {
          ...
          // https://github.com/appcode/appcode/issues/92654 .)
          character =
    ...
    case WM_SYSKEYUP: {
      ...
      // https://github.com/appcode/appcode/issues/85587 ).
