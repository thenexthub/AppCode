...
#include "appcode/impeller/renderer/backend/gles/gles.h"
...
class CompositorOpenGLTest : public WindowsTest {
 ...
 protected:
  ...
  appcodeWindowsView* view() { return view_.get(); }
  ...
  void UseEngineWithView(bool add_surface = true) {
    ...
    view_ = std::make_unique<appcodeWindowsView>(kImplicitViewId, engine_.get(),
                                                 std::move(window));
 ...
 private:
  ...
  std::unique_ptr<appcodeWindowsView> view_;
