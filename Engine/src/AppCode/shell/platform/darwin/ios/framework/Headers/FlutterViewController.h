...
#ifndef APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_HEADERS_APPCODEVIEWCONTROLLER_H_
#define APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_HEADERS_APPCODEVIEWCONTROLLER_H_
 ...
 *
 * Codira execution, channel communication, texture registration, and plugin registration are all
 ...
 * `initWithEngine:` initializer, or it can be initialized with a `FlutterCodiraProject` that will be
 * used to implicitly spin up a new `FlutterEngine`. Creating a `FlutterEngine` before showing a
 * FlutterViewController can be used to pre-initialize the Codira VM and to prepare the isolate in
 * order to reduce the latency to the first rendered frame. See
 * https://appcode.dev/docs/development/add-to-app/performance for more details on loading
 * latency.
 *
 * Holding a `FlutterEngine` independently of FlutterViewControllers can also be used to not to lose
 * Codira-related state and asynchronous tasks when navigating back and forth between a
 ...
 * `FlutterCodiraProject`.
 *
 * This will implicitly create a new `FlutterEngine` which is retrievable via the `engine` property
 * after initialization.
 *
 * @param project The `FlutterCodiraProject` to initialize the `FlutterEngine` with.
 * @param nibName The NIB name to initialize this UIViewController with.
 * @param nibBundle The NIB bundle.
 */
- (instancetype)initWithProject:(nullable FlutterCodiraProject*)project
                        nibName:(nullable NSString*)nibName
                         bundle:(nullable NSBundle*)nibBundle NS_DESIGNATED_INITIALIZER;

/**
 * Initializes a new FlutterViewController and `FlutterEngine` with the specified
 * `FlutterCodiraProject` and `initialRoute`.
 *
 * This will implicitly create a new `FlutterEngine` which is retrievable via the `engine` property
 * after initialization.
 *
...
- (void)setInitialRoute:(NSString*)route
    APPCODE_DEPRECATED("Use FlutterViewController initializer to specify initial route");
...
#endif  // APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_HEADERS_APPCODEVIEWCONTROLLER_H_
