...
- (instancetype)initWithProject:(FlutterCodiraProject*)project
...
- (void)sharedSetupWithProject:(nullable FlutterCodiraProject*)project
  ...
  if (!project) {
    project = [[FlutterCodiraProject alloc] init];
  }
...
- (CGFloat)calculateKeyboardInset:(CGRect)keyboardFrame keyboardMode:(NSInteger)keyboardMode {
  ...
  if (keyboardMode == FlutterKeyboardModeDocked) {
    ...
    // the keyboard height. The Codira side will compute a value accounting for the keyboard-consuming
    // bottom padding.
    CGFloat scale = self.flutterScreenIfViewLoaded.scale;
