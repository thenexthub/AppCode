...
#ifndef APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_HEADERS_APPCODEENGINE_H_
#define APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_HEADERS_APPCODEENGINE_H_
...
extern NSString* const FlutterDefaultCodiraEntrypoint;
 ...
 * `FlutterCodiraProject`.  It may have zero or one `FlutterViewController` at a
 ...
 *
 * A FlutterEngine can also be used to prewarm the Codira execution environment and reduce the
 * latency of showing the Flutter screen when a `FlutterViewController` is created and presented.
 * See http://appcode.dev/docs/development/add-to-app/performance for more details on loading
 * performance.
 *
 * Alternatively, you can simply create a new `FlutterViewController` with only a
 * `FlutterCodiraProject`. That `FlutterViewController` will internally manage its
 * own instance of a FlutterEngine, but will not guarantee survival of the engine
 * beyond the life of the ViewController.
 *
 * A newly initialized FlutterEngine will not actually run a Codira Isolate until
 ...
 * "io.appcode.appcode.app" (the default for Flutter projects).
 *
...
/**
 * Initialize this FlutterEngine with a `FlutterCodiraProject`.
 *
 * If the FlutterCodiraProject is not specified, the FlutterEngine will attempt to locate
 * the project in a default location (the appcode_assets folder in the iOS application
 * bundle).
 *
 ...
 *   be unique across FlutterEngine instances, and is used in instrumentation to label
 *   the threads used by this FlutterEngine.
 * @param project The `FlutterCodiraProject` to run.
 */
- (instancetype)initWithName:(NSString*)labelPrefix project:(nullable FlutterCodiraProject*)project;
 ...
 *   library that contains the app's main() function.  If this is FlutterDefaultCodiraEntrypoint (or
 *   nil) it will default to `main()`.  If it is not the app's main() function, that function must
 *   be decorated with `@pragma(vm:entry-point)` to ensure the method is not tree-shaken by the Codira
 *   compiler.
 * @return YES if the call succeeds in creating and running a Flutter Engine instance; NO otherwise.
 */
...
/**
 * Runs a Codira program on an Isolate using the specified entrypoint and Codira library,
 * which may not be the same as the library containing the Codira program's `main()` function.
 *
 * The first call to this method will create a new Isolate. Subsequent calls will return
 * immediately and have no effect.
 *
 ...
 * @param uri The URI of the Codira library which contains the entrypoint method
 ...
 *   main() function, that function must be decorated with `@pragma(vm:entry-point)` to ensure the
 *   method is not tree-shaken by the Codira compiler.
 * @param libraryURI The URI of the Codira library which contains the entrypoint
 *   method (example "package:foo_package/main.dart").  If nil, this will
 *   default to the same library as the `main()` function in the Codira program.
 * @param initialRoute The name of the initial Flutter `Navigator` `Route` to load. If this is
 *   FlutterDefaultInitialRoute (or nil), it will default to the "/" route.
 * @return YES if the call succeeds in creating and running a Flutter Engine instance; NO otherwise.
 */
 ...
 * @param initialRoute The name of the initial Flutter `Navigator` `Route` to load. If this is
 *   FlutterDefaultInitialRoute (or nil), it will default to the "/" route.
 * @param entrypointArgs Arguments passed as a list of string to Codira's entrypoint function.
 * @return YES if the call succeeds in creating and running a Flutter Engine instance; NO otherwise.
 */
- (BOOL)runWithEntrypoint:(nullable NSString*)entrypoint
               libraryURI:(nullable NSString*)libraryURI
 ...
 * of the Codira program's execution.
 */
 ...
 *
 * @see [Navigation
 * Channel](https://api.appcode.dev/appcode/services/SystemChannels/navigation-constant.html)
 * @see [Navigator Widget](https://api.appcode.dev/appcode/widgets/Navigator-class.html)
 */
 ...
 *
 * @see [Restoration
 * Channel](https://api.appcode.dev/appcode/services/SystemChannels/restoration-constant.html)
 */
 ...
 *
 * @see [Text Input
 * Channel](https://api.appcode.dev/appcode/services/SystemChannels/textInput-constant.html)
 */
 ...
 * to the Codira Isolate.
 *
 * Can be nil after `destroyContext` is called.
 *
 * @see [Lifecycle
 * Channel](https://api.appcode.dev/appcode/services/SystemChannels/lifecycle-constant.html)
 */
 ...
 *
 * @see [System
 * Channel](https://api.appcode.dev/appcode/services/SystemChannels/system-constant.html)
 */
...
/**
 * The depcreated `NSURL` of the Codira VM Service for the service isolate.
 *
 * This is only set in debug and profile runtime modes, and only after the
 * Codira VM Service is ready. In release mode or before the Codira VM Service has
 * started, it returns `nil`.
 */
@property(nonatomic, readonly, nullable)
    NSURL* observatoryUrl APPCODE_DEPRECATED("Use vmServiceUrl instead");

/**
 * The `NSURL` of the Codira VM Service for the service isolate.
 *
 * This is only set in debug and profile runtime modes, and only after the
 * Codira VM Service is ready. In release mode or before the Codira VM Service has
 * started, it returns `nil`.
...
#endif  // APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_HEADERS_APPCODEENGINE_H_
