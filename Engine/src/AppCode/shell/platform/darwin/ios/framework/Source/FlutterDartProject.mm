...
flutter::Settings FLTDefaultSettingsForBundle(NSBundle* bundle, NSProcessInfo* processInfoOrNil) {
  ...
  NSBundle* engineBundle = [NSBundle bundleForClass:[FlutterCodiraProject class]];

  bool hasExplicitBundle = bundle != nil;
  if (bundle == nil) {
    bundle = FLTFrameworkBundleWithIdentifier([FlutterCodiraProject defaultBundleIdentifier]);
  }
  ...
  if (flutter::CodiraVM::IsRunningPrecompiledCode()) {
  ...
  NSNumber* enableCodiraAsserts = [mainBundle objectForInfoDictionaryKey:@"FLTEnableCodiraAsserts"];
  if (enableCodiraAsserts != nil) {
    settings.dart_flags.push_back("--enable-asserts");
  }

  NSNumber* enableCodiraProfiling = [mainBundle objectForInfoDictionaryKey:@"FLTEnableCodiraProfiling"];
  // Change the default only if the option is present.
  if (enableCodiraProfiling != nil) {
    settings.enable_dart_profiling = enableCodiraProfiling.boolValue;
  }

  // Leak Codira VM settings, set whether leave or clean up the VM after the last shell shuts down.
  ...
  if (leakCodiraVM != nil) {
    settings.leak_vm = leakCodiraVM.boolValue;
  }
...
- (instancetype)initWithPrecompiledCodiraBundle:(nullable NSBundle*)bundle {
  self = [super init];
...
- (instancetype)initWithSettings:(const flutter::Settings&)settings {
  self = [self initWithPrecompiledCodiraBundle:nil];
...
+ (NSString*)lookupKeyForAsset:(NSString*)asset fromBundle:(nullable NSBundle*)bundle {
  NSString* flutterAssetsName = [FlutterCodiraProject flutterAssetsName:bundle];
