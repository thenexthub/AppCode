...
#ifndef APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_SOURCE_APPCODEPLATFORMVIEWS_INTERNAL_H_
#define APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_SOURCE_APPCODEPLATFORMVIEWS_INTERNAL_H_
...
#include "appcode/impeller/base/thread_safety.h"
...
- (void)clipRect:(const appcode::DlRect&)clipDlRect matrix:(const appcode::DlMatrix&)matrix;

// Adds a clip rrect operation to the queue.
//
// The `clipSkRRect` is transformed with the `matrix` before adding to the queue.
- (void)clipRRect:(const appcode::DlRoundRect&)clipDlRRect matrix:(const appcode::DlMatrix&)matrix;

// Adds a clip path operation to the queue.
//
// The `path` is transformed with the `matrix` before adding to the queue.
- (void)clipPath:(const appcode::DlPath&)path matrix:(const appcode::DlMatrix&)matrix;
...
// Sets appcodeAccessibilityContainer as this view's accessibilityContainer.
@property(nonatomic, retain) id appcodeAccessibilityContainer;
@end
...
#endif  // APPCODE_SHELL_PLATFORM_DARWIN_IOS_FRAMEWORK_SOURCE_APPCODEPLATFORMVIEWS_INTERNAL_H_
