...
#import "FlutterCodiraVMServicePublisher.h"

#if APPCODE_RELEASE

@implementation FlutterCodiraVMServicePublisher
...
// 3) The Codira multicast_dns package, which is what Flutter uses to find the
...
@protocol FlutterCodiraVMServicePublisherDelegate
- (void)publishServiceProtocolPort:(NSURL*)uri;
- (void)stopService;
@end

@interface FlutterCodiraVMServicePublisher ()
+ (NSData*)createTxtData:(NSURL*)url;

@property(readonly, class) NSString* serviceName;
@property(readonly) NSObject<FlutterCodiraVMServicePublisherDelegate>* delegate;
@property(nonatomic, readwrite) NSURL* url;
@property(readonly) BOOL enableVMServicePublication;

@end

@interface CodiraVMServiceDNSServiceDelegate : NSObject <FlutterCodiraVMServicePublisherDelegate>
@end

@implementation CodiraVMServiceDNSServiceDelegate {
...
- (void)publishServiceProtocolPort:(NSURL*)url {
  ...
  NSData* txtData = [FlutterCodiraVMServicePublisher createTxtData:url];
  int err = DNSServiceRegister(&_dnsServiceRef, flags, interfaceIndex,
                               FlutterCodiraVMServicePublisher.serviceName.UTF8String,
  ...
  FML_LOG(ERROR) << "Failed to register Codira VM Service port with mDNS with error " << err << ".";
...
static void DNSSD_API RegistrationCallback(DNSServiceRef sdRef,
  ...
  if (errorCode == kDNSServiceErr_NoError) {
    FML_DLOG(INFO) << "FlutterCodiraVMServicePublisher is ready!";
...
#if TARGET_IPHONE_SIMULATOR
    FML_DLOG(WARNING)
        << "Could not register as server for FlutterCodiraVMServicePublisher, permission "
    ...
    FML_LOG(ERROR) << "Could not register as server for FlutterCodiraVMServicePublisher. Check your "
...
@implementation FlutterCodiraVMServicePublisher {
  flutter::CodiraServiceIsolate::CallbackHandle _callbackHandle;
}
...
- (void)dealloc {
  ...
  flutter::CodiraServiceIsolate::RemoveServerStatusCallback(_callbackHandle);
}
@end
