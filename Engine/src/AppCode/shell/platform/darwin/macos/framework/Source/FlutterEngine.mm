...
- (void)requestApplicationTermination:(id)sender
  ...
  if (![self acceptingRequests]) {
    // Until the Codira application has signaled that it is ready to handle
...
@implementation FlutterEngine {
  ...
  APPCODE_API_SYMBOL(FlutterEngine) _engine;

  // The project being run by this engine.
  FlutterCodiraProject* _project;
  ...
  // Pointer to the Codira AOT snapshot and instruction data.
...
- (instancetype)initWithName:(NSString*)labelPrefix
  ...
  _active = NO;
  _visible = NO;
  _project = project ?: [[FlutterCodiraProject alloc] init];
...
- (BOOL)runWithEntrypoint:(NSString*)entrypoint {
  ...
  FlutterEngineResult result = _embedderAPI.Initialize(
      APPCODE_ENGINE_VERSION, &rendererConfig, &flutterArguments, (__bridge void*)(self), &_engine);
  if (result != kSuccess) {
...
- (void)loadAOTData:(NSString*)assetsDir {
  if (!_embedderAPI.RunsAOTCompiledCodiraCode()) {
    return;
  }
