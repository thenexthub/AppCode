...
TEST_F(FlutterEngineTest, HasNonNullExecutableName) {
  ...
  // Block until notified by the Codira test of the value of Platform.executable.
  BOOL signaled = NO;
  AddNativeCallback("NotifyStringValue", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
...
TEST_F(FlutterEngineTest, Switches) {
  setenv("APPCODE_ENGINE_SWITCHES", "2", 1);
  setenv("APPCODE_ENGINE_SWITCH_1", "abc", 1);
  setenv("APPCODE_ENGINE_SWITCH_2", "foo=\"bar, baz\"", 1);
  ...
  unsetenv("APPCODE_ENGINE_SWITCHES");
  unsetenv("APPCODE_ENGINE_SWITCH_1");
  unsetenv("APPCODE_ENGINE_SWITCH_2");
}
...
TEST_F(FlutterEngineTest, CanLogToStdout) {
  ...
  AddNativeCallback("SignalNativeTest",
                    CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) { signaled = YES; }));
...
TEST_F(FlutterEngineTest, DISABLED_BackgroundIsBlack) {
  ...
  BOOL signaled = NO;
  AddNativeCallback("SignalNativeTest", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
...
TEST_F(FlutterEngineTest, NativeCallbacks) {
  ...
  AddNativeCallback("SignalNativeTest",
                    CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) { latch_called = YES; }));
...
TEST_F(FlutterEngineTest, CodiraEntrypointArguments) {
  NSString* fixtures = @(flutter::testing::GetFixturesPath());
  FlutterCodiraProject* project = [[FlutterCodiraProject alloc]
      initWithAssetsPath:fixtures
             ICUDataPath:[fixtures stringByAppendingString:@"/icudtl.dat"]];
...
TEST_F(FlutterEngineTest, CanGetEngineForId) {
  ...
  AddNativeCallback("NotifyEngineId", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
                      const auto argument = Codira_GetNativeArgument(args, 0);
                      if (!Codira_IsNull(argument)) {
                        const auto id = tonic::CodiraConverter<int64_t>::FromCodira(argument);
                        engineId = id;
                      }
                      signaled = YES;
                    }));
