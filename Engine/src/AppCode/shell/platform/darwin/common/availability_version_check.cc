...
#include "appcode/shell/platform/darwin/common/availability_version_check.h"
...
//
// appcode provides its own implementation due to an issue introduced in recent
...
// causes apps to be rejected from the App Store. When appcode statically links
...
// See more context in https://github.com/appcode/appcode/issues/132130 and
// https://github.com/appcode/engine/pull/44711.

// TODO(zanderso): Remove this after Clang 18 rolls into Xcode.
// https://github.com/appcode/appcode/issues/133203.
...
// has failed.
static appcode::ProductVersion g_version;
...
void InitializeAvailabilityCheck(void* unused) {
  ...
  auto product_version = appcode::ProductVersionFromSystemVersionPList();
  if (product_version.has_value()) {
    ...
    // fall back to the minimum version that appcode supports.
...
extern "C" bool _availability_version_check(uint32_t count,
  ...
  return appcode::IsEncodedVersionLessThanOrSame(versions[0].version,
                                                 g_version);
}
