...
TEST(AvailabilityVersionCheck, CanDecodeSystemPlist) {
  auto maybe_product_version = appcode::ProductVersionFromSystemVersionPList();
...
TEST(AvailabilityVersionCheck, CanParseAndCompareVersions) {
  ...
  ASSERT_TRUE(appcode::IsEncodedVersionLessThanOrSame(encoded_lower_version,
                                                      rhs_version));

  uint32_t encoded_higher_version = ConstructVersion(42, 0, 1);
  ASSERT_FALSE(appcode::IsEncodedVersionLessThanOrSame(encoded_higher_version,
                                                       rhs_version));
}
