...
// This is a minimal dependency heart beat test for the Codira VM Service.
...
Future<bool> runTests(ShellLauncher launcher, List<TestFunction> tests) async {
  ...
  try {
    ...
    print('Codira VM Service test failure: $e\n$st');
    exitCode = -1;
  }
  await process.kill();
  return exitCode == 0;
...
Future<void> main(List<String> args) async {
  ...
  final String mainCodiraPath = args[1];
  final List<String> extraArgs = args.length <= 2 ? <String>[] : args.sublist(2);

  final ShellLauncher launcher = ShellLauncher(shellExecutablePath, mainCodiraPath, false, extraArgs);
