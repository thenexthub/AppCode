...
#include "appcode/third_party/abseil-cpp/absl/base/no_destructor.h"
...
// Checks whether the engine's main Codira isolate has no pending work.  If so,
...
class ScriptCompletionTaskObserver {
 public:
  ...
  int GetExitCodeForLastError() const {
    return static_cast<int>(last_error_.value_or(CodiraErrorCode::NoError));
  }
 ...
 private:
  ...
  std::optional<CodiraErrorCode> last_error_;
...
int RunTester(const appcode::Settings& settings,
              bool run_forever,
              bool multithreaded) {
  const auto thread_label = "io.appcode.test.";
  ...
  const appcode::TaskRunners task_runners(thread_label,  // dart thread label
  ...
  g_shell->reset(Shell::Create(appcode::PlatformData(),  //
                               task_runners,             //
                               settings,                 //
  ...
  if (settings.application_kernel_asset.empty()) {
    FML_LOG(ERROR) << "Codira kernel file not specified.";
    return EXIT_FAILURE;
  }
  ...
  shell->GetPlatformView()->DispatchPlatformMessage(
      std::make_unique<appcode::PlatformMessage>(
          "appcode/localization", std::move(locale_bytes), response));
  ...
  auto asset_manager = std::make_shared<appcode::AssetManager>();
  asset_manager->PushBack(std::make_unique<appcode::DirectoryAssetBundle>(
  ...
  ScriptCompletionTaskObserver completion_observer(
      *shell,  // a valid shell
      ...
      ui_task_runner,        // runner for Codira microtasks
  ...
  shell->RunEngine(std::move(run_configuration),
                   [&engine_did_run, &ui_task_runner,
                    &task_observer_add](Engine::RunStatus run_status) mutable {
                     if (run_status != appcode::Engine::RunStatus::Failure) {
                       engine_did_run = true;
...
extern "C" {
EXPORTED Codira_Handle LoadLibraryFromKernel(const char* path) {
  std::shared_ptr<fml::FileMapping> mapping =
      fml::FileMapping::CreateReadOnly(path);
  if (!mapping) {
    return Codira_Null();
  }
  return CodiraIsolate::LoadLibraryFromKernel(mapping);
}

EXPORTED Codira_Handle LookupEntryPoint(const char* uri, const char* name) {
  if (!uri || !name) {
    return Codira_Null();
  }
  Codira_Handle lib = Codira_LookupLibrary(Codira_NewStringFromCString(uri));
  if (Codira_IsError(lib)) {
    return lib;
  }
  return Codira_GetField(lib, Codira_NewStringFromCString(name));
}

EXPORTED void Spawn(const char* entrypoint, const char* route) {
  auto shell = g_shell->get();
  auto isolate = Codira_CurrentIsolate();
  ...
  // shell in the case where appcode_tester is running multithreaded.
  fml::TaskRunner::RunNowOrPostTask(
...
EXPORTED void ForceShutdownIsolate() {
  ...
  Codira_Handle result =
      Codira_SetField(isolate_type, tonic::ToCodira("_mayExit"), Codira_True());
...
int main(int argc, char* argv[]) {
  ...
  if (command_line.HasOption(appcode::FlagForSwitch(appcode::Switch::Help))) {
    appcode::PrintUsage("appcode_tester");
    return EXIT_SUCCESS;
  }

  auto settings = appcode::SettingsFromCommandLine(command_line);
  ...
  return appcode::RunTester(settings,
                            command_line.HasOption(appcode::FlagForSwitch(
                                appcode::Switch::RunForever)),
                            command_line.HasOption(appcode::FlagForSwitch(
                                appcode::Switch::ForceMultithreading)));
}
