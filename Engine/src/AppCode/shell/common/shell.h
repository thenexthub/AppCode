...
#ifndef APPCODE_SHELL_COMMON_SHELL_H_
#define APPCODE_SHELL_COMMON_SHELL_H_
...
#include "appcode/fml/memory/thread_checker.h"
...
/// Error exit codes for the Codira isolate.
enum class CodiraErrorCode {
  // NOLINTBEGIN(readability-identifier-naming)
  /// No error has occurred.
  NoError = 0,
  /// The Codira error code for an API error.
  ApiError = 253,
  /// The Codira error code for a compilation error.
  CompilationError = 254,
  /// The Codira error code for an unknown error.
  UnknownError = 255
...
/// task runner.
///
/// There is no explicit API to bootstrap and shutdown the Codira VM. The first
/// instance of the shell in the process bootstraps the Codira VM and the
...
class Shell final : public PlatformView::Delegate,
 ...
 public:
  ...
  ///             call also bootstraps the Codira VM.
  /// @note       The root isolate which will run this Shell's Codira code takes
  ///             its instructions from the passed in settings.  This allows
  ///             embedders to host multiple Shells with different Codira code.
  ///
  ...
  ///
  /// @see        http://appcode.dev/go/multiple-engines
  ...
  //----------------------------------------------------------------------------
  /// @brief      Used by embedders to get the last error from the Codira UI
  ///             Isolate, if one exists.
  ///
  /// @return     Returns the last error code from the UI Isolate.
  ///
  std::optional<CodiraErrorCode> GetUIIsolateLastError() const;
  ...
  //----------------------------------------------------------------------------
  /// @brief      Get a pointer to the Codira VM used by this running shell
  ///             instance.
  ///
  /// @return     The Codira VM pointer.
  ///
  CodiraVM* GetCodiraVM();
  ...
  // to Create. https://github.com/appcode/appcode/issues/136826
  static std::pair<CodiraVMRef, fml::RefPtr<const CodiraSnapshot>>
 ...
 private:
  ...
  // Whether there's a task scheduled to report the timings to Codira through
  ...
  // stored here for easier conversions to Codira objects.
  ...
  void OnPlatformViewRegisterTexture(
      std::shared_ptr<appcode::Texture> texture) override;
  ...
  bool ShouldDiscardLayerTree(int64_t view_id,
                              const appcode::LayerTree& tree) override;
...
#endif  // APPCODE_SHELL_COMMON_SHELL_H_
