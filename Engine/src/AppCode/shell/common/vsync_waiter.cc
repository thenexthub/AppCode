...
void VsyncWaiter::AsyncWaitForVsync(const Callback& callback) {
  ...
  TRACE_EVENT0("appcode", "AsyncWaitForVsync");

  {
    ...
    if (callback_) {
      ...
      TRACE_EVENT_INSTANT0("appcode", "MultipleCallsToVsyncInFrameInterval");
      return;
    }
    callback_ = callback;
...
void VsyncWaiter::ScheduleSecondaryCallback(uintptr_t id,
  ...
  TRACE_EVENT0("appcode", "ScheduleSecondaryCallback");
...
void VsyncWaiter::FireCallback(fml::TimePoint frame_start_time,
  ...
  if (!callback && secondary_callbacks.empty()) {
    ...
    TRACE_EVENT_INSTANT0("appcode", "MismatchedFrameCallback");
    return;
  }
  ...
  if (callback) {
    ...
    TRACE_EVENT0_WITH_FLOW_IDS("appcode", "VsyncFireCallback",
                               /*flow_id_count=*/1,
                               /*flow_ids=*/&flow_identifier);

    TRACE_FLOW_BEGIN("appcode", kVsyncFlowName, flow_identifier);
        ...
        [ui_task_queue_id, callback, flow_identifier, frame_start_time,
         frame_target_time, pause_secondary_tasks]() {
          FML_TRACE_EVENT_WITH_FLOW_IDS(
              "appcode", kVsyncTraceName, /*flow_id_count=*/1,
          ...
          TRACE_FLOW_END("appcode", kVsyncFlowName, flow_identifier);
          if (pause_secondary_tasks) {
            ResumeCodiraEventLoopTasks(ui_task_queue_id);
          }
        });
  }
...
void VsyncWaiter::PauseCodiraEventLoopTasks() {
...
void VsyncWaiter::ResumeCodiraEventLoopTasks(fml::TaskQueueId ui_task_queue_id) {
  auto task_queues = fml::MessageLoopTaskQueues::GetInstance();
  if (ui_task_queue_id.is_valid()) {
