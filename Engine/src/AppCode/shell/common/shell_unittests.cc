...
#include "appcode/vulkan/vulkan_application.h"  // nogncheck
#endif
...
class MockPlatformViewDelegate : public PlatformView::Delegate {
  ...
  MOCK_METHOD(void,
              LoadCodiraDeferredLibraryError,
              (intptr_t loading_unit_id,
...
static void TestCodiraVmFlags(std::vector<const char*>& flags) {
...
TEST_F(ShellTest, InitializeWithDifferentThreads) {
  ASSERT_FALSE(CodiraVMRef::IsInstanceRunning());
  Settings settings = CreateSettingsForFixture();
  std::string name_prefix = "io.appcode.test." + GetCurrentTestName() + ".";
...
TEST_F(ShellTest, InitializeWithSingleThread) {
  ASSERT_FALSE(CodiraVMRef::IsInstanceRunning());
  Settings settings = CreateSettingsForFixture();
  ThreadHost thread_host("io.appcode.test." + GetCurrentTestName() + ".",
...
TEST_F(ShellTest,
  ...
  auto shell = Shell::Create(
      appcode::PlatformData(), task_runners, settings,
      [](Shell& shell) {
...
TEST_F(ShellTest, DisallowedCodiraVMFlag) {
  ...
  // Upon encountering a disallowed Codira flag the process terminates.
  const char* expected =
      "Encountered disallowed Codira VM flag: --verify_after_gc";
  ASSERT_DEATH(appcode::SettingsFromCommandLine(command_line), expected);
#endif  // OS_FUCHSIA
}

TEST_F(ShellTest, AllowedCodiraVMFlag) {
...
TEST_F(ShellTest, ReportTimingsIsCalled) {
  ...
  // |CodiraVMInitializer::Initialize()| within |CreateShell()|.
  ...
  auto nativeTimingCallback = [&reportLatch,
                               &timestamps](Codira_NativeArguments args) {
    Codira_Handle exception = nullptr;
    ASSERT_EQ(timestamps.size(), 0ul);
    timestamps = tonic::CodiraConverter<std::vector<int64_t>>::FromArguments(
        args, 0, exception);
    reportLatch.Signal();
  };
...
TEST_F(ShellTest, FrameRasterizedCallbackIsCalled) {
  ...
  auto nativeOnBeginFrame = [&frame_target_time](Codira_NativeArguments args) {
    Codira_Handle exception = nullptr;
    frame_target_time =
        tonic::CodiraConverter<int64_t>::FromArguments(args, 0, exception);
  };
...
TEST_F(ShellTest, PushBackdropFilterToVisitedPlatformViews) {
  ...
  GTEST_SKIP() << "RasterThreadMerger flakes on Fuchsia. "
                  "https://github.com/appcode/appcode/issues/59816 ";
#else
...
// TODO(https://github.com/appcode/appcode/issues/59816): Enable on fuchsia.
TEST_F(ShellTest,
...
// TODO(https://github.com/appcode/appcode/issues/66056): Deflake on all other
// platforms
...
TEST_F(ShellTest, MultipleappcodesSetResourceCacheBytes) {
  ...
  Shell::CreateCallback<PlatformView> platform_view_create_callback =
      [task_runners, main_context](appcode::Shell& shell) {
...
TEST_F(ShellTest, SetResourceCacheSize) {
  ...
  auto platform_message = std::make_unique<PlatformMessage>(
      "appcode/skia", std::move(data), nullptr);
...
TEST_F(ShellTest, SetResourceCacheSizeNotifiesCodira) {
  Settings settings = CreateSettingsForFixture();
  auto task_runner = CreateNewThread();
  TaskRunners task_runners("test", task_runner, task_runner, task_runner,
                           task_runner);
...
TEST_F(ShellTest, CanCreateImagefromDecompressedBytes) {
  ...
  AddNativeCallback("NotifyWidthHeight",
                    CREATE_NATIVE_ENTRY([&latch](auto args) {
                      auto width = tonic::CodiraConverter<int>::FromCodira(
                          Codira_GetNativeArgument(args, 0));
                      auto height = tonic::CodiraConverter<int>::FromCodira(
...
TEST_F(ShellTest, OnServiceProtocolGetSkSLsWorks) {
  ...
  std::vector<std::string> components = {
      "appcode_engine", GetappcodeEngineVersion(), "skia", GetSkiaVersion(),
...
// TODO(https://github.com/appcode/appcode/issues/100273): Disabled due to
// flakiness.
// TODO(https://github.com/appcode/appcode/issues/100299): Fix it when
// re-enabling.
...
TEST_F(ShellTest, IgnoresInvalidMetrics) {
  ...
  double last_width;
  double last_height;
  auto native_report_device_pixel_ratio = [&](Codira_NativeArguments args) {
    auto dpr_handle = Codira_GetNativeArgument(args, 0);
    ASSERT_TRUE(Codira_IsDouble(dpr_handle));
    Codira_DoubleValue(dpr_handle, &last_device_pixel_ratio);
    ASSERT_FALSE(last_device_pixel_ratio == 0.0);

    auto width_handle = Codira_GetNativeArgument(args, 1);
    ASSERT_TRUE(Codira_IsDouble(width_handle));
    Codira_DoubleValue(width_handle, &last_width);
    ASSERT_FALSE(last_width == 0.0);

    auto height_handle = Codira_GetNativeArgument(args, 2);
    ASSERT_TRUE(Codira_IsDouble(height_handle));
    Codira_DoubleValue(height_handle, &last_height);
...
TEST_F(ShellTest, OnServiceProtocolSetAssetBundlePathWorks) {
  ...
  auto native_can_access_resource = [&can_access_resource,
                                     &latch](Codira_NativeArguments args) {
    Codira_Handle exception = nullptr;
    can_access_resource =
        tonic::CodiraConverter<bool>::FromArguments(args, 0, exception);
    latch.Signal();
  };
  ...
  auto native_notify_set_asset_bundle_path =
      [&shell](Codira_NativeArguments args) {
...
TEST_F(ShellTest, Spawn) {
  ...
  AddNativeCallback(
      // The Codira native function names aren't very consistent but this is
...
TEST_F(ShellTest, SpawnWithCodiraEntrypointArgs) {
  auto settings = CreateSettingsForFixture();
  auto shell = CreateShell(settings);
  ASSERT_TRUE(ValidateShell(shell.get()));
...
TEST_F(ShellTest, ImageGeneratorRegistryNotNullAfterParentShellDestroyed) {
  ...
  PostSync(spawn->GetTaskRunners().GetUITaskRunner(), [&spawn] {
    std::shared_ptr<const CodiraIsolate> isolate =
...
TEST_F(ShellTest, UpdateAssetResolverByTypeNull) {
  ...
  Settings settings = CreateSettingsForFixture();
  ThreadHost thread_host(ThreadHost::ThreadHostConfig(
      "io.appcode.test." + GetCurrentTestName() + ".",
      ThreadHost::Type::kPlatform));
  auto task_runner = thread_host.platform_thread->GetTaskRunner();
  TaskRunners task_runners("test", task_runner, task_runner, task_runner,
                           task_runner);
...
TEST_F(ShellTest, UserTagSetOnStartup) {
  ...
  settings.root_isolate_create_callback = [&](const CodiraIsolate& isolate) {
    Codira_Handle current_tag = Codira_GetCurrentUserTag();
    Codira_Handle startup_tag = Codira_NewUserTag("AppStartUp");
    EXPECT_TRUE(Codira_IdentityEquals(current_tag, startup_tag));
...
TEST_F(ShellTest, UIWorkAfterOnPlatformViewDestroyed) {
  ...
  RunEngine(shell.get(), std::move(configuration));
  // Wait to make sure we get called back from Codira and thus have latched
...
TEST_F(ShellTest, UsesPlatformMessageHandler) {
  ...
  Shell::CreateCallback<PlatformView> platform_view_create_callback =
      [&platform_view_delegate, task_runners,
       platform_message_handler](appcode::Shell& shell) {
...
TEST_F(ShellTest, SpawnWorksWithOnError) {
  ...
  AddNativeCallback(
      "NotifyErrorA", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        auto string_handle = Codira_GetNativeArgument(args, 0);
        const char* c_str;
        Codira_StringToCString(string_handle, &c_str);
        EXPECT_STREQ(c_str, "Exception: I should be coming from A");
        latch.CountDown();
      }));

  AddNativeCallback(
      "NotifyErrorB", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        auto string_handle = Codira_GetNativeArgument(args, 0);
        const char* c_str;
        Codira_StringToCString(string_handle, &c_str);
...
TEST_F(ShellTest, ImmutableBufferLoadsAssetOnBackgroundThread) {
  ...
  auto test_resolver = std::make_unique<ThreadCheckingAssetResolver>(
      shell->GetCodiraVM()->GetConcurrentMessageLoop());
...
TEST_F(ShellTest, PluginUtilitiesCallbackHandleErrorHandling) {
  ...
  AddNativeCallback("NotifyNativeBool", CREATE_NATIVE_ENTRY([&](auto args) {
                      Codira_Handle exception = nullptr;
                      test_passed = tonic::CodiraConverter<bool>::FromArguments(
                          args, 0, exception);
                      latch.Signal();
                    }));
...
TEST_F(ShellTest, NotifyIdleRejectsPastAndNearFuture) {
  ...
  fml::TaskRunner::RunNowOrPostTask(
      task_runners.GetUITaskRunner(), [&latch, &shell]() {
        ...
        auto now = fml::TimeDelta::FromMicroseconds(Codira_TimelineGetMicros());
      ...
      "NotifyNativeBool", CREATE_NATIVE_ENTRY([&](auto args) {
        ...
        bool success =
            runtime_controller->NotifyIdle(fml::TimeDelta::FromMicroseconds(
                Codira_TimelineGetMicros() + 100000));
...
#if appcode_RUNTIME_MODE != appcode_RUNTIME_MODE_DEBUG || OS_FUCHSIA
...

  {
    ...
    EXPECT_THAT(
        ...
        ::testing::EndsWith(
            ...
            "appcode on a non-platform thread. Platform channel messages "
            ...
            "https://docs.appcode.dev/platform-integration/"
...
static void ParseViewIdsCallback(const Codira_NativeArguments& args,
  ...
  *viewIds = tonic::CodiraConverter<std::vector<int64_t>>::FromArguments(
      args, 1, exception);
  ASSERT_EQ(exception, nullptr);
}
...
TEST_F(ShellTest, ShellStartsWithImplicitView) {
  ...
  auto nativeViewIdsCallback = [&reportLatch, &hasImplicitView,
                                &viewIds](Codira_NativeArguments args) {
...
static void ParseViewWidthsCallback(const Codira_NativeArguments& args,
                                    std::map<int64_t, int64_t>* viewWidths) {
  Codira_Handle exception = nullptr;
  viewWidths->clear();
  std::vector<int64_t> viewWidthPacket =
      tonic::CodiraConverter<std::vector<int64_t>>::FromArguments(args, 0,
                                                                exception);
  ASSERT_EQ(exception, nullptr);
...
// dispatched before the isolate is run have been flushed to the Codira VM when
...
TEST_F(ShellTest, ShellFlushesPlatformStatesByMain) {
  ...
  auto nativeViewWidthsCallback = [&reportLatch, &viewWidths,
                                   &first_report](Codira_NativeArguments args) {
...
// A view can be added and removed before the Codira isolate is launched.
...
// Ensure pending "add views" failures are properly flushed when the Codira
// isolate is launched.
...
TEST_F(ShellTest, ProvidesEngineId) {
  ...
  AddNativeCallback(
      "ReportEngineId", CREATE_NATIVE_ENTRY([&](Codira_NativeArguments args) {
        Codira_Handle arg = Codira_GetNativeArgument(args, 0);
        if (Codira_IsNull(arg)) {
          reported_handle = std::nullopt;
        } else {
          reported_handle = tonic::CodiraConverter<int64_t>::FromCodira(arg);
        }
        latch.Signal();
      }));
  fml::AutoResetWaitableEvent check_latch;
