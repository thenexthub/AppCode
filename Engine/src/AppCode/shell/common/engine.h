...
#ifndef APPCODE_SHELL_COMMON_ENGINE_H_
#define APPCODE_SHELL_COMMON_ENGINE_H_
...
#include "appcode/shell/common/animator.h"
...
/// but it is not in the |CodiraIsolate::Phase::Running| phase yet. It only moves
...
///           (usually in Codira code) to peer into the lower level functionality.
...
class Engine final : public RuntimeDelegate, PointerDataDispatcher::Delegate {
 public:
  ...
  enum class RunStatus {
    ...
    /// `CodiraIsolate::Phase::Running` phase with its entry-point invocation
    ...
    /// not be moved into the `CodiraIsolate::Phase::Running` phase.
    ///
    ...
    ///   AOT mode operation of the Codira VM.
    ///
    Failure,
  ...
  class Delegate {
   public:
    ...
    ///             sent back to Codira code via `Engine::ReportTimings`.
    ///
    ...
    ///                              collected and send back to Codira.
    ///
  ...
  /// @see        Engine::Engine
  /// @see        CodiraIsolate::SpawnIsolate
  ///
  ...
  //----------------------------------------------------------------------------
  /// @brief      Moves the root isolate to the `CodiraIsolate::Phase::Running`
  ...
  ///             via `Codira_TimelineGetMicros` from `dart_api.h` for
  ///             consistency. In reality, the clocks used by Codira, FML and
  ///             std::steady_clock are all the same and the timepoints can be
  ///             converted from on clock type to another.
  ///
  ///             The Codira VM uses this notification to schedule book-keeping
  ...
  ///             collection, just gives the Codira VM more hints about opportune
  ...
  //----------------------------------------------------------------------------
  /// @brief      Notifies the engine that the attached appcode view has been
  ///             destroyed.
  ///             This enables the engine to notify the Codira VM so it can do
  ...
  ///             reported back to Codira code. The shell reports this extra
  ...
  //  Codira but hurts readability. The phases and the units of the timepoints are
  ...
  ///
  Codira_Port GetUIIsolateMainPort();
  ...
  //----------------------------------------------------------------------------
  /// @brief      It is an unexpected challenge to determine when a Codira
  ...
  //----------------------------------------------------------------------------
  /// @brief      Errors that are unhandled on the Codira message loop are kept
...
#endif  // APPCODE_SHELL_COMMON_ENGINE_H_
