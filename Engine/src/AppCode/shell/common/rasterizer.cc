...
Rasterizer::Rasterizer(Delegate& delegate,
    ...
    : delegate_(delegate),
      ...
      compositor_context_(std::make_unique<appcode::CompositorContext>(*this)),
...
std::shared_ptr<appcode::TextureRegistry> Rasterizer::GetTextureRegistry() {
...
appcode::LayerTree* Rasterizer::GetLastLayerTree(int64_t view_id) {
  auto found = view_records_.find(view_id);
  if (found == view_records_.end()) {
    return nullptr;
  }
...
DrawStatus Rasterizer::Draw(const std::shared_ptr<FramePipeline>& pipeline) {
  TRACE_EVENT0("appcode", "GPURasterizer::Draw");
  if (raster_thread_merger_ &&
...
std::unique_ptr<Rasterizer::GpuImageResult> Rasterizer::MakeSkiaGpuImage(
  ...
  return nullptr;
#else   // SLIMPELLER
  TRACE_EVENT0("appcode", "Rasterizer::MakeGpuImage");
  FML_DCHECK(display_list);
...
Rasterizer::DoDrawResult Rasterizer::DoDraw(
    std::unique_ptr<FrameTimingsRecorder> frame_timings_recorder,
    std::vector<std::unique_ptr<LayerTreeTask>> tasks) {
  TRACE_EVENT_WITH_FRAME_NUMBER(frame_timings_recorder, "appcode",
...
// see: https://github.com/appcode/appcode/issues/56598
...
Rasterizer::DoDrawResult Rasterizer::DrawToSurfaces(
    ...
    std::vector<std::unique_ptr<LayerTreeTask>> tasks) {
  TRACE_EVENT0("appcode", "Rasterizer::DrawToSurfaces");
  FML_DCHECK(surface_);
  frame_timings_recorder.AssertInState(FrameTimingsRecorder::State::kBuildEnd);
...
std::unique_ptr<FrameItem> Rasterizer::DrawToSurfacesUnsafe(
  ...
  // TODO (https://github.com/appcode/appcode/issues/105596): this can be in
  ...
  // See https://github.com/appcode/appcode/issues/135530, item 4.
  frame_timings_recorder.RecordRasterEnd(
...
DrawSurfaceStatus Rasterizer::DrawToSurfaceUnsafe(
  ...
  if (external_view_embedder_) {
    external_view_embedder_->PrepareappcodeView(
  ...
  if (compositor_frame) {
    ...
    if (frame->framebuffer_info().supports_partial_repaint) {
      // Disable partial repaint if external_view_embedder_ SubmitappcodeView is
    ...
    if (external_view_embedder_ &&
      ...
      external_view_embedder_->SubmitappcodeView(
...
static sk_sp<SkData> ScreenshotLayerTreeAsPicture(
    appcode::LayerTree* tree,
    appcode::CompositorContext& compositor_context) {
#if SLIMPELLER
  return nullptr;
#else  // SLIMPELLER
  ...
  // https://github.com/appcode/appcode/issues/23435
...
Rasterizer::Screenshot Rasterizer::ScreenshotLastLayerTree(
  ...
  // https://github.com/appcode/appcode/issues/135534
  // https://github.com/appcode/appcode/issues/135535
  auto* layer_tree = GetLastLayerTree(kappcodeImplicitViewId);
...
void Rasterizer::SetResourceCacheMaxBytes(size_t max_bytes, bool from_user) {
  ...
  if (!from_user && user_override_resource_cache_bytes_) {
    ...
    // value for this over the appcode/skia channel.
    return;
  }
