...
static void TestSimulatedInputEvents(
  ...
  auto nativeOnPointerDataPacket = [&events_consumed_at_frame,
                                    ...
                                    &frame_drawn](Codira_NativeArguments args) {
    events_consumed += 1;
  ...
  // https://github.com/appcode/appcode/issues/40863 won't happen again.
...
TEST_F(ShellTest, CanCorrectlyPipePointerPacket) {
  ...
  auto nativeOnPointerDataPacket = [&reportLatch, &result_sequence](
    ...
    result_sequence = tonic::CodiraConverter<std::vector<int64_t>>::FromArguments(
        args, 0, exception);
    reportLatch.Signal();
  };
  // Starts engine.
