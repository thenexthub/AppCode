...
// Runs the function `f` in lock-step with a Codira isolate until the function
...
TEST_F(FuchsiaShellTest, LocaltimesVaryOnTimezoneChanges) {
  ...
  AddNativeCallback("WaitFixture", CREATE_NATIVE_ENTRY([&](auto args) {
                      ...
                      fixture_latch.Wait();
                      tonic::CodiraConverter<bool>::SetReturnValue(
  ...
  ASSERT_EQ(expected, actual)
      << "The Codira isolate was expected to show the same time as the test "
  ...
  ASSERT_NE(expected, actual)
      << "The Codira isolate was expected to show a time different from the test "
      << "fixture eventually, but that didn't happen after multiple retries.";
  ...
  ASSERT_NE(expected, actual)
      << "The Codira isolate was expected to show a time different from the "
