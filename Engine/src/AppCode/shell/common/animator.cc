...
Animator::Animator(Delegate& delegate,
    ...
    : delegate_(delegate),
      ...
      // https://github.com/appcode/engine/pull/9132 for discussion.
...
void Animator::BeginFrame(
  ...
  TRACE_EVENT_ASYNC_END0("appcode", "Frame Request Pending",
  ...
  TRACE_EVENT_WITH_FRAME_NUMBER(frame_timings_recorder_, "appcode",
                                "Animator::BeginFrame", flow_id_count,
                                flow_ids.get());

  while (!trace_flow_ids_.empty()) {
    uint64_t trace_flow_id = trace_flow_ids_.front();
    TRACE_FLOW_END("appcode", "PointerEvent", trace_flow_id);
  ...
  if (!producer_continuation_) {
    ...
    if (!producer_continuation_) {
      ...
      // frame interval.
      TRACE_EVENT0("appcode", "PipelineFull");
      RequestFrame();
      return;
    }
  }
    ...
    task_runners_.GetUITaskRunner()->PostDelayedTask(
        [self = weak_factory_.GetWeakPtr()]() {
          ...
          if (!self->frame_scheduled_) {
            auto now =
                fml::TimeDelta::FromMicroseconds(Codira_TimelineGetMicros());
            if (now > self->dart_frame_deadline_) {
              TRACE_EVENT0("appcode", "BeginFrame idle callback");
...
void Animator::DrawLastLayerTrees(
  ...
  // files.
  TRACE_EVENT0("appcode", "Animator::DrawLastLayerTrees");
...
void Animator::RequestFrame(bool regenerate_layer_trees) {
  if (regenerate_layer_trees && !regenerate_layer_trees_) {
    ...
    // happen.
    TRACE_EVENT_ASYNC_BEGIN0("appcode", "Frame Request Pending",
                             frame_request_number_);
      ...
      reinterpret_cast<uintptr_t>(this), [self = weak_factory_.GetWeakPtr()] {
        ...
        if (!self->frame_scheduled_ && !self->trace_flow_ids_.empty()) {
          ...
          TRACE_EVENT0_WITH_FLOW_IDS(
              "appcode", "Animator::ScheduleMaybeClearTraceFlowIds - callback",
              flow_id_count, flow_ids.get());

          while (!self->trace_flow_ids_.empty()) {
            auto flow_id = self->trace_flow_ids_.front();
            TRACE_FLOW_END("appcode", "PointerEvent", flow_id);
