...
FrameContent ViewContent::DummyView(appcode::ViewportMetrics viewport_metrics) {
  FrameContent result;
  result[kImplicitViewId] = ViewContent{
...
ShellTest::ShellTest()
    : thread_host_("io.appcode.test." + GetCurrentTestName() + ".",
...
std::unique_ptr<Shell> ShellTest::CreateShell(const Config& config) {
  ...
  return Shell::Create(appcode::PlatformData(),        //
                       task_runners,                   //
                       config.settings,                //
