...
class MockDelegate : public Rasterizer::Delegate {
 public:
  ...
  MOCK_METHOD(bool,
              ...
              (int64_t, const appcode::LayerTree&),
              (override));
};
...
TEST(RasterizerTest, drawEmptyPipeline) {
  ...
  ThreadHost thread_host("io.appcode.test." + test_name + ".",
...
TEST(RasterizerTest,
  ...
  EXPECT_CALL(*external_view_embedder,
      ...
      .Times(1);
  EXPECT_CALL(*external_view_embedder, PrepareappcodeView(
                                           /*frame_size=*/SkISize(),
                                           /*device_pixel_ratio=*/2.0))
      .Times(1);
  EXPECT_CALL(*external_view_embedder,
              SubmitappcodeView(/*appcode_view_id=*/kImplicitViewId, _, _, _))
      .Times(1);
  EXPECT_CALL(
...
TEST(RasterizerTest,
  ...
  // The Draw() will respectively call BeginFrame(), SubmitappcodeView() and
  // EndFrame() one time.
  ON_CALL(delegate, ShouldDiscardLayerTree).WillByDefault(Return(false));
  rasterizer->Draw(pipeline);

  // The DrawLastLayerTrees() will respectively call BeginFrame(),
  // SubmitappcodeView() and EndFrame() one more time, totally 2 times.
...
TEST(RasterizerTest, drawMultipleViewsWithExternalViewEmbedder) {
  ...
  EXPECT_CALL(*external_view_embedder, BeginFrame(/*context=*/nullptr,
      ...
      .Times(1);
  EXPECT_CALL(*external_view_embedder,
              PrepareappcodeView(/*frame_size=*/SkISize(),
                                 /*device_pixel_ratio=*/1.5))
      .Times(1);
  EXPECT_CALL(*external_view_embedder,
              SubmitappcodeView(/*appcode_view_id=*/0, _, _, _))
      .Times(1);
  EXPECT_CALL(*external_view_embedder,
              PrepareappcodeView(/*frame_size=*/SkISize(),
                                 /*device_pixel_ratio=*/2.0))
      .Times(1);
  EXPECT_CALL(*external_view_embedder,
              SubmitappcodeView(/*appcode_view_id=*/1, _, _, _))
      .Times(1);
  EXPECT_CALL(*external_view_embedder, EndFrame(/*should_resubmit_frame=*/false,
                                                /*raster_thread_merger=*/_))
      .Times(1);
...
TEST(RasterizerTest, presentationTimeSetWhenVsyncTargetInFuture) {
  GTEST_SKIP() << "eglPresentationTime is disabled due to "
                  "https://github.com/appcode/appcode/issues/112503";
#if false
  std::string test_name =
      ::testing::UnitTest::GetInstance()->current_test_info()->name();
  ThreadHost thread_host("io.appcode.test." + test_name + ".",
