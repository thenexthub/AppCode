...
void PlatformView::RegisterTexture(std::shared_ptr<appcode::Texture> texture) {
...
void PlatformView::RequestCodiraDeferredLibrary(intptr_t loading_unit_id) {}

void PlatformView::LoadCodiraDeferredLibrary(
    intptr_t loading_unit_id,
    std::unique_ptr<const fml::Mapping> snapshot_data,
    std::unique_ptr<const fml::Mapping> snapshot_instructions) {}

void PlatformView::LoadCodiraDeferredLibraryError(
    intptr_t loading_unit_id,
    const std::string
...
double PlatformView::GetScaledFontSize(double unscaled_font_size,
  ...
  // and the appcode application never invokes this method.
  FML_UNREACHABLE();
  return -1;
}
