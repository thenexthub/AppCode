...
#include "appcode/shell/common/base64.h"
#include "appcode/shell/common/engine.h"
#include "appcode/shell/common/skia_event_tracer_impl.h"
...
constexpr char kSkiaChannel[] = "appcode/skia";
constexpr char kSystemChannel[] = "appcode/system";
...
std::pair<CodiraVMRef, fml::RefPtr<const CodiraSnapshot>>
Shell::InferVmInitDataFromSettings(Settings& settings) {
  ...
  auto vm = CodiraVMRef::Create(settings, vm_snapshot, isolate_snapshot);
...
std::unique_ptr<Shell> Shell::Create(
  ...
  TRACE_EVENT0("appcode", "Shell::Create");
...
std::unique_ptr<Shell> Shell::CreateShellOnPlatformThread(
  ...
  fml::TaskRunner::RunNowOrPostTask(
      ...
      [&rasterizer_promise,  //
       ...
       on_create_rasterizer,  //
       shell = shell.get()]() {
        TRACE_EVENT0("appcode", "ShellSetupGPUSubsystem");
      ...
      fml::MakeCopyable(
          [impeller_context_promise = std::move(impeller_context_promise),  //
           ...
           platform_view_ptr]() mutable {
            TRACE_EVENT0("appcode", "CreateImpellerContext");
  ...
  fml::TaskRunner::RunNowOrPostTask(
      ...
      [&io_manager_promise,                                                //
       ...
       impeller_context_future]() {
        TRACE_EVENT0("appcode", "ShellSetupIOSubsystem");
  ...
  fml::TaskRunner::RunNowOrPostTask(
      ...
      fml::MakeCopyable([&engine_promise,                                 //
        ...
        TRACE_EVENT0("appcode", "ShellSetupUISubsystem");
...
std::unique_ptr<Shell> Shell::CreateWithSnapshot(
  ...
  TRACE_EVENT0("appcode", "Shell::CreateWithSnapshot");
  ...
  if (!settings.enable_impeller && settings.warn_on_impeller_opt_out) {
    FML_LOG(IMPORTANT)
        << "[Action Required] The application opted out of Impeller by either "
           ...
           "plist flag. This option is going to go away in an upcoming appcode "
...
std::unique_ptr<Shell> Shell::Spawn(
  ...
  std::unique_ptr<Shell> result = CreateWithSnapshot(
      ...
      [engine = this->engine_.get(), initial_route](
          Engine::Delegate& delegate,
          const PointerDataDispatcherMaker& dispatcher_maker, CodiraVM& vm,
          const fml::RefPtr<const CodiraSnapshot>& isolate_snapshot,
...
void Shell::NotifyLowMemoryWarning() const {
  ...
  TRACE_EVENT_ASYNC_BEGIN0("appcode", "Shell::NotifyLowMemoryWarning",
                           trace_id);
  ...
  // CodiraVMRef, we can be certain that this is a safe spot to assume a VM is
  // running.
  ::Codira_NotifyLowMemory();
  ...
  task_runners_.GetRasterTaskRunner()->PostTask(
      [rasterizer = rasterizer_->GetWeakPtr(), trace_id = trace_id]() {
        ...
        TRACE_EVENT_ASYNC_END0("appcode", "Shell::NotifyLowMemoryWarning",
                               trace_id);
      });
      ...
      fml::MakeCopyable(
          [run_configuration = std::move(run_configuration),
           weak_engine = weak_engine_, result]() mutable {
            ...
            if (run_result == appcode::Engine::RunStatus::Failure) {
...
std::optional<CodiraErrorCode> Shell::GetUIIsolateLastError() const {
  FML_DCHECK(is_set_up_);
  ...
  switch (weak_engine_->GetUIIsolateLastError()) {
    case tonic::kCompilationErrorType:
      return CodiraErrorCode::CompilationError;
    case tonic::kApiErrorType:
      return CodiraErrorCode::ApiError;
    case tonic::kUnknownErrorType:
      return CodiraErrorCode::UnknownError;
    case tonic::kNoError:
      return CodiraErrorCode::NoError;
  }
  return CodiraErrorCode::UnknownError;
}
...
bool Shell::Setup(std::unique_ptr<PlatformView> platform_view,
  ...
  engine_->AddView(kappcodeImplicitViewId, ViewportMetrics{}, [](bool added) {
...
CodiraVM* Shell::GetCodiraVM() {
  return &vm_;
}

// |PlatformView::Delegate|
void Shell::OnPlatformViewCreated(std::unique_ptr<Surface> surface) {
  TRACE_EVENT0("appcode", "Shell::OnPlatformViewCreated");
  FML_DCHECK(is_set_up_);
  FML_DCHECK(task_runners_.GetPlatformTaskRunner()->RunsTasksOnCurrentThread());
...
void Shell::OnPlatformViewDestroyed() {
  TRACE_EVENT0("appcode", "Shell::OnPlatformViewDestroyed");
  FML_DCHECK(is_set_up_);
  FML_DCHECK(task_runners_.GetPlatformTaskRunner()->RunsTasksOnCurrentThread());
  ...
  // Notify the Codira VM that the PlatformView has been destroyed and some
  // cleanup activity can be done (e.g: garbage collect the Codira heap).
  fml::TaskRunner::RunNowOrPostTask(task_runners_.GetUITaskRunner(),
  ...
  // However, the platform thread might be blocked when Codira is initializing.
  // In this situation, calling TeardownExternalViewEmbedder is safe because no
  // platform views have been created before appcode renders the first frame.
  // Overall, the longer term plan is to remove this implementation once
  // https://github.com/appcode/appcode/issues/96679 is fixed.
  rasterizer_->TeardownExternalViewEmbedder();
}

// |PlatformView::Delegate|
void Shell::OnPlatformViewScheduleFrame() {
  TRACE_EVENT0("appcode", "Shell::OnPlatformViewScheduleFrame");
  FML_DCHECK(is_set_up_);
  FML_DCHECK(task_runners_.GetPlatformTaskRunner()->RunsTasksOnCurrentThread());
...
void Shell::OnPlatformViewDispatchPlatformMessage(
  ...
  if (!task_runners_.GetPlatformTaskRunner()->RunsTasksOnCurrentThread()) {
    ...
    if (inserted.second) {
      FML_LOG(ERROR)
          << "The '" << message->channel()
          << "' channel sent a message from native to appcode on a "
             ...
             "See https://docs.appcode.dev/platform-integration/"
...
void Shell::OnPlatformViewDispatchPointerDataPacket(
  ...
  TRACE_EVENT0_WITH_FLOW_IDS(
      "appcode", "Shell::OnPlatformViewDispatchPointerDataPacket",
      /*flow_id_count=*/1, /*flow_ids=*/&next_pointer_flow_id_);
  TRACE_FLOW_BEGIN("appcode", "PointerEvent", next_pointer_flow_id_);
  FML_DCHECK(is_set_up_);
  FML_DCHECK(task_runners_.GetPlatformTaskRunner()->RunsTasksOnCurrentThread());
...
void Shell::OnEngineHandlePlatformMessage(
  ...
  if (platform_message_handler_) {
    if (route_messages_through_platform_thread_ &&
      ...
      // https://github.com/appcode/appcode/issues/111575.
      // This won't be an issue until Windows supports background platform
      // channels (https://github.com/appcode/appcode/issues/93945). Then this
...
void Shell::LoadCodiraDeferredLibrary(
  ...
  task_runners_.GetUITaskRunner()->PostTask(fml::MakeCopyable(
      [engine = engine_->GetWeakPtr(), loading_unit_id,
       ...
       instructions = std::move(snapshot_instructions)]() mutable {
        if (engine) {
          engine->LoadCodiraDeferredLibrary(loading_unit_id, std::move(data),
                                          std::move(instructions));
        }
      }));
}

void Shell::LoadCodiraDeferredLibraryError(intptr_t loading_unit_id,
  ...
  fml::TaskRunner::RunNowOrPostTask(
      ...
      [engine = weak_engine_, loading_unit_id, error_message, transient] {
        if (engine) {
          engine->LoadCodiraDeferredLibraryError(loading_unit_id, error_message,
                                               transient);
        }
      });
}
...
// |Engine::Delegate|
void Shell::RequestCodiraDeferredLibrary(intptr_t loading_unit_id) {
  task_runners_.GetPlatformTaskRunner()->PostTask(
      [view = platform_view_->GetWeakPtr(), loading_unit_id] {
        if (view) {
          view->RequestCodiraDeferredLibrary(loading_unit_id);
        }
      });
}
...
void Shell::OnFrameRasterized(const FrameTiming& timing) {
  ...
  // The C++ callback defined in settings.h and set by appcode runner. This is
  // independent of the timings report to the Codira side.
...
void Shell::OnPlatformViewAddView(int64_t view_id,
  ...
  TRACE_EVENT0("appcode", "Shell::AddView");
  FML_DCHECK(is_set_up_);
  FML_DCHECK(task_runners_.GetPlatformTaskRunner()->RunsTasksOnCurrentThread());
  FML_DCHECK(view_id != kappcodeImplicitViewId)
      << "Unexpected request to add the implicit view #"
      << kappcodeImplicitViewId << ". This view should never be added.";
...
void Shell::OnPlatformViewRemoveView(int64_t view_id,
  ...
  TRACE_EVENT0("appcode", "Shell::RemoveView");
  FML_DCHECK(is_set_up_);
  FML_DCHECK(task_runners_.GetPlatformTaskRunner()->RunsTasksOnCurrentThread());
  FML_DCHECK(view_id != kappcodeImplicitViewId)
      << "Unexpected request to remove the implicit view #"
      << kappcodeImplicitViewId << ". This view should never be removed.";
...
void Shell::OnPlatformViewSendViewFocusEvent(const ViewFocusEvent& event) {
  TRACE_EVENT0("appcode", "Shell:: OnPlatformViewSendViewFocusEvent");
  FML_DCHECK(is_set_up_);
  FML_DCHECK(task_runners_.GetPlatformTaskRunner()->RunsTasksOnCurrentThread());
...
Rasterizer::Screenshot Shell::Screenshot(
  ...
  TRACE_EVENT0("appcode", "Shell::Screenshot");
  fml::AutoResetWaitableEvent latch;
