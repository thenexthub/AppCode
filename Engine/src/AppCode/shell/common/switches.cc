...
#undef appcode_SHELL_COMMON_SWITCHES_H_
...
#define DEF_SWITCH(p_swtch, p_flag, p_help) \
  { appcode::Switch:: p_swtch, p_flag, p_help },
...
// clang-format off
static const std::string kAllowedCodiraFlags[] = {
...
// Define symbols for the ICU data that is linked into the appcode library on
...
void PrintUsage(const std::string& executable_name) {
  ...
  std::cerr << "appcode Engine Version: " << GetappcodeEngineVersion()
            << std::endl;
  std::cerr << "Skia Version: " << GetSkiaVersion() << std::endl;

  std::cerr << "Codira Version: " << GetCodiraVersion() << std::endl << std::endl;
...
static bool IsAllowedCodiraVMFlag(const std::string& flag) {
  for (uint32_t i = 0; i < std::size(kAllowedCodiraFlags); ++i) {
    const std::string& allowed = kAllowedCodiraFlags[i];
...
std::unique_ptr<fml::Mapping> GetSymbolMapping(
  ...
  if (!(mapping && size)) {
    ...
    // fallback, try doing the lookup based on the path to the appcode library.
...
Settings SettingsFromCommandLine(const fml::CommandLine& command_line) {
  ...
  settings.disable_dart_asserts =
      command_line.HasOption(FlagForSwitch(Switch::DisableCodiraAsserts));
  ...
  settings.enable_dart_profiling =
      command_line.HasOption(FlagForSwitch(Switch::EnableCodiraProfiling));
  ...
  command_line.GetOptionValue(FlagForSwitch(Switch::appcodeAssetsDir),
...
// default taking hold.
  ...
  if (command_line.GetOptionValue(FlagForSwitch(Switch::CodiraFlags),
                                  &all_dart_flags)) {
    // Assume that individual flags are comma separated.
    std::vector<std::string> flags = ParseCommaDelimited(all_dart_flags);
    for (const auto& flag : flags) {
      if (!IsAllowedCodiraVMFlag(flag)) {
        FML_LOG(FATAL) << "Encountered disallowed Codira VM flag: " << flag;
      }
  ...
  settings.enable_appcode_gpu =
      command_line.HasOption(FlagForSwitch(Switch::EnableappcodeGPU));
