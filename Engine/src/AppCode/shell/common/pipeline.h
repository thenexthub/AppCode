...
#ifndef APPCODE_SHELL_COMMON_PIPELINE_H_
#define APPCODE_SHELL_COMMON_PIPELINE_H_
...
class Pipeline {
 public:
  ...
  class ProducerContinuation {
   public:
    ...
    ~ProducerContinuation() {
      if (continuation_) {
        ...
        TRACE_EVENT_ASYNC_END0("appcode", "PipelineProduce", trace_id_);
        // The continuation is being dropped on the floor. End the flow.
        TRACE_FLOW_END("appcode", "PipelineItem", trace_id_);
        TRACE_EVENT_ASYNC_END0("appcode", "PipelineItem", trace_id_);
      }
    }
    ...
    [[nodiscard]] PipelineProduceResult Complete(ResourcePtr resource) {
      ...
      if (continuation_) {
        ...
        TRACE_EVENT_ASYNC_END0("appcode", "PipelineProduce", trace_id_);
        TRACE_FLOW_STEP("appcode", "PipelineItem", trace_id_);
      }
      return result;
    }
   ...
   private:
    ...
    ProducerContinuation(const Continuation& continuation, uint64_t trace_id)
      ...
      TRACE_EVENT_ASYNC_BEGIN0_WITH_FLOW_IDS("appcode", "PipelineItem",
                                             trace_id_, /*flow_id_count=*/1,
                                             /*flow_ids=*/&trace_id);
      TRACE_FLOW_BEGIN("appcode", "PipelineItem", trace_id_);
      TRACE_EVENT_ASYNC_BEGIN0("appcode", "PipelineProduce", trace_id_);
    }
  ...
  ProducerContinuation Produce() {
    ...
    ++inflight_;
    FML_TRACE_COUNTER("appcode", "Pipeline Depth",
  ...
  [[nodiscard]] PipelineConsumeResult Consume(const Consumer& consumer) {
    ...
    TRACE_FLOW_END("appcode", "PipelineItem", trace_id);
    TRACE_EVENT_ASYNC_END0("appcode", "PipelineItem", trace_id);
...
#endif  // APPCODE_SHELL_COMMON_PIPELINE_H_
