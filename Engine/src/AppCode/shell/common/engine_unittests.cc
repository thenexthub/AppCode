...
class MockDelegate : public Engine::Delegate {
 public:
  ...
  MOCK_METHOD(std::unique_ptr<std::vector<std::string>>,
              ...
              (override));
  MOCK_METHOD(void, RequestCodiraDeferredLibrary, (intptr_t), (override));
...
class MockRuntimeDelegate : public RuntimeDelegate {
 public:
  ...
  MOCK_METHOD(void,
              Render,
              (int64_t, std::unique_ptr<appcode::LayerTree>, float),
              (override));
  MOCK_METHOD(void,
              UpdateSemantics,
              (int64_t, SemanticsNodeUpdates, CustomAccessibilityActionUpdates),
...
class MockRuntimeController : public RuntimeController {
 public:
  ...
  MOCK_METHOD(void,
              ...
              (override));
  MOCK_METHOD(CodiraVM*, GetCodiraVM, (), (const, override));
...
TEST_F(EngineTest, DispatchPlatformMessageInitialRoute) {
  PostUITaskSync([this] {
    ...
    std::unique_ptr<PlatformMessage> message =
        MakePlatformMessage("appcode/navigation", values, response);
    engine->DispatchPlatformMessage(std::move(message));
...
TEST_F(EngineTest, SpawnSharesFontLibrary) {
  PostUITaskSync([this] {
    ...
    auto mock_runtime_controller =
        std::make_unique<MockRuntimeController>(client, task_runners_);
    auto vm_ref = CodiraVMRef::Create(settings_);
    EXPECT_CALL(*mock_runtime_controller, GetCodiraVM())
...
TEST_F(EngineTest, PassesLoadCodiraDeferredLibraryErrorToRuntime) {
  PostUITaskSync([this] {
    ...
    EXPECT_CALL(*mock_runtime_controller,
                LoadCodiraDeferredLibraryError(error_id, error_message, true))
        .Times(1);
    auto engine = std::make_unique<Engine>(
        /*delegate=*/delegate_,
        /*dispatcher_maker=*/dispatcher_maker_,
    ...
    engine->LoadCodiraDeferredLibraryError(error_id, error_message, true);
  });
}
