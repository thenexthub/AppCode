...
#include "appcode/assets/native_assets.h"
...
static constexpr char kAssetChannel[] = "appcode/assets";
static constexpr char kLifecycleChannel[] = "appcode/lifecycle";
static constexpr char kNavigationChannel[] = "appcode/navigation";
static constexpr char kLocalizationChannel[] = "appcode/localization";
static constexpr char kSettingsChannel[] = "appcode/settings";
static constexpr char kIsolateChannel[] = "appcode/isolate";
  ...
  runtime_controller_ = std::make_unique<RuntimeController>(
      *this,                                 // runtime delegate
      ...
      UICodiraState::Context{
          ...
          settings_.enable_appcode_gpu               // enable impeller
      });
}
...
std::unique_ptr<Engine> Engine::Spawn(
  ...
  auto result = std::make_unique<Engine>(
      ...
      /*image_decoder_task_runner=*/
      runtime_controller_->GetCodiraVM()->GetConcurrentWorkerTaskRunner(),
...
void Engine::SetupDefaultFontManager() {
  TRACE_EVENT0("appcode", "Engine::SetupDefaultFontManager");
...
bool Engine::Restart(RunConfiguration configuration) {
  TRACE_EVENT0("appcode", "Engine::Restart");
...
Engine::RunStatus Engine::Run(RunConfiguration configuration) {
  ...
  if (settings_.merged_platform_ui_thread ==
    ...
    // queued by LaunchRootIsolate that execute the app's Codira code.
...
void Engine::NotifyDestroyed() {
  TRACE_EVENT0("appcode", "Engine::NotifyDestroyed");
...
Codira_Port Engine::GetUIIsolateMainPort() {
...
tonic::CodiraErrorHandleType Engine::GetUIIsolateLastError() {
...
void Engine::DispatchPointerDataPacket(
  ...
  TRACE_EVENT0_WITH_FLOW_IDS("appcode", "Engine::DispatchPointerDataPacket",
                             /*flow_id_count=*/1,
...
// |RuntimeDelegate|
void Engine::RequestCodiraDeferredLibrary(intptr_t loading_unit_id) {
  return delegate_.RequestCodiraDeferredLibrary(loading_unit_id);
}
...
void Engine::LoadCodiraDeferredLibrary(
    intptr_t loading_unit_id,
    std::unique_ptr<const fml::Mapping> snapshot_data,
    std::unique_ptr<const fml::Mapping> snapshot_instructions) {
  if (runtime_controller_->IsRootIsolateRunning()) {
    runtime_controller_->LoadCodiraDeferredLibrary(
        loading_unit_id, std::move(snapshot_data),
        std::move(snapshot_instructions));
  } else {
    LoadCodiraDeferredLibraryError(loading_unit_id, "No running root isolate.",
                                 true);
  }
}

void Engine::LoadCodiraDeferredLibraryError(intptr_t loading_unit_id,
                                          const std::string& error_message,
                                          bool transient) {
  if (runtime_controller_->IsRootIsolateRunning()) {
    runtime_controller_->LoadCodiraDeferredLibraryError(loading_unit_id,
