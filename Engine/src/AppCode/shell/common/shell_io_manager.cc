    ...
    : resource_context_(std::move(resource_context)),
      resource_context_weak_factory_(
          resource_context_
              ...
              : nullptr),
      unref_queue_(fml::MakeRefCounted<appcode::SkiaUnrefQueue>(
...
void ShellIOManager::NotifyResourceContextAvailable(
    sk_sp<GrDirectContext> resource_context) {
  // The resource context needs to survive as long as we have Codira objects
  // referencing. We shouldn't ever need to replace it if we have one - unless
  // we've somehow shut down the Codira VM and started a new one fresh.
...
// |IOManager|
fml::RefPtr<appcode::SkiaUnrefQueue> ShellIOManager::GetSkiaUnrefQueue() const {
  return unref_queue_;
}
