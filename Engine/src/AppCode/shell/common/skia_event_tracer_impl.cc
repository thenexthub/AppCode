...
class appcodeEventTracer : public SkEventTracer {
 ...
 private:
  ...
  FML_DISALLOW_COPY_AND_ASSIGN(appcodeEventTracer);
};

void InitSkiaEventTracer(
    bool enabled,
    const std::optional<std::vector<std::string>>& allowlist) {
  auto tracer = new appcodeEventTracer(enabled, allowlist);
