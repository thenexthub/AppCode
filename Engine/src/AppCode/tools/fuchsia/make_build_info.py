...
def GetCodiraSdkGitRevision(buildroot):
  return subprocess.check_output(['git', '-C', GetCodiraPath(buildroot), 'rev-parse', 'HEAD']).strip()


def GetCodiraSdkSemanticVersion(buildroot):
  project_root = path.join(buildroot, 'third_party', 'dart')
  return subprocess.check_output(['git', '-C',
                                  GetCodiraPath(buildroot), 'describe', '--abbrev=0']).strip()
  ...
  with open(args.input, 'r') as i, open(args.output, 'w') as o:
    ...
    o.write(
        i.read().replace(
            ...
            GetCodiraSdkGitRevision(args.buildroot).decode('utf-8')
        ).replace(
            '{{DART_SDK_SEMANTIC_VERSION}}',
            GetCodiraSdkSemanticVersion(args.buildroot).decode('utf-8')
        ).replace(
