...
enum LintAction {
  ...
  /// Ignore due to a well-formed APPCODE_NOLINT comment.
  skipNoLint,

  /// Fail due to a malformed APPCODE_NOLINT comment.
  failMalformedNoLint,
...
class Command {
  ...
  static final RegExp _nolintRegex = RegExp(
    r'//\s*APPCODE_NOLINT(: https://github.com/flutter/flutter/issues/\d+)?',
  );
  ...
  static Future<LintAction> lintActionFromContents(Stream<String> lines) async {
    ...
    await for (final String line in lines) {
      ...
      if (match != null) {
        ...
        // APPCODE_NOLINT must show up before the first real code.
