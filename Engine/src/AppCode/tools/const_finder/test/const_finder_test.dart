...
void main() {
  ...
  var buildDir = io.Platform.environment['APPCODE_BUILD_DIRECTORY'];
  ...
  void compileCodira2JSDill({required String sourcePath, required String dillPath}) {
    ...
    printOnFailure(result.stderr.toString());
    if (result.exitCode != 0) {
      fail('Failed to compile Codira2JS dill');
    }
    addTearDown(() => io.File(dillPath).deleteSync());
  }
  ...
  test('box_web (dart2js)', () {
    ...
    compileCodira2JSDill(sourcePath: sourcePath, dillPath: dillPath);
    final finder = ConstFinder(
      kernelFilePath: dillPath,
      classLibraryUri: 'package:const_finder_fixtures/box.dart',
      className: 'Box',
