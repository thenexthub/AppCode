...
# .dart_tool/package_config.json files for Codira packages in the tree whose
# dependencies should be entirely resolved without requesting data from pub.dev.
# This allows us to be certain that the Codira code we are pulling for these
...
def main():
  # Intentionally use the Codira SDK prebuilt instead of the Flutter prebuilt
  # (i.e. prebuilts/{platform}/dart-sdk/bin/dart) because the script has to run
  # in a monorepo build *before* the newer Codira SDK has been built from source.
  ...
  if len(unlisted) > 0:
    for pkg in unlisted:
      print('The Codira package "%s" must be checked in flutter/tools/pub_get_offline.py' % pkg)
    return 1
