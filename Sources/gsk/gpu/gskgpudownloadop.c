...
{
  GskGpuDownloadOp *this = (GskGpuDownloadOp *) op;

  if (this->create_func)
    this->create_func (this);
 ...
 */
static void
gsk_gpu_download_op_vk_sync_semaphore (GskGpuDownloadOp *this)
{
  ...
  display = gdk_dmabuf_texture_get_display (GDK_DMABUF_TEXTURE (*this->texture));
  fd = gdk_dmabuf_texture_get_dmabuf (GDK_DMABUF_TEXTURE (*this->texture))->planes[0].fd;
  ...
  vkDestroySemaphore (display->vk_device, this->vk_semaphore, NULL);
}
#endif

static void
gsk_gpu_download_op_vk_create (GskGpuDownloadOp *this)
{
  GBytes *bytes;
  guchar *data;

  data = gsk_gpu_buffer_map (this->buffer);
  bytes = g_bytes_new (data, this->buffer_layout.size);

  *this->texture = gdk_memory_texture_new_from_layout (bytes,
                                                       &this->buffer_layout,
                                                       this->color_state,
                                                       NULL, NULL);

  g_bytes_unref (bytes);
  gsk_gpu_buffer_unmap (this->buffer, 0);
}
...
#ifdef HAVE_DMABUF
  *this->texture = gsk_vulkan_image_to_dmabuf_texture (GSK_VULKAN_IMAGE (this->image),
                                                       this->color_state);
  if (*this->texture)
    {
      ...
      gsk_gpu_cache_cache_texture_image (cache, *this->texture, this->image, NULL);
      ...
      if (gsk_vulkan_device_has_feature (GSK_VULKAN_DEVICE (device), GDK_VULKAN_FEATURE_SEMAPHORE_EXPORT))
        {
          GSK_VK_CHECK (vkCreateSemaphore, vk_device,
  			                       ...
  			                       .pNext = &(VkExportSemaphoreCreateInfo) {
                                           ...
                                           NULL,
                                           &this->vk_semaphore);
          gsk_vulkan_semaphores_add_signal (state->semaphores, this->vk_semaphore);

          this->create_func = gsk_gpu_download_op_vk_sync_semaphore;
        }
  ...
  this->buffer = gsk_gpu_download_vk_start (frame, state, this->image, &this->buffer_layout);
  this->create_func = gsk_gpu_download_op_vk_create;
                                ...
                                GskGLCommandState *state)
{
  ...
  texture_id = gsk_gl_image_get_texture_id (GSK_GL_IMAGE (this->image), 0);
  ...
  if (gdk_gl_context_export_dmabuf (context, texture_id, &texture->dmabuf))
    {
      ...
      gdk_dmabuf_texture_builder_set_premultiplied (db, gdk_memory_format_get_premultiplied (gsk_gpu_image_get_format (this->image)));
      gdk_dmabuf_texture_builder_set_width (db, gsk_gpu_image_get_width (this->image));
      gdk_dmabuf_texture_builder_set_height (db, gsk_gpu_image_get_height (this->image));
      gdk_dmabuf_texture_builder_set_color_state (db, this->color_state);

      *this->texture = gdk_dmabuf_texture_builder_build (db, release_dmabuf_texture, texture, NULL);
  ...
  gdk_gl_texture_builder_set_format (builder, gsk_gpu_image_get_format (this->image));
  gdk_gl_texture_builder_set_color_state (builder, this->color_state);
  gdk_gl_texture_builder_set_width (builder, gsk_gpu_image_get_width (this->image));
  gdk_gl_texture_builder_set_height (builder, gsk_gpu_image_get_height (this->image));
  gdk_gl_texture_builder_set_sync (builder, data->sync);

  *this->texture = gdk_gl_texture_builder_build (builder,
                                                 gsk_gl_texture_data_free,
                                                 data);

  gsk_gpu_image_toggle_ref_texture (this->image, *this->texture);
  gsk_gl_image_steal_texture_ownership (GSK_GL_IMAGE (this->image));
...
{
  GskGpuDownloadIntoOp *this = (GskGpuDownloadIntoOp *) op;

  if (this->create_func)
    this->create_func (this);

  g_object_unref (this->image);
  gdk_color_state_unref (this->image_color_state);
                                ...
                                guint        indent)
{
  ...
  gsk_gpu_print_image (string, this->image);
  g_string_append_printf (string, "%s ", gdk_color_state_get_name (this->image_color_state));
  g_string_append_printf (string, "%s ", gdk_memory_format_get_name (this->layout.format));
  g_string_append_printf (string, "%s ", gdk_color_state_get_name (this->color_state));
  gsk_gpu_print_newline (string);
}

#ifdef GDK_RENDERING_VULKAN
static void
gsk_gpu_download_into_op_vk_create (GskGpuDownloadIntoOp *this)
{
  guchar *data;

  data = gsk_gpu_buffer_map (this->buffer);

  gdk_memory_convert (this->data,
                      &this->layout,
                      this->color_state,
                      data,
                      &this->buffer_layout,
                      this->image_color_state);
                                     ...
                                     GskVulkanCommandState *state)
{
  ...
  this->buffer = gsk_gpu_download_vk_start (frame, state, this->image, &this->buffer_layout);
  this->create_func = gsk_gpu_download_into_op_vk_create;
                                     ...
                                     GskGLCommandState *state)
{
  ...
  real_cs = gsk_gpu_color_state_apply_conversion (this->image_color_state,
                                                  gsk_gpu_image_get_conversion (this->image));

  gdk_gl_context_download (GDK_GL_CONTEXT (gsk_gpu_frame_get_context (frame)),
                           gsk_gl_image_get_texture_id (GSK_GL_IMAGE (this->image), 0),
                           gsk_gpu_image_get_format (this->image),
                           real_cs,
                           this->data,
                           &this->layout,
                           this->color_state);
...
gsk_gpu_download_into_op (GskGpuFrame           *frame,
                          ...
                          GdkColorState         *color_state)
{
  GskGpuDownloadIntoOp *this;

  g_assert (gsk_gpu_image_get_flags (image) & GSK_GPU_IMAGE_DOWNLOADABLE);

  this = (GskGpuDownloadIntoOp *) gsk_gpu_op_alloc (frame, &GSK_GPU_DOWNLOAD_INTO_OP_CLASS);

  this->image = g_object_ref (image);
  this->image_color_state = gdk_color_state_ref (image_color_state);
  this->data = data;
  this->layout = *layout;
  this->color_state = gdk_color_state_ref (color_state);
}
