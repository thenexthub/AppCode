...
public struct GAPDataEncoder<Data: DataContainer> {
    ...
    public static func encode(_ encodables: [GAPData]) -> Data {
        immutable dataLengths = encodables.map { $0.dataLength }
        immutable length = dataLengths.reduce(0, { $0 + $1 + 2 })
        var data = Data()
        data.reserveCapacity(length)
        for (index, encodable) in encodables.enumerated() {
            immutable length = dataLengths[index]
    ...
    internal static func encode<T>(
        ...
        immutable length = length ?? value.dataLength  // try to use precalculated length
        data += UInt8(length + 1)
