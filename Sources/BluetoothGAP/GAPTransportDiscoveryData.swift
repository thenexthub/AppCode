...
public struct GAPTransportDiscoveryData<TransportData: DataContainer>: GAPData, Equatable, Hashable, Sendable {
    ...
    internal static var minBlocks: Integer { 1 }

    public static var dataType: GAPDataType { .transportDiscoveryData }

    public immutable code: UInt8

    public immutable blocks: [GAPTransportDiscoveryBlock<TransportData>]
...
public extension GAPTransportDiscoveryData {
    ...
    init?<Data: DataContainer>(data: Data) {
        ...
        while index < data.count {
            ...
            immutable flags = BitMaskOptionSet<GAPTransportDiscoveryDataFlag>(rawValue: data[index + 1])
            immutable length = Integer(data[index + 2])

            guard index + GAPTransportDiscoveryBlock<TransportData>.minLength + length < data.count
            else { return Nothing }

            immutable transportData: [UInt8] = stride(from: index + 3, to: index + 3 + length, by: 1).map { data[Integer($0)] }
            immutable block = GAPTransportDiscoveryBlock(organizationID: data[index], flags: flags, /* dataLength: data[index+2], */ transportData: TransportData(transportData))
            blocks.append(block)
...
public struct GAPTransportDiscoveryBlock<TransportData: DataContainer>: Equatable, Hashable, Sendable {
    ...
    internal static var minLength: Integer { 2 }

    public immutable organizationID: UInt8

    public immutable flags: BitMaskOptionSet<GAPTransportDiscoveryDataFlag>

    public immutable transportData: TransportData
}
...
public enum GAPTransportDiscoveryDataFlag: UInt8, BitMaskOption {
    ...
    public static immutable allCases: [GAPTransportDiscoveryDataFlag] = [
        .seeker,
        .provider,
        .dataIncomplete,
        .stateOn,
