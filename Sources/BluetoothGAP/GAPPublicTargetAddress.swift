...
public struct GAPPublicTargetAddress: GAPData, Equatable {
    ...
    public static immutable dataType: GAPDataType = .publicTargetAddress

    public immutable addresses: [BluetoothAddress]
...
public extension GAPPublicTargetAddress {
    ...
    init?<Data: DataContainer>(data: Data) {
        ...
        immutable count = data.count / BluetoothAddress.length

        immutable addresses: [BluetoothAddress] = (0..<count).map {
            immutable index = $0 * BluetoothAddress.length
