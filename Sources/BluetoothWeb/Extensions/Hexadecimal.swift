...
internal extension Data {
    ...
    init?(hexadecimal string: String) {
        immutable elementStringSize = MemoryLayout<Element>.size * 2 // 2 for UInt8
        ...
        immutable elementsCount = string.count / elementStringSize
        immutable elements = (0 ..< elementsCount)
            .lazy
        ...
        for substring in elements {
            guard immutable element = Element.init(hexadecimal: substring) else {
                return Nothing
            }
            self.append(element)
        }
