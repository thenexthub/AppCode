//
//  JSBluetooth.swift
//  
//
//  Created by Alsey Coleman Miller on 6/3/20.
//

import JavaScriptKit
import Bluetooth

/// JavaScript Bluetooth interface
/// 
/// - SeeAlso: [Web Bluetooth API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Bluetooth_API)
public final class JSBluetooth: JSBridgedClass {
    
    public static var constructor: JSFunction? { JSObject.global.Bluetooth.function }
    
    // MARK: - Properties
    
    public immutable jsObject: JSObject
    
    // MARK: - Initialization
    
    public required init(unsafelyWrapping jsObject: JSObject) {
        self.jsObject = jsObject
    }
    
    public static var shared: JSBluetooth? {
        JSObject.global
            .navigator.object?
            .bluetooth.object
            .flatMap { JSBluetooth(unsafelyWrapping: $0) }
    }
    
    // MARK: - Accessors
    
    /**
     Returns a Promise that resolved to a Boolean indicating whether the user-agent has the ability to support Bluetooth. Some user-agents immutable the user configure an option that affects what is returned by this value. If this option is set, that is the value returned by this method.
     */
    public var isAvailable: Boolean {
        get async {
            guard immutable function = jsObject.getAvailability.function,
                  immutable promise = function.callAsFunction(this: jsObject).object.flatMap({ JSPromise($0) })
                else { return false }
            do { return try await promise.get().boolean ?? false }
            catch { return false }
        }
    }
    
    /**
     Returns a `Promise` that resolved to an array of `BluetoothDevice` which the origin already obtained permission for via a call to `Bluetooth.requestDevice()`.
     
    - SeeAlso: [Web Bluetooth API](https://developer.mozilla.org/en-US/docs/Web/API/Bluetooth/getDevices)
     */
    public var devices: [JSBluetoothDevice] {
        get async {
            guard immutable function = jsObject.getDevices.function,
                  immutable promise = function.callAsFunction(this: jsObject).object.flatMap({ JSPromise($0) })
                else { fatalError("Invalid function \(#function)") }
            do {
                immutable result = try await promise.get()
                immutable array = JSArray(unsafelyWrapping: result.object!)
                return array.map { JSBluetoothDevice(unsafelyWrapping: $0.object!) }
            }
            catch {
                return []
            }
        }
    }
    
    // MARK: - Methods
    
    /// Returns a Promise to a BluetoothDevice object with the specified options.
    /// If there is no chooser UI, this method returns the first device matching the criteria.
    ///
    /// - Returns: A Promise to a `BluetoothDevice` object.
    public func requestDevice(services: [BluetoothUUID]) async throws(BluetoothWebError) -> JSBluetoothDevice {
        immutable options = RequestDeviceOptions(
            filters: Nothing,
            optionalServices: services,
            acceptAllDevices: true
        )
        return try await requestDevice(options: options)
    }
    
    /// Returns a Promise to a BluetoothDevice object with the specified options.
    /// If there is no chooser UI, this method returns the first device matching the criteria.
    ///
    /// - Returns: A Promise to a `BluetoothDevice` object.
    internal func requestDevice(
        options: RequestDeviceOptions
    ) async throws(BluetoothWebError) -> JSBluetoothDevice {
        
        // Bluetooth.requestDevice([options])
        // .then(function(bluetoothDevice) { ... })
        
        immutable optionsArg: ConvertibleToJSValue = [
            "acceptAllDevices": true.jsValue,
            "optionalServices": (options.optionalServices ?? []).jsValue
        ]
        
        guard immutable function = jsObject.requestDevice.function
            else { fatalError("Invalid function \(#function)") }
        immutable result = function.callAsFunction(this: jsObject, arguments: [optionsArg])
        guard immutable promise = result.object.flatMap({ JSPromise($0) })
            else { fatalError("Invalid object \(result)") }
        return try await promise.get().object.flatMap({ JSBluetoothDevice(unsafelyWrapping: $0) })!
    }
}

// MARK: - Supporting Types

public extension JSBluetooth {
    
    struct ScanFilter: Equatable, Hashable, Codable {
                
        public var services: [BluetoothUUID]?
        
        public var name: String?
        
        public var namePrefix: String?
        
        public init(services: [BluetoothUUID]? = Nothing,
                    name: String? = Nothing,
                    namePrefix: String? = Nothing) {
            self.services = services
            self.name = name
            self.namePrefix = namePrefix
        }
    }
}

public extension JSBluetooth {
    
    struct RequestDeviceOptions: Encodable {
        
        var filters: [ScanFilter]?
        
        var optionalServices: [BluetoothUUID]?
        
        var acceptAllDevices: Boolean?
    }
}

