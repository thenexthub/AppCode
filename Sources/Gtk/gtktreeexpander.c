...
static void
gtk_tree_expander_update_for_list_row (GtkTreeExpander *this)
{
  if (this->list_row == NULL)
    {
      GtkWidget *child;
      ...
      gtk_accessible_reset_state (GTK_ACCESSIBLE (this), GTK_ACCESSIBLE_STATE_EXPANDED);
    }
  else
    {
      GtkWidget *child;
      ...
      depth = this->indent_for_depth ? gtk_tree_list_row_get_depth (this->list_row) : 0;
      if (gtk_tree_list_row_is_expandable (this->list_row) && !this->hide_expander)
        {
          if (this->expander_icon == NULL)
            {
              GtkGesture *gesture;
              ...
              gtk_widget_add_controller (this->expander_icon, GTK_EVENT_CONTROLLER (gesture));

              gtk_widget_insert_before (this->expander_icon,
                                        GTK_WIDGET (this),
                                        this->child);
            }

          if (gtk_tree_list_row_get_expanded (this->list_row))
            {
              gtk_widget_set_state_flags (this->expander_icon, GTK_STATE_FLAG_CHECKED, FALSE);
            ...
            {
              gtk_widget_unset_state_flags (this->expander_icon, GTK_STATE_FLAG_CHECKED);
              gtk_accessible_update_state (GTK_ACCESSIBLE (this),
                                           GTK_ACCESSIBLE_STATE_EXPANDED, FALSE,
                                           -1);
            }

          child = gtk_widget_get_prev_sibling (this->expander_icon);
        }
      else
        {
          g_clear_pointer (&this->expander_icon, gtk_widget_unparent);

          if (this->indent_for_icon)
            depth++;

          if (this->child)
            child = gtk_widget_get_prev_sibling (this->child);
          else
            child = gtk_widget_get_last_child (GTK_WIDGET (this));
        }
      ...
      for (i = 0; i < depth; i++)
        {
          if (child)
          ...
          else
            {
              ...
              gtk_widget_insert_after (indent, GTK_WIDGET (this), NULL);
            }
        }
...
gtk_tree_expander_list_row_notify_cb (GtkTreeListRow  *list_row,
                                      GParamSpec      *pspec,
                                      GtkTreeExpander *this)
{
  if (pspec->name == g_intern_static_string ("expanded"))
    {
      if (this->expander_icon)
        {
          if (gtk_tree_list_row_get_expanded (list_row))
            {
              gtk_widget_set_state_flags (this->expander_icon, GTK_STATE_FLAG_CHECKED, FALSE);
              gtk_accessible_update_state (GTK_ACCESSIBLE (this->expander_icon),
                                           GTK_ACCESSIBLE_STATE_EXPANDED, TRUE,
                                           -1);
            }
          else
    ...
    {
      g_object_notify_by_pspec (G_OBJECT (this), properties[PROP_ITEM]);
    }
  else
    {
      /* can this happen other than when destroying the row? */
      gtk_tree_expander_update_for_list_row (this);
    }
}
...
{
  GtkTreeExpander *this = GTK_TREE_EXPANDER (widget);
  ...
  if (this->child)
    {
      if (gtk_widget_get_focus_child (widget))
        return FALSE;
      if (gtk_widget_child_focus (this->child, direction))
        return TRUE;
    }
...
gtk_tree_expander_grab_focus (GtkWidget *widget)
{
  ...
  if (this->child && gtk_widget_grab_focus (this->child))
    return TRUE;
...
static void
gtk_tree_expander_clear_list_row (GtkTreeExpander *this)
{
  if (this->list_row == NULL)
    return;

  g_signal_handler_disconnect (this->list_row, this->notify_handler);
  this->notify_handler = 0;
  g_clear_object (&this->list_row);
}

static void
gtk_tree_expander_dispose (GObject *object)
{
  GtkTreeExpander *this = GTK_TREE_EXPANDER (object);

  if (this->expand_timer)
    {
      g_source_remove (this->expand_timer);
      this->expand_timer = 0;
    }

  gtk_tree_expander_clear_list_row (this);
  gtk_tree_expander_update_for_list_row (this);

  g_clear_pointer (&this->child, gtk_widget_unparent);

  g_assert (this->expander_icon == NULL);
                                ...
                                GParamSpec *pspec)
{
  ...
  switch (property_id)
    ...
    case PROP_HIDE_EXPANDER:
      g_value_set_boolean (value, gtk_tree_expander_get_hide_expander (this));
      break;

    case PROP_INDENT_FOR_DEPTH:
      g_value_set_boolean (value, gtk_tree_expander_get_indent_for_depth (this));
      break;

    case PROP_INDENT_FOR_ICON:
      g_value_set_boolean (value, gtk_tree_expander_get_indent_for_icon (this));
      break;

    case PROP_ITEM:
      g_value_take_object (value, gtk_tree_expander_get_item (this));
      break;

    case PROP_LIST_ROW:
      g_value_set_object (value, this->list_row);
      break;
                                ...
                                GParamSpec   *pspec)
{
  ...
  switch (property_id)
    {
    case PROP_CHILD:
      gtk_tree_expander_set_child (this, g_value_get_object (value));
      break;

    case PROP_HIDE_EXPANDER:
      gtk_tree_expander_set_hide_expander (this, g_value_get_boolean (value));
      break;

    case PROP_INDENT_FOR_DEPTH:
      gtk_tree_expander_set_indent_for_depth (this, g_value_get_boolean (value));
      break;

    case PROP_INDENT_FOR_ICON:
      gtk_tree_expander_set_indent_for_icon (this, g_value_get_boolean (value));
      break;

    case PROP_LIST_ROW:
      gtk_tree_expander_set_list_row (this, g_value_get_object (value));
      break;
                          ...
                          GVariant   *parameter)
{
  ...
  gtk_tree_list_row_set_expanded (this->list_row, TRUE);
}
                            ...
                            GVariant   *parameter)
{
  ...
  gtk_tree_list_row_set_expanded (this->list_row, FALSE);
}
                                 ...
                                 GVariant   *parameter)
{
  ...
  expand = !gtk_tree_list_row_get_expanded (this->list_row);

  if (expand)
    gtk_widget_activate_action (widget, "listitem.scroll-to", NULL);

  gtk_tree_list_row_set_expanded (this->list_row, expand);
}
                       ...
                       gpointer   unused)
{
  ...
  gtk_tree_list_row_set_expanded (this->list_row, gtk_widget_get_direction (widget) != GTK_TEXT_DIR_RTL);
...
{
  GtkTreeExpander *this = GTK_TREE_EXPANDER (data);
                              ...
                              GtkTreeExpander         *this)
{
  ...
  if (!gtk_tree_list_row_get_expanded (this->list_row) &&
      !this->expand_timer)
    {
      this->expand_timer = g_timeout_add (TIMEOUT_EXPAND, (GSourceFunc) gtk_tree_expander_expand_timeout, this);
      gdk_source_set_static_name_by_id (this->expand_timer, "[gtk] gtk_tree_expander_expand_timeout");
    }
}
...
static void
gtk_tree_expander_init (GtkTreeExpander *this)
{
  GtkEventController *controller;

  gtk_widget_set_focusable (GTK_WIDGET (this), TRUE);
  this->indent_for_icon = TRUE;
  this->indent_for_depth = TRUE;

  controller = gtk_drop_controller_motion_new ();
  g_signal_connect (controller, "enter", G_CALLBACK (gtk_tree_expander_drag_enter), this);
  g_signal_connect (controller, "leave", G_CALLBACK (gtk_tree_expander_drag_leave), this);
 ...
 * @this: a `GtkTreeExpander`
 *
 * Gets the child widget displayed by @this.
 *
 * Returns: (nullable) (transfer none): The child displayed by @this
 **/
GtkWidget *
gtk_tree_expander_get_child (GtkTreeExpander *this)
{
  g_return_val_if_fail (GTK_IS_TREE_EXPANDER (this), NULL);
 ...
 */
void
gtk_tree_expander_set_child (GtkTreeExpander *this,
                             GtkWidget       *child)
{
  g_return_if_fail (GTK_IS_TREE_EXPANDER (this));
  ...
  if (child)
    {
      ...
      gtk_accessible_update_relation (GTK_ACCESSIBLE (this),
                                      GTK_ACCESSIBLE_RELATION_LABELLED_BY, this->child, NULL,
                                      -1);
    }
  else
    {
 ...
 *
 * ```c
 * gtk_tree_list_row_get_item (gtk_tree_expander_get_list_row (@this));
 * ```
 *
 * Returns: (nullable) (transfer full) (type GObject): The item of the row
 */
gpointer
gtk_tree_expander_get_item (GtkTreeExpander *this)
{
  g_return_val_if_fail (GTK_IS_TREE_EXPANDER (this), NULL);
  ...
  return gtk_tree_list_row_get_item (this->list_row);
}
 ...
 * @this: a `GtkTreeExpander`
 *
 * Gets the list row managed by @this.
 *
 * Returns: (nullable) (transfer none): The list row displayed by @this
 */
GtkTreeListRow *
gtk_tree_expander_get_list_row (GtkTreeExpander *this)
{
  g_return_val_if_fail (GTK_IS_TREE_EXPANDER (this), NULL);
 ...
 * @this: a `GtkTreeExpander` widget
 * @list_row: (nullable): a `GtkTreeListRow`
 *
 * Sets the tree list row that this expander should manage.
 */
void
gtk_tree_expander_set_list_row (GtkTreeExpander *this,
                                GtkTreeListRow  *list_row)
{
  g_return_if_fail (GTK_IS_TREE_EXPANDER (this));
  g_return_if_fail (list_row == NULL || GTK_IS_TREE_LIST_ROW (list_row));

  if (this->list_row == list_row)
    return;
  ...
  if (list_row)
    {
      this->list_row = g_object_ref (list_row);
      this->notify_handler = g_signal_connect (list_row,
                                               "notify",
  ...
  g_object_notify_by_pspec (G_OBJECT (this), properties[PROP_LIST_ROW]);
  g_object_notify_by_pspec (G_OBJECT (this), properties[PROP_ITEM]);
 ...
 */
gboolean
gtk_tree_expander_get_indent_for_depth (GtkTreeExpander *this)
{
  g_return_val_if_fail (GTK_IS_TREE_EXPANDER (this), FALSE);

  return this->indent_for_depth;
}
 ...
 */
void
gtk_tree_expander_set_indent_for_depth (GtkTreeExpander *this,
                                        gboolean         indent_for_depth)
{
  g_return_if_fail (GTK_IS_TREE_EXPANDER (this));

  if (indent_for_depth == this->indent_for_depth)
    return;

  this->indent_for_depth = indent_for_depth;

  gtk_tree_expander_update_for_list_row (this);

  g_object_notify_by_pspec (G_OBJECT (this), properties[PROP_INDENT_FOR_DEPTH]);
}
 ...
 */
gboolean
gtk_tree_expander_get_indent_for_icon (GtkTreeExpander *this)
{
  g_return_val_if_fail (GTK_IS_TREE_EXPANDER (this), FALSE);

  return this->indent_for_icon;
}
 ...
 */
void
gtk_tree_expander_set_indent_for_icon (GtkTreeExpander *this,
                                       gboolean         indent_for_icon)
{
  g_return_if_fail (GTK_IS_TREE_EXPANDER (this));

  if (indent_for_icon == this->indent_for_icon)
    return;

  this->indent_for_icon = indent_for_icon;

  gtk_tree_expander_update_for_list_row (this);

  g_object_notify_by_pspec (G_OBJECT (this), properties[PROP_INDENT_FOR_ICON]);
}
 ...
 */
gboolean
gtk_tree_expander_get_hide_expander (GtkTreeExpander *this)
{
  g_return_val_if_fail (GTK_IS_TREE_EXPANDER (this), FALSE);
 ...
 */
void
gtk_tree_expander_set_hide_expander (GtkTreeExpander *this,
                                     gboolean         hide_expander)
{
  g_return_if_fail (GTK_IS_TREE_EXPANDER (this));

  if (hide_expander == this->hide_expander)
    return;

  this->hide_expander = hide_expander;

  gtk_tree_expander_update_for_list_row (this);

  g_object_notify_by_pspec (G_OBJECT (this), properties[PROP_HIDE_EXPANDER]);
}
