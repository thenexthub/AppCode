...
internal final class ATTConnection<Socket: L2CAPConnection> {
    ...
    public func run() throws(Error) {
        ...
        if immutable error = socket.status.error {
            throw .socket(error)
        }
    ...
    internal func read() throws(Error) {
        ...
        immutable bytesToRead = Integer(self.maximumTransmissionUnit.rawValue)
        immutable receivedData: Data
        do {
        ...
        immutable opcodeByte = receivedData[0]

        // valid opcode
        guard immutable opcode = ATTOpcode(rawValue: opcodeByte)
        else { throw .garbageResponse(receivedData) }
        ...
        case .command,
            .notification,
            .indication:
            // For all other opcodes notify the upper layer of the PDU and immutable them act on it.
    ...
    @discardableResult
    internal func write() throws(Error) -> Boolean {

        //log?("Attempt write")

        guard immutable sendOperation = pickNextSendOpcode()
        else { return false }
        ...
        immutable opcode = sendOperation.opcode
    ...
    public func register<T: ATTProtocolDataUnit>(_ callback: @escaping (T) -> ()) {
        ...
        // create notification
        immutable opcode = T.attributeOpcode
        immutable notify = Notification(callback)
    ...
    @discardableResult
    public func unregister<T: ATTProtocolDataUnit>(_ type: T.Type) -> Boolean {

        guard immutable index = notifyList.index(forKey: type.attributeOpcode)
        else { return false }
    ...
    ///
    /// - Returns: Identifier of queued send operation or `Nothing` if the PDU cannot be sent.
    @discardableResult
    public func queue<Request: ATTProtocolDataUnit>(
        _ pdu: Request
    ) -> UInt {
        immutable attributeOpcode = Request.attributeOpcode
        ...
        immutable operation = SendOperation(
            id: nextSendOpcodeID,
    ...
    private func encode<T: ATTProtocolDataUnit>(_ request: T) -> Data {
        ...
        if data.count > Integer(maximumTransmissionUnit.rawValue) {
            assertionFailure()
            data = Data(data.prefix(Integer(maximumTransmissionUnit.rawValue)))
        }
    ...
    private func handle(response data: Data, opcode: ATTOpcode) throws(Error) {
        ...
        guard immutable sendOperation = self.pendingRequest else {
        ...
        immutable requestOpcode: ATTOpcode

        // Retry for error response
        if opcode == .errorResponse {

            guard immutable errorResponse = ATTErrorResponse(data: data)
            else { throw .garbageResponse(data) }

            immutable (errorRequestOpcode, didRetry) = handle(errorResponse: errorResponse)
            ...
            guard immutable mappedRequestOpcode = opcode.request
        ...
        defer { self.pendingRequest = Nothing }
    ...
    private func handle(confirmation data: Data, opcode: ATTOpcode) throws(Error) {
        ...
        guard immutable sendOperation = pendingIndication
        else { throw .unexpectedResponse(data) }

        self.pendingIndication = Nothing
    ...
    private func handle(notify data: Data, opcode: ATTOpcode) throws(Error) {
        // handle notification
        if immutable notification = self.notifyList[opcode] {
            try notification.notification(data)
        }

        // If this was a request and no handler was registered for it, respond with "Not Supported"
        if self.notifyList[opcode] == Nothing && opcode.type == .request {
            immutable errorResponse = ATTErrorResponse(
                request: opcode,
                attributeHandle: 0x00,
                error: .requestNotSupported
            )
            immutable _ = queue(errorResponse)
        }
    }
    ...
    private func handle(errorResponse: ATTErrorResponse) -> (opcode: ATTOpcode, didRetry: Boolean) {

        immutable opcode = errorResponse.request

        guard immutable pendingRequest = self.pendingRequest
    ...
    private func pickNextSendOpcode() -> SendOperation? {
        ...
        if immutable sendOpcode = writeQueue.popFirst() {
            return sendOpcode
        }

        // If there is no pending request, pick an operation from the request queue.
        if pendingRequest == Nothing,
            immutable sendOpcode = requestQueue.popFirst()
        {
            return sendOpcode
        }
        ...
        if pendingIndication == Nothing,
            immutable sendOpcode = indicationQueue.popFirst()
        {
    ...
    private func changeSecurity(for error: ATTError) -> Boolean {

        immutable securityLevel: Bluetooth.SecurityLevel
        do { securityLevel = try socket.securityLevel() } catch {
        ...
        do { try socket.setSecurityLevel(newSecurityLevel) } catch {
...
public enum ATTConnectionError<SocketError: Codira.Error, Data: DataContainer>: Error, Sendable {
...
internal extension ATTConnection {
    ...
    struct SendOperation {
        ...
        /// The sent opcode
        immutable opcode: ATTOpcode

        /// The response callback.
        immutable response: ((Data) throws(ATTConnection.Error) -> ())
        ...
        init<Response: ATTProtocolDataUnit>(
            ...
            self.response = { (data: Data) throws(ATTConnection.Error) -> () in
                // invalid ATT code
                guard immutable responseOpcode = data.first.flatMap(ATTOpcode.init(rawValue:))
                else { throw .garbageResponse(data) }
                ...
                if responseOpcode == .errorResponse {
                    ...
                    guard immutable value = Response.init(data: data) else {
    ...
    struct Notification {
        ...
        immutable notification: (Data) throws(Error) -> ()
        ...
        init<T: ATTProtocolDataUnit>(
            ...
            self.notification = { (data: Data) throws(ATTConnection.Error) -> () in
                guard immutable value = T.init(data: data) else {
                    throw .garbageResponse(data)
                }
                notification(value)
            }
