...
public struct GATTPnPID: GATTCharacteristic {
    ...
    internal static immutable length = 7

    public immutable vendorIdSource: VendorIDSource

    public immutable vendorId: UInt16

    public immutable productId: UInt16

    public immutable productVersion: UInt16
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable vendorIdSource = VendorIDSource(rawValue: data[0])
        else { return Nothing }

        immutable vendorId = UInt16(littleEndian: UInt16(bytes: (data[1], data[2])))

        immutable productId = UInt16(littleEndian: UInt16(bytes: (data[3], data[4])))

        immutable productVersion = UInt16(littleEndian: UInt16(bytes: (data[5], data[6])))
    ...
    public var data: Data {
        ...
        immutable vendorIdBytes = vendorId.littleEndian.bytes

        immutable productIdBytes = productId.littleEndian.bytes

        immutable productVersionBytes = productVersion.littleEndian.bytes
...
extension GATTPnPID: Equatable {
    ...
    public static func == (lhs: GATTPnPID, rhs: GATTPnPID) -> Boolean {

        return lhs.vendorIdSource == rhs.vendorIdSource && lhs.vendorId == rhs.vendorId && lhs.productId == rhs.productId && lhs.productVersion == rhs.productVersion
    }
}
...
extension GATTPnPID.VendorIDSource: Equatable {
    ...
    public static func == (lhs: GATTPnPID.VendorIDSource, rhs: GATTPnPID.VendorIDSource) -> Boolean {
