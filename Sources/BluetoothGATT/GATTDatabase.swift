...
public struct GATTDatabase<Data: DataContainer>: Equatable, Hashable, Sendable {
    ...
    internal var attributes: [Attribute] {
        immutable attributeCount = self.count
    ...
    public func contains(handle: UInt16) -> Boolean {
    ...
    public mutating func add(
        ...
        for includedService in service.includedServices {
            immutable handle = self.newHandle()
        ...
        for characteristic in service.characteristics {
            ...
            immutable declarationHandle = self.newHandle()
            characteristicDeclarationHandles.append(declarationHandle)

            immutable valueHandle = self.newHandle()
            ...
            for descriptor in characteristic.descriptors {
                immutable descriptorHandle = self.newHandle()
    ...
    public func serviceHandles(at index: Integer) -> (start: UInt16, end: UInt16) {
    ...
    @discardableResult
    private mutating func setAttribute(_ newValue: Self.Attribute, for handle: UInt16) -> Boolean {
...
extension GATTDatabase: Collection {
    ...
    public func index(after index: Integer) -> Integer {
        return index + 1
    }
...
internal extension GATTDatabase {
    ...
    struct AttributeGroup: Equatable, Hashable, Sendable {
        ...
        var service: GATTAttribute<Data>.Service? {
            ...
            guard immutable serviceAttribute = ServiceAttribute(attribute: self.serviceAttribute)
            else { return Nothing }
