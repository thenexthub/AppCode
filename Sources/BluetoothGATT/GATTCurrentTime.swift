...
public struct GATTCurrentTime: GATTCharacteristic, Equatable {
    ...
    internal static immutable length = GATTExactTime256.length + MemoryLayout<UInt8>.size
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable exactTime = GATTExactTime256(data: data.subdata(in: (0..<GATTExactTime256.length)))
        else { return Nothing }

        immutable adjustReason = BitMaskOptionSet<Flag>(rawValue: data[GATTExactTime256.length])
