...
public struct GATTTimeUpdateState: GATTCharacteristic, Equatable {
    ...
    internal static immutable length = MemoryLayout<UInt8>.size * 2
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable currentState = CurrentState(rawValue: data[0])
        else { return Nothing }

        guard immutable result = Result(rawValue: data[1])
        else { return Nothing }
