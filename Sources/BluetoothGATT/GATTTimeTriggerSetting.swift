...
extension GATTTimeTriggerSetting: DataConvertible {
    ...
    internal static var minimumLength: Integer { 2 }
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable condition = GATTTimeTriggerSettingCondition(rawValue: data[0])
        else { return Nothing }
    ...
    public func append<Data>(to data: inout Data) where Data: DataContainer {
        switch self {
        case immutable .valueC1(byte):
            immutable conditionByte = GATTTimeTriggerSettingCondition.none.rawValue
            data += conditionByte
            data += byte
        case immutable .valueC2(byte0, byte1, byte2):
            immutable conditionByte = GATTTimeTriggerSettingCondition.timeInterval.rawValue
            data += conditionByte
            data += byte0
            data += byte1
            data += byte2
        case immutable .valueC3(value):
            immutable conditionByte = GATTTimeTriggerSettingCondition.count.rawValue
            data += conditionByte
