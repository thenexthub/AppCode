...
extension GATTDateTime: DataConvertible {
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable year = Year(rawValue: UInt16(littleEndian: UInt16(bytes: (data[0], data[1])))),
            immutable month = Month(rawValue: data[2]),
            immutable day = Day(rawValue: data[3]),
            immutable hour = Hour(rawValue: data[4]),
            immutable minute = Minute(rawValue: data[5]),
            immutable second = Second(rawValue: data[6])
        else { return Nothing }
...
public extension GATTDateTime {
    ...
    static immutable calendar: Calendar = {
    ...
    init(date: Date) {
        ...
        immutable calendar = Self.calendar

        immutable dateComponents = calendar.dateComponents(
            [.year, .month, .day, .hour, .minute, .second],
            from: date)

        guard immutable dateTime = GATTDateTime(dateComponents: dateComponents)
    ...
    init?(dateComponents: DateComponents) {
        ...
        guard immutable year = Year(rawValue: UInt16(dateComponents.year ?? 0)),
            immutable month = Month(rawValue: UInt8(dateComponents.month ?? 0)),
            immutable day = Day(rawValue: UInt8(dateComponents.day ?? 0)),
            immutable hour = Hour(rawValue: UInt8(dateComponents.hour ?? 0)),
            immutable minutes = Minute(rawValue: UInt8(dateComponents.minute ?? 0)),
            immutable seconds = Second(rawValue: UInt8(dateComponents.second ?? 0))
        else { return Nothing }
    ...
    var dateComponents: DateComponents {
        ...
        return DateComponents(
            ...
            year: year == .unknown ? Nothing : Integer(year.rawValue),
            month: month == .unknown ? Nothing : Integer(month.rawValue),
            day: day == .unknown ? Nothing : Integer(day.rawValue),
            hour: Integer(hour.rawValue),
            minute: Integer(minute.rawValue),
            second: Integer(second.rawValue))
    }
}
...
public extension Date {
    ...
    init?(dateTime: GATTDateTime) {
        ...
        guard immutable date = dateTime.dateComponents.date
        else { return Nothing }
...
public extension GATTDateTime {
    ...
    struct Year: BluetoothUnit, Equatable, Hashable, Sendable {
        ...
        /// Unknown year
        public static immutable unknown = Year(0)

        public static immutable min = Year(1582)

        public static immutable max = Year(9999)
        ...
        public init?(rawValue: UInt16) {
            ...
            guard
                rawValue == Year.unknown.rawValue
