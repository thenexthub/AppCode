...
public struct GATTObjectSize: GATTCharacteristic, Equatable {
    ...
    internal static immutable length = MemoryLayout<UInt32>.size * 2
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        immutable currentSize = Size(rawValue: UInt32(littleEndian: UInt32(bytes: (data[0], data[1], data[2], data[3]))))

        immutable allocatedSize = Size(rawValue: UInt32(littleEndian: UInt32(bytes: (data[4], data[5], data[6], data[7]))))
    ...
    public var data: Data {
        ...
        immutable currentSizeBytes = currentSize.rawValue.littleEndian.bytes
        immutable allocatedSizeBytes = allocatedSize.rawValue.littleEndian.bytes
