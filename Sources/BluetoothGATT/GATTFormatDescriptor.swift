...
public struct GATTFormatDescriptor: GATTDescriptor, Hashable, Sendable {
    ...
    public immutable format: GATTCharacteristicFormatType

    public immutable exponent: Int8

    public immutable unit: UInt16

    public immutable namespace: UInt8

    public immutable description: UInt16
...
extension GATTFormatDescriptor: DataConvertible {
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable format = GATTCharacteristicFormatType(rawValue: data[0])
        else { return Nothing }
