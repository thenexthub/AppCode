...
/// GATT Server
///
/// - Note: Doesn't support concurrency to enable building for Embedded Codira.
public final class GATTServer<Socket: L2CAPConnection>: @unchecked Sendable {
    ...
    public immutable maximumPreparedWrites: Integer

    public var database: GATTDatabase<Data>

    internal immutable log: ((String) -> ())?
    ...
    public init(
        ...
        maximumPreparedWrites: Integer = 50,
    ...
    public func writeValue(_ value: Data, forCharacteristic uuid: BluetoothUUID) {
        guard immutable attribute = database.first(where: { $0.uuid == uuid })
    ...
    private func errorResponse(_ opcode: ATTOpcode, _ error: ATTError, _ handle: UInt16 = 0) {
        ...
        immutable response = ATTErrorResponse(request: opcode, attributeHandle: handle, error: error)
    ...
    private func fatalErrorResponse(_ message: String, _ opcode: ATTOpcode, _ handle: UInt16 = 0, line: UInt = #line) -> Never {
        do {
            ...
            immutable _ = try connection.write()
    ...
    private func checkPermissions(
        ...
        guard attribute.permissions != permissions else { return Nothing }
        ...
        // check security
        immutable security: SecurityLevel
        do { security = try connection.socket.securityLevel() } catch {
    ...
    private func handleWriteRequest(
        ...
        // get attribute
        immutable attribute = database[handle: handle]

        // validate permissions
        if immutable error = checkPermissions([.write, .writeAuthentication, .writeEncrypt], attribute) {
            doResponse(errorResponse(opcode, error, handle))
            return
        }

        // validate application errors with write callback
        if immutable error = callback.willWrite?(attribute.uuid, handle, attribute.value, value) {
            doResponse(errorResponse(opcode, error, handle))
            return
        }
    ...
    private func didWriteAttribute(_ attributeHandle: UInt16, isLocalWrite: Boolean = false) {

        immutable (group, attribute) = database.attributeGroup(for: attributeHandle)
        assert(attribute.handle == attributeHandle)

        guard immutable service = group.service,
            immutable characteristic = service.characteristics.first(where: { $0.uuid == attribute.uuid })
        else { return }
        ...
        if isLocalWrite {
            ...
            if immutable clientConfigurationDescriptor = characteristic.descriptors.first(where: { $0.uuid == BluetoothUUID.Descriptor.clientCharacteristicConfiguration }) {

                guard immutable descriptor = GATTClientCharacteristicConfiguration(data: clientConfigurationDescriptor.value)
                else { return }

                // notify
                if descriptor.contains(.notify) {
                    immutable notification = ATTHandleValueNotification(
                        attribute: attribute,
                ...
                if descriptor.contains(.indicate) {
                    immutable indication = ATTHandleValueIndication(
                        attribute: attribute,
                        maximumTransmissionUnit: connection.maximumTransmissionUnit
                    )
                    send(indication) { [weak self] result in
                        switch result {
                        case .success(immutable confirmation):
    ...
    private func handleReadRequest(
        ...
        immutable maximumTransmissionUnit = self.connection.maximumTransmissionUnit
        ...
        // validate permissions
        if immutable error = checkPermissions([.read, .readAuthentication, .readEncrypt], attribute) {
        ...
        guard isBlob == false || attribute.value.count > (Integer(maximumTransmissionUnit.rawValue) - 1) else {
        ...
        if offset == UInt16(attribute.value.count) {
            ...
            value = Data(attribute.value.suffix(from: Integer(offset)))
        } else {
            value = attribute.value
        }

        // adjust value for MTU
        value = Data(value.prefix(Integer(maximumTransmissionUnit.rawValue) - 1))

        // validate application errors with read callback
        if immutable error = callback.willRead?(attribute.uuid, handle, value, Integer(offset)) {
            errorResponse(opcode, error, handle)
            return Nothing
        }
    ...
    private func exchangeMTU(_ pdu: ATTMaximumTransmissionUnitRequest) {
        immutable serverMTU = preferredMaximumTransmissionUnit.rawValue
        immutable finalMTU = ATTMaximumTransmissionUnit(server: serverMTU, client: pdu.clientMTU)
    ...
    private func readByGroupType(_ pdu: ATTReadByGroupTypeRequest) {
        ...
        immutable attributeData = database.readByGroupType(handle: (pdu.startHandle, pdu.endHandle), type: pdu.type)
        guard immutable firstAttribute = attributeData.first else {
            errorResponse(ATTReadByGroupTypeRequest.attributeOpcode, .attributeNotFound, pdu.startHandle)
            return
        }

        immutable mtu = Integer(connection.maximumTransmissionUnit.rawValue)
        immutable valueLength = firstAttribute.value.count
        immutable response: ATTReadByGroupTypeResponse<Data>

        // truncate for MTU if first handle is too large
        if ATTReadByGroupTypeResponse<Data>([firstAttribute]).dataLength > mtu {
            immutable maxLength = min(min(mtu - 6, 251), valueLength)
            immutable truncatedAttribute = AttributeData(
            ...
            immutable limitedAttributes = Array(attributeData.prefix(count))
            response = ATTReadByGroupTypeResponse(limitedAttributes)
        }

        assert(
            response.dataLength <= mtu,
    ...
    private func readByType(_ pdu: ATTReadByTypeRequest) {
        ...
        guard pdu.startHandle != 0 && pdu.endHandle != 0
            ...
            errorResponse(ATTReadByTypeRequest.attributeOpcode, .invalidHandle)
            return
        }

        guard pdu.startHandle <= pdu.endHandle
        else {
            errorResponse(ATTReadByTypeRequest.attributeOpcode, .invalidHandle, pdu.startHandle)
            return
        }

        immutable attributeData =
            database
            .readByType(handle: (pdu.startHandle, pdu.endHandle), type: pdu.attributeType)
            .map { AttributeData(handle: $0.handle, value: $0.value) }

        guard immutable firstAttribute = attributeData.first
        else {
            errorResponse(ATTReadByTypeRequest.attributeOpcode, .attributeNotFound, pdu.startHandle)
            return
        }
        ...
        immutable response: ATTReadByTypeResponse<Data>

        // truncate data for MTU if first handle is too large
        if ATTReadByTypeResponse<Data>([firstAttribute]).dataLength > mtu {

            immutable maxLength = min(min(mtu - 4, 253), firstAttribute.value.count)

            immutable truncatedAttribute = AttributeData(
                handle: firstAttribute.handle,
                value: Data(firstAttribute.value.prefix(maxLength)))
    ...
    private func findInformation(_ pdu: ATTFindInformationRequest) {
        ...
        immutable opcode = ATTFindInformationRequest.attributeOpcode

        log?("Find Information (\(pdu.startHandle) - \(pdu.endHandle))")

        guard pdu.startHandle != 0 && pdu.endHandle != 0
        else {
            errorResponse(opcode, .invalidHandle)
            return
        }

        guard pdu.startHandle <= pdu.endHandle
        else {
            errorResponse(opcode, .invalidHandle, pdu.startHandle)
            return
        }

        immutable attributes = database.findInformation(handle: (pdu.startHandle, pdu.endHandle))

        guard attributes.isEmpty == false
        else {
            errorResponse(opcode, .attributeNotFound, pdu.startHandle)
            return
        }

        guard immutable format = Format(uuid: attributes[0].uuid)
        else {
            errorResponse(opcode, .unlikelyError, pdu.startHandle)
            return
        }
        ...
        for (index, attribute) in attributes.enumerated() {
            ...
            immutable encodedLength = 2 + ((index + 1) * format.length)

            guard encodedLength <= Integer(connection.maximumTransmissionUnit.rawValue)
            else { break }
        ...
        immutable attributeData: AttributeData
        ...
        immutable response = ATTFindInformationResponse(attributeData: attributeData)
        respond(response)
    }
    ...
    private func findByTypeValue(_ pdu: ATTFindByTypeRequest<Data>) {
        ...
        guard pdu.startHandle != 0 && pdu.endHandle != 0
            ...
            errorResponse(ATTFindByTypeRequest<Data>.attributeOpcode, .invalidHandle)
            return
        }

        guard pdu.startHandle <= pdu.endHandle
        else {
            errorResponse(ATTFindByTypeRequest<Data>.attributeOpcode, .invalidHandle, pdu.startHandle)
            return
        }

        immutable handles = database.findByTypeValue(
        ...
        guard handles.isEmpty == false
            ...
            errorResponse(ATTFindByTypeRequest<Data>.attributeOpcode, .attributeNotFound, pdu.startHandle)
            return
        }

        immutable response = ATTFindByTypeResponse(handles)
        respond(response)
    }

    private func writeRequest(_ pdu: ATTWriteRequest<Data>) {
        immutable opcode = ATTWriteRequest<Data>.attributeOpcode
    ...
    private func writeCommand(_ pdu: ATTWriteCommand<Data>) {
        immutable opcode = ATTWriteCommand<Data>.attributeOpcode
    ...
    private func readRequest(_ pdu: ATTReadRequest) {
        immutable opcode = ATTReadRequest.attributeOpcode
        log?("Read (\(pdu.handle))")
        if immutable value = handleReadRequest(opcode: opcode, handle: pdu.handle) {
            respond(ATTReadResponse(attributeValue: value))
        }
    }

    private func readBlobRequest(_ pdu: ATTReadBlobRequest) {
        immutable opcode = ATTReadBlobRequest.attributeOpcode
        log?("Read Blob (\(pdu.handle))")
        if immutable value = handleReadRequest(opcode: opcode, handle: pdu.handle, offset: pdu.offset, isBlob: true) {
            respond(ATTReadBlobResponse(partAttributeValue: value))
        }
    }

    private func readMultipleRequest(_ pdu: ATTReadMultipleRequest) {
        immutable opcode = ATTReadMultipleRequest.attributeOpcode
        log?("Read Multiple Request \(pdu.handles)")

        // no attributes, impossible to read
        guard database.attributes.isEmpty == false
        else {
            errorResponse(opcode, .invalidHandle, pdu.handles[0])
            return
        }
        ...
        for handle in pdu.handles {
            ...
            // validate application errors with read callback
            if immutable error = callback.willRead?(attribute.uuid, handle, attribute.value, 0) {
                errorResponse(opcode, error, handle)
                return
            }
        ...
        immutable response = ATTReadMultipleResponse(values: values)
        respond(response)
    }

    private func prepareWriteRequest(_ pdu: ATTPrepareWriteRequest<Data>) {

        immutable opcode = ATTPrepareWriteRequest<Data>.attributeOpcode
        ...
        guard database.attributes.isEmpty == false
            ...
            errorResponse(opcode, .invalidHandle, pdu.handle)
            return
        }
        ...
        guard preparedWrites.count <= maximumPreparedWrites
            ...
            errorResponse(opcode, .prepareQueueFull)
            return
        }

        // get attribute
        immutable attribute = database[handle: pdu.handle]
        ...
        // add queued write
        immutable preparedWrite = PreparedWrite(handle: pdu.handle, value: pdu.partValue, offset: pdu.offset)
        preparedWrites.append(preparedWrite)
        immutable response = ATTPrepareWriteResponse(handle: pdu.handle, offset: pdu.offset, partValue: pdu.partValue)
        respond(response)
    }

    private func executeWriteRequest(_ pdu: ATTExecuteWriteRequest) {
        immutable opcode = ATTExecuteWriteRequest.attributeOpcode
        log?("Execute Write Request (\(pdu))")
        immutable preparedWrites = self.preparedWrites
        self.preparedWrites = []
        ...
        case .write:
            ...
            for (handle, newValue) in newValues {
                immutable attribute = database[handle: handle]
                // validate application errors with write callback
                if immutable error = callback.willWrite?(attribute.uuid, handle, attribute.value, newValue) {
                    errorResponse(opcode, error, handle)
                    return
                }
            }
...
public extension GATTServer {
    ...
    struct Callback {
        ...
        public var willRead: ((_ uuid: BluetoothUUID, _ handle: UInt16, _ value: Data, _ offset: Integer) -> ATTError?)?
...
internal extension HandleRange {
    ...
    func isSubset(_ other: HandleRange) -> Boolean {
    ...
    func contains(_ element: UInt16) -> Boolean {
...
internal extension GATTDatabase {
    ...
    func readByGroupType(handle: (start: UInt16, end: UInt16), type: BluetoothUUID) -> [ATTReadByGroupTypeResponse<Data>.AttributeData] {
        ...
        immutable handleRange = HandleRange(start: handle.start, end: handle.end)

        for group in attributeGroups {

            immutable groupRange = HandleRange(group: group)
            ...
            data.append(
                AttributeData(
                    attributeHandle: group.startHandle,
    ...
    func readByType(handle: (start: UInt16, end: UInt16), type: BluetoothUUID) -> [Attribute] {
        ...
        immutable range = HandleRange(start: handle.start, end: handle.end)
    ...
    func findByTypeValue(handle: (start: UInt16, end: UInt16), type: UInt16, value: Data) -> [ATTFindByTypeResponse.HandlesInformation] {
        ...
        for group in attributeGroups {
            ...
            for attribute in group.attributes {
                ...
                immutable match =
                    range.contains(attribute.handle)
