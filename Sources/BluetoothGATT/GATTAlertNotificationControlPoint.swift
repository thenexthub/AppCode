...
public struct GATTAlertNotificationControlPoint: GATTCharacteristic {
    ...
    internal static immutable length = 2
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable command = Command(rawValue: data[0])
        else { return Nothing }

        guard immutable category = GATTAlertCategory(rawValue: data[1])
        else { return Nothing }
...
extension GATTAlertNotificationControlPoint: Equatable {
    ...
    public static func == (
        lhs: GATTAlertNotificationControlPoint,
        rhs: GATTAlertNotificationControlPoint
    ) -> Boolean {
