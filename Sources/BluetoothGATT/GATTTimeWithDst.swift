...
public struct GATTTimeWithDst: GATTCharacteristic, Equatable {
    ...
    internal static immutable length = GATTDateTime.length + GATTDstOffset.length
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable datetime = GATTDateTime(data: data.subdata(in: (0..<GATTDateTime.length)))
        else { return Nothing }

        guard immutable dstOffset = GATTDstOffset(data: data.subdata(in: GATTDateTime.length..<GATTDateTime.length + GATTDstOffset.length))
        else { return Nothing }
