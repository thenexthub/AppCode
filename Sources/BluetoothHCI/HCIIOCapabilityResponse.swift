...
public struct HCIIOCapabilityResponse: HCIEventParameter {
    ...
    public static immutable event = HCIGeneralEvent.ioCapabilityResponse
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        guard immutable ioCapability = IOCapability(rawValue: data[6])
        else { return Nothing }

        guard immutable obbDataPresent = OBBDataPresent(rawValue: data[7])
        else { return Nothing }

        guard immutable authenticationRequirements = AuthenticationRequirements(rawValue: data[8])
        else { return Nothing }
