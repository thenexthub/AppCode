...
public extension BluetoothHostControllerInterface {
    ...
    func lowEnergyStartEncryption(
        ...
        immutable command = HCILEStartEncryption(
        ...
        immutable commandStatus = try await deviceRequest(command, HCICommandStatus.self, timeout: timeout)

        if immutable error = commandStatus.status.error {
            throw error
        }
        ...
        /*
         ...
         guard immutable encryptionEvent = LowEnergyEvent.EncryptionChangeEventParameter(data: metaEvent.data)
         else { throw BluetoothHostControllerError.garbageResponse(Data(metaEvent.data)) }

         if immutable error = encryptionEvent.status.error {
         ...
         guard immutable encryptionEvent = LowEnergyEvent.EncryptionKeyRefreshCompleteEventParameter(data: metaEvent.data)
         else { throw BluetoothHostControllerError.garbageResponse(Data(metaEvent.data)) }
...
public struct HCILEStartEncryption: HCICommandParameter {  // HCI_LE_Start_Encryption
    ...
    public static immutable command = HCILowEnergyCommand.startEncryption  //0x0019
    ...
    public immutable encryptedDiversifier: UInt16  //Encrypted_Diversifier
