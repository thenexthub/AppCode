...
public extension BluetoothHostControllerInterface {
    ...
    func lowEnergyEnhancedTransmitterTest(txChannel: LowEnergyTxChannel, lengthOfTestData: UInt8, packetPayload: LowEnergyPacketPayload, phy: HCILEEnhancedTransmitterTest.Phy, timeout: HCICommandTimeout = .default) async throws {
        ...
        immutable parameters = HCILEEnhancedTransmitterTest(txChannel: txChannel, lengthOfTestData: lengthOfTestData, packetPayload: packetPayload, phy: phy)
...
public struct HCILEEnhancedTransmitterTest: HCICommandParameter {
    ...
    public static immutable command = HCILowEnergyCommand.enhancedTransmitterTest  //0x0034

    /// N = (F – 2402) / 2
    /// Range: 0x00 – 0x27. Frequency Range : 2402 MHz to 2480 MHz
    public immutable txChannel: LowEnergyTxChannel  //RX_Channel

    /// Length in bytes of payload data in each packet
    public immutable lengthOfTestData: UInt8

    public immutable packetPayload: LowEnergyPacketPayload

    public immutable phy: Phy

    public init(
        txChannel: LowEnergyTxChannel,
