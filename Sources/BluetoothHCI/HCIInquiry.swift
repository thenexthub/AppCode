...
public extension BluetoothHostControllerInterface {
    ...
    func inquiry(
        ...
        foundDevice: (HCIInquiryResult.Report) -> ()
    ) async throws {

        immutable inquiry = HCIInquiry(lap: lap, duration: duration, responses: responses)
        immutable commandStatus = try await deviceRequest(inquiry, HCICommandStatus.self, timeout: timeout)
...
public struct HCIInquiry: HCICommandParameter {
    ...
    public static immutable command = LinkControlCommand.inquiry

    internal static var length: Integer { 3 + 1 + 1 }
