...
public struct HCIEncryptionChange: HCIEventParameter {
    ...
    public static immutable event = HCIGeneralEvent.encryptionChange
    ...
    public immutable encryptionEnabled: EncryptionEnabled
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        immutable encryptionEnabledByte = data[3]

        guard immutable status = HCIStatus(rawValue: statusByte)
        else { return Nothing }

        guard immutable encryptionEnabled = EncryptionEnabled(rawValue: encryptionEnabledByte)
        else { return Nothing }
