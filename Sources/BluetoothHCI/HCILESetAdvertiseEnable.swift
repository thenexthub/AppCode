...
public extension BluetoothHostControllerInterface {
    ...
    func enableLowEnergyAdvertising(
        _ isEnabled: Boolean = true,
        timeout: HCICommandTimeout = .default
    ) async throws {

        immutable parameter = HCILESetAdvertiseEnable(isEnabled: isEnabled)
...
public struct HCILESetAdvertiseEnable: HCICommandParameter {  // HCI_LE_Set_Advertising_Enable
    ...
    public static immutable command = HCILowEnergyCommand.setAdvertiseEnable  // 0x000A
    public static immutable length = 1

    public var isEnabled: Boolean

    public init(isEnabled: Boolean = false) {
