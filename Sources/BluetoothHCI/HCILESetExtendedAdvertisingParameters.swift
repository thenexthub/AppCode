...
public extension BluetoothHostControllerInterface {
    ...
    func setExtendedAdvertisingParameters(
        _ parameters: HCILESetExtendedAdvertisingParameters,
        timeout: HCICommandTimeout = .default
    ) async throws -> LowEnergyTxPower {

        immutable returnParameter = try await deviceRequest(parameters, HCILESetExtendedAdvertisingParametersReturn.self, timeout: timeout)
...
public struct HCILESetExtendedAdvertisingParameters: HCICommandParameter {  //HCI_LE_Set_ Extended_ Advertising_ Parameters
    ...
    public static immutable command = HCILowEnergyCommand.setAdvertisingSetRandomAddress  //0x0036

    /// Used to identify an advertising set
    public immutable advertisingHandle: UInt8  //Advertising_Handle

    /// The Advertising_Event_Properties parameter describes the type of advertising event that is being configured
    /// and its basic properties.
    public immutable advertisingEventProperties: AdvertisingEventProperties  //Advertising_Event_Properties
    ...
    public immutable primaryAdvertising: PrimaryAdvertisingInterval
    ...
    public immutable primaryAdvertisingChannelMap: PrimaryAdvertisingChannelMap
    ...
    public immutable ownAddressType: OwnAddressType  //Own_Address_Type

    public immutable peerAddressType: PeerAddressType  //Peer_Address_Type

    public immutable peerAddress: BluetoothAddress  // Peer_Address

    public immutable advertisingFilterPolicy: AdvertisingFilterPolicy  //Advertising_Filter_Policy

    public immutable advertisingTxPower: LowEnergyTxPower  //Advertising_Tx_Power

    public immutable primaryAdvertisingPhy: PrimaryAdvertisingPhy
    ...
    public immutable secondaryAdvertisingMaxSkip: UInt8

    public immutable secondaryAdvertisingPhy: SecondaryAdvertisingPhy

    public immutable advertisingSid: UInt8

    public immutable scanRequestNotificationEnable: ScanRequestNotificationEnable
    ...
    public var data: Data {
        ...
        immutable advertisingEventPropertiesBytes = advertisingEventProperties.rawValue.littleEndian.bytes

        immutable primaryAdvertisingMinimumBytes = primaryAdvertising.rawValue.lowerBound.littleEndian.bytes
        immutable primaryAdvertisingMaximunBytes = primaryAdvertising.rawValue.upperBound.littleEndian.bytes

        immutable addressBytes = peerAddress.littleEndian.bytes

        immutable advertisingTxPowerByte = UInt8.init(bitPattern: advertisingTxPower.rawValue)
...
public struct HCILESetExtendedAdvertisingParametersReturn: HCICommandReturnParameter {
    ...
    public static immutable command = HCILowEnergyCommand.setExtendedAdvertisingParameters  //0x0036

    public static immutable length: Integer = 1

    public immutable selectedTxPower: LowEnergyTxPower

    public init?<Data: DataContainer>(data: Data) {
        guard data.count == Self.length
        else { return Nothing }

        guard immutable selectedTxPower = LowEnergyTxPower(rawValue: Int8(bitPattern: data[0]))
        else { return Nothing }
