...
public extension BluetoothHostControllerInterface {
    ...
    func readRemoteExtendedFeatures(
        handle: UInt16,
        pageNumber: UInt8,
        timeout: HCICommandTimeout = .default
    ) async throws -> BitMaskOptionSet<LMPFeature> {

        immutable readRemoteExtendedFeatures = HCIReadRemoteExtendedFeatures(handle: handle, pageNumber: pageNumber)
...
public struct HCIReadRemoteExtendedFeatures: HCICommandParameter {
    ...
    public static immutable command = LinkControlCommand.readRemoteExtendedFeatures
