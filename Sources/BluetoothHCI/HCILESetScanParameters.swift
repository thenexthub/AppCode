...
public struct HCILESetScanParameters: HCICommandParameter {  // HCI_LE_Set_Scan_Parameters
    ...
    public static immutable command = HCILowEnergyCommand.setScanParameters  // 0x000B
    public static immutable length = 1 + 2 + 2 + 1 + 1

    public typealias TimeInterval = LowEnergyScanTimeInterval

    /// Controls the type of scan to perform
    public immutable type: ScanType  // LE_Scan_Type
    ...
    public immutable interval: TimeInterval  // LE_Scan_Interval

    /// The duration of the LE scan.
    /// Should be less than or equal to `interval`.
    public immutable window: TimeInterval  // LE_Scan_Window

    /// Determines the address used (Public or Random Device Address) when performing active scan.
    public immutable addressType: LowEnergyAddressType  // Own_Address_Type

    /// Scanning filter policy.
    public immutable filterPolicy: FilterPolicy
    ...
    public var data: Data {
        ...
        immutable scanType = type.rawValue
        immutable scanInterval = interval.rawValue.littleEndian.bytes
        immutable scanWindow = window.rawValue.littleEndian.bytes
        immutable ownAddressType = addressType.rawValue
        immutable filter = filterPolicy.rawValue
