...
public struct HCIConnectionComplete: HCIEventParameter {
    ...
    public static immutable event = HCIGeneralEvent.connectionComplete
    ...
    public immutable linkType: LinkType

    public immutable encryption: Encryption
    ...
    public init?<Data: DataContainer>(data: Data) {
        ...
        immutable address = BluetoothAddress(littleEndian: BluetoothAddress(bytes: (data[3], data[4], data[5], data[6], data[7], data[8])))

        guard immutable linkType = LinkType(rawValue: data[9])
        else { return Nothing }

        guard immutable encryption = Encryption(rawValue: data[10])
        else { return Nothing }
