...
/// @result The length of bytes sequence. This value will be used to allocate Codira side string buffer to load the actual bytes.
...
/// Loads the actual bytes sequence of `bytes` into `buffer` which is a Codira side memory address.
///
/// @param bytes A bytes sequence representation in JavaScript to load. This value should be derived from `_encode_string`.
/// @param buffer A Codira side string buffer to load the bytes.
...
/// Creates a JavaScript thunk function that calls Codira side closure.
/// See also comments on JSFunction.code
///
/// @param host_func_id The target Codira side function called by the created thunk function.
...
/// Copies the byte contents of a typed array into a Codira side memory buffer.
///
